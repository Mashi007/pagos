...
-- ============================================================================
-- COLUMNAS CORRECTAS: id, cliente_id, producto, estado, total_financiamiento
--                     numero_cuotas, fecha_aprobacion
...
FROM prestamos p
LEFT JOIN cuotas c ON p.id = c.prestamo_id
GROUP BY p.id, p.cliente_id, p.producto, p.estado, 
...
FROM prestamos p
LEFT JOIN cuotas c ON p.id = c.prestamo_id
GROUP BY p.id, p.cliente_id, p.producto, p.estado, p.numero_cuotas
...
SELECT 
  ...
  ROUND(100.0 * COUNT(DISTINCT c.prestamo_id) / COUNT(*), 2) AS porcentaje_con_cuotas
FROM prestamos p
LEFT JOIN cuotas c ON p.id = c.prestamo_id
GROUP BY p.estado
ORDER BY p.estado;
...
SELECT 
  'Prestamos CON cuotas' AS metrica,
...
SELECT 
  'Prestamos SIN cuotas' AS metrica,
...
SELECT 
  'Prestamos con cobertura completa' AS metrica,
  COUNT(*)::TEXT AS valor
FROM (
  SELECT p.id
  FROM prestamos p
...
SELECT 
  'Prestamos con cobertura parcial' AS metrica,
  COUNT(*)::TEXT AS valor
FROM (
  SELECT p.id
  FROM prestamos p
