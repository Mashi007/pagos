#!/usr/bin/env python3
"""
Script para corregir automáticamente el patrón "tiene 4 espacios, pero debería tener 8 espacios"
"""

import os

def fix_indentation_automatically():
    """Corregir automáticamente los problemas de indentación identificados"""
    
    files_to_fix = [
        "backend/app/api/v1/endpoints/critical_error_monitor.py",
        "backend/app/api/v1/endpoints/intermittent_failure_analyzer.py", 
        "backend/app/api/v1/endpoints/network_diagnostic.py",
        "backend/app/api/v1/endpoints/temporal_analysis.py"
    ]
    
    total_fixed = 0
    
    for file_path in files_to_fix:
        print(f"Corrigiendo {file_path}...")
        
        if not os.path.exists(file_path):
            print(f"  ERROR: Archivo no encontrado: {file_path}")
            continue
        
        # Leer archivo
        with open(file_path, 'r', encoding='utf-8') as f:
            lines = f.readlines()
        
        fixed_count = 0
        i = 0
        
        while i < len(lines):
            line = lines[i]
            line_num = i + 1
            
            # Detectar líneas que empiecen con 4 espacios pero deberían tener 8
            if line.startswith('    ') and not line.startswith('        '):
                stripped = line.strip()
                
                # Si es un método o función, probablemente debería tener 8 espacios
                if stripped.startswith('def ') or stripped.startswith('class '):
                    # Verificar si estamos dentro de una clase
                    in_class = False
                    for j in range(i-1, -1, -1):
                        if lines[j].strip().startswith('class '):
                            in_class = True
                            break
                        elif lines[j].strip().startswith('def ') and not lines[j].startswith('        '):
                            break
                    
                    if in_class:
                        # Corregir a 8 espacios
                        corrected_line = '        ' + stripped + '\n'
                        lines[i] = corrected_line
                        print(f"  CORREGIDO linea {line_num}: {stripped}")
                        fixed_count += 1
            
            i += 1
        
        # Escribir archivo corregido
        if fixed_count > 0:
            with open(file_path, 'w', encoding='utf-8') as f:
                f.writelines(lines)
            print(f"  OK: {fixed_count} problemas corregidos en {file_path}")
            total_fixed += fixed_count
        else:
            print(f"  OK: No se encontraron problemas en {file_path}")
    
    print(f"\nRESUMEN: Total de problemas corregidos: {total_fixed}")
    return total_fixed

if __name__ == "__main__":
    print("Corrigiendo patrones de indentacion automaticamente...")
    fixed = fix_indentation_automatically()
    print(f"Correccion completada. {fixed} problemas corregidos.")
