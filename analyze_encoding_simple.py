    ...
    for encoding in encodings:
        try:
            with open(file_path, 'r', encoding=encoding) as f:
                content = f.read()
            print(f"OK: Leido exitosamente con {encoding}")
...
def analyze_problematic_chars(content, file_path):
    ...
    print(f"\n--- Analisis de caracteres problematicos ---")
...
def analyze_raw_bytes(file_path):
    ...
    print(f"\n--- Analisis de bytes raw ---")
    
    try:
        with open(file_path, 'rb') as f:
            raw_bytes = f.read()
        
        print(f"Tamano del archivo: {len(raw_bytes)} bytes")
...
def main():
    ...
    print("ANALISIS DE CARACTERES INVISIBLES Y PROBLEMAS DE CODIFICACION")
    print("=" * 70)
