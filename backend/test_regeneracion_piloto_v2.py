...
class TestRegeneracionPiloto:
    ...
    def test_4_fifo_validacion(self):
        """Test 4: FIFO validacion (no hay cuota posterior pagada si anterior impaga)"""
        test_name = "TEST 4: FIFO Validacion"
        try:
            for pid in self.PRESTAMO_IDS:
                cuotas = (
                    self.db.query(Cuota)
                        ...
                        for siguiente in cuotas[i + 1 :]:
                            if (siguiente.total_pagado or 0) > 0:
                                raise AssertionError(
                                    ...
                                    f"pagada pero {c.numero_cuota} impaga (violacion FIFO)"
                                )
                        break
