...
def upgrade() -> None:
    bind = op.get_bind()
    insp = inspect(bind)
    if not insp.has_table("revision_manual_solicitudes_reapertura"):
        op.create_table(
            ...
            sa.ForeignKeyConstraint(
                ["resuelto_por_usuario_id"], ["usuarios.id"], ondelete="SET NULL"
            ),
            sa.ForeignKeyConstraint(
                ["solicitante_usuario_id"], ["usuarios.id"], ondelete="SET NULL"
            ),
        ...
        op.execute(
            "CREATE INDEX IF NOT EXISTS ix_revision_manual_sol_reap_prestamo_id "
            "ON revision_manual_solicitudes_reapertura (prestamo_id);"
        )
        op.execute(
            "CREATE INDEX IF NOT EXISTS ix_revision_manual_sol_reap_solicitante_usuario_id "
            "ON revision_manual_solicitudes_reapertura (solicitante_usuario_id);"
        )
        op.execute(
            "CREATE INDEX IF NOT EXISTS ix_revision_manual_sol_reap_estado "
            "ON revision_manual_solicitudes_reapertura (estado);"
        )
        op.execute(
            "CREATE UNIQUE INDEX IF NOT EXISTS uq_rm_sol_reap_un_pendiente_por_prestamo "
            "ON revision_manual_solicitudes_reapertura (prestamo_id) "
