...
def upgrade() -> None:
    ...
    result = connection.execute(sa.text("SELECT COUNT(*) FROM usuarios WHERE is_admin = TRUE"))
    admin_count = result.scalar()
    
    if admin_count == 0:
        # Si no hay admins, hacer el primer usuario admin
        op.execute("UPDATE usuarios SET is_admin = TRUE WHERE id = (SELECT MIN(id) FROM usuarios)")
