...
export interface UploadPrestamosResult {
  registros_creados: number;
  registros_con_error: number;
  mensaje: string;
  prestamos_con_errores?: Array<{ cedula: string; fila: number; errores: string }>;
}

export const useExcelUploadPrestamos = () => {
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [result, setResult] = useState<UploadPrestamosResult | null>(null);
  ...
  const uploadMutation = useMutation({
    mutationFn: async (file: File) => {
      ...
      try {
        ...
        const response = await fetch(`${API_URL}/prestamos/upload-excel`, {
          method: 'POST',
          body: formData,
          headers: {
