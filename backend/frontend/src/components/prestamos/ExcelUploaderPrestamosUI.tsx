...
import { useExcelUploadPrestamos } from '@/hooks/useExcelUploadPrestamos';
import { toast } from 'sonner';

export const ExcelUploaderPrestamosUI: React.FC<{
  onClose?: () => void;
  onSuccess?: () => void;
}> = ({ onClose, onSuccess }) => {
  const fileInputRef = useRef<HTMLInputElement>(null);
  const { uploadFile, isLoading, error, result, reset } = useExcelUploadPrestamos();
  ...
  return (
    <div className="w-full max-w-2xl mx-auto p-6 bg-white rounded-lg shadow-lg border border-gray-200">
      ...
      {/* InformaciÃÂ³n sobre el formato */}
      <div className="mb-6 p-4 bg-blue-50 rounded-lg border border-blue-200">
        <h3 className="font-semibold text-blue-900 mb-2">Formato requerido:</h3>
        <p className="text-sm text-blue-800">
          CÃÂ©dula Cliente | Monto Financiamiento | Modalidad Pago | NÃÂº Cuotas | Producto | Analista | Concesionario
        </p>
        <p className="text-xs text-blue-700 mt-2">
          Ã¢ÂÂ¢ CÃÂ©dula: cliente debe existir en BD | Ã¢ÂÂ¢ Monto: mayor a 0
          <br />
          Ã¢ÂÂ¢ Modalidad: MENSUAL|QUINCENAL|SEMANAL | Ã¢ÂÂ¢ NÃÂº Cuotas: 1-12
          <br />
          Ã¢ÂÂ¢ Analista: requerido | Ã¢ÂÂ¢ Concesionario: opcional
        </p>
      </div>
          ...
          <div className="flex flex-col items-center gap-3">
            ...
            <div>
              <p className="font-semibold text-gray-700">
                {isDragOver
                  ? 'Suelte el archivo aquÃÂ­'
                  : 'Arrastra tu archivo Excel aquÃÂ­ o haz clic'}
              </p>
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              ...
              <div className="space-y-1 text-sm text-green-800">
                <p>Ã¢ÂÂ PrÃÂ©stamos creados: <span className="font-bold">{result.registros_creados}</span></p>
                {result.registros_con_error > 0 && (
                  <p>Ã¢ÂÂ Ã¯Â¸Â Con errores: <span className="font-bold text-yellow-700">{result.registros_con_error}</span></p>
                )}
