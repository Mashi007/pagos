  ...
  if (error) {
    return (
      <div className="p-4 bg-red-50 rounded-lg border border-red-200 text-red-800">
        Error al cargar prÃÂ©stamos con errores
      </div>
    );
  }
  ...
  if (!data?.items || data.items.length === 0) {
    return (
      <div className="p-8 text-center bg-gray-50 rounded-lg border border-gray-200">
        <div className="text-gray-500">No hay prÃÂ©stamos con errores</div>
      </div>
    );
  }
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-xl font-bold text-gray-800 flex items-center gap-2">
          ...
          Revisar prÃÂ©stamos ({data.total || 0})
        </h2>
        <button
          onClick={() => refetch()}
        ...
        <table className="w-full text-sm">
          <thead className="bg-gray-50 border-b">
            <tr>
              ...
              <th className="px-4 py-3 text-left font-semibold text-gray-700">CÃÂ©dula</th>
      ...
      {data.total > perPage && (
        <div className="flex justify-center gap-2 mt-4">
          ...
          <span className="px-4 py-2 text-sm text-gray-600">
            PÃÂ¡gina {page} de {Math.ceil(data.total / perPage)}
          </span>
          <button
