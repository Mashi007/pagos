...
Basado en el reporte de verificación - elimina solo los roles que realmente no existen
"""
import os
import re
...
class LegacyRoleCleaner:
    def __init__(self, backend_path: str = "."):
        ...
        # Roles que SÍ deben ser eliminados (no existen en el sistema actual)
        self.legacy_roles_to_remove = {
            'USER',
            'USER', 
            'USER',
            'ADMIN',
            'GERENTE',
            'USER_USER',
            'USER'  # Este también aparece en el reporte como extra
        }
        # Roles válidos que SÍ existen
        self.valid_roles = {'USER', 'ADMIN', 'GERENTE', 'COBRANZAS'}
        self.files_to_fix = []
        
    def scan_and_fix_files(self):
        """Escanear y corregir archivos con roles legacy"""
        print("ESCANEANDO ARCHIVOS CON ROLES LEGACY QUE DEBEN SER ELIMINADOS")
        print("=" * 70)
    ...
    def fix_legacy_roles(self, content: str) -> str:
        ...
        # Reemplazos específicos para roles que SÍ deben ser eliminados
        replacements = {
            # ADMIN -> ADMIN
            r'"ADMIN"': '"ADMIN"',
            r"'ADMIN'": "'ADMIN'",
            ...
            # USER -> USER (este también aparece como extra en el reporte)
            r'"USER"': '"USER"',
            r"'USER'": "'USER'",
            r'USER': 'USER',
            
            # Listas de roles legacy que deben ser corregidas
            ...
            # Roles en filtros de SQLAlchemy
    ...
    def get_changes_summary(self, original: str, fixed: str) -> List[str]:
        ...
        for legacy_role in self.legacy_roles_to_remove:
    ...
    def generate_cleanup_report(self):
        ...
        for role in sorted(self.legacy_roles_to_remove):
            print(f"  - {role}")
