...
def verificar_modelos_vehiculos():
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            result = conn.execute(text("SELECT COUNT(*) FROM modelos_vehiculos WHERE activo = true"))
            registros_activos = result.scalar()
            logger.info(f"✅ Registros activos: {registros_activos}")

            # Mostrar todos los registros
            result = conn.execute(text("SELECT id, nombre, activo FROM modelos_vehiculos ORDER BY nombre"))
            registros = result.fetchall()

            logger.info("📋 Registros en la tabla:")
            for registro in registros:
                estado = "✅ Activo" if registro.activo else "❌ Inactivo"
                logger.info(f"  ID: {registro.id} | Nombre: {registro.nombre} | {estado}")

            if registros_activos == 0:
                logger.warning("⚠️ No hay registros activos. Esto explicaría por qué no se cargan los modelos.")
