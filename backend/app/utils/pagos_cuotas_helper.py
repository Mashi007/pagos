...
def obtener_pagos_cuota(
    ...
    if cedula and monto_cuota:
        ...
        ).order_by(func.abs(Pago.monto_pagado - monto_cuota)).limit(1).all()
        ...
        if pagos_encontrados:
            logger.debug(
                f"✅ [obtener_pagos_cuota] Encontrado 1 pago "
                f"por cedula={cedula} + monto similar a {monto_cuota}"
            )
            return pagos_encontrados
...
def calcular_monto_pagado_mes(
    ...
    query_cuotas = db.query(Cuota).join(
    ...
    if pago.cedula and pago.fecha_pago:
        ...
        for prestamo in prestamos:
            ...
            cuota = db.query(Cuota).filter(
                ...
                func.abs(func.extract('day', Cuota.fecha_vencimiento - fecha_pago_date))
            ).first()
