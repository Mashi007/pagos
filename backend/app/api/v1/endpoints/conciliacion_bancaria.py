...
def generar_template_conciliacion(    current_user:
 User = Depends(get_current_user),):
    """Generar template Excel para conciliaciÃ³n bancaria"""    try:
        logger.info(            f"Usuario {current_user.email} generando template de conciliaciÃ³n"        )        # Crear workbook        
from openpyxl 
import Workbook        
from openpyxl.worksheet.datavalidation 
import DataValidation        wb = Workbook()        # HOJA 1:
 INSTRUCCIONES        ws_instrucciones = wb.active        ws_instrucciones.title = "Instrucciones"        instrucciones = [            ["INSTRUCCIONES PARA CONCILIACIÃ“N BANCARIA"],            [""],            ["1. FORMATO DE ARCHIVO:
"],            ["   - Archivo Excel (.xlsx)"],            ["   - Primera fila:
 encabezados de columnas"],            ["   - Datos desde la segunda fila"],            ["   - MÃ¡ximo 100 registros por archivo"],            [""],            ["2. COLUMNAS REQUERIDAS:
"],            ["   - fecha:
 Fecha real del pago (formato YYYY-MM-DD)"],            ["   - numero_documento:
 NÃºmero de documento del pago"],            [""],            ["3. PROCESO DE CONCILIACIÃ“N:
"],            [                "   - El sistema compara el nÃºmero de documento con la \                base de datos"            ],            ["   - Si hay coincidencia EXACTA:
 se marca como CONCILIADO"],            ["   - Si NO hay coincidencia:
 se marca como PENDIENTE"],            [""],            ["4. VALIDACIONES:
"],            ["   - Fecha debe estar en formato YYYY-MM-DD"],            ["   - NÃºmero de documento debe coincidir EXACTAMENTE"],            ["   - No se permiten caracteres especiales adicionales"],            [""],            ["5. EJEMPLOS DE DATOS VÃLIDOS:
"],            ["   - fecha:
 '2024-01-15'"],            ["   - numero_documento:
 'DOC001234'"],            [""],            ["6. NOTAS IMPORTANTES:
"],            ["   - No eliminar las columnas"],            ["   - No cambiar el orden de las columnas"],            ["   - Usar solo caracteres ASCII"],            ["   - Verificar que los nÃºmeros de documento sean exactos"],            ["   - Un archivo por vez"],            [""],            ["7. RESULTADO:
"],            ["   - Los pagos conciliados aparecerÃ¡n en el resumen"],            ["   - Los pendientes requerirÃ¡n revisiÃ³n manual"],            [""],            ["8. DESCONCILIACIÃ“N:
"],            ["   - Se puede desconciliar un pago ya conciliado"],            ["   - Requiere formulario con justificaciÃ³n"],            ["   - Se registra en auditorÃ­a"],            [""],            [                "FECHA DE GENERACIÃ“N:
...
%S")            ],            ["GENERADO POR:
 ...
 TEMPLATE VACÃA        ws_template = wb.create_sheet("Template_Conciliacion")        # Encabezados        encabezados = ["fecha", "numero_documento"]        for i, encabezado in enumerate(encabezados, 1):
            ...
            ws_template.cell(row=2, column=i, value=valor)        # Aplicar validaciones        # ValidaciÃ³n para fecha (formato YYYY-MM-DD)        dv_fecha = DataValidation(            type="date", formula1="2020-01-01", formula2="2030-12-31"        )        dv_fecha.add("A2:
A100")        ws_template.add_data_validation(dv_fecha)        # Guardar en memoria        output = io.BytesIO()        wb.save(output)        output.seek(0)        logger.info("Template de conciliaciÃ³n generado exitosamente")        return {            "success":
 True,            "message":
 ...
 output.getvalue(),        }    except Exception as e:
        logger.error(f"Error generando template de conciliaciÃ³n:
...
def procesar_conciliacion(    file:
 ...
 User = Depends(get_current_user),):
    """Procesar archivo Excel de conciliaciÃ³n bancaria"""    try:
        logger.info(            f"Usuario {current_user.email} procesando conciliaciÃ³n bancaria"        )        # Validar tipo de archivo        if not file.filename.endswith((".xlsx", ".xls")):
 ...
 {required_columns}",            )        # Validar lÃ­mite de registros        if len(df) > 100:
            raise HTTPException(                status_code=status.HTTP_400_BAD_REQUEST,                detail="MÃ¡ximo 100 registros por archivo",            )        # Procesar cada registro        resultados = []        conciliados = 0        pendientes = 0        for index, row in df.iterrows():
 ...
 estado,                    "pago_id":
 pago.id if pago else None,                }            )        # Guardar cambios        db.commit()        logger.info(            f"ConciliaciÃ³n procesada:
 {conciliados} "    f"conciliados, {pendientes} pendientes"        )        return {            "success":
 True,            "message":
 "ConciliaciÃ³n procesada exitosamente",            "resumen":
 {                "total_registros":
 len(df),                "conciliados":
 ...
 resultados,        }    except HTTPException:
        ...
        db.rollback()        logger.error(f"Error procesando conciliaciÃ³n:
 {e}")        raise HTTPException(            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,            detail=f"Error interno del servidor:
 ...
 User = Depends(get_current_user),):
    """Desconciliar un pago ya conciliado"""    try:
        logger.info(            f"Usuario {current_user.email} desconciliando pago "    f"{conciliacion_data.numero_documento_anterior}"        )        # Buscar el pago a desconciliar        pago = (            db.query(Pago)            .filter(                and_(                    Pago.activo,                    Pago.numero_documento                    == conciliacion_data.numero_documento_anterior,                    Pago.conciliado,                )            )            .first()        )        if not pago:
            raise HTTPException(                status_code=status.HTTP_404_NOT_FOUND,                detail="Pago no encontrado o no estÃ¡ conciliado",            )        # Desconciliar el pago        pago.conciliado = False        pago.fecha_conciliacion = None        # Crear registro de auditorÃ­a (simulado - se integrarÃ¡ con mÃ³dulo de        # auditorÃ­a)        conciliacion_record = {            "id":
...
def obtener_estado_conciliacion(    db:
 Session = Depends(get_db),    current_user:
 User = Depends(get_current_user),):
    """Obtener estado general de conciliaciÃ³n"""    try:
        # EstadÃ­sticas generales        total_pagos = db.query(Pago).filter(Pago.activo).count()        pagos_conciliados = (            db.query(Pago).filter(and_(Pago.activo, Pago.conciliado)).count()        )        pagos_pendientes = total_pagos - pagos_conciliados        # Porcentaje de conciliaciÃ³n        porcentaje_conciliacion = (            (pagos_conciliados / total_pagos * 100) if total_pagos > 0 else 0        )        return {            "success":
 True,            "estadisticas":
 {                "total_pagos":
 ...
 datetime.now(),        }    except Exception as e:
        logger.error(f"Error obteniendo estado de conciliaciÃ³n:
 {e}")        raise HTTPException(            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,            detail=f"Error interno del servidor:
 {str(e)}",        )
