"""Endpoint de AnÃ¡lisis de Impacto en PerformanceProporciona mÃ©tricas y anÃ¡lisis de impacto del sistema"""
import logging
import time
from typing 
import Any, Dict
...
def get_health_impact_analysis_public():
    """    Obtener anÃ¡lisis de impacto de health checks (PÃšBLICO)    - MÃ©tricas de performance de health checks    - Impacto en recursos del sistema    - Alertas y recomendaciones    """    try:
 ...
 time.time(),        }    except Exception as e:
        logger.error(f"Error obteniendo anÃ¡lisis de health:
 {e}")        raise HTTPException(            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,            detail=f"Error obteniendo anÃ¡lisis de health:
 {str(e)}",        )@router.get("/impact/health", response_model=Dict[str, Any])async 
def get_health_impact_analysis(    current_user:
 User = Depends(get_current_user),):
    """    Obtener anÃ¡lisis de impacto de health checks    - MÃ©tricas de performance de health checks    - Impacto en recursos del sistema    - Alertas y recomendaciones    """    try:
        analyzer = get_impact_analyzer()        status_data = analyzer.get_current_status()        return {            "status":
 "success",            "data":
 status_data,            "timestamp":
 time.time(),        }    except Exception as e:
...
def get_performance_impact_analysis(    current_user:
 User = Depends(get_current_user),):
    """    Obtener anÃ¡lisis de impacto en performance    - MÃ©tricas de performance de endpoints    - AnÃ¡lisis de impacto en recursos    - Recomendaciones de optimizaciÃ³n    """    try:
        analyzer = get_impact_analyzer()        report = analyzer.get_performance_report()        return {"status":
 "success", "data":
 report, "timestamp":
 time.time()}    except Exception as e:
        logger.error(f"Error obteniendo anÃ¡lisis de performance:
 {e}")        raise HTTPException(            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,            detail=f"Error obteniendo anÃ¡lisis de performance:
 {str(e)}",        )@router.get("/impact/errors", response_model=Dict[str, Any])async 
def get_error_impact_analysis(    current_user:
 User = Depends(get_current_user),):
    """    Obtener anÃ¡lisis de impacto de errores    - Tasa de error del sistema    - AnÃ¡lisis de impacto de errores    - Estado de circuit breakers    - Recomendaciones de manejo de errores    """    try:
 ...
 time.time(),        }    except Exception as e:
        logger.error(f"Error obteniendo anÃ¡lisis de errores:
 {e}")        raise HTTPException(            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,            detail=f"Error obteniendo anÃ¡lisis de errores:
...
def get_comprehensive_impact_analysis(    current_user:
 ...
 time.time(),            },            "timestamp":
 time.time(),        }    except Exception as e:
        logger.error(f"Error obteniendo anÃ¡lisis integral:
 {e}")        raise HTTPException(            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,            detail=f"Error obteniendo anÃ¡lisis integral:
...
def _generate_general_recommendations(    health_status:
 ...
 Any) -> list:
    """Generar recomendaciones generales del sistema"""    recommendations = []    # Recomendaciones basadas en health    if health_status.get("active_alerts", 0) > 0:
        ...
        recommendations.append("Implementar manejo de errores mÃ¡s robusto")    if error_analysis.consecutive_errors > 3:
        recommendations.append(            "Activar circuit breakers para endpoints problemÃ¡ticos"        )    if not recommendations:
        recommendations.append(            "Sistema funcionando dentro de parÃ¡metros normales"        )    return recommendations
