                ...
                anomalies.append(
                    {
                        ...
                        "description": (
                            f"Error count spike: {
                                avg_recent_errors:.1f} vs {
                                historical_avg:.1f} historical avg"),
                        "confidence": 0.8,
...
async def collect_authentication_metrics(db: Session = Depends(get_db)):
    """
    ...
    if predictions.get("status") == "success":
        ...
        if "success_rate_critical" in pred_data:
            ...
            recommendations.append(
                f"🚨 Predicción: Tasa de éxito crítica en {
                    pred['days_to_critical']} días (probabilidad: {
                    pred['probability']:.1%})")

        if "response_time_warning" in pred_data:
            pred = pred_data["response_time_warning"]
            recommendations.append(
                f"⚠️ Predicción: Tiempo de respuesta alto en {
                    pred['days_to_warning']} días (probabilidad: {
                    pred['probability']:.1%})")
