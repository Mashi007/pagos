...
def get_pagos_por_dia_mes(
    ...
    anos: Optional[str] = Query(None, description="Años separados por coma"),
...
def get_pagos_por_mes(
    ...
    """Pagos agrupados por mes/año. Cada mes tiene lista de pagos."""
    hoy = date.today()
    resultado: dict = {"meses": []}
...
def _generar_excel_pagos_por_mes(data_por_mes: dict) -> bytes:
    ...
    for idx, mes_data in enumerate(meses_data):
        ...
        ws.append([f"Total pagos: {total_pagos} | Total cédulas: {total_cedulas} | Total monto: ${total_monto:.2f}"])
        ws.append([])
...
def exportar_pagos(
    ...
    """Exporta reporte de pagos. Excel: una pestaña por mes. PDF: requiere fecha_inicio y fecha_fin."""
    if formato == "excel":
        data_por_mes = _pagos_por_dia_periodos(db, _periodos_desde_filtros(anos, meses_list, meses))
    ...
    return Response(
        ...
        headers={"Content-Disposition": f"attachment; filename=reporte_pagos_{fi.isoformat()}_{ff.isoformat()}.pdf"},
    )
