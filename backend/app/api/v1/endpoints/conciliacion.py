...
# y manual de movimientos bancarios"""\nimport io\nimport logging\nfrom datetime \nimport date, datetime\nfrom decimal
...
# pd.to_datetime(fila["fecha"]).date() return fecha, advertencias except Exception:\n advertencias.append(f"Fila {index +
...
# list]:\n """Validar referencia de una fila""" advertencias = [] duplicados = [] referencia =
...
# duplicados.append( { "fila":\n index + 1, "referencia":\n referencia, "monto":\n float(fila["monto"]), } ) return None,
# advertencias, duplicados referencias_vistas.add(referencia) return referencia, advertencias, duplicados\ndef
...
# duplicados = [] try:\n # Validar fecha fecha, fecha_advertencias = _validar_fecha(fila, index)
# advertencias.extend(fecha_advertencias) if fecha is None:\n return None, advertencias, errores, duplicados # Validar monto
# monto, monto_advertencias = _validar_monto(fila, index) advertencias.extend(monto_advertencias) if monto is None:\n return
# None, advertencias, errores, duplicados # Validar referencia referencia, ref_advertencias, ref_duplicados =
# _validar_referencia( fila, index, referencias_vistas ) advertencias.extend(ref_advertencias)
# duplicados.extend(ref_duplicados) if referencia is None:\n return None, advertencias, errores, duplicados # Validar cédula
...
# matching_data.get("pago_sugerido") return movimiento, advertencias, errores, duplicados except Exception as e:\n
...
# List[str], List[str], List[str],]:\n """Procesar archivo completo y extraer movimientos válidos""" advertencias = []
# movimientos_validos = [] duplicados = [] cedulas_no_registradas = [] errores = [] referencias_vistas = set() for index,
# fila in df.iterrows():\n movimiento, fila_advertencias, fila_errores, fila_duplicados = ( _procesar_fila_movimiento(fila,
# index, referencias_vistas, db) ) advertencias.extend(fila_advertencias) errores.extend(fila_errores)
# duplicados.extend(fila_duplicados) if movimiento:\n movimientos_validos.append(movimiento) # Verificar cédulas no
...
# cedulas_no_registradas.append(movimiento.cedula_pagador) return ( movimientos_validos, advertencias, errores, duplicados,
# cedulas_no_registradas, )@router.post("/validar-archivo", response_model=ValidacionArchivoBancario)async \ndef
...
# errores=errores, vista_previa=[], ) # 4. Procesar archivo completo ( movimientos_validos, advertencias,
# errores_procesamiento, duplicados, cedulas_no_registradas, ) = _procesar_archivo_completo(df, db) return
# ValidacionArchivoBancario( archivo_valido=len(errores_procesamiento) == 0, formato_detectado=formato, total_filas=len(df),
# filas_validas=len(movimientos_validos), errores=errores_procesamiento, advertencias=advertencias,
# duplicados_encontrados=duplicados, cedulas_no_registradas=cedulas_no_registradas, vista_previa=movimientos_validos[ :\n10
# ], # Primeros 10 para vista previa ) except Exception as e:\n raise HTTPException( status_code=500, detail=f"Error
...
# Monto aproximado (±2%) 3° Nº Referencia conocido """ exactos = [] # ✅ COINCIDENCIA EXACTA parciales = [] # ⚠️ COINCIDENCIA
# PARCIAL sin_match = [] # ❌ SIN COINCIDENCIA for mov in movimientos:\n match_encontrado = False # 1° PRIORIDAD:\n Cédula +
...
# _buscar_pago_por_referencia(mov.referencia, db) if pago_existente:\n exactos.append(_crear_match_referencia(mov,
# pago_existente)) match_encontrado = True # Sin coincidencia if not match_encontrado:\n
# sin_match.append(_crear_sin_match(mov)) return ResultadoConciliacion( total_movimientos=len(movimientos), total_pagos=0, #
# No relevante en este contexto conciliados=len(exactos), sin_conciliar_banco=len(sin_match),
# sin_conciliar_sistema=len(parciales), porcentaje_conciliacion=( round((len(exactos) / len(movimientos) * 100), 2) if
# movimientos else 0 ), detalle_conciliados=exactos, detalle_sin_conciliar_banco=sin_match,
# detalle_sin_conciliar_sistema=parciales, )@router.post("/confirmar-conciliacion/{pago_id}")\ndef confirmar_conciliacion(
...
# EstadoConciliacion.CONCILIADO pago.fecha_conciliacion = datetime.now() db.commit() db.refresh(pago) return {"message":\n
...
# Depends(get_db),):\n """ Obtiene pagos pendientes de conciliar. """ query = db.query(Pago).filter( Pago.estado_conciliacion
...
# date(anio, mes, 1) ultimo_dia = monthrange(anio, mes)[1] fecha_fin = date(anio, mes, ultimo_dia) total_pagos = (
# db.query(Pago) .filter(Pago.fecha_pago >= fecha_inicio, Pago.fecha_pago <= fecha_fin) .count() ) conciliados = (
# db.query(Pago) .filter( Pago.fecha_pago >= fecha_inicio, Pago.fecha_pago <= fecha_fin, Pago.estado_conciliacion ==
# EstadoConciliacion.CONCILIADO, ) .count() ) pendientes = total_pagos - conciliados return { "mes":\n mes, "anio":\n anio,
# "total_pagos":\n total_pagos, "conciliados":\n conciliados, "pendientes":\n pendientes, "porcentaje_conciliacion":\n round(
# (conciliados / total_pagos * 100) if total_pagos > 0 else 0, 2 ), }# ============================================# REVISIÓN
# MANUAL# ============================================@router.post("/revision-manual")\ndef procesar_revision_manual(
# revision:\n RevisionManual, db:\n Session = Depends(get_db), current_user:\n User = Depends(get_current_user),):\n """
...
# simplicidad, crear pago directamente db_pago = Pago( prestamo_id=( cuota.prestamo_id if cuota else cliente.prestamos[0].id
...
# revision.accion == "RECHAZAR":\n # Marcar como rechazado (requeriría tabla de movimientos # temporales) return {
...
# background_tasks:\n BackgroundTasks, db:\n Session = Depends(get_db), current_user:\n User = Depends(get_current_user),):\n
# """ Aplicar conciliación masiva de movimientos """ try:\n pagos_creados = [] errores = [] total_monto = Decimal("0.00")
# clientes_afectados = set() # Procesar cada movimiento (esto requeriría almacenamiento temporal de movimientos) # Por
# simplicidad, simulamos el proceso for mov_id in conciliacion_data.movimientos_a_aplicar:\n try:\n# En implementación real,
# obtendríamos el # movimiento de almacenamiento temporal # Por ahora simulamos la creación de pago # Simular creación de
# pago exitoso pago_id = len(pagos_creados) + 1000 # ID simulado pagos_creados.append(pago_id) total_monto +=
# Decimal("500.00") # Monto simulado clientes_afectados.add(f"Cliente-{mov_id}") except Exception as e:\n
# errores.append({"movimiento_id":\n mov_id, "error":\n str(e)}) # Generar reporte en background background_tasks.add_task(
# _generar_reporte_conciliacion, user_id=current_user.id, pagos_creados=pagos_creados, total_monto=float(total_monto), )
# return ResultadoConciliacionMasiva( total_procesados=len(conciliacion_data.movimientos_a_aplicar),
# exitosos=len(pagos_creados), fallidos=len(errores), pagos_creados=pagos_creados, errores=errores, resumen_financiero={
# "total_monto_aplicado":\n float(total_monto), "clientes_afectados":\n len(clientes_afectados), "promedio_pago":\n (
...
# Depends(get_current_user),):\n """ Obtener historial de conciliaciones procesadas """ # Por ahora retornamos datos
# simulados # En implementación real, habría una tabla de conciliaciones historial_simulado = [ { "id":\n 1,
...
# in h["usuario_proceso"].lower() ] return resultado@router.get("/tabla-resultados/{proceso_id}")\ndef
# obtener_tabla_resultados( proceso_id:\n int, db:\n Session = Depends(get_db), current_user:\n User =
# Depends(get_current_user),):\n """ Obtener tabla de resultados visual como el diagrama """ # Datos simulados para la tabla
# visual tabla_resultados = [ { "banco_ref":\n "7400874", "monto":\n "$500.00", "cliente":\n "Juan P.", "cedula":\n
...
# Pasos del flujo:\n 1. ✅ COBRANZAS descarga extracto del banco (Excel) 2. ✅ Ingresa a "Conciliación Bancaria" 3. ✅ Carga el
# archivo Excel 4. ✅ Sistema valida formato y datos 5. ✅ Sistema muestra vista previa 6. ✅ Cobranzas confirma "Procesar" 7. ✅
# Sistema ejecuta matching automático 8. ✅ Sistema muestra tabla de resultados 9. ✅ Cobranzas revisa casos parciales/manuales
# 10. ✅ Sistema muestra resumen final 11. ✅ Cobranzas confirma "Aplicar todos" 12. ✅ Sistema ejecuta en lote 13. ✅ Sistema
...
# status_code=403, detail="Sin permisos para conciliación bancaria", ) # Validar archivo (reutilizar endpoint existente)
...
# validacion.advertencias, "mensaje":\n "❌ Archivo no válido - Corrija los errores y vuelva a intentar", } #
# ============================================ # PASOS 6-7:\n MATCHING AUTOMÁTICO #
# ============================================ movimientos_extendidos = validacion.vista_previa # Ejecutar matching
# automático resultado_matching = matching_automatico( movimientos=movimientos_extendidos, db=db, current_user=current_user,
# ) # ============================================ # PASO 8:\n TABLA DE RESULTADOS #
# ============================================ tabla_resultados = [] # Procesar coincidencias exactas for match in
# resultado_matching.detalle_conciliados:\n tabla_resultados.append( { "ref_banco":\n match["movimiento"].referencia,
...
# resultado_matching.detalle_sin_conciliar_sistema:\n tabla_resultados.append( { "ref_banco":\n
...
# ============================================ exactos = len([r for r in tabla_resultados if "✅" in r["estado"]]) revision =
# len([r for r in tabla_resultados if "⚠️" in r["estado"]]) manuales = len([r for r in tabla_resultados if "❌" in
# r["estado"]]) total_monto_aplicable = sum( float(r["monto"].replace("$", "").replace(",", "")) for r in tabla_resultados if
# "✅" in r["estado"] ) clientes_afectados = len( set( r["cedula"] for r in tabla_resultados if "✅" in r["estado"] and
# r["cedula"] != "Desconocida" ) ) resumen_final = { "total_movimientos":\n len(tabla_resultados),
# "pagos_aplicar_automatico":\n exactos, "requieren_revision":\n revision, "busqueda_manual":\n manuales,
# "total_monto_aplicable":\n total_monto_aplicable, "clientes_afectados":\n clientes_afectados, "tasa_exito_automatico":\n (
# round((exactos / len(tabla_resultados) * 100), 2) if tabla_resultados else 0 ), } # Guardar datos temporalmente para
# aplicación posterior # En implementación real, usarías Redis o tabla temporal proceso_id =
# f"CONC-{datetime.now().strftime('%Y%m%d%H%M%S')}" return { "paso":\n "8_TABLA_RESULTADOS", "proceso_id":\n proceso_id,
...
# }, "acciones_disponibles":\n { "aplicar_exactos":\n f"POST /conciliacion/aplicar-exactos/ \ {proceso_id}",
# "revisar_parciales":\n f"POST /conciliacion/revisar-parciales/ \ {proceso_id}", "aplicar_todos":\n f"POST
# /conciliacion/aplicar-todos/{proceso_id}", }, "mensaje":\n ( f"✅ Archivo procesado - {exactos} coincidencias exactas,
# {revision} requieren revisión" ), } except Exception as e:\n raise HTTPException( status_code=500, detail=f"Error en flujo
# de conciliación:\n {str(e)}" )@router.post("/aplicar-exactos/{proceso_id}")async \ndef aplicar_coincidencias_exactas(
# proceso_id:\n str, background_tasks:\n BackgroundTasks, db:\n Session = Depends(get_db), current_user:\n User =
# Depends(get_current_user),):\n """ 🚀 PASO 11a:\n Aplicar solo coincidencias exactas automáticamente """ try:\n# En
# implementación real, recuperarías datos # del proceso desde Redis/BD temporal # Por ahora simulamos la aplicación
# pagos_creados = [] clientes_afectados = set() total_aplicado = Decimal("0.00") errores = [] # Simular aplicación de pagos
# exactos # En implementación real, iterarías sobre los movimientos exactos # guardados for i in range(5):\n # Simulación de
# 5 pagos exactos try:\n # Crear pago simulado pago_simulado = { "id":\n 1000 + i, "monto":\n 500.00, "cliente":\n
# f"Cliente-{i + 1}", "cuota":\n i + 1, } pagos_creados.append(pago_simulado)
# clientes_afectados.add(pago_simulado["cliente"]) total_aplicado += Decimal(str(pago_simulado["monto"])) except Exception as
...
# descripcion=f"Conciliación masiva - Proceso {proceso_id}", datos_nuevos={ "proceso_id":\n proceso_id, "pagos_aplicados":\n
# len(pagos_creados), "total_monto":\n float(total_aplicado), "clientes_afectados":\n len(clientes_afectados), }, )
# db.add(auditoria) db.commit() # Generar reporte en background background_tasks.add_task(
# _generar_reporte_conciliacion_completo, proceso_id=proceso_id, user_id=current_user.id, pagos_creados=pagos_creados,
# total_monto=float(total_aplicado), ) # Notificar a admin background_tasks.add_task( _notificar_admin_conciliacion,
# proceso_id=proceso_id, usuario_proceso=current_user.full_name, pagos_aplicados=len(pagos_creados),
...
# "estados_clientes_actualizados":\n True, "auditoria_registrada":\n True, "emails_confirmacion_programados":\n True,
# "reporte_pdf_programado":\n True, "admin_notificado":\n True, }, "mensaje":\n ( f"✅ {len(pagos_creados)} pagos aplicados
...
# Conciliación Bancaria → Nuevo Proceso", "permisos_requeridos":\n ["COBRANZAS", "ADMIN", "GERENTE"], "siguiente_paso":\n
...
# "contenido_reporte":\n [ "Encabezado con fecha y usuario", "Resumen ejecutivo de la conciliación", "Detalle de movimientos
# procesados", "Lista de pagos aplicados exitosamente", "Lista de errores o rechazados", "Estadísticas finales", "Firmas y
...
# /conciliacion/matching-automatico", "tabla_resultados":\n "GET /conciliacion/tabla-resultados/{proceso_id}",
...
# user_id:\n int, pagos_creados:\n List[dict], total_monto:\n float,):\n """ 📄 PASO 13:\n Generar reporte PDF de conciliación
...
# {reporte_data}") # En implementación real:\n # 1. Crear PDF con reportlab # 2. Incluir tabla de movimientos procesados # 3.
# Agregar estadísticas y gráficos # 4. Guardar en sistema de archivos # 5. Enviar por email al usuario except Exception as
# e:\n logger = logging.getLogger(__name__) logger.error(f"Error generando reporte completo:\n {str(e)}")async \ndef
# _notificar_admin_conciliacion( proceso_id:\n str, usuario_proceso:\n str, pagos_aplicados:\n int, total_monto:\n float,):\n
...
# conciliacion_{proceso_id}.pdf Disponible en sistema de archivosACCIONES RECOMENDADAS:\n Revisar reporte detallado Verificar
# pagos aplicados Confirmar actualización de carteraAcceder al sistema:\n https:\n//pagos-f2qf.onrender.comSaludos. """ notif
# = Notificacion( user_id=admin.id, tipo="EMAIL", categoria="GENERAL", asunto=f"✅ Conciliación Completada - {proceso_id}",
# mensaje=mensaje, estado="PENDIENTE", programada_para=datetime.now(), prioridad="NORMAL", ) db.add(notif) db.commit() #
...
# subject=notif.asunto, body=notif.mensaje, notificacion_id=notif.id, ) db.close() except Exception as e:\n logger =
