...
def reporte_cartera(
    ...
    try:
        ...
        cantidad_prestamos_activos = db.query(Prestamo).filter(
            Prestamo.estado == EstadoPrestamo.ACTIVO
        ).count()

        cantidad_prestamos_mora = db.query(Prestamo).filter(
            Prestamo.estado == EstadoPrestamo.EN_MORA
        ).count()

        # Distribución por monto
        distribucion_por_monto = db.query(
