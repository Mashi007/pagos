# backend/app/api/v1/endpoints/reportes.py"""Sistema de Reportes del Sistema de PréstamosGeneración de reportes de cartera,
# morosidad, cobranza y análisis financiero"""\nimport io\nimport logging\nfrom datetime \nimport date, datetime\nfrom
...
# ReporteMorosidad,)logger = logging.getLogger(__name__)router = APIRouter()@router.get("/cartera",
...
# ).scalar() or Decimal("0") # Préstamos activos total_activos = ( db.query(Prestamo) .filter(Prestamo.estado ==
# EstadoPrestamo.ACTIVO) .count() ) # Préstamos en mora total_mora = ( db.query(Prestamo) .filter(Prestamo.estado ==
# EstadoPrestamo.EN_MORA) .count() ) # Tasa de morosidad tasa_morosidad = ( (total_mora / total_activos * 100) if
# total_activos > 0 else 0 ) # Distribución por rango de montos distribucion = ( db.query(
...
# total_prestamos_activos=total_activos, total_prestamos_mora=total_mora, tasa_morosidad=round(tasa_morosidad, 2),
# distribucion_montos=[ {"rango":\n d[0], "cantidad":\n d[1], "monto":\n d[2]} for d in distribucion ],
# )@router.get("/morosidad", response_model=ReporteMorosidad)\ndef reporte_morosidad( dias_mora_minimo:\n int = 1, db:\n
# Session = Depends(get_db)):\n """ Genera reporte detallado de morosidad. """ hoy = date.today() # Préstamos en mora por
# rango de días rangos_mora = [ {"nombre":\n "1-30 días", "min":\n 1, "max":\n 30}, {"nombre":\n "31-60 días", "min":\n 31,
# "max":\n 60}, {"nombre":\n "61-90 días", "min":\n 61, "max":\n 90}, {"nombre":\n "Más de 90 días", "min":\n 91, "max":\n
# 999999}, ] detalle_rangos = [] for rango in rangos_mora:\n prestamos = ( db.query(Prestamo) .filter( Prestamo.estado ==
# EstadoPrestamo.EN_MORA, Prestamo.dias_mora >= rango["min"], Prestamo.dias_mora <= rango["max"], ) .all() ) cantidad =
# len(prestamos) monto_mora = sum(p.saldo_pendiente for p in prestamos) detalle_rangos.append( { "rango":\n rango["nombre"],
...
# return ReporteMorosidad( fecha_reporte=hoy, total_prestamos_mora=total_mora, monto_total_mora=monto_total_mora,
# detalle_por_rango=detalle_rangos, )@router.get("/cobranza", response_model=ReporteCobranza)\ndef reporte_cobranza(
# fecha_inicio:\n date, fecha_fin:\n date, db:\n Session = Depends(get_db)):\n """ Genera reporte de gestión de cobranza. """
# # Pagos recibidos en el período pagos = ( db.query(Pago) .filter(Pago.fecha_pago >= fecha_inicio, Pago.fecha_pago <=
# fecha_fin) .all() ) total_recaudado = sum(p.monto for p in pagos) cantidad_pagos = len(pagos) # Pagos por concepto
...
# total_recaudado=total_recaudado, cantidad_pagos=cantidad_pagos, promedio_pago=( total_recaudado / cantidad_pagos if
# cantidad_pagos > 0 else Decimal("0") ), detalle_por_concepto=[ {"concepto":\n c[0], "cantidad":\n c[1], "monto":\n c[2]}
# for c in por_concepto ], eficiencia_cobranza=round(eficiencia, 2), )\ndef _crear_estilos_excel():\n """Crear estilos para
...
# ws.append(["Fecha:\n", date.today().strftime("%d/%m/%Y"), "", ""]) ws.append([]) headers = [ "ID", "Cliente", "Monto
# Total", "Saldo Pendiente", "Estado", "Días Mora", ] ws.append(headers) # Aplicar estilo a encabezados for cell in ws[4]:\n
# cell.fill = header_fill cell.font = header_font prestamos = ( db.query(Prestamo) .filter( Prestamo.estado.in_(
# [EstadoPrestamo.ACTIVO, EstadoPrestamo.EN_MORA] ) ) .all() ) for p in prestamos:\n ws.append( [ p.id, f"{p.cliente.nombres}
# {p.cliente.apellidos}", float(p.monto_total), float(p.saldo_pendiente), p.estado.value, p.dias_mora or 0, ] )\ndef
# _crear_reporte_pagos( ws, header_fill, header_font, fecha_inicio, fecha_fin, db):\n """Crear reporte de pagos""" ws.title =
# "Reporte de Pagos" ws.append(["Reporte de Pagos", "", "", ""]) ws.append(["Período:\n", f"{fecha_inicio} - {fecha_fin}",
# "", ""]) ws.append([]) headers = [ "Fecha", "Préstamo ID", "Cliente", "Monto", "Concepto", "Referencia", ]
# ws.append(headers) for cell in ws[4]:\n cell.fill = header_fill cell.font = header_font pagos = ( db.query(Pago)
# .filter(Pago.fecha_pago >= fecha_inicio, Pago.fecha_pago <= fecha_fin) .all() ) for p in pagos:\n ws.append( [
# p.fecha_pago.strftime("%d/%m/%Y"), p.prestamo_id, f"{p.prestamo.cliente.nombres} {p.prestamo.cliente.apellidos}",
...
# no está instalado. Ejecuta:\n pip install openpyxl", ) try:\n wb = openpyxl.Workbook() ws = wb.active # Crear estilos
# header_fill, header_font = _crear_estilos_excel() # Crear reporte según tipo if tipo_reporte == "cartera":\n
# _crear_reporte_cartera(ws, header_fill, header_font, db) elif tipo_reporte == "pagos":\n _crear_reporte_pagos( ws,
...
# f"reporte_{tipo_reporte}_{date.today().strftime('%Y%m%d')}.xlsx" ) return StreamingResponse( output,
# media_type="application/vnd.openxmlformats-officedocument. \ spreadsheetml.sheet", headers={ "Content-Disposition":\n
# f"attachment;\n filename={filename}" }, ) except Exception as e:\n raise HTTPException( status_code=500, detail=f"Error
# generando Excel:\n {str(e)}" )@router.get("/clientes-top")\ndef clientes_top(limite:\n int = 10, db:\n Session =
# Depends(get_db)):\n """ Obtiene los top clientes por monto prestado. """ resultado = ( db.query( Cliente,
# func.count(Prestamo.id).label("total_prestamos"), func.sum(Prestamo.monto_total).label("monto_total"), ) .join(Prestamo)
...
# - Datos del cliente - Tabla de amortización - Historial de pagos - Saldo pendiente """ try:\n # Obtener datos del cliente
...
# pagesize=letter) # Encabezado p.setFont("Helvetica-Bold", 16) p.drawString(50, 750, "ESTADO DE CUENTA") # Datos del cliente
# p.setFont("Helvetica", 12) y_pos = 700 p.drawString(50, y_pos, f"Cliente:\n {cliente.nombre_completo}") p.drawString(50,
# y_pos - 20, f"Cédula:\n {cliente.cedula}") p.drawString(50, y_pos - 40, f"Teléfono:\n {cliente.telefono or 'N/A'}")
# p.drawString(50, y_pos - 60, f"Vehículo:\n {cliente.vehiculo_completo}") # Resumen financiero resumen =
# cliente.calcular_resumen_financiero(db) y_pos = 600 p.setFont("Helvetica-Bold", 14) p.drawString(50, y_pos, "RESUMEN
# FINANCIERO") p.setFont("Helvetica", 12) p.drawString( 50, y_pos - 30, f"Total Financiado:\n
...
# {resumen['cuotas_t \ otales']}", ) p.drawString( 50, y_pos - 110, f"% Avance:\n {resumen['porcentaje_avance']}%" )
...
# Session = Depends(get_db)):\n """ 2. Tabla de amortización por cliente (PDF) - Plan de pagos completo - Fechas de
# vencimiento - Montos por cuota - Estado de cada cuota """ try:\n \nfrom reportlab.lib \nimport colors \nfrom
...
# SimpleDocTemplate(buffer, pagesize=A4) styles = getSampleStyleSheet() story = [] # Título title = Paragraph( f"<b>TABLA DE
...
# "❌", }.get(cuota.estado, "❓") data.append( [ str(cuota.numero_cuota), cuota.fecha_vencimiento.strftime("%d/%m/%Y"),
...
# detail="reportlab no está instalado" )@router.get("/cobranza-diaria/pdf")async \ndef reporte_cobranza_diaria_pdf( fecha:\n
# Optional[date] = Query( None, description="Fecha del reporte (default:\n hoy)" ), db:\n Session = Depends(get_db),):\n """
# 2. Reporte de cobranza diaria (PDF/Excel) - Pagos recibidos hoy - Vencimientos de hoy - Pendientes del día """ if not
# fecha:\n fecha = date.today() # Obtener datos con joins explícitos pagos_hoy = ( db.query(Pago) .select_from(Pago)
# .join(Prestamo, Pago.prestamo_id == Prestamo.id) .join(Cliente, Prestamo.cliente_id == Cliente.id) .filter(Pago.fecha_pago
# == fecha, Pago.estado != "ANULADO") .all() ) vencimientos_hoy = ( db.query(Cuota) .select_from(Cuota) .join(Prestamo,
...
# description="Modelos separados por coma" ), estado:\n Optional[str] = Query(None, description="AL_DIA, MORA, TODOS"),
...
# Depends(get_current_user),):\n """ 🔍 Generador de reportes personalizados con filtros """ # Construir query base query =
# db.query(Cliente) # Aplicar filtros if fecha_inicio:\n query = query.filter(Cliente.fecha_registro >= fecha_inicio) if
...
# asesor_ids.split(",")] query = query.filter(Cliente.asesor_id.in_(ids)) if concesionarios:\n concesionarios_list =
# [c.strip() for c in concesionarios.split(",")] query = query.filter(Cliente.concesionario.in_(concesionarios_list)) if
# modelos:\n modelos_list = [m.strip() for m in modelos.split(",")] query =
# query.filter(Cliente.modelo_vehiculo.in_(modelos_list)) if estado and estado != "TODOS":\n if estado == "AL_DIA":\n query =
# query.filter(Cliente.dias_mora == 0) elif estado == "MORA":\n query = query.filter(Cliente.dias_mora > 0) if modalidad:\n
# query = query.filter(Cliente.modalidad_pago == modalidad) # Obtener resultados clientes = query.all() # Construir respuesta
# según columnas seleccionadas resultados = [] for cliente in clientes:\n cliente_data = {"id":\n cliente.id} if
...
# ============================================@router.get("/cartera-mensual/pdf")async \ndef reporte_mensual_cartera_pdf(
...
# mes = datetime.now().month if not anio:\n anio = datetime.now().year # Calcular KPIs del mes inicio_mes = date(anio, mes,
...
# .filter(Cliente.activo, Cliente.estado_financiero == "EN_MORA") .count() ) # Pagos del mes pagos_mes = (
...
# Cobrado", f"${float(pagos_mes):\n,.2f}", "-"], ] kpis_table = Table(kpis_data) kpis_table.setStyle( TableStyle( [
...
# Análisis de mora por rangos mora_data = [ ["Rango de Mora", "Cantidad", "Porcentaje"], [ "0 días (Al día)",
...
# buffer.seek(0) return StreamingResponse( buffer, media_type="application/pdf", headers={ "Content-Disposition":\n
...
# Query(None), db:\n Session = Depends(get_db), current_user:\n User = Depends(get_current_user),):\n """ 5. Reporte por
# asesor (PDF) - Clientes del asesor - Ventas del período - Estado de cobranza - Ranking vs otros asesores - Cartera asignada
# """ try:\n \nfrom reportlab.lib \nimport colors \nfrom reportlab.lib.pagesizes \nimport A4 \nfrom reportlab.lib.styles
...
# styles["Title"] ) story.append(title) story.append(Spacer(1, 30)) # Información del período periodo_info = f"""
...
# resumen_data = [ ["Métrica", "Valor"], ["Total Clientes Asignados", str(len(clientes_asesor))], ["Ventas en el Período",
...
# Lista de clientes (primeros 20) if clientes_asesor:\n clientes_data = [ ["Cliente", "Cédula", "Vehículo", "Estado",
...
# colors.black), ] ) ) story.append( Paragraph( "<b>Clientes Asignados (Top 20)</b>", styles["Heading2"] ) )
# story.append(clientes_table) doc.build(story) buffer.seek(0) return StreamingResponse( buffer,
# media_type="application/pdf", headers={ "Content-Disposition":\n f"attachment;\n filename=reporte_asesor_{asesor."
...
# verificar_reportes_pdf_implementados( current_user:\n User = Depends(get_current_user),):\n """ 📋 Verificación completa de
# reportes PDF implementados """ return { "titulo":\n "✅ REPORTES PDF CONFIRMADOS", "fecha_verificacion":\n
...
# "descripcion":\n ( "Datos del cliente, resumen financiero, tabla de amortización con estado, historial de pagos, saldo
...
# /api/v1/reportes/tabla-amortizacion/{cliente_id}/pdf", "descripcion":\n ( "Plan de pagos completo, fechas de vencimiento,
# montos por cuota, estado de cada cuota" ), "implementado":\n True, "formato":\n "PDF", "ejemplo_url":\n
# "/api/v1/reportes/tabla-amortizacion/123/pdf", }, "3_cobranza_diaria":\n { "nombre":\n "✅ Reporte de cobranza diaria",
# "endpoint":\n "GET /api/v1/reportes/cobranza-diaria/pdf", "descripcion":\n ( "Pagos recibidos del día, vencimientos del
# día, pagos pendientes, resumen de efectividad" ), "implementado":\n True, "formato":\n "PDF/JSON", "ejemplo_url":\n
...
# estado de cobranza, ranking vs otros asesores, cartera asignada" ), "implementado":\n True, "formato":\n "PDF",
# "ejemplo_url":\n "/api/v1/reportes/asesor/1/pdf", }, }, "caracteristicas_implementadas":\n { "generacion_pdf":\n "✅ Usando
# reportlab con tablas profesionales", "descarga_directa":\n "✅ StreamingResponse con headers apropiados",
# "estilos_profesionales":\n "✅ Colores, fuentes y formato empresarial", "datos_dinamicos":\n "✅ Consultas en tiempo real a
# la base de datos", "filtros_por_rol":\n "✅ Respeta permisos de acceso por rol", "manejo_errores":\n "✅ Validaciones y
# mensajes de error apropiados", }, "dependencias_requeridas":\n { "reportlab":\n "Para generación de PDFs", "nota":\n "Si
# reportlab no está instalado, se devuelve error 500 con mensaje claro", }, "ejemplos_uso":\n { "estado_cuenta":\n ( "curl -X
# GET 'https:\n//pagos-f2qf.onrender.com/api/v1/reportes/estado-cuenta/123/pdf' -H 'Authorization:\n Bearer TOKEN'" ),
# "tabla_amortizacion":\n ( "curl -X GET 'https:\n//pagos-f2qf.onrender.com/api/v1/reportes/tabla-amortizacion/123/pdf' -H
