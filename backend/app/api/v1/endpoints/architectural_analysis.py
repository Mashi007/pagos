...
class ArchitecturalAnalysisSystem:
    ...
    def __init__(self):
        ...
        self.failure_patterns = defaultdict(list)  # Patrones de fallo por componente
    ...
    def _initialize_system_components(self):
        ...
        self.system_components = {
            'jwt_handler': {
                ...
                'health_checks': ['token_creation', 'token_validation', 'token_decoding']
            },
            'database_layer': {
                'name': 'Database Layer',
                'description': 'Capa de acceso a base de datos',
                'dependencies': ['database_connection', 'sqlalchemy_session'],
                'health_checks': ['connection_test', 'query_performance', 'transaction_integrity']
            },
            'authentication_middleware': {
                'name': 'Authentication Middleware',
                'description': 'Middleware de autenticación',
                'dependencies': ['jwt_handler', 'user_model'],
                'health_checks': ['token_extraction', 'user_validation', 'permission_check']
            },
            'user_model': {
                'name': 'User Model',
                'description': 'Modelo de usuario',
                'dependencies': ['database_layer'],
                'health_checks': ['user_lookup', 'user_validation', 'permission_verification']
            },
            'api_endpoints': {
                'name': 'API Endpoints',
                'description': 'Endpoints de la API',
                'dependencies': ['authentication_middleware', 'database_layer'],
                'health_checks': ['endpoint_availability', 'response_time', 'error_handling']
            },
            'frontend_integration': {
                'name': 'Frontend Integration',
                'description': 'Integración con frontend',
                'dependencies': ['api_endpoints', 'cors_config'],
                'health_checks': ['cors_headers', 'response_format', 'error_propagation']
            }
        }
    ...
    def _perform_health_check(self, component_id: str, check_name: str) -> Dict[str, Any]:
        ...
        if component_id == 'jwt_handler':
            ...
            return {'status': 'unknown', 'score': 0, 'error': 'Componente no reconocido'}
        ...
        if check_name == 'token_creation':
            ...
            try:
                ...
                try:
                    ...
                    return {'status': 'error', 'score': 0, 'error': 'Token malformado no detectado'}
                except:
                    return {
                        'status': 'success',
                        'score': 1.0,
    ...
    def analyze_component_dependencies(self) -> Dict[str, Any]:
        ...
        critical_components = ['jwt_handler', 'database_layer', 'authentication_middleware']
