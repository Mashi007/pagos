...
def mover_a_pagos_normales(
    ...
    for idx, pid in enumerate(ids, start=1):
        ...
        # Aislar cada fila con SAVEPOINT: si una explota (constraint, cascada, etc.)
        # se revierte solo esa fila, sin perder movimientos previos del mismo lote.
        row_movido = 0
        row_cuotas_aplicadas = 0
        row_ya_cargado_eliminado: dict[str, Any] | None = None
        try:
            ...
            if row_ya_cargado_eliminado is not None:
                ya_cargado_eliminados.append(row_ya_cargado_eliminado)
            if row_movido:
                movidos += row_movido
                cuotas_aplicadas += row_cuotas_aplicadas
