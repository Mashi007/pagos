...
def limpiar_pagos_con_error_ya_cargados(
    ...
    Aislamiento por fila: si una falla, se revierte solo su savepoint y se sigue.
    """
    _ = current_user
    ...
    for row in rows:
        pid = int(row.id)
        try:
            detalle_eliminado: dict[str, Any] | None = None
            ...
            if detalle_eliminado is not None:
                detalles.append(detalle_eliminado)
                eliminados += 1
...
def mover_a_pagos_normales(
    ...
    for idx, pid in enumerate(ids, start=1):
        ...
        # Aislar el procesamiento de cada fila con savepoint: si una explota (constraint,
        # cascada, etc.) revertimos solo esa fila y seguimos con las demás.
        try:
            ya_cargado_info: dict[str, Any] | None = None
            nuevo_pago_id: int | None = None
            cc_aplicadas = 0
            cp_aplicadas = 0
            pago_movido = False
            ...
            if ya_cargado_info is not None:
