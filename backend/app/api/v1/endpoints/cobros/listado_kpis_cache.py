...
_COBROS_LISTADO_KPIS_CACHE_TTL_SEC = 1200  # 20 minutos
...
_cobros_listado_kpis_mutation_generation = 0
...
def _cobros_listado_kpis_cache_generation() -> int:
    """Generación actual; cambia tras toda mutación que invalida el listado."""
    with _cobros_listado_kpis_mem_lock:
        return _cobros_listado_kpis_mutation_generation


def _cobros_listado_kpis_cache_set(
    cache_payload: str,
    data: dict,
    *,
    ...
    """
    Publica un snapshot solo si ninguna mutación ocurrió desde que empezó el cálculo.

    La comprobación y la escritura comparten el lock con la invalidación. Así un
    cálculo lento nunca puede reponer filas anteriores después de un commit.
    """
    key = _cobros_listado_kpis_storage_key(cache_payload)
    ...
    with _cobros_listado_kpis_mem_lock:
        if (
            ...
            and _cobros_listado_kpis_mutation_generation != expected_generation
        ):
            return False
        if redis_client is not None:
            try:
...
def _cobros_listado_kpis_cache_delete(cache_payload: str) -> None:
    """Elimina el snapshot fresco y stale de una consulta concreta."""
    key = _cobros_listado_kpis_storage_key(cache_payload)
    stale_key = _cobros_listado_kpis_stale_storage_key(cache_payload)
    with _cobros_listado_kpis_mem_lock:
        _cobros_listado_kpis_mem_cache.pop(key, None)
        _cobros_listado_kpis_mem_stale_cache.pop(stale_key, None)

    redis_client = get_redis_client()
    if redis_client is None:
        return
    try:
        redis_client.delete(key, stale_key)
    except Exception as e:
        logger.warning("[COBROS_CACHE] Redis delete listado-y-kpis falló: %s", e)


def _invalidate_cobros_listado_kpis_cache() -> None:
    global _cobros_listado_kpis_mutation_generation
    with _cobros_listado_kpis_mem_lock:
        _cobros_listado_kpis_mutation_generation += 1
        _cobros_listado_kpis_mem_cache.clear()
        _cobros_listado_kpis_mem_stale_cache.clear()
...
def _drop_pagos_from_listado_kpis_cache(
    ...
    *,
    ...
    if not ids:
        return
    # Esta función se llama después del commit de aprobar/rechazar/eliminar. Además
    # de parchear snapshots existentes, invalida cualquier cálculo que haya empezado
    # antes del commit para impedir que vuelva a publicar las filas eliminadas.
    global _cobros_listado_kpis_mutation_generation
    with _cobros_listado_kpis_mem_lock:
        _cobros_listado_kpis_mutation_generation += 1
