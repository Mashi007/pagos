# backend/app/api/v1/endpoints/modelos_vehiculos.py"""Endpoints para gestión de modelos de vehículos"""\nimport logging\nfrom
...
# listar_modelos_vehiculos( # Paginación page:\n int = Query(1, ge=1, description="Número de página"), limit:\n int =
# Query(20, ge=1, le=1000, description="Tamaño de página"), # Búsqueda search:\n Optional[str] = Query(None,
# description="Buscar en modelo"), # Filtros activo:\n Optional[bool] = Query( None, description="Filtrar por estado activo"
# ), db:\n Session = Depends(get_db), current_user:\n User = Depends(get_current_user),):\n """ 📋 Listar modelos de vehículos
# con paginación y filtros """ try:\n # Construir query base query = db.query(ModeloVehiculo) # Aplicar filtros if search:\n
...
# total total = query.count() # Paginación offset = (page - 1) * limit modelos = query.offset(offset).limit(limit).all() #
# Serializar respuesta modelos_response = [ ModeloVehiculoResponse.model_validate(modelo) for modelo in modelos ] return
# ModeloVehiculoListResponse( items=modelos_response, total=total, page=page, page_size=limit, total_pages=(total + limit -
# 1) // limit, ) except Exception as e:\n logger.error(f"Error listando modelos de vehículos:\n {e}") raise HTTPException(
# status_code=500, detail="Error interno del servidor" )@router.get("/activos",
# response_model=List[ModeloVehiculoResponse])\ndef listar_modelos_activos( db:\n Session = Depends(get_db), # TEMPORALMENTE
# SIN AUTENTICACIÓN PARA DROPDOWNS # current_user:\n User = Depends(get_current_user)):\n """ 📋 Listar solo modelos de
# vehículos activos (para formularios) """ try:\n modelos = ( db.query(ModeloVehiculo) .filter(ModeloVehiculo.activo)
# .order_by(ModeloVehiculo.modelo) .all() ) return [ ModeloVehiculoResponse.model_validate(modelo) for modelo in modelos ]
...
# detail="Error interno del servidor" )@router.post("/", response_model=ModeloVehiculoResponse)\ndef crear_modelo_vehiculo(
...
# un modelo de vehículo con ese nombre", ) # Actualizar campos if modelo_data.modelo is not None:\n modelo.modelo =
...
# # HARD DELETE - eliminar completamente de la base de datos modelo_nombre = modelo.modelo # Guardar nombre para log
# db.delete(modelo) db.commit() logger.info( f"Modelo de vehículo ELIMINADO COMPLETAMENTE:\n {modelo_nombre} (ID:\n
# {modelo_id})" ) return { "message":\n "Modelo de vehículo eliminado completamente de la base de datos" } except
