...
@router.post(
    "/prestamo/{prestamo_id}/crear-cuotas", response_model=List[CuotaResponse]
)
...
@router.get(
    "/prestamo/{prestamo_id}/cuotas", response_model=List[CuotaResponse]
)
...
@router.post(
    "/prestamo/{prestamo_id}/recalcular-mora",
...
def recalcular_mora(
    ...
    try:
        ...
        return RecalcularMoraResponse(
            ...
            mensaje=f"Se recalculó la"
            + f"mora de {resultado['cuotas_ \
            actualizadas']} cuotas",
        )
...
@router.get(
    "/prestamo/{prestamo_id}/estado-cuenta",
...
@router.post(
    "/prestamo/{prestamo_id}/proyeccion-pago",
    response_model=ProyeccionPagoResponse,
)
def proyectar_pago(
    prestamo_id: int,
...
def obtener_informacion_adicional(
    ...
    todas_cuotas = (
        db.query(Cuota).filter(Cuota.prestamo_id == prestamo_id).all()
    )
    ...
    if proxima_cuota:
        dias_hasta_vencimiento = (
            proxima_cuota.fecha_vencimiento - date.today()
        ).days
        proximo_vencimiento = {
    ...
    return {
        ...
        "financiero": {
            ...
            "porcentaje_pagado": (
                round(
                    (float(total_pagado) / float(prestamo.monto_total) * 100),
                    2,
                )
        ...
        tabla_visual.append(
            {
                ...
                "fecha_vencimiento": cuota.fecha_vencimiento.strftime(
                    "%d/%m/%Y"
                ),
