...
def _sanear_cedula_ingresada(cedula: str) -> str:
    """NFKC + mayúsculas; solo V, E, G, J y dígitos (puntos, comas, espacios, etc. se descartan)."""
    if not cedula:
        return ""
    s = unicodedata.normalize("NFKC", cedula.strip()).upper()
    s = re.sub(r"[\u200B-\u200D\uFEFF]", "", s)
...
def validate_cedula(cedula: str) -> dict[str, Any]:
    ...
    cedula_clean = _sanear_cedula_ingresada(cedula)
    ...
    if not re.match(r"^[VEGJ]?\d+$", cedula_clean):
        return {
            ...
            "error": "Solo se permiten letra V, E, G o J y números. Revise el texto ingresado.",
        }
