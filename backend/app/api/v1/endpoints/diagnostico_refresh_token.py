"""Endpoint de diagnÃ³stico especÃ­fico para problemas de refresh token"""
import logging
from datetime 
import datetime
import jwt
...
def diagnosticar_refresh_token(    request:
 ...
 Session = Depends(get_db)):
    """    ðŸ” DiagnÃ³stico especÃ­fico para problemas de refresh token    """    try:
        logger.info("ðŸ” Iniciando diagnÃ³stico de refresh token")        # Obtener refresh token del body        body = await request.json()        refresh_token = body.get("refresh_token")        if not refresh_token:
            return {                "timestamp":
 datetime.now().isoformat(),                "status":
 "error",                "error":
 "No refresh token provided",                "recomendacion":
 (                    "Verificar que el frontend estÃ© enviando el refresh_token \                    correctamente"                ),            }        logger.info(f"ðŸ” Refresh token recibido:
 {refresh_token[:
20]}...")        # 1. Verificar formato del token        try:
            # Decodificar sin verificar para obtener informaciÃ³n bÃ¡sica            payload_unverified = jwt.decode(                refresh_token, options={"verify_signature":
 ...
 payload_unverified.get("iat"),            }            # Verificar si estÃ¡ expirado            if payload_unverified.get("exp"):
 ...
 {str(e)}",                "recomendacion":
 "El refresh token tiene un formato invÃ¡lido",            }        # 2. Verificar con decode_token (con verificaciÃ³n)        try:
 ...
 {str(e)}",                    "token_info":
 token_info,                    "recomendacion":
 "El refresh token es invÃ¡lido o    corrupto.",                }        # 3. Verificar tipo de token        if payload_verified.get("type") != "refresh":
            return {                "timestamp":
 datetime.now().isoformat(),                "status":
 "error",                "error":
 "Token type is not 'refresh'",                "token_info":
 token_info,                "recomendacion":
 "El token enviado no es un refresh token    vÃ¡lido.",            }        # 4. Verificar usuario en BD        user_id = payload_verified.get("sub")        if not user_id:
            return {                "timestamp":
 datetime.now().isoformat(),                "status":
 "error",                "error":
 "No user ID in token",                "token_info":
 token_info,                "recomendacion":
 "El refresh token no contiene un user_id    vÃ¡lido.",            }        user = db.query(User).filter(User.id == int(user_id)).first()        if not user:
            return {                "timestamp":
 datetime.now().isoformat(),                "status":
 "error",                "error":
 ...
 "El usuario estÃ¡ inactivo. Contactar    administrador.",            }        # 5. Intentar generar nuevos tokens        try:
 ...
 datetime.now().isoformat(),                "status":
 "success",                "message":
 "Refresh token vÃ¡lido y nuevos tokens generados",                "token_info":
 token_info,                "user_info":
 {                    "email":
 user.email,                    "active":
 user.is_active,                    "admin":
 ...
 (                    "El refresh token es vÃ¡lido. El problema puede estar    en el frontend."                ),            }        except Exception as e:
            return {                "timestamp":
 datetime.now().isoformat(),                "status":
 "error",                "error":
 ...
 "Error interno al generar nuevos tokens.",            }    except Exception as e:
        logger.error(f"âŒ Error en diagnÃ³stico de refresh token:
 {e}")        return {            "timestamp":
 datetime.now().isoformat(),            "status":
 "error",            "error":
 f"Error interno:
...
def estado_refresh_tokens(    db:
 ...
 User = Depends(get_current_user),):
    """    ðŸ“Š Estado general de los refresh tokens en el sistema    """    try:
        logger.info(            f"ðŸ“Š Verificando estado de refresh tokens -     Usuario:
 {current_user.email}"        )        # InformaciÃ³n del usuario actual        user_info = {            "id":
 ...
 7,            },            "recomendacion":
 (                "Tokens generados correctamente. Verificar configuraciÃ³n    del frontend."            ),        }    except Exception as e:
        logger.error(f"âŒ Error verificando estado de refresh tokens:
