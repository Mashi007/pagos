"""Sistema de Monitoreo en Tiempo Real para AutenticaciÃ³nAnÃ¡lisis continuo de tokens, requests y patrones de error"""
import logging
import threading
import time
from collections 
...
def __init__(self):
        self.request_history = deque(maxlen=1000)  # Ãšltimos 1000 requests        self.token_analysis = defaultdict(list)  # AnÃ¡lisis por token        self.error_patterns = defaultdict(int)  # Patrones de error        self.performance_metrics = deque(maxlen=100)  # MÃ©tricas de rendimiento        self.active_sessions = {}  # Sesiones activas        self.lock = threading.Lock()        # Iniciar monitoreo en background        self._start_background_monitoring()    
...
def monitor_loop():
 ...
 {e}")                    time.sleep(60)  # Esperar mÃ¡s tiempo si hay error        thread = threading.Thread(target=monitor_loop, daemon=True)        thread.start()        logger.info("ðŸ” Monitor de autenticaciÃ³n en tiempo real iniciado")    
def log_request(self, request_data:
 Dict[str, Any]):
        """Registrar request para anÃ¡lisis"""        with self.lock:
            request_data["timestamp"] = datetime.now()            self.request_history.append(request_data)            # Analizar token si estÃ¡ presente            if "token" in request_data:
                self._analyze_token(request_data["token"], request_data)    
def _analyze_token(self, token:
 str, request_data:
 Dict[str, Any]):
        """Analizar token especÃ­fico"""        try:
 ...
 request_data.get("success", True),            }            self.token_analysis[token_id].append(analysis)            # Mantener solo Ãºltimos 50 anÃ¡lisis por token            if len(self.token_analysis[token_id]) > 50:
...
def _analyze_patterns(self):
        """Analizar patrones en tiempo real"""        with self.lock:
            # Analizar Ãºltimos 5 minutos            cutoff_time = datetime.now() - timedelta(minutes=5)            recent_requests = [                req                for req in self.request_history                if req["timestamp"] > cutoff_time            ]            # Contar errores por tipo            error_counts = defaultdict(int)            for req in recent_requests:
                ...
                # Mantener solo los mÃ¡s frecuentes                sorted_patterns = sorted(                    self.error_patterns.items(),                    key=lambda x:
...
def get_real_time_status(self) -> Dict[str, Any]:
        """Obtener estado en tiempo real"""        with self.lock:
            current_time = datetime.now()            # AnÃ¡lisis de Ãºltimos 5 minutos            cutoff_time = current_time - timedelta(minutes=5)            recent_requests = [                req                for req in self.request_history                if req["timestamp"] > cutoff_time            ]            # EstadÃ­sticas bÃ¡sicas            total_requests = len(recent_requests)            failed_requests = len(                [                    req                    for req in recent_requests                    if not req.get("success", True)                ]            )            success_rate = (                ((total_requests - failed_requests) / total_requests * 100)                if total_requests > 0                else 100            )            # AnÃ¡lisis de tokens            active_tokens = len(self.token_analysis)            expiring_tokens = 0            for token_id, analyses in self.token_analysis.items():
                if analyses:
...
def _calculate_peak_error_rate(self) -> float:
        """Calcular tasa mÃ¡xima de errores"""        if not self.performance_metrics:
...
def get_real_time_status():
    """    ðŸ“Š Estado en tiempo real del sistema de autenticaciÃ³n    """    try:
...
def analyze_user_tokens(    user_id:
 ...
 User = Depends(get_current_user),):
    """    ðŸ” AnÃ¡lisis detallado de tokens de un usuario especÃ­fico    """    try:
        # Verificar que el usuario existe        user = db.query(User).filter(User.id == user_id).first()        if not user:
            return {                "timestamp":
 datetime.now().isoformat(),                "status":
 "error",                "error":
 "Usuario no encontrado",            }        # Buscar anÃ¡lisis de tokens para este usuario        user_token_analyses = []        for token_id, analyses in auth_monitor.token_analysis.items():
            if analyses and str(analyses[0]["user_id"]) == str(user_id):
                user_token_analyses.extend(analyses)        # Ordenar por tiempo mÃ¡s reciente        user_token_analyses.sort(key=lambda x:
 x["request_time"], reverse=True)        # AnÃ¡lisis estadÃ­stico        total_requests = len(user_token_analyses)        successful_requests = len(            [a for a in user_token_analyses if a.get("success", True)]        )        success_rate = (            (successful_requests / total_requests * 100)            if total_requests > 0            else 0        )        # Endpoints mÃ¡s usados        endpoint_usage = defaultdict(int)        for analysis in user_token_analyses:
...
10]                ),                "recent_requests":
 user_token_analyses[                    :
20                ],  # Ãšltimos 20 requests            },        }    except Exception as e:
...
def log_request_data(request:
 ...
 Dict[str, Any]):
    """    ðŸ“ Endpoint para registrar datos de request (usado por middleware)    """    try:
        # Agregar informaciÃ³n adicional del request        enhanced_data = {            **request_data,            "client_ip":
...
def get_error_patterns():
    """    ðŸš¨ Patrones de error detectados    """    try:
...
def get_performance_metrics():
    """    âš¡ MÃ©tricas de rendimiento del sistema    """    try:
        status = auth_monitor.get_real_time_status()        return {            "timestamp":
 datetime.now().isoformat(),            "status":
 "success",            "metrics":
 ...
 _generate_performance_recommendations(status),        }    except Exception as e:
        logger.error(f"Error obteniendo mÃ©tricas de rendimiento:
 {e}")        return {            "timestamp":
 datetime.now().isoformat(),            "status":
 "error",            "error":
 str(e),        }
def _generate_performance_recommendations(status:
 Dict[str, Any]) -> List[str]:
    """Generar recomendaciones basadas en mÃ©tricas"""    recommendations = []    metrics = status.get("metrics", {})    success_rate = metrics.get("success_rate_percent", 100)    if success_rate < 95:
        recommendations.append(            f"âš ï¸ Tasa de Ã©xito baja"            + f"({success_rate}%) - Revisar c \            onfiguraciÃ³n de tokens"        )    expiring_tokens = metrics.get("expiring_tokens", 0)    if expiring_tokens > 0:
        recommendations.append(            f"ðŸ”„ {expiring_tokens} tokens expirando pronto - Verificar  \            auto-refresh"        )    avg_response_time = status.get("performance", {}).get(        "avg_response_time", 0    )    if avg_response_time > 2.0:
        recommendations.append(            f"ðŸŒ Tiempo de respuesta alto ({avg_response_time}s) -     Optimizar queries"        )    if not recommendations:
        recommendations.append("âœ… Sistema funcionando correctamente")    return recommendations
