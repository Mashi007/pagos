...
def _generar_recomendaciones_configuracion(
    estado_servicios: Dict[str, Any],
) -> List[str]:
    """Generar recomendaciones de configuración"""
    return ["Recomendación 1", "Recomendación 2"]


def _generar_alertas_configuracion(
    db: Session, estado_categorias: Dict[str, Any]
) -> List[Dict[str, Any]]:
...
def obtener_configuracion_validadores():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error obteniendo configuración de validadores: {str(e)}",
        )


@router.post("/validadores/probar")
def probar_validadores(datos_prueba: Dict[str, Any]):
    """
    ...
    try:
        ...
        if "telefono" in datos_prueba:
            ...
            resultados["telefono"] = ValidadorTelefono.validar_y_formatear_telefono(
                telefono, pais
            )
        ...
        return {
            ...
            "resumen": {
                ...
                "invalidos": sum(
                    1 for r in resultados.values() if not r.get("valido", False)
                ),
            },
        }
...
def obtener_configuracion_categoria(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error obteniendo configuración de categoría: {str(e)}",
        )
    ...
    try:
        ...
        for categoria, configs in configuraciones.items():
            for clave, nuevo_valor in configs.items():
                try:
                    ...
                    if not config:
                        errores.append(
                            f"Configuración {categoria}.{clave} no encontrada"
                        )
                        continue

                    if config.solo_lectura:
                        errores.append(
                            f"Configuración {categoria}.{clave} es de solo lectura"
                        )
                        continue
        ...
        return {
            "mensaje": (
                ...
                else "❌ No se pudo actualizar ninguna configuración"
            ),
        ...
        db.rollback()
        raise HTTPException(
            status_code=500, detail=f"Error actualizando configuraciones: {str(e)}"
        )
...
def obtener_estado_servicios(
    ...
    try:
        ...
        if not current_user.is_admin:
            raise HTTPException(
                status_code=403,
                detail="Solo administradores pueden ver estado de servicios",
            )
        ...
        estado_servicios = {
            "ai": {
                ...
                "estado": (
                    "✅ ACTIVO" if ConfigHelper.is_ai_enabled(db) else "❌ INACTIVO"
                ),
            },
            "email": {
            ...
            "whatsapp": {
                ...
                "configurado": bool(
                    ConfigHelper.get_config(db, "WHATSAPP", "META_ACCESS_TOKEN")
                ),
                "estado": (
                    "✅ ACTIVO"
        ...
        return {
            ...
            "estado_general": {
                ...
                "estado": (
                    ...
                    else "⚠️ PARCIAL" if servicios_activos > 0 else "❌ CRÍTICO"
                ),
            },
...
def inicializar_configuraciones_default(
    ...
    if not current_user.is_admin:
        raise HTTPException(
            status_code=403,
            detail="Solo administradores pueden inicializar configuraciones",
        )
...
def obtener_configuracion_ia(
    ...
    try:
        ...
        if not current_user.is_admin:
            raise HTTPException(
                status_code=403,
                detail="Solo administradores pueden ver configuración de IA",
            )
        configs_ia = (
            db.query(ConfiguracionSistema)
            .filter(
        ...
        for config in configs_ia:
            ...
            configuracion[config.clave] = {
                "valor": valor_mostrar,
                "valor_real": (
                    config.valor_procesado if config.tipo_dato != "PASSWORD" else None
                ),
                "descripcion": config.descripcion,
                "tipo_dato": config.tipo_dato,
                "requerido": config.requerido,
        ...
        estado_ia = {
            ...
            "prediccion_mora": {
                "habilitado": configuracion.get("AI_PREDICTION_ENABLED", {}).get(
                    "valor", False
                ),
        ...
        raise HTTPException(
            status_code=500, detail=f"Error obteniendo configuración IA: {str(e)}"
        )
...
def obtener_configuracion_email(
    ...
    try:
        ...
        for config in configs_email:
            ...
            if config.tipo_dato == "PASSWORD" and config.valor:
                valor_mostrar = (
                    "****" + config.valor[-4:] if len(config.valor) > 4 else "****"
                )
...
def actualizar_configuracion_email(
    ...
    if not current_user.is_admin:
        raise HTTPException(
            status_code=403, detail="Solo administradores pueden configurar email"
        )
...
def dashboard_configuracion_sistema(
    ...
    try:
        ...
        configs_configuradas = (
            db.query(ConfiguracionSistema)
            .filter(
                ConfiguracionSistema.valor.isnot(None), ConfiguracionSistema.valor != ""
            )
            .count()
        )
        ...
        for (categoria,) in categorias:
            ...
            porcentaje = (
                round(configuradas_cat / total_cat * 100, 1) if total_cat > 0 else 0
            )
        ...
        return {
            ...
            "servicios_criticos": {
                ...
                "ia": {
                    "estado": (
                        "✅ ACTIVA" if ConfigHelper.is_ai_enabled(db) else "❌ INACTIVA"
                    ),
...
def actualizar_configuracion_tasas(
    ...
    if not current_user.is_admin:
        raise HTTPException(
            status_code=403, detail="Solo administradores pueden modificar tasas"
        )
...
def actualizar_configuracion_limites(
    ...
    if not current_user.is_admin:
        raise HTTPException(
            status_code=403, detail="Solo administradores pueden modificar límites"
        )
...
def actualizar_configuracion_notificaciones(
    ...
    if not current_user.is_admin:
        raise HTTPException(
            status_code=403, detail="Sin permisos para modificar notificaciones"
        )
...
def restablecer_configuracion_defecto(
    ...
    if not current_user.is_admin:
        raise HTTPException(
            status_code=403,
            detail="Solo administradores pueden restablecer configuración",
        )
...
def validar_limites_cliente(
    ...
    try:
        ...
        # Validaciones
        validaciones = {"puede_solicitar": True, "mensajes": []}
