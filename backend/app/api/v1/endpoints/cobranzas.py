    ...
    try:
        ...
        query = (
            db.query(
                ...
                func.sum(
                    case((Cuota.fecha_vencimiento < hoy - timedelta(days=30), Cuota.monto_cuota), else_=0)
        ...
        query = query.group_by(
            Cliente.cedula, Cliente.nombres, Cliente.telefono,
            Prestamo.usuario_proponente, Prestamo.id, Prestamo.total_financiamiento
        )
    ...
    try:
        ...
        query = (
            db.query(
                ...
                func.avg(
                    case((Cuota.fecha_vencimiento < hoy, 
...
def informe_antiguedad_saldos(
    ...
    try:
        ...
        query = (
            db.query(
                case(
                    ((func.extract('day', hoy - Cuota.fecha_vencimiento) <= 30), "0-30 días"),
                    ((func.extract('day', hoy - Cuota.fecha_vencimiento) <= 60), "31-60 días"),
                    ((func.extract('day', hoy - Cuota.fecha_vencimiento) <= 90), "61-90 días"),
                    ((func.extract('day', hoy - Cuota.fecha_vencimiento) <= 180), "91-180 días"),
        ...
        orden_rangos = {"0-30 días": 1, "31-60 días": 2, "61-90 días": 3, "91-180 días": 4, "Más de 180 días": 5}
    ...
    try:
        ...
        datos_resumen = {
            ...
            "resumen_general": {
                ...
                "promedio_deuda_cliente": float(monto_total_adeudado / clientes_atrasados) if clientes_atrasados > 0 else 0.0,
            },
            "top_analistas": [
                {
                    "analista": row.analista,
...
def _generar_excel_clientes_atrasados(datos: List[Dict]) -> StreamingResponse:
    ...
    # Encabezados
    headers = ["Cédula", "Nombres", "Teléfono", "Analista", "Préstamo ID", 
               "Total Financiamiento", "Cuotas Vencidas", "Total Adeudado",
               "Fecha Primera Vencida", "Días Retraso", "Monto >30 días"]
...
def _generar_excel_rendimiento_analista(datos: List[Dict]) -> StreamingResponse:
    ...
    headers = ["Analista", "Total Clientes", "Total Préstamos", 
               "Monto Total Adeudado", "Cuotas Vencidas", "Promedio Días Retraso"]
    ...
    for row_idx, registro in enumerate(datos, 2):
        ...
        ws.cell(row=row_idx, column=6, value=round(registro.get("promedio_dias_retraso", 0), 2))
...
def _generar_excel_montos_periodo(datos: List[Dict]) -> StreamingResponse:
    ...
    for row_idx, registro in enumerate(datos, 2):
        ws.cell(row=row_idx, column=1, value=registro.get("mes_display") or registro.get("mes"))
...
def _generar_pdf_resumen_ejecutivo(datos: Dict) -> StreamingResponse:
    ...
    resumen_data = [
        ...
        ["Promedio Deuda por Cliente", f"${resumen.get('promedio_deuda_cliente', 0):,.2f}"],
    ]
    ...
    if datos.get("top_analistas"):
        ...
        analistas_table.setStyle(TableStyle([
            ('BACKGROUND', (0, 0), (-1, 0), colors.HexColor('#366092')),
            ('TEXTCOLOR', (0, 0), (-1, 0), colors.whitesmoke),
            ('ALIGN', (0, 0), (-1, -1), 'LEFT'),
            ('FONTNAME', (0, 0), (-1, 0), 'Helvetica-Bold'),
