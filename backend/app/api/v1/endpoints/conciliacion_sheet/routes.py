...
- POST /conciliacion-sheet/sync — cron externo con header X-Conciliacion-Sheet-Sync-Secret.
- POST /conciliacion-sheet/sync-now — mismo trabajo, con sesión staff (admin / operador / gerente).
- GET /conciliacion-sheet/status — metadatos, última corrida y si el snapshot alcanza para Clientes/Préstamos Drive.
...
def _run_sync_to_db(db: Session) -> Dict[str, Any]:
    """Sync CONCILIACIÓN → BD (snapshot + tabla drive)."""
...
# Columna S = índice 18: hace falta al menos 19 cabeceras (A..S) para Clientes/Préstamos Drive.
_MIN_HEADERS_FOR_DRIVE_SNAPSHOT = 19


def _hoja_snapshot_hint_and_blocker(
    *,
    hoja_snapshot_ready: bool,
    ...
    """
    Texto para operadores (hoja_snapshot_hint) y código estable (hoja_snapshot_blocker)
    cuando el snapshot aún no alcanza para Clientes/Préstamos Drive.
    """
    if hoja_snapshot_ready:
        return None, None

    min_h = _MIN_HEADERS_FOR_DRIVE_SNAPSHOT
    n_headers = len(hdrs)
    ...
    if not headers_ok:
        return (
            f"Las cabeceras guardadas ({n_headers}) no alcanzan la columna S (se necesitan al "
    ...
    if drive_row_count == 0 and snapshot_row_count > 0:
        return (
            "Hay filas en conciliacion_sheet_rows pero la tabla drive está vacía; reintente sync-now.",
            "drive_table_empty",
        )

    return (
        "Aún no es posible usar Clientes/Préstamos Drive. Ejecute sync-now o revise la configuración.",
        "unknown",
    )
...
def _build_operator_checklist(
    *,
    ...
    out: List[str] = [
        ...
        "Con personal autorizado: use el botón «Actualización manual» o POST /api/v1/conciliacion-sheet/sync-now.",
    ]
    ...
    if not scheduled_jobs_enabled:
        out.append(
            "Si ENABLE_AUTOMATIC_SCHEDULED_JOBS=false, los jobs 01:00/02:00 (America/Caracas) no arrancan en el servidor; "
            "use cron externo (Render Cron, etc.) o sync manual."
        )
    else:
        out.append(
            "Con ENABLE_AUTOMATIC_SCHEDULED_JOBS=true el scheduler sincroniza la hoja a las 01:00 (Clientes) y 02:00 (Préstamos) America/Caracas."
        )
    if blocker == "headers_below_S":
        out.append(
            "Amplíe CONCILIACION_SHEET_COLUMNS_RANGE para incluir al menos hasta la columna S y vuelva a sincronizar."
        )
...
def get_conciliacion_sheet_status(
    ...
    headers_ok = len(hdrs) >= _MIN_HEADERS_FOR_DRIVE_SNAPSHOT
    hoja_snapshot_ready = (
        spreadsheet_configured
        and bool(hdrs)
        and headers_ok
        and snapshot_row_count > 0
        and drive_row_count > 0
        and (last_run is None or bool(last_run.success))
    )
    hint, blocker = _hoja_snapshot_hint_and_blocker(
    ...
    logger.info(
        "[conciliacion_sheet] GET /status hoja_snapshot_ready=%s filas_snapshot=%s filas_drive=%s n_headers=%s "
    ...
    return {
        ...
        "hoja_snapshot_ready": hoja_snapshot_ready,
        "hoja_snapshot_blocker": blocker,
        "hoja_snapshot_hint": hint,
