...
def update_user(
    ...
    try:
        ...
        logger.info(
            f"🔍 [UPDATE_USER] Iniciando actualización - user_id={user_id}, email={user.email}, nombre={user.nombre} {user.apellido}"
        )
        ...
        logger.info(
            f"🔍 [UPDATE_USER] Verificación de usuario - user_id={user_id}, user.id={user.id}, user.email={user.email}"
        )
