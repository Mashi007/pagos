"""Sistema de Monitoreo en Tiempo Real EspecÃ­ficoMonitorea especÃ­ficamente los momentos cuando ocurren fallos 401 intermitentes"""
import logging
import threading
import time
from collections 
...
import Userlogger = logging.getLogger(__name__)router = APIRouter()# ============================================# SISTEMA DE MONITOREO EN TIEMPO REAL ESPECÃFICO# ============================================
class RealTimeSpecificMonitor:
    """Monitor especÃ­fico para fallos 401 intermitentes"""    
def __init__(self):
        self.active_monitoring = False        self.monitoring_sessions = {}  # Sesiones de monitoreo activas        self.real_time_events = deque(maxlen=1000)  # Eventos en tiempo real        self.failure_moments = deque(            maxlen=100        )  # Momentos especÃ­ficos de fallo        self.success_moments = deque(            maxlen=100        )  # Momentos especÃ­ficos de Ã©xito        self.lock = threading.Lock()    
def start_specific_monitoring(        self, session_id:
 str, target_endpoints:
 List[str] = None    ) -> Dict[str, Any]:
        """Iniciar monitoreo especÃ­fico"""        with self.lock:
 ...
 0,            }            self.monitoring_sessions[session_id] = monitoring_session            self.active_monitoring = True            logger.info(f"ðŸ” Monitoreo especÃ­fico iniciado:
 {session_id}")            return monitoring_session    
def stop_specific_monitoring(self, session_id:
 str) -> Dict[str, Any]:
        """Detener monitoreo especÃ­fico"""        with self.lock:
            if session_id not in self.monitoring_sessions:
                return {"error":
 "SesiÃ³n no encontrada"}            session = self.monitoring_sessions[session_id]            session["status"] = "stopped"            session["end_time"] = datetime.now()            session["duration_seconds"] = (                session["end_time"] - session["start_time"]            ).total_seconds()            # Si no hay mÃ¡s sesiones activas, detener monitoreo global            active_sessions = [                s                for s in self.monitoring_sessions.values()                if s["status"] == "active"            ]            if not active_sessions:
                self.active_monitoring = False            logger.info(f"â¹ï¸ Monitoreo especÃ­fico detenido:
 {session_id}")            return session    
def capture_auth_event(self, event_type:
 str, event_data:
 Dict[str, Any]):
        """Capturar evento de autenticaciÃ³n en tiempo real"""        if not self.active_monitoring:
 ...
 self._get_session_context(),            }            self.real_time_events.append(event)            # Actualizar contadores de sesiones activas            for session in self.monitoring_sessions.values():
                if session["status"] == "active":
                    session["events_captured"] += 1                    if event_type in [                        "auth_failure",                        "token_expired",                        "validation_failed",                    ]:
                        ...
                        session["successes_captured"] += 1                        self.success_moments.append(event)            logger.debug(f"ðŸ“¡ Evento capturado:
 {event_type}")    
...
def analyze_failure_moments(self) -> Dict[str, Any]:
        """Analizar momentos especÃ­ficos de fallo"""        with self.lock:
...
def _analyze_failure_timing(self) -> Dict[str, Any]:
        """Analizar timing de momentos de fallo"""        if len(self.failure_moments) < 2:
            return {"error":
 "Datos insuficientes para anÃ¡lisis de timing"}        timestamps = [moment["timestamp"] for moment in self.failure_moments]        timestamps.sort()        intervals = []        for i in range(1, len(timestamps)):
...
def _analyze_failure_context(self) -> Dict[str, Any]:
 ...
 contexts[-5:
],  # Ãšltimos 5 contextos        }    
def _analyze_failure_correlations(self) -> Dict[str, Any]:
        """Analizar correlaciones entre fallos y Ã©xitos"""        if not self.success_moments:
            return {"error":
 "No hay momentos de Ã©xito para correlaciÃ³n"}        # Buscar patrones de alternancia entre Ã©xito y fallo        all_moments = []        for moment in self.failure_moments:
...
def get_monitoring_session_details(        self, session_id:
 str    ) -> Dict[str, Any]:
        """Obtener detalles de sesiÃ³n de monitoreo especÃ­fica"""        with self.lock:
            if session_id not in self.monitoring_sessions:
                return {"error":
 "SesiÃ³n no encontrada"}            session = self.monitoring_sessions[session_id]            # Filtrar eventos de esta sesiÃ³n            session_events = []            for event in self.real_time_events:
 ...
 session_events[-20:
],  # Ãšltimos 20 eventos                "total_events_in_session":
 len(session_events),            }# Instancia global del monitor especÃ­ficoreal_time_monitor = RealTimeSpecificMonitor()# ============================================# ENDPOINTS DE MONITOREO ESPECÃFICO# ============================================@router.post("/start-specific-monitoring")async 
def start_specific_monitoring_endpoint(    monitoring_request:
 Dict[str, Any],    db:
 Session = Depends(get_db),    current_user:
 User = Depends(get_current_user),):
    """    ðŸ” Iniciar monitoreo especÃ­fico de fallos 401    """    try:
 ...
 session,        }    except Exception as e:
        logger.error(f"Error iniciando monitoreo especÃ­fico:
 {e}")        return {            "timestamp":
 datetime.now().isoformat(),            "status":
 "error",            "error":
...
def stop_specific_monitoring_endpoint(    session_id:
 ...
 Session = Depends(get_db),    current_user:
 User = Depends(get_current_user),):
    """    â¹ï¸ Detener monitoreo especÃ­fico    """    try:
        session = real_time_monitor.stop_specific_monitoring(session_id)        return {            "timestamp":
 datetime.now().isoformat(),            "status":
 "success",            "monitoring_session":
 session,        }    except Exception as e:
        logger.error(f"Error deteniendo monitoreo especÃ­fico:
 {e}")        return {            "timestamp":
 datetime.now().isoformat(),            "status":
 "error",            "error":
...
def capture_auth_event_endpoint(    event_data:
 ...
 Session = Depends(get_db),    current_user:
 User = Depends(get_current_user),):
    """    ðŸ“¡ Capturar evento de autenticaciÃ³n en tiempo real    """    try:
 ...
 "Evento capturado",        }    except HTTPException:
        ...
        logger.error(f"Error capturando evento de autenticaciÃ³n:
 {e}")        return {            "timestamp":
 datetime.now().isoformat(),            "status":
 "error",            "error":
...
def get_failure_moments_analysis(    db:
 Session = Depends(get_db),    current_user:
 User = Depends(get_current_user),):
    """    ðŸ” AnÃ¡lisis de momentos especÃ­ficos de fallo    """    try:
...
def get_real_time_status_endpoint(    db:
 Session = Depends(get_db),    current_user:
 User = Depends(get_current_user),):
    """    ðŸ“Š Estado del monitoreo en tiempo real    """    try:
...
def get_monitoring_session_details_endpoint(    session_id:
 ...
 Session = Depends(get_db),    current_user:
 User = Depends(get_current_user),):
    """    ðŸ“‹ Detalles de sesiÃ³n de monitoreo especÃ­fica    """    try:
 ...
 details,        }    except Exception as e:
        logger.error(f"Error obteniendo detalles de sesiÃ³n:
 {e}")        return {            "timestamp":
 datetime.now().isoformat(),            "status":
 "error",            "error":
 str(e),        }
