...
class NetworkDiagnosticSystem:
    ...
    def analyze_network_health(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            recent_tests = [
                test for test in self.connection_tests
                if datetime.fromisoformat(test['timestamp']) > cutoff_time
            ]

            recent_latency = [
                test for test in self.latency_history
                if datetime.fromisoformat(test['timestamp']) > cutoff_time
            ]
    ...
    def _analyze_connectivity(self, recent_tests: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        overall_success_rate = sum(stats['success_rate'] for stats in connectivity_stats.values()) / len(connectivity_stats) if connectivity_stats else 0
    ...
    def _analyze_latency(self, recent_latency: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if not recent_latency:
            return {'status': 'no_data', 'message': 'No hay datos de latencia recientes'}
    ...
    try:
        with network_diagnostic.lock:
            ...
            connectivity_stats = {
                ...
                'recent_tests': list(network_diagnostic.connection_tests)[-10:] if network_diagnostic.connection_tests else []
            }
            ...
            if network_diagnostic.latency_history:
                all_latencies = [
                    test['latency_ms'] for test in network_diagnostic.latency_history
                    if test['status'] == 'success' and test['latency_ms'] is not None
                ]
