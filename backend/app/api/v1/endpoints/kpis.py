...
# Depends(get_current_user),):\n """ KPIs principales para el dashboard 📊 KPIs mostrados:\n - 💰 Cartera Total - ✅ Clientes al
# Día - ⚠️ Clientes en Mora - 📈 Tasa de Morosidad - 💸 Cobrado Hoy - 📅 Vencimientos Hoy """ if not fecha_corte:\n fecha_corte
...
# tasa_morosidad = ( (clientes_en_mora / total_clientes * 100) if total_clientes > 0 else 0 ) # 💸 COBRADO HOY cobrado_hoy =
# db.query(func.sum(Pago.monto_pagado)).filter( Pago.fecha_pago == fecha_corte, Pago.estado != "ANULADO" ).scalar() or
# Decimal("0") # 📅 VENCIMIENTOS HOY vencimientos_hoy = ( db.query(Cuota) .filter( Cuota.fecha_vencimiento == fecha_corte,
...
# f"{clientes_en_mora:\n,}", "icono":\n "⚠️", "color":\n "warning", }, "tasa_morosidad":\n { "valor":\n round(tasa_morosidad,
# 2), "formato":\n f"{tasa_morosidad:\n.2f}%", "icono":\n "📈", "color":\n "danger" if tasa_morosidad > 10 else "warning", },
...
# / total_clientes * 100), 2) if total_clientes > 0 else 0 ), "porcentaje_mora":\n round(tasa_morosidad, 2), },
# }@router.get("/financieros")\ndef kpis_financieros( periodo:\n str = Query("mes", description="dia, semana, mes, año"),
# fecha_inicio:\n Optional[date] = Query(None), fecha_fin:\n Optional[date] = Query(None), db:\n Session = Depends(get_db),
# current_user:\n User = Depends(get_current_user),):\n """ 📈 KPIs Financieros - Cartera total - Total cobrado por período -
# Proyección de flujo de caja - Ingresos por período - Tasa de recuperación - Rentabilidad por modalidad """ # Calcular
# fechas según período hoy = date.today() if not fecha_inicio or not fecha_fin:\n if periodo == "dia":\n fecha_inicio =
# fecha_fin = hoy elif periodo == "semana":\n fecha_inicio = hoy - timedelta(days=hoy.weekday()) fecha_fin = fecha_inicio +
# timedelta(days=6) elif periodo == "mes":\n fecha_inicio = hoy.replace(day=1) fecha_fin = (fecha_inicio +
# timedelta(days=32)).replace( day=1 ) - timedelta(days=1) elif periodo == "año":\n fecha_inicio = hoy.replace(month=1,
# day=1) fecha_fin = hoy.replace(month=12, day=31) # Cartera total (saldos pendientes) cartera_total = db.query(
...
# "ANULADO", ).scalar() or Decimal("0") # Proyección próximos 30 días fecha_proyeccion = hoy + timedelta(days=30)
# flujo_proyectado = db.query(func.sum(Cuota.monto_cuota)).filter( Cuota.fecha_vencimiento >= hoy, Cuota.fecha_vencimiento <=
# fecha_proyeccion, Cuota.estado.in_(["PENDIENTE", "PARCIAL"]), ).scalar() or Decimal("0") # Ingresos por período (capital +
# interés + mora) ingresos_capital = db.query(func.sum(Pago.monto_capital)).filter( Pago.fecha_pago >= fecha_inicio,
# Pago.fecha_pago <= fecha_fin ).scalar() or Decimal("0") ingresos_interes = db.query(func.sum(Pago.monto_interes)).filter(
# Pago.fecha_pago >= fecha_inicio, Pago.fecha_pago <= fecha_fin ).scalar() or Decimal("0") ingresos_mora =
...
# User = Depends(get_current_user),):\n """ 👥 KPIs de Cobranza - Tasa de morosidad general - Tasa de morosidad por analista -
# Promedio de días de retraso - Porcentaje de cumplimiento - Top 10 clientes morosos - Evolución mensual """ # Tasa de
# morosidad general total_clientes = db.query(Cliente).filter(Cliente.activo).count() clientes_mora = (
# db.query(Cliente).filter(Cliente.activo, Cliente.dias_mora > 0).count() ) clientes_al_dia = ( db.query(Cliente)
# .filter(Cliente.activo, Cliente.dias_mora == 0) .count() ) tasa_morosidad_general = ( (clientes_mora / total_clientes *
# 100) if total_clientes > 0 else 0 ) # Tasa de morosidad por analista morosidad_por_analista = ( db.query( User.full_name,
...
# .filter(Cliente.activo, Cliente.dias_mora > 0) .scalar() or 0 ) # Porcentaje de cumplimiento de pagos (cuotas pagadas a
...
# clientes morosos top_morosos = ( db.query( Cliente.id, Cliente.nombres, Cliente.apellidos, Cliente.cedula,
# Cliente.dias_mora, Cliente.total_financiamiento, ) .filter(Cliente.activo, Cliente.dias_mora > 0)
# .order_by(Cliente.dias_mora.desc()) .limit(10) .all() ) # Evolución mensual de mora (últimos 6 meses) evolucion_mensual =
# [] for i in range(6):\n mes_fecha = date.today().replace(day=1) - timedelta(days=30 * i) # Simulación - en implementación
# real calcularías histórico evolucion_mensual.append( { "mes":\n mes_fecha.strftime("%Y-%m"), "tasa_morosidad":\n
# round(tasa_morosidad_general - (i * 0.5), 2), "clientes_mora":\n max(0, clientes_mora - (i * 10)), } ) return {
...
# "tasa_morosidad":\n ( round((mora / total * 100), 2) if total > 0 else 0 ), } for analista, total, mora in
# morosidad_por_analista ], "top_clientes_morosos":\n [ { "id":\n cliente_id, "nombre":\n f"{nombres} {apellidos}",
# "cedula":\n cedula, "dias_mora":\n dias_mora, "monto_financiamiento":\n float(monto or 0), } for cliente_id, nombres,
# apellidos, cedula, dias_mora, monto in top_morosos ], "evolucion_mensual":\n evolucion_mensual,
...
# ) .outerjoin(Cliente, Analista.id == Cliente.analista_id) .filter(Analista.activo, Cliente.activo) .group_by(User.id,
# User.full_name) .all() ) # Procesar datos para ranking ranking_ventas = [] ranking_cobranza = [] for analista_id, nombre,
...
# 0 ), }, }@router.get("/productos")\ndef kpis_productos( db:\n Session = Depends(get_db), current_user:\n User =
# Depends(get_current_user),):\n """ 🏍️ KPIs de Producto - Modelo más/menos vendido - Ventas por modelo - Ticket promedio por
...
# Cliente.modelo_vehiculo.isnot(None)) .group_by(Cliente.modelo_vehiculo) .all() ) # Procesar datos modelos_data = [] for
# modelo, marca, ventas, monto, ticket in ventas_por_modelo:\n # Buscar datos de mora para este modelo mora_data = next( (m
# for m in mora_por_modelo if m[0] == modelo), (modelo, 0, 0) ) tasa_mora_modelo = ( (mora_data[2] / mora_data[1] * 100) if
# mora_data[1] > 0 else 0 ) modelos_data.append( { "modelo":\n modelo, "marca":\n marca, "total_ventas":\n ventas,
...
# Comparativa entre concesionarios """ # Ventas por concesionario ventas_por_concesionario = ( db.query(
...
# "tasa_mora_promedio":\n ( sum(c["tasa_mora"] for c in concesionarios_data) / len(concesionarios_data) if
# concesionarios_data else 0 ), }, }
