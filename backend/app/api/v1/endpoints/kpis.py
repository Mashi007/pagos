...
def kpis_financieros(
    ...
    # Rentabilidad por modalidad
    rentabilidad_modalidad = db.query(
...
def kpis_cobranza(
    ...
    cuotas_pagadas_tiempo = db.query(Cuota).filter(
        Cuota.estado == "PAGADA",
...
def kpis_productos(
    ...
    return {
        ...
        "estadisticas": {
            ...
            "ticket_promedio_general": sum(m["ticket_promedio"] for m in modelos_data) / len(modelos_data) if modelos_data else 0
        }
    }
...
def kpis_concesionarios(
    ...
    """
    # Ventas por concesionario
    ventas_por_concesionario = db.query(
    ...
    # Mora por concesionario
    mora_por_concesionario = db.query(
        Cliente.concesionario,
        func.count(Cliente.id).label('total_clientes'),
        func.sum(case((Cliente.dias_mora > 0, 1), else_=0)).label('clientes_mora'),
    ...
    for concesionario, ventas, monto, ticket in ventas_por_concesionario:
        ...
        mora_data = next((m for m in mora_por_concesionario if m[0] == concesionario), 
                        (concesionario, 0, 0, 0))
    ...
    return {
        ...
        "estadisticas_generales": {
            ...
            "ticket_promedio_general": sum(c["ticket_promedio"] for c in concesionarios_data) / len(concesionarios_data) if concesionarios_data else 0,
            "tasa_mora_promedio": sum(c["tasa_mora"] for c in concesionarios_data) / len(concesionarios_data) if concesionarios_data else 0
        }
    }
