...
class Prestamo(Base):
    ...
    # Montos
    monto_total = Column(Numeric(NUMERIC_PRECISION, NUMERIC_SCALE), nu \
    llable=False)
    monto_financiado = Column(Numeric(NUMERIC_PRECISION, NUMERIC_SCALE) \
    , nullable=False)
    ...
    # Estado financiero
    saldo_pendiente = Column(Numeric(NUMERIC_PRECISION, NUMERIC_SCALE) \
    , nullable=False)
    ...
    total_pagado = Column(Numeric(NUMERIC_PRECISION, NUMERIC_SCALE), d \
    efault=0.00)
    ...
    actualizado_en = Column(TIMESTAMP, server_default=func.now(), onup \
    date=func.now())
