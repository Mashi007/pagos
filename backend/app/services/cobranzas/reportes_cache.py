"""Resumen de mora para cache del dashboard (worker programado)."""
...
def ejecutar_actualizacion_reportes(db: Session) -> dict:
    """
    Cuotas impagas vencidas en prestamos APROBADO.
    Invocado por el worker de cache del dashboard si ENABLE_AUTOMATIC_SCHEDULED_JOBS=true.
    """
    hoy = date.today()
    q_resumen = (
        select(
