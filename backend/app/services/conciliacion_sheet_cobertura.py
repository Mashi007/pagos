...
# Ventana alrededor de la última fila conocida para no leer toda la columna A (~7k+ filas).
...
def _last_nonempty_sheet_row_in_column_values(
    ...
    for i, row in enumerate(values or []):
        if row and _cell_str(row[0] if row else ""):
...
def compute_scan_coverage_from_db(db: Session) -> Dict[str, Any]:
    ...
    if google_tail is not None and max_row_i is not None:
        if google_tail > max_row_i:
            cola_ok = False
            cola_mensaje = (
                f"Google tiene datos hasta la fila {google_tail}, pero la BD solo hasta {max_row_i}. "
                "Ejecute sincronización manual con Drive."
            )
        elif google_tail < max_row_i:
            cola_ok = False
            cola_mensaje = (
                f"La BD marca última fila {max_row_i} pero en Google la cola A llega solo hasta {google_tail}. "
                "Revise cabecera LOTE o re-sincronice."
            )
        else:
            cola_ok = True
            cola_mensaje = f"Cola alineada: última fila con dato en columna A = {google_tail}."
...
def resolve_sync_end_row_from_column_a(
    ...
    *,
    ...
    """
    Paso previo al sync: lee columna A completa, detecta cabecera (marcador) y la última
    fila con dato en A. El sync de A:S debe acotarse a esa fila.
    """
    from app.services.conciliacion_sheet_sync import (
        _find_header_row,
        fetch_sheet_column_a_full,
    )

    sheet_title, col_a_values = fetch_sheet_column_a_full(spreadsheet_id, tab_name)
    if not col_a_values:
        raise ValueError("La columna A de la hoja devolvió 0 filas.")

    h_idx, marker_hit = _find_header_row(col_a_values, marker)
    if not marker_hit:
        logger.warning(
            "[conciliacion_sheet_cobertura] Marcador %r no hallado en columna A; cabecera índice 0.",
            marker,
        )

    last_in_a: Optional[int] = None
    for i, row in enumerate(col_a_values):
        if row and _cell_str(row[0] if row else ""):
            last_in_a = i + 1

    if last_in_a is None:
        raise ValueError("No hay celdas con dato en la columna A.")

    header_row_1based = h_idx + 1
    if last_in_a < header_row_1based:
        raise ValueError(
            f"Última fila con dato en A ({last_in_a}) está antes de la cabecera ({header_row_1based})."
        )

    logger.info(
        "[conciliacion_sheet_cobertura] columna A: filas=%s cabecera_fila=%s ultima_fila_a=%s marker_hit=%s",
        len(col_a_values),
        header_row_1based,
        last_in_a,
        marker_hit,
    ...
    return {
        ...
        "column_a_last_row": last_in_a,
        "sync_end_row": last_in_a,
        "marker_hit": marker_hit,
        "column_a_rows_returned": len(col_a_values),
    }
...
def probe_google_sheet_tail_row(
    ...
    *,
    ...
    """
    Lee un tramo de la columna A en Google alrededor de la última fila conocida en BD
    y devuelve la última fila con contenido. Opcionalmente persiste en conciliacion_sheet_meta.
    """
    from app.services.conciliacion_sheet_sync import fetch_sheet_column_a_slice
    ...
    for _ in range(4):
        sheet_title, values = fetch_sheet_column_a_slice(
            spreadsheet_id, tab_name, row_start, row_end
        )
    ...
    logger.info(
        "[conciliacion_sheet_cobertura] probe tail sheet=%r rango=A%s:A%s google_last=%s bd_max=%s",
        sheet_title,
        row_start,
        row_end,
        google_last,
    ...
    return {
        "ok": True,
        "sheet_title": sheet_title,
        "probe_range": f"A{row_start}:A{row_end}",
