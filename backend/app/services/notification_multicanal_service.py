...
# notificaciones por Email + WhatsAppSin IA - Basado en templates y reglas de negocio"""import loggingfrom datetime import
# date, datetime, timedeltafrom enum import Enumfrom typing import Any, Dict, Listfrom sqlalchemy import funcfrom
...
# logging.getLogger(__name__)class TipoNotificacionCliente(str, Enum): """Tipos de notificaciones a clientes"""
...
# Obtener preferencias de notificación del cliente Por defecto: AMBOS canales si están configurados """ # TODO: Implementar
...
# self.whatsapp_service = WhatsAppService() # Configuración de horarios self.HORARIOS_ENVIO = {
...
# resultados = { "fecha_procesamiento": datetime.now().isoformat(), "notificaciones_procesadas": 0, "exitosas": 0,
...
# resultados["notificaciones_procesadas"] += resultado_tipo[ "total" ] resultados["exitosas"] += resultado_tipo["exitosas"]
# resultados["fallidas"] += resultado_tipo["fallidas"] resultados["por_tipo"][tipo_notif.value] = resultado_tipo except
# Exception as e: error_msg = ( f"Error procesando {tipo_notif.value}: {str(e)}" ) logger.error(error_msg)
# resultados["errores"].append(error_msg) # Generar reporte diario si es necesario if self._es_hora_reporte_diario(): await
# self._generar_reporte_diario(resultados) logger.info( f"✅ Procesamiento completado:" + f"{resultados['exitosas']} \
# exitosas, {resultados['fallidas']} fallidas" ) return resultados except Exception as e: logger.error(f"Error en
...
# "total": len(clientes_objetivo), "exitosas": 0, "fallidas": 0, "detalles": [], } for cliente_data in clientes_objetivo:
...
# resultado_envio["exitoso"]: resultado["exitosas"] += 1 else: resultado["fallidas"] += 1
# resultado["detalles"].append(resultado_envio) except Exception as e: logger.error( f"Error enviando notificación" + f"a
# cliente {cliente_data.get('cliente_id')}: {e}" ) resultado["fallidas"] += 1 return resultado except Exception as e:
# logger.error(f"Error procesando tipo {tipo.value}: {e}") return { "tipo": tipo.value, "total": 0, "exitosas": 0,
...
# tipo == TipoNotificacionCliente.RECORDATORIO_3_DIAS: # Cuotas que vencen en 3 días fecha_objetivo = hoy + timedelta(days=3)
...
# TipoNotificacionCliente.RECORDATORIO_1_DIA: # Cuotas que vencen mañana fecha_objetivo = hoy + timedelta(days=1) cuotas = (
...
# timedelta(days=1) cuotas = ( self.db.query(Cuota) .join(Prestamo) .join(Cliente) .filter( Cuota.fecha_vencimiento ==
# fecha_vencida, Cuota.estado.in_(["PENDIENTE", "PARCIAL", "VENCIDA"]), Cliente.activo, ) .all() ) elif tipo ==
# TipoNotificacionCliente.MORA_3_DIAS: # Cuotas con 3 días de mora fecha_vencida = hoy - timedelta(days=3) cuotas = (
...
# TipoNotificacionCliente.MORA_5_DIAS: # Cuotas con 5 días de mora fecha_vencida = hoy - timedelta(days=5) cuotas = (
# self.db.query(Cuota) .join(Prestamo) .join(Cliente) .filter( Cuota.fecha_vencimiento == fecha_vencida,
# Cuota.estado.in_(["PENDIENTE", "PARCIAL", "VENCIDA"]), Cliente.activo, ) .all() ) else: cuotas = [] # Convertir cuotas a
# datos de cliente for cuota in cuotas: cliente = cuota.prestamo.cliente clientes_objetivo.append( { "cliente_id":
...
# "canales_intentados": [], "canales_exitosos": [], "canales_fallidos": [], "exitoso": False, "timestamp":
# datetime.now().isoformat(), } # Determinar canales a usar canales_envio = [] if preferencias in [ CanalNotificacion.EMAIL,
...
# Enviar por cada canal for canal in canales_envio: resultado["canales_intentados"].append(canal) try: if canal == "EMAIL":
# exito_email = await self._enviar_email_cliente( cliente_data, tipo ) if exito_email:
# resultado["canales_exitosos"].append("EMAIL") else: resultado["canales_fallidos"].append("EMAIL") elif canal == "WHATSAPP":
# exito_whatsapp = await self._enviar_whatsapp_cliente( cliente_data, tipo ) if exito_whatsapp:
# resultado["canales_exitosos"].append("WHATSAPP") else: resultado["canales_fallidos"].append("WHATSAPP") except Exception as
# e: logger.error(f"Error enviando por {canal}: {e}") resultado["canales_fallidos"].append(canal) # Considerar exitoso si al
# menos un canal funcionó resultado["exitoso"] = len(resultado["canales_exitosos"]) > 0 # Registrar en historial await
# self._registrar_en_historial(cliente_data, tipo, resultado) return resultado except Exception as e: logger.error(f"Error en
# envío multicanal: {e}") return { "cliente_id": cliente_data["cliente_id"], "exitoso": False, "error": str(e), } async def
...
# html_content=template_email["cuerpo_html"], ) return resultado.get("exitoso", False) except Exception as e:
...
# resultado.get("exitoso", False) except Exception as e: logger.error(f"Error enviando WhatsApp: {e}") return False def
...
# f"${cliente_data['monto_cuota']:,.0f}", "fecha": cliente_data["fecha_vencimiento"].strftime("%d/%m/%Y"), "dias_mora":
...
# días", "cuerpo_html": ( f""" <div style="font-family: Arial, sans-serif; max-width: \ 600px; margin: 0 auto;"> <div
...
# style="background: white; padding: 20px; border-radius: 8px;"> <h2>Hola {variables['nombre']},</h2> <p>Te recordamos que tu
...
# "cuerpo_html": ( f""" <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;"> <div
...
# ['dias_mora']}</p> </div> <p><strong>Para evitar cargos por mora, realiza tu pago hoy mismo.</strong></p> <p>Si tienes
# alguna dificultad, contáctanos inmediatamente al {variables['telefono_empresa']}.</p> <div style="text-align: center;
...
# <p><strong>✅ Hemos recibido tu pago de \ la cuota #{variables['cuota']}.</strong></p> </div> <div style="background:
...
# """Obtener template de WhatsApp según tipo (más cortos que email)""" # Variables comunes nombre =
# cliente_data["nombre"].split()[0] cuota = cliente_data["cuota_numero"] monto = f"${cliente_data['monto_cuota']:,.0f}" fecha
# = cliente_data["fecha_vencimiento"].strftime("%d/%m/%Y") dias_mora = cliente_data["dias_mora"] vehiculo =
# cliente_data["vehiculo"] templates = { TipoNotificacionCliente.RECORDATORIO_3_DIAS: { "mensaje": ( f"""👋 Hola {nombre}, Te
# recordamos que tu cuota #{cuota} de tu {vehiculo} vence el {fecha}. Monto: {monto}Por favor realiza tu pago a tiempo.
...
# atraso: {dias_mora}Para evitar cargos adicionales, paga hoy.Necesitas ayuda? Responde este mensaje.Financiamiento
...
# f"diario de notificaciones" ) return False # Verificar intervalo mínimo ultima_notificacion = ( self.db.query(Notificacion)
# .filter(Notificacion.usuario_id == cliente_id) .order_by(Notificacion.creado_en.desc()) .first() ) if ultima_notificacion:
# tiempo_transcurrido = ( datetime.now() - ultima_notificacion.creado_en ) if tiempo_transcurrido.total_seconds() < (
...
# resultado["canales_intentados"]: estado = ( "ENTREGADO" if canal in resultado["canales_exitosos"] else "ERROR" )
...
# datetime.now() # Generar reporte a las 18:00 (6 PM) return ahora.hour == 18 and ahora.minute < 10 async def
# _generar_reporte_diario(self, resultados: Dict): """Generar reporte diario de notificaciones""" try: # Obtener usuarios de
# cobranzas para enviar reporte usuarios_cobranzas = ( self.db.query(User) .filter( User.is_admin, # Cambio clave: rol →
# is_admin User.is_active, User.email.isnot(None), ) .all() ) # Crear reporte reporte_html = f""" <h2>📊 Reporte Diario de
# Notificaciones</h2> <p><strong>Fecha:</strong> {date.today().strftime('%d/%m/%Y')}</p> <h3>📈 Resumen:</h3> <ul> <li>Total
# procesadas: {resultados['notificaciones_procesadas']}</li> <li>✅ Exitosas: {resultados['exitosas']}</li> <li>❌ Fallidas:
# {resultados['fallidas']}</li> <li>📊 Tasa de éxito: {(resultados['exitosas'] / resultados['notificaciones_procesadas'] *
# 100):.1f}%</li> </ul> <h3>📋 Por Tipo de Notificación:</h3> <ul> """ for tipo, datos in resultados["por_tipo"].items():
# reporte_html += f"<li><strong>{tipo}:</strong> {datos['exitosas']}/{datos['total']} exitosas</li>" reporte_html += "</ul>"
# if resultados["errores"]: reporte_html += """ <h3>⚠️ Errores Detectados:</h3> <ul> """ for error in resultados["errores"]:
# reporte_html += f"<li>{error}</li>" reporte_html += "</ul>" # Enviar reporte a equipo de cobranzas for usuario in
# usuarios_cobranzas: await self.email_service.send_email( to_email=usuario.email, subject=f"📊 Reporte Diario de
# Notificaciones - {date.today().strftime('%d/%m/%Y')}", html_content=reporte_html, ) logger.info("📧 Reporte diario enviado
...
# está ejecutándose") return {"mensaje": "Ciclo ya en ejecución"} try: self.is_running = True logger.info("🔔 Iniciando ciclo
# automático de notificaciones") # Crear servicio de notificaciones servicio_notif = NotificacionMulticanal(db) # Procesar
# todas las notificaciones resultados = ( await servicio_notif.procesar_notificaciones_automaticas() ) # Log de resultados
# logger.info( f"✅ Ciclo completado: " f"{resultados.get('exitosas', 0)} exitosas, " f"{resultados.get('fallidas', 0)}
# fallidas" ) return { "mensaje": "✅ Ciclo de notificaciones completado exitosamente", "timestamp":
# datetime.now().isoformat(), "resultados": resultados, } except Exception as e: logger.error(f"❌ Error en ciclo de
# notificaciones: {e}") return { "mensaje": "❌ Error en ciclo de notificaciones", "error": str(e), "timestamp":
# datetime.now().isoformat(), } finally: self.is_running = False def verificar_configuracion_servicios( self, db: Session )
# -> Dict[str, bool]: """Verificar que los servicios estén configurados""" try: from app.models.configuracion_sistema import
# ConfigHelper return { "email_configurado": ConfigHelper.is_email_configured(db), "whatsapp_habilitado":
# ConfigHelper.is_whatsapp_enabled(db), "puede_enviar_notificaciones": True, # Al menos email siempre debe estar } except
...
# GESTIÓN DE REINTENTOS# ============================================class GestorReintentos: """ 🔄 Gestor de reintentos para
# notificaciones fallidas """ @staticmethod async def procesar_reintentos_pendientes(db: Session) -> Dict[str, Any]: """ 🔄
# Procesar reintentos de notificaciones fallidas """ try: # Buscar notificaciones fallidas que requieren reintento
# notificaciones_reintento = ( db.query(Notificacion) .filter( Notificacion.estado == "ERROR", Notificacion.intentos <
# Notificacion.max_intentos, Notificacion.creado_en >= datetime.now() - timedelta(hours=24), # Solo últimas 24h ) .all() )
# resultados = { "total_reintentos": len(notificaciones_reintento), "exitosos": 0, "fallidos": 0, "detalles": [], } for
# notificacion in notificaciones_reintento: try: # Incrementar contador de intentos notificacion.intentos += 1 # Intentar
# reenvío if notificacion.canal == "EMAIL": exito = await GestorReintentos._reintentar_email( notificacion ) elif
# notificacion.canal == "WHATSAPP": exito = await GestorReintentos._reintentar_whatsapp( notificacion ) else: exito = False
# if exito: notificacion.estado = "ENTREGADO" notificacion.fecha_entrega = datetime.now() resultados["exitosos"] += 1 else:
# resultados["fallidos"] += 1 # Si agotó reintentos, notificar a admin if notificacion.intentos >= notificacion.max_intentos:
# await GestorReintentos._notificar_admin_fallo_critico( notificacion, db ) db.commit() except Exception as e: logger.error(
# f"Error en reintento de notificación " f"{notificacion.id}: {e}" ) resultados["fallidos"] += 1 return resultados except
# Exception as e: logger.error(f"Error procesando reintentos: {e}") return {"error": str(e)} @staticmethod async def
# _reintentar_email(notificacion: Notificacion) -> bool: """Reintentar envío de email""" try: # Reenviar email usando datos
# guardados email_service = EmailService() resultado = await email_service.send_email(
# to_email=notificacion.destinatario_email, subject=notificacion.titulo, html_content=notificacion.mensaje, ) return
# resultado.get("exitoso", False) except Exception as e: logger.error(f"Error reintentando email: {e}") return False
# @staticmethod async def _reintentar_whatsapp(notificacion: Notificacion) -> bool: """Reintentar envío de WhatsApp""" try: #
# Reenviar WhatsApp usando datos guardados whatsapp_service = WhatsAppService() resultado = await
# whatsapp_service.send_message( to_phone=notificacion.destinatario_telefono, message=notificacion.mensaje, ) return
# resultado.get("exitoso", False) except Exception as e: logger.error(f"Error reintentando WhatsApp: {e}") return False
...
# prioridad="ALTA", titulo="🚨 Fallo crítico en notificaciones", mensaje=f"""Fallo crítico en sistema de
# notificaciones:Cliente: {notificacion.destinatario_nombre}Canal: {notificacion.canal}Tipo: {notificacion.tipo}Intentos
# realizados: {notificacion.intentos}ltimo error: {notificacion.error_mensaje}Acción requerida: Revisar configuración del
# servicio. """, estado="PENDIENTE", ) db.add(notif_admin) db.commit() logger.warning( "🚨 Administradores notificados sobre
...
# ').title()}", } @staticmethod def listar_todos_templates() -> List[Dict]: """Listar todos los templates disponibles"""
