...
# y trazabilidad"""import jsonimport loggingimport sysimport uuidfrom contextvars import ContextVarfrom datetime import
# datetimefrom typing import Any, Dict, Optional# Context variables para trazabilidadrequest_id: ContextVar[Optional[str]] =
...
# Formatter para logs estructurados en formato JSON """ def format(self, record: logging.LogRecord) -> str: """ Formatear log
# record como JSON estructurado """ # Datos base del log log_data = { "timestamp": datetime.utcnow().isoformat() + "Z",
...
# } # Agregar datos adicionales del record if hasattr(record, "extra_data"): log_data["extra_data"] = record.extra_data
# return json.dumps(log_data, ensure_ascii=False, default=str)class ServiceLogger: """ Logger especializado para servicios
...
# contexto de trazabilidad """ # Crear record con datos adicionales record = self.logger.makeRecord( self.logger.name, level,
# "", 0, message, (), None ) if extra_data: record.extra_data = extra_data # Agregar kwargs como atributos del record for
...
# None,): """ Log estructurado para llamadas a servicios """ logger = get_service_logger(f"service.{service_name}") log_data
# = { "service": service_name, "method": method_name, "success": success, "duration_ms": duration_ms, } if params: # Filtrar
# datos sensibles safe_params = {} for key, value in params.items(): if key.lower() in ["password", "token", "secret",
...
# Any]] = None,): """ Log de eventos de negocio para auditoría """ logger = get_service_logger("business_events") event_data
...
# str = "MEDIUM", details: Optional[Dict[str, Any]] = None, threat_level: str = "UNKNOWN",): """ Log de eventos de seguridad
...
# Configuración global de loggingdef configure_service_logging(): """ Configurar logging global para servicios """ #
# Configurar nivel de logging logging.basicConfig( level=logging.INFO, format="%(asctime)s - %(name)s - %(levelname)s -
# %(message)s", ) # Configurar loggers específicos service_loggers = [ "app.services.auth_service",
...
# logging automático de métodosdef log_method_calls(logger_name: str): """ Decorador para logging automático de llamadas a
# métodos """ def decorator(func): def wrapper(*args, **kwargs): logger = get_service_logger(logger_name) start_time =
# datetime.utcnow() try: logger.info(f"Method call started: {func.__name__}") result = func(*args, **kwargs) duration = (
# datetime.utcnow() - start_time ).total_seconds() * 1000 logger.info( f"Method call completed: {func.__name__}",
# duration_ms=duration, ) return result except Exception as e: duration = ( datetime.utcnow() - start_time ).total_seconds()
