...
# services_dir.exists(): print(f"Error: Directorio {services_dir} no existe") return None print
...
# not in service["metrics"]: metrics = service["metrics"] print(f" Funciones: {metrics.get('total_functions', 0)}") print
# Clases: {metrics.get('total_classes', 0)}") print(f" Lineas: {metrics.get('total_lines', 0)}") print
# report["critical_issues"]: print(f"ERROR {issue['file']}: {issue['score']:.2f}%") for problem in issue["issues"]: print
...
# 80: print("Calidad aceptable") return 0 elif overall_score >= 70: print("Calidad mejorable") return 1 else: print
