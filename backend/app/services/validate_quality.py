...
# trazabilidad"""import jsonimport sysfrom datetime import datetimefrom pathlib import Pathfrom app.services.logging_config
...
# # Configurar logging configure_service_logging() # Directorio de servicios services_dir = Path(__file__).parent if not
...
# calidad""" print("Ejecutando analisis de calidad...") report = apply_quality_standards(str(services_dir)) # Mostrar
# resultados print("RESULTADOS DEL ANALISIS") print("-" * 40) print(f"Servicios analizados: {report['services_analyzed']}")
# print(f"Score general: {report['overall_score']:.2f}%") print(f"Timestamp: {report['timestamp']}") print() return reportdef
# _mostrar_detalles_por_servicio(report: dict): """Mostrar detalles por servicio""" print("DETALLES POR SERVICIO") print("-"
...
# "NARANJA" else: score_color = "ROJO" print(f"{score_color} {service_name}: {score:.2f}%") # Mostrar validaciones
# específicas validations = service["validations"] for validation, passed in validations.items(): status = "OK" if passed
# else "ERROR" print(f" {status} {validation}") # Mostrar métricas si están disponibles if "metrics" in service and "error"
...
# {metrics.get('cyclomatic_complexity', 0)}") print()def _mostrar_problemas_criticos(report: dict): """Mostrar problemas
# críticos""" if report["critical_issues"]: print("PROBLEMAS CRITICOS") print("-" * 40) for issue in
# report["critical_issues"]: print(f"ERROR {issue['file']}: {issue['score']:.2f}%") for problem in issue["issues"]: print(f"
# • {problem}") print()def _mostrar_recomendaciones(report: dict): """Mostrar recomendaciones""" if
...
# f"quality_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json" ) with open(report_file, "w", encoding="utf-8") as f:
...
# análisis de calidad report = _ejecutar_analisis_calidad(services_dir) # Mostrar detalles
# _mostrar_detalles_por_servicio(report) _mostrar_problemas_criticos(report) _mostrar_recomendaciones(report) # Guardar
