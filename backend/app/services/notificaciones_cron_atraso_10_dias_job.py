# -*- coding: utf-8 -*-
"""
Cron servidor: envío automático «atraso-10-dias» (PAGO_10_DIAS_ATRASADO / menor a 60).

Requiere ENABLE_AUTOMATIC_SCHEDULED_JOBS + líder de scheduler, y
ENABLE_CRON_NOTIFICACIONES_ATRASO_10_DIAS (ver settings).
Horario America/Caracas: CRON_ATRASO_10_DIAS_HOUR / CRON_ATRASO_10_DIAS_MINUTE
(defecto 13:15, todos los días).

Idempotencia: una ejecución «terminal» por fecha Caracas
(clave configuracion notificaciones_cron_atraso_10_dias_estado).
"""
...
CLAVE_CRON_ATRASO_10_DIAS_ESTADO = "notificaciones_cron_atraso_10_dias_estado"
...
def _cargar_estado(db: Session) -> Dict[str, Any]:
    try:
        row = db.get(Configuracion, CLAVE_CRON_ATRASO_10_DIAS_ESTADO)
        if row and row.valor:
            data = json.loads(row.valor)
            if isinstance(data, dict):
                return data
    except json.JSONDecodeError as e:
        logger.warning("[cron_atraso10] estado en BD no es JSON valido: %s", e)
    except Exception as e:
        logger.exception("[cron_atraso10] leer estado: %s", e)
    return {}
...
def _persistir_estado(db: Session, body: Dict[str, Any]) -> None:
    ...
    if row:
        ...
        db.add(Configuracion(clave=CLAVE_CRON_ATRASO_10_DIAS_ESTADO, valor=raw))


def ejecutar_cron_atraso_10_dias(db: Session) -> Dict[str, Any]:
    """
    Ejecuta el envío PAGO_10_DIAS_ATRASADO (atraso-10-dias) para hoy Caracas si aplica.
    ...
    if debe_omitir_cron_por_estado_persistido(prev, hoy_s):
        logger.info(
            "[cron_atraso10] omitido: ya hubo resultado terminal hoy (%s) estado=%s",
            hoy_s,
            prev.get("estado"),
        )
        return {
    ...
    if isinstance(tipo_cfg, dict) and tipo_cfg.get("habilitado") is False:
        ...
        db.commit()
        logger.info("[cron_atraso10] omitido: envío desactivado para %s", TIPO_CASO)
        return {
            "omitido": True,
    ...
    max_try = int(getattr(settings, "CRON_ATRASO_10_DIAS_INTENTOS_JOB", 3) or 3)
    max_try = max(1, min(max_try, 10))
    sleep_s = int(
        getattr(settings, "CRON_ATRASO_10_DIAS_SLEEP_ENTRE_INTENTOS_SEG", 60) or 60
    )
    ...
    for attempt in range(1, max_try + 1):
        try:
            ...
            db.commit()
            logger.info(
                "[cron_atraso10] ok fecha=%s enviados=%s total_lista=%s fallidos=%s intento=%s/%s",
                hoy_s,
                out.get("enviados"),
            ...
            try:
                ...
                pass
            logger.warning(
                "[cron_atraso10] intento %s/%s falló: %s",
                attempt,
                max_try,
                e,
    ...
    db.commit()
    logger.error(
        "[cron_atraso10] error definitivo fecha=%s tras %s intentos", hoy_s, max_try
    )
    return {
        "omitido": False,
        "error": True,
...
def job_cron_atraso_10_dias_scheduler() -> None:
    """Punto de entrada APScheduler: sesión propia."""
    from app.core.database import SessionLocal

    db = SessionLocal()
    try:
        ejecutar_cron_atraso_10_dias(db)
    except Exception as e:
        logger.exception("[cron_atraso10] job no controlado: %s", e)
    finally:
        db.close()
