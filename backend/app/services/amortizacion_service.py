...
# Generar fechas de vencimiento fechas = calculate_payment_dates( request.fecha_primer_vencimiento, n_cuotas,
...
# Decimal("12") else: tasa_periodo = tasa_anual / Decimal("12") # Capital fijo por cuota capital_fijo = (monto /
...
# Decimal("0.00") total_interes = Decimal("0.00") for i in range(n_cuotas): # Interés sobre saldo interes = (saldo *
...
# saldo_capital_inicial=cuota_detalle.saldo_inicial, saldo_capital_final=cuota_detalle.saldo_final,
# capital_pendiente=cuota_detalle.capital, interes_pendiente=cuota_detalle.interes, estado="PENDIENTE", ) db.add(cuota)
# cuotas_creadas.append(cuota) db.commit() return cuotas_creadas @staticmethod def obtener_cuotas_prestamo( db: Session,
...
# tasa_mora_diaria total_mora_nueva += nueva_mora cuotas_actualizadas += 1 db.commit() return { "cuotas_actualizadas":
# cuotas_actualizadas, "total_mora_anterior": total_mora_anterior, "total_mora_nueva": total_mora_nueva, "diferencia":
