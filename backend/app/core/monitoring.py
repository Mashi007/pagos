...
# para tracking de errores Args: app: Instancia de FastAPI """ if not settings.SENTRY_DSN: logging.warning
# configurado - Sentry deshabilitado") return sentry_sdk.init
...
# PII por defecto attach_stacktrace=True, max_breadcrumbs=50, ) logging.info
...
# logging.info(f"Configuración aplicada: {config_applied}") except Exception as e: logging.error
...
# "bulk_migration_total", total_records, {"type": migration_type} ) track_business_metrics
...
# self.operation_name = operation_name self.context = kwargs def __enter__(self): logging.info
