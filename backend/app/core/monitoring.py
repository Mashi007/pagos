...
# estructurado"""import loggingfrom datetime import datetimefrom typing import Optionalimport sentry_sdkfrom fastapi import
...
# PII por defecto attach_stacktrace=True, max_breadcrumbs=50, ) logging.info(f"Sentry configurado - Environment:
# {settings.ENVIRONMENT}")def before_send_sentry(event, hint): """ Hook para modificar eventos antes de enviar a Sentry Útil
...
# Instrumentar la app instrumentator.instrument(app) # Exponer endpoint /metrics instrumentator.expose(app,
...
# jsonlogger.JsonFormatter( "%(asctime)s %(name)s %(levelname)s %(message)s %(pathname)s % \ (lineno)d", rename_fields={
# "asctime": "timestamp", "levelname": "level", "pathname": "file", "lineno": "line", }, )
...
# específicos loggers_to_configure = [ "uvicorn", "uvicorn.access", "uvicorn.error", "sqlalchemy.engine", "app", ] for
# logger_name in loggers_to_configure: logger = logging.getLogger(logger_name) logger.setLevel(log_level) logger.handlers =
# [] logger.addHandler(log_handler) logger.propagate = False logging.info(f"Logging estructurado configurado - Level:
...
# config_applied["prometheus"] = True logging.info("Sistema de monitoreo configurado exitosamente")
...
# track_business_metrics("mora_acumulada", dias_mora, {"cliente_id": \ 123}) track_business_metrics("conciliacion_exitosa",
# 1, {"banco": "Popular", "registros": 50}) """ try: # Log estructurado para métricas de negocio logging.info(
# f"BUSINESS_METRIC: {metric_name}", extra={ "metric_name": metric_name, "metric_value": value, "labels": labels or {},
# "timestamp": datetime.now().isoformat(), "system": "financiamiento_automotriz", }, ) # Si Prometheus está habilitado,
# registrar métrica if settings.PROMETHEUS_ENABLED: # Implementar contadores específicos del negocio pass except Exception as
...
# cliente user_id: ID del usuario que ejecuta additional_data: Datos adicionales """ track_business_metrics(
...
# user_role: str, processing_time_seconds: Optional[float] = None,) -> None: """ Trackear flujo de aprobaciones Args:
# workflow_step: SOLICITUD, APROBACION, RECHAZO, EJECUCION request_type: MODIFICAR_PAGO, ANULAR_PAGO, EDITAR_CLIENTE
# user_role: Rol del usuario processing_time_seconds: Tiempo de procesamiento """ track_business_metrics(
# f"approval_workflow_{workflow_step.lower()}", processing_time_seconds or 1, { "workflow_step": workflow_step,
...
# self.operation_name = operation_name self.context = kwargs def __enter__(self): logging.info( f"Iniciando operación:
...
# monitoreo """ return { "titulo": "🔍 ESTADO DEL SISTEMA DE MONITOREO", "fecha_verificacion": datetime.now().isoformat(),
# "servicios_monitoreo": { "sentry": { "habilitado": bool(getattr(settings, "SENTRY_DSN", None)), "dsn_configurado":
...
# "nivel": getattr(settings, "LOG_LEVEL", "INFO"), "descripcion": "Logs estructurados para análisis", }, },
# "metricas_negocio_disponibles": { "financieras": [ "clientes_creados", "pagos_procesados", "prestamos_aprobados",
# "mora_acumulada", "conciliacion_exitosa", ], "operacionales": [ "approval_workflow_solicitud",
...
# "beneficios_implementacion": [ "🔍 Tracking automático de errores con Sentry", "📊 Métricas de performance con Prometheus",
# "📋 Logs estructurados para análisis", "⚡ Alertas automáticas de problemas", "📈 Métricas de negocio específicas", "🔧
# Debugging mejorado en producción", ], "pasos_integracion": [ "1. Agregar variables de entorno (SENTRY_DSN, etc.)", "2.
# Importar setup_monitoring en main.py", "3. Llamar setup_monitoring(app) al inicio", "4. Usar track_operation en endpoints
# críticos", "5. Configurar alertas en Sentry/Prometheus", ], "utilidad_para_financiamiento": { "alta": "✅ Muy útil para
# sistema empresarial", "razones": [ "Tracking de operaciones financieras críticas", "Monitoreo de flujos de aprobación",
# "Alertas de errores en pagos/conciliación", "Métricas de rendimiento de analistaes", "Análisis de patrones de mora",
