...
# logging.getLogger("security_audit")def log_security_event( event_type: SecurityEventType, user_email: Optional[str] = None,
...
# security_audit_logger.warning(f"SECURITY EVENT: {event_data}") else: security_audit_logger.info(f"SECURITY EVENT:
# {event_data}")def log_login_attempt( email: str, ip_address: str, success: bool, reason: Optional[str] = None): """Registra
# un intento de login""" log_security_event( event_type=( SecurityEventType.LOGIN_SUCCESS if success else
...
# cambio de contraseña""" log_security_event( event_type=SecurityEventType.PASSWORD_CHANGE, user_email=user_email,
# user_id=user_id, ip_address=ip_address, success=success, )def log_unauthorized_access( endpoint: str, user_email:
