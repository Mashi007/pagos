...
def _credentials_oauth(scopes: List[str]) -> Optional[Any]:
    ...
    try:
        ...
        err_str = str(e).lower()
        # invalid_scope suele deberse a pedir un alcance distinto al del consentimiento; callers
        # pueden reintentar con otro scope. Evitar traceback a nivel ERROR para no simular caída.
        if "invalid_scope" in err_str:
            logger.warning(
                "[INFORME_PAGOS] OAuth: refresh rechazado (invalid_scope) con scopes=%s: %s. "
                "Suele ocurrir si el consentimiento emitió otros alcances literales (p. ej. spreadsheets vs spreadsheets.readonly). "
                "Si la operación sigue fallando, en Configuración > Google pulse «Conectar con Google» y autorice de nuevo.",
                scopes,
                e,
            )
        else:
