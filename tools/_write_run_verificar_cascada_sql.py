...
p = ROOT / "sql" / "run_verificar_cascada.py"
p.write_text(
...
fifo = ROOT / "sql" / "run_verificar_fifo.py"
fifo.write_text(
    r'''#!/usr/bin/env python3
"""Compat: delega en run_verificar_cascada (mismo flujo)."""
import runpy
import sys
from pathlib import Path

p = Path(__file__).resolve().parent / "run_verificar_cascada.py"
sys.exit(runpy.run_path(str(p), run_name="__main__") or 0)
''',
    encoding="utf-8",
    newline="\n",
)
