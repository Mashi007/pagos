# -*- coding: utf-8 -*-
"""Incluye estado de cuota (reglas Caracas) en recibos PDF."""
...
def patch_recibo_pdf() -> None:
    ...
    if "estado_cuota: Optional[str] = None" in t:
        print("recibo_pdf: signature already patched")
    else:
        t = t.replace(
    ...
    if old_block not in t:
        if "if estado_cuota_lbl:" in t and "_info_style" in t:
    ...
    if old_saldos not in t:
        if "_estado_cell" in t:
...
def patch_prestamos_endpoint() -> None:
    ...
    old_call = """    pdf_bytes = generar_recibo_cuota_amortizacion(
    ...
    # Compute block before pdf_bytes - insert if not present
    if "estado_cuota_lbl" not in t.split("def get_recibo_cuota_pdf")[1].split("def _obtener_cuotas_para_export")[0]:
        insert = """
    fv_c = cuota.fecha_vencimiento
    fv_date_c = fv_c.date() if fv_c and hasattr(fv_c, "date") else fv_c
    estado_codigo = estado_cuota_para_mostrar(
        total_pagado, monto_cuota, fv_date_c, hoy_negocio()
    )
...
"""
    ...
    elif "estado_cuota=estado_cuota_lbl" in t:
        print("prestamos: call already extended")
    else:
        raise SystemExit("prestamos.py: generar_recibo_cuota_amortizacion call not found")
