-- ============================================
-- CREAR TABLA TEMPORAL PARA IMPORTACIÓN DE PRÉSTAMOS
-- ============================================
-- Esta tabla temporal permite importar préstamos desde otra base de datos
-- sin restricciones de Foreign Keys, facilitando la validación y limpieza
-- antes de insertar en la tabla final.
-- ============================================
...
BEGIN
    IF EXISTS (
        ...
        RAISE NOTICE '⚠️ La tabla prestamos_temporal ya existe. Eliminándola...';
        DROP TABLE prestamos_temporal CASCADE;
    END IF;
END $$;

-- Crear tabla temporal con la misma estructura que prestamos
CREATE TABLE prestamos_temporal (
    -- IDENTIFICACIÓN
...
CREATE INDEX idx_prestamos_temporal_cedula ON prestamos_temporal(cedula);
CREATE INDEX idx_prestamos_temporal_estado_validacion ON prestamos_temporal(estado_validacion);
CREATE INDEX idx_prestamos_temporal_cliente_id_mapeado ON prestamos_temporal(cliente_id_mapeado);

-- Comentarios en la tabla
COMMENT ON TABLE prestamos_temporal IS 'Tabla temporal para importación de préstamos desde otra base de datos';
COMMENT ON COLUMN prestamos_temporal.cedula_original IS 'Cédula original de la fuente (para validación)';
COMMENT ON COLUMN prestamos_temporal.cliente_id_mapeado IS 'cliente_id mapeado después de validar existencia en tabla clientes';
COMMENT ON COLUMN prestamos_temporal.errores_validacion IS 'Errores encontrados durante la validación (JSON o texto)';
COMMENT ON COLUMN prestamos_temporal.estado_validacion IS 'Estado de validación: PENDIENTE, VALIDADO, ERROR';
COMMENT ON COLUMN prestamos_temporal.observaciones_importacion IS 'Observaciones específicas de la importación';
