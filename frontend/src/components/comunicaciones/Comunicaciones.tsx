...
export function Comunicaciones({
  ...
  const [comunicacionSeleccionada, setComunicacionSeleccionada] =
    useState<ComunicacionUnificada | null>(null)

  // Estado para carga de mensajes (solo cuando se selecciona)

  const [mensajesCargados, setMensajesCargados] = useState<
    Map<string, ComunicacionUnificada[]>
  >(new Map())
  ...
  const [clienteRecienCreado, setClienteRecienCreado] = useState<{
    contacto: string
    tipo: string
  } | null>(null)
  ...
  const {
    ...
    queryFn: () =>
      comunicacionesService.listarComunicaciones(1, 100, undefined, clienteId),
  ...
  const todasComunicaciones = useMemo(() => {
    if (comunicacionesData?.comunicaciones)
      return comunicacionesData.comunicaciones
  ...
  useEffect(() => {
    if (conversacionIdFromUrl == null || todasComunicaciones.length === 0)
      return
    ...
    const idBase = comm.cliente_id
      ? `cliente_${comm.cliente_id}`
      : `contacto_${comm.from_contact}`
  ...
  useEffect(() => {
    ...
    const idSeleccionar = tieneWhatsApp
      ? idWhatsApp
      : tieneEmail
        ? idEmail
        : null
  ...
  const conversacionesAgrupadas = useMemo(() => {
    ...
    todasComunicaciones.forEach(comm => {
      ...
      if (!grupos.has(id)) {
        ...
        let nombre =
          comm.nombre_contacto && comm.nombre_contacto.trim()
            ? comm.nombre_contacto.trim()
            : comm.from_contact
        ...
        if (
          ...
          nombre =
            mockNombresClientes[comm.cliente_id] ||
            `Cliente #${comm.cliente_id}`
        }
      ...
      if (
        new Date(comm.timestamp) > new Date(grupo.ultimaComunicacion.timestamp)
      ) {
    ...
    if (clienteId && clienteDesdeUrl) {
      const nombreCliente =
        (clienteDesdeUrl.nombres || '').trim() || `Cliente #${clienteId}`

      const telefono = (clienteDesdeUrl.telefono || '').trim()

      const email = (clienteDesdeUrl.email || '').trim()

      const contactoWhatsApp = telefono.startsWith('+')
        ? telefono
        : `+${telefono}`
  ...
  const conversacionActual = useMemo(() => {
    ...
    return (
      conversacionesAgrupadas.find(c => c.id === conversacionSeleccionada) ||
      null
    )
  ...
  useEffect(() => {
    if (clienteRecienCreado && conversacionesAgrupadas.length > 0) {
      const conversacionActualizada = conversacionesAgrupadas.find(
        c =>
          c.contacto === clienteRecienCreado.contacto &&
          c.tipo === clienteRecienCreado.tipo
      )
  ...
  const whatsappContacto =
    conversacionActual?.tipo === 'whatsapp' ? conversacionActual.contacto : null
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const res =
        await comunicacionesService.listarMensajesWhatsApp(whatsappContacto)
  ...
  useEffect(() => {
    ...
    const mensajes = todasComunicaciones.filter(comm => {
      const idBase = comm.cliente_id
        ? `cliente_${comm.cliente_id}`
        : `contacto_${comm.from_contact}`
  ...
  const mensajesOrdenados = useMemo(() => {
    ...
    if (
      ...
      mensajesWhatsAppData !== undefined
    )
  ...
  const handleSeleccionarConversacion = (
  ...
  const handleCrearCliente = async (conversacion: ConversacionAgrupada) => {
    ...
    try {
      ...
      const resultado =
        await comunicacionesService.crearClienteAutomatico(request)
    ...
    try {
      ...
      const ticketData: TicketCreate = {
        ...
        conversacion_whatsapp_id:
          comunicacionSeleccionada.tipo === 'whatsapp'
            ? comunicacionSeleccionada.id
            : undefined,

        comunicacion_email_id:
          comunicacionSeleccionada.tipo === 'email'
            ? comunicacionSeleccionada.id
            : undefined,

        asignado_a: ticketForm.responsable_id
          ? usuarios.find(u => u.id === ticketForm.responsable_id)?.email
          : undefined,
      ...
      if (conversacionActual?.cliente_id ?? clienteId) {
        const response = await ticketsService.getTickets({
          cliente_id: conversacionActual?.cliente_id ?? clienteId,
        })
  ...
  const handleEnviarMensaje = async () => {
    ...
    if (conversacionActual.tipo === 'whatsapp') {
      ...
      const numeroDestino =
        (ultimaComunicacion.direccion === 'INBOUND'
      ...
      if (soloDigitos.length < 10) {
        toast.error(
          'Número de WhatsApp inválido o incompleto (incluye código de país). No se puede enviar.'
        )
    ...
    try {
      if (conversacionActual!.tipo === 'whatsapp') {
        ...
        const resultado = await comunicacionesService.enviarWhatsApp(
          numeroDestino,
          mensajeTexto.trim()
        )
        ...
        if (resultado?.success) {
          ...
          queryClient.invalidateQueries({
            queryKey: ['comunicaciones-mensajes', conversacionActual!.contacto],
          })
        } else {
          toast.error(
            resultado?.mensaje ||
              'No se pudo enviar por WhatsApp. Revisa Configuración > WhatsApp.'
          )
        }
  ...
  return (
    <div className="flex h-[calc(100vh-120px)] gap-0 overflow-hidden bg-white">
      {/* COLUMNA IZQUIERDA: Lista de conversaciones */}

      <div className="flex w-80 flex-shrink-0 flex-col border-r border-gray-200 bg-white">
        {/* Header con búsqueda */}

        <div className="border-b border-gray-200 bg-gradient-to-r from-blue-50 to-white p-3">
          <div className="mb-2 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                <RefreshCw
                  className={`h-4 w-4 text-blue-600 ${isLoading ? 'animate-spin' : ''}`}
                />
              </Button>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto">
          {isError && (
            <div className="flex items-center gap-2 border-b border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800">
              ...
              <span>
                Mostrando datos de ejemplo. Error al conectar con el servidor.
              </span>
              ...
              <Button
                ...
                size="sm"
                onClick={() => refetch()}
                className="ml-auto h-6 text-amber-700"
              >
                Reintentar
              </Button>
            </div>
          ...
          {isLoading && (
            <div className="flex items-center gap-2 border-b border-blue-200 bg-blue-50 px-3 py-2 text-xs text-blue-700">
          ...
          {conversacionesFiltradas.length === 0 ? (
            ...
            <div className="divide-y divide-gray-100">
              ...
              {conversacionesFiltradas.some(
                c => !c.leido || c.noLeidos > 0
              ) && (
                <div className="border-b-2 border-blue-300 bg-gradient-to-r from-blue-50 to-blue-100 px-4 py-2.5">
              ...
              {conversacionesFiltradas.map((conversacion, index) => {
                ...
                const mostrarSeparadorLeidos =
                  ...
                  (!conversacionesFiltradas[index - 1].leido ||
                    conversacionesFiltradas[index - 1].noLeidos > 0)
                ...
                return (
                  <div key={conversacion.id}>
                    ...
                    <div
                      onClick={() =>
                        handleSeleccionarConversacion(conversacion)
                      }
                      className={`cursor-pointer p-3 transition-all duration-200 ${
                        conversacionSeleccionada === conversacion.id
                          ? 'border-l-4 border-blue-600 bg-gradient-to-r from-blue-50 to-blue-100'
                          : 'border-l-4 border-transparent hover:bg-gray-50'
                      } ${conversacion.noLeidos > 0 ? 'bg-blue-50/50 font-semibold' : ''}`}
                    >
                      <div className="mb-1 flex items-start justify-between">
                        <div className="flex min-w-0 flex-1 items-center gap-2">
                          ...
                          <div className="min-w-0 flex-1">
                            <div className="flex items-center gap-2">
                              <span
                                className={`truncate text-sm ${conversacion.noLeidos > 0 ? 'text-gray-900' : 'text-gray-700'}`}
                              >
                                {conversacion.nombre}
                              </span>
                              ...
                              {!conversacion.operado && (
                                <Badge
                                  variant="destructive"
                                  className="animate-pulse px-2 py-0.5 text-xs font-bold"
                                >
                                  NUEVO
                                </Badge>
                              )}
                        ...
                        {conversacion.noLeidos > 0 && (
                          <Badge className="ml-2 min-w-[24px] flex-shrink-0 justify-center bg-blue-600 font-bold text-white">
                            {conversacion.noLeidos}
                          </Badge>
                        )}
                      </div>

                      <div className="mt-2 line-clamp-2 truncate text-xs text-gray-600">
                        {conversacion.ultimaComunicacion.body?.substring(
                          0,
                          60
                        ) ||
                          conversacion.ultimaComunicacion.subject ||
                          '[Sin contenido]'}

                        {conversacion.ultimaComunicacion.body &&
                          conversacion.ultimaComunicacion.body.length > 60 &&
                          '...'}
                      </div>

                      {conversacion.tipo === 'whatsapp' &&
                        conversacion.ultimaComunicacion.cedula && (
                      ...
                      <div className="mt-2 flex items-center gap-1 text-xs text-gray-400">
                        ...
                        {formatearFecha(
                          conversacion.ultimaComunicacion.timestamp
                        )}
                      </div>
                    </div>
                  </div>
      ...
      <div className="flex min-w-0 flex-1 flex-col bg-white">
        {!conversacionActual ? (
          ...
          <>
            ...
            <div className="border-b border-gray-200 bg-gradient-to-r from-white to-gray-50 p-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    {conversacionActual.tipo === 'whatsapp' &&
                      conversacionActual.ultimaComunicacion?.cedula && (
                ...
                {conversacionActual.cliente_id && (
                  <Button
                    ...
                    onClick={async () => {
                      try {
                        const c = await clienteService.getCliente(
                          String(conversacionActual.cliente_id)
                        )
            ...
            <div
              ...
              className="min-h-0 flex-1 space-y-3 overflow-y-auto bg-gray-50 p-3"
            >
                      ...
                      >
                        <div className="mb-1 flex items-center gap-2">
                          {mensaje.tipo === 'whatsapp' ? (
                            <MessageSquare
                              className={`h-3 w-3 ${mensaje.direccion === 'OUTBOUND' ? 'text-white' : 'text-green-600'}`}
                            />
                          ) : (
                            <Mail
                              className={`h-3 w-3 ${mensaje.direccion === 'OUTBOUND' ? 'text-white' : 'text-purple-600'}`}
                            />
                          )}

                          <span
                            className={`text-xs ${mensaje.direccion === 'OUTBOUND' ? 'text-blue-100' : 'text-gray-500'}`}
                          >
                        ...
                        {mensaje.subject && (
                          <div
                            className={`mb-1 font-semibold ${mensaje.direccion === 'OUTBOUND' ? 'text-white' : 'text-gray-900'}`}
                          >
                            {mensaje.subject}
                          </div>
                        )}
                        ...
                        <div
                          className={`whitespace-pre-wrap text-sm ${mensaje.direccion === 'OUTBOUND' ? 'text-white' : 'text-gray-700'}`}
                        >
            ...
            <div className="border-t border-gray-200 bg-gradient-to-r from-white to-gray-50 p-3">
              ...
              {conversacionActual.tipo === 'whatsapp' && modoAutomatico ? (
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="mt-1 text-xs text-blue-700">
                        ...
                        recibidos. Cambia a modo manual para responder tú desde
                        esta misma pantalla.
                      </p>
                    </div>
                  </div>
                ...
                <div className="space-y-2">
                  <p className="text-xs font-medium text-gray-600">
                    Modo Manual - Responde desde aquí (Enter para enviar,
                ...
                <div className="flex gap-2">
                  <Textarea
                    placeholder={
                      conversacionActual.tipo === 'whatsapp'
                        ? 'Escribe un mensaje... (Enter enviar)'
      ...
      <div className="flex w-96 flex-shrink-0 flex-col border-l border-gray-200 bg-white">
        {!conversacionActual ? (
          <div className="flex h-full items-center justify-center bg-gradient-to-br from-gray-50 to-white p-4">
            <div className="text-center">
              ...
              <p className="text-sm font-medium text-gray-600">
                Selecciona una conversación para ver los tickets
              </p>
          ...
          <>
            <div className="border-b border-gray-200 bg-gradient-to-r from-blue-50 to-white p-3">
              ...
              {!(conversacionActual?.cliente_id ?? clienteId) && (
                <p className="mt-2 rounded bg-orange-50 px-2 py-1 text-xs font-medium text-orange-600">
            ...
            <div className="flex-1 space-y-3 overflow-y-auto p-3">
              ...
              {!ticketEditando && (
                <Card className="border-2 border-blue-100 shadow-md">
                  <CardHeader className="bg-gradient-to-r from-blue-50 to-white px-3 pb-2 pt-3">
                    ...
                    <div>
                      ...
                      <Select
                        ...
                        onValueChange={value =>
                          setTicketForm({
                            ...ticketForm,
                            responsable_id: value ? parseInt(value) : undefined,
                          })
                        }
                ...
                <Card>
                  <CardHeader className="pb-3">
                    <div className="flex items-center justify-between">
                      <CardTitle className="text-sm">
                        Actualizar Ticket #{ticketEditando.id}
                      </CardTitle>
                    ...
                    <p className="mt-1 text-xs text-gray-500">
                      Título y descripción no se pueden editar después de
                      guardar
                    </p>
                  </CardHeader>
                  ...
                  <CardContent className="space-y-3">
                    ...
                    <div>
                      ...
                      <Textarea
                        value={ticketEditando.descripcion}
                        disabled
                    ...
                    <Select
                      ...
                      onValueChange={value =>
                        setTicketEditando({ ...ticketEditando, estado: value })
                      }
                    >
                      <SelectTrigger className="text-xs">
                        <SelectValue />
              ...
              {ticketsCliente.length > 0 && (
                <div className="space-y-2">
                  ...
                  {ticketsCliente.map(ticket => (
                    <Card
                      key={ticket.id}
                      className="cursor-pointer border-2 p-3 transition-all duration-200 hover:border-blue-300 hover:shadow-md"
                    ...
                    >
                      ...
                      {ticket.fecha_limite && (
                        <div className="mt-2 flex items-center gap-1 rounded bg-blue-50 px-2 py-1 text-xs font-medium text-blue-600">
      ...
      {mostrarCrearCliente && conversacionActual && (
        <CrearClienteForm
          cliente={{
            ...
            nombres:
              conversacionActual.nombre !== conversacionActual.contacto
                ? conversacionActual.nombre
                : '',
          }}
          onClose={() => {
          ...
          onSuccess={() => {
            ...
            toast.success(
              'Cliente creado exitosamente. Las comunicaciones se han actualizado.'
            )
          }}
          onClienteCreated={() => {
      ...
      {mostrarEditarCliente && clienteParaEditar && (
        <CrearClienteForm
          cliente={
            clienteParaEditar as unknown as {
              id?: number
              cedula?: string
              nombre?: string
              apellido?: string
          ...
          onSuccess={() => {
            ...
            toast.success(
              'Cliente actualizado. Teléfono y email se han guardado en la BD.'
            )
          }}
        />
      )}
