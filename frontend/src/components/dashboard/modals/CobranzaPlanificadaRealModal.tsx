...
export function CobranzaPlanificadaRealModal({
  isOpen,
  onClose,
}: CobranzaPlanificadaRealModalProps) {
  ...
  const datosGrafico = useMemo(() => {
    ...
    return cobranzaPorDiaData.dias.map(d => {
      ...
      const porcentajeCumplimiento =
        planificada > 0 ? (real / planificada) * 100 : 0
  ...
  const metricas = useMemo(() => {
    ...
    const totalPlanificada = datosGrafico.reduce(
      (sum, d) => sum + d.cobranza_planificada,
      0
    )
    ...
    const porcentajeCumplimiento =
      totalPlanificada > 0 ? (totalReal / totalPlanificada) * 100 : 0
  ...
  const CustomTooltip = ({
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="min-w-[200px] rounded-lg border border-gray-200 bg-white p-4 shadow-lg">
          <p className="mb-3 font-semibold text-gray-800">{label}</p>
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center justify-between border-t pt-2">
              ...
              <span
                className={`font-semibold ${data?.diferencia >= 0 ? 'text-emerald-600' : 'text-red-600'}`}
              >
                {data?.diferencia >= 0 ? '+' : ''}
            ...
            <div className="flex items-center justify-between">
              ...
              <span
                className={`font-semibold ${data?.porcentaje_cumplimiento >= 100 ? 'text-emerald-600' : 'text-orange-600'}`}
              >
      ...
      <div className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button
                      variant={
                        vistaRapida === 'ultimos3' ? 'default' : 'outline'
                      }
                      size="sm"
                    ...
                    <Button
                      variant={
                        vistaRapida === 'personalizado' ? 'default' : 'outline'
                      }
                      size="sm"
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
          <Card>
            ...
            <CardContent>
              ...
              <div className="mt-1 text-xs text-gray-500">
                Promedio diario:{' '}
                {formatCurrency(metricas.promedioDiarioPlanificada)}
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div
                className={`text-2xl font-bold ${metricas.diferenciaTotal >= 0 ? 'text-emerald-600' : 'text-red-600'}`}
              >
                {metricas.diferenciaTotal >= 0 ? '+' : ''}
                {formatCurrency(metricas.diferenciaTotal)}
              </div>
          ...
          <Card>
            ...
            <CardContent>
              <div
                className={`text-2xl font-bold ${metricas.porcentajeCumplimiento >= 100 ? 'text-emerald-600' : 'text-orange-600'}`}
              >
                {metricas.porcentajeCumplimiento.toFixed(1)}%
              </div>

              <div className="mt-1 text-xs text-gray-500">
                {metricas.diasCumplidos} días cumplidos /{' '}
                {metricas.diasNoCumplidos} no cumplidos
              </div>
            </CardContent>
          </Card>
        </div>
