...
export function CobranzaPlanificadaRealModal({ isOpen, onClose }: CobranzaPlanificadaRealModalProps) {
  ...
  // Calcular fechas segÃÂºn vista rÃÂ¡pida
  const calcularFechasVista = (vista: VistaRapida) => {
    const hoy = new Date()
    hoy.setHours(0, 0, 0, 0)
    const manana = new Date(hoy)
    manana.setDate(manana.getDate() + 1)
    // ÃÂÃÂ¡ltimos 3 dÃÂ­as: desde hace 2 dÃÂ­as hasta hoy (3 dÃÂ­as en total)
    ...
    switch (vista) {
      ...
      default:
        // personalizado - usar dÃÂ­as mostrar
  ...
  const { data: cobranzaPorDiaData, isLoading: loadingPorDia, refetch } = useQuery({
    ...
    queryFn: async (): Promise<CobranzaPorDiaResponse> => {
      ...
      Object.entries(params).forEach(([key, value]) => {
        // ÃÂ¢ÃÂÃ¢ÂÂ¦ Evitar duplicar fecha_inicio y fecha_fin ya que se agregan manualmente despuÃÂ©s
  ...
  // Procesar datos para el grÃÂ¡fico
  ...
  // Renderizar grÃÂ¡fico segÃÂºn tipo
  const renderGrafico = () => {
    const commonProps = {
      data: datosGrafico,
  ...
  return (
    <BaseModal
      isOpen={isOpen}
      onClose={onClose}
      title="Cobranza Planificada vs Real por DÃÂ­a"
      size="xlarge"
    >
      <div className="space-y-6">
        ...
        <div className="space-y-4">
          ...
          {/* Controles de visualizaciÃÂ³n */}
          <Card>
            <CardContent className="pt-6">
              <div className="space-y-4">
                {/* Vistas rÃÂ¡pidas */}
                <div className="flex items-center gap-2">
                  <span className="text-sm font-medium text-gray-700">Vista rÃÂ¡pida:</span>
                ...
                {/* Tipo de grÃÂ¡fico y dÃÂ­as (solo visible en personalizado) */}
                <div className="flex flex-wrap items-center gap-4">
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-medium text-gray-700">Tipo de grÃÂ¡fico:</span>
                    <div className="flex gap-2">
                      <Button
                  ...
                  {vistaRapida === 'personalizado' && (
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-medium text-gray-700">DÃÂ­as a mostrar:</span>
                      <select
                        value={diasMostrar}
                        onChange={(e) => {
                      ...
                      >
                        <option value={7}>7 dÃÂ­as</option>
                        <option value={15}>15 dÃÂ­as</option>
                        <option value={30}>30 dÃÂ­as</option>
                        <option value={60}>60 dÃÂ­as</option>
                        <option value={90}>90 dÃÂ­as</option>
                      </select>
                    </div>
                  )}
                </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <Card>
            ...
            <CardContent>
              ...
              <div className="text-xs text-gray-500 mt-1">
                {metricas.diasCumplidos} dÃÂ­as cumplidos / {metricas.diasNoCumplidos} no cumplidos
              </div>
            </CardContent>
          </Card>
        </div>

        {/* GrÃÂ¡fico principal */}
        <Card>
          <CardHeader>
