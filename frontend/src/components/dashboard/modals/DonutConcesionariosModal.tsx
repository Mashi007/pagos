...
export function DonutConcesionariosModal({
  isOpen,
  onClose,
}: DonutConcesionariosModalProps) {
  ...
  const datosPrincipales = datosGrafico.filter(
    d => d.porcentaje >= umbralPorcentaje
  )
      ...
      ? [
          {
            ...
            cantidad_prestamos: datosOtros.reduce(
              (sum, d) => sum + d.cantidad_prestamos,
              0
            ),
          },
        ]
  ...
  const CustomTooltip = ({
    ...
    payload?: Array<{
      name?: string
      value?: number
      payload?: { porcentaje?: number; cantidad_prestamos?: number }
    }>
  }) => {
      ...
      return (
        <div className="rounded-lg border border-gray-200 bg-white p-3 shadow-lg">
          ...
          <p className="mb-1 text-sm">
            ...
            {typeof data.payload?.porcentaje === 'number'
              ? data.payload.porcentaje.toFixed(2)
              : '0.00'}
            %
          </p>
          ...
          <p className="text-sm">
            ...
            {typeof data.payload?.cantidad_prestamos === 'number'
              ? data.payload.cantidad_prestamos
              : 0}{' '}
            préstamos
          </p>
        </div>
    ...
    >
      <div className="space-y-6">
        ...
        <Card>
          <CardHeader>
            <CardTitle>
              Distribución de Préstamos por Concesionario (Porcentaje)
            </CardTitle>
          </CardHeader>
          ...
          <CardContent>
            {loadingPrestamos ? (
              ...
              <div className="flex h-[500px] items-center justify-center text-gray-500">
                No hay datos de préstamos disponibles para el período
                seleccionado
              </div>
            ) : (
                    ...
                    <PieChart>
                      ...
                      <Legend
                        ...
                        formatter={(value: string) =>
                          `${value} (${datosFinales.find(d => d.concesionario === value)?.porcentaje.toFixed(1)}%)`
                        }
                      />
                    </PieChart>
                  </ResponsiveContainer>
