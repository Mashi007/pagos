...
export function TendenciasModal({ isOpen, onClose }: TendenciasModalProps) {
  ...
  // ConfiguraciÃÂ³n de polling (cada 10 minutos por defecto)
  ...
  const { data: tendenciasData, isLoading: loadingTendencias, refetch } = useQuery({
    ...
    staleTime: 5 * 60 * 1000,
    refetchInterval: isOpen ? POLLING_INTERVAL : false, // Polling solo si el modal estÃÂ¡ abierto
  })

  // Efecto para actualizaciÃÂ³n automÃÂ¡tica cuando cambian los crÃÂ©ditos/amortizaciones
  useEffect(() => {
    if (!isOpen) return
  ...
  // Separar datos histÃÂ³ricos de proyecciones
  ...
  // Combinar datos para el grÃÂ¡fico
            ...
            return (
              <p key={index} className="text-sm" style={{ color: entry.color }}>
                ...
                {dataKey.includes('cuotas_en_dias')
                  ? `${typeof value === 'number' ? value.toFixed(0) : 'N/A'} dÃÂ­as`
                  : typeof value === 'number'
  ...
  return (
    <BaseModal
      isOpen={isOpen}
      onClose={onClose}
      title="Tendencias de Cuentas por Cobrar y Cuotas en DÃÂ­as"
      size="xlarge"
    >
      <div className="space-y-6">
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-4 flex-wrap">
            <div className="flex items-center gap-2">
              <label className="text-sm font-medium">Meses de ProyecciÃÂ³n:</label>
              <Select
            ...
            <div className="flex items-center gap-2">
              ...
              >
                ...
                <SelectContent>
                  ...
                  <SelectItem value="proximos_n_dias">PrÃÂ³ximos N DÃÂ­as</SelectItem>
                  <SelectItem value="hasta_fin_anio">Hasta Fin de AÃÂ±o</SelectItem>
            ...
            {granularidad === 'proximos_n_dias' && (
              <div className="flex items-center gap-2">
                <label className="text-sm font-medium">DÃÂ­as:</label>
                <Select
                ...
                >
                  ...
                  <SelectContent>
                    <SelectItem value="7">7 dÃÂ­as</SelectItem>
                    <SelectItem value="15">15 dÃÂ­as</SelectItem>
                    <SelectItem value="30">30 dÃÂ­as</SelectItem>
                    <SelectItem value="60">60 dÃÂ­as</SelectItem>
                    <SelectItem value="90">90 dÃÂ­as</SelectItem>
                  </SelectContent>
                </Select>
              </div>
            )}
            ...
            {tendenciasData?.ultima_actualizacion && (
              <div className="flex items-center gap-2 ml-auto">
                ...
                <span className="text-xs text-gray-500">
                  ÃÂltima actualizaciÃÂ³n: {new Date(tendenciasData.ultima_actualizacion).toLocaleString('es-ES')}
                </span>
              </div>
            )}
          </div>
        ...
        {/* GrÃÂ¡fico de Tendencias */}
        <Card>
          <CardHeader>
            <CardTitle>Cuentas por Cobrar y Cuotas en DÃÂ­as (con ProyecciÃÂ³n)</CardTitle>
          </CardHeader>
          <CardContent>
            {loadingTendencias ? (
              <div className="h-[500px] flex items-center justify-center">
                <div className="animate-pulse text-gray-400">Cargando grÃÂ¡fico...</div>
              </div>
            ) : datosGrafico.length === 0 ? (
              <div className="h-[500px] flex items-center justify-center text-gray-500">
                No hay datos disponibles para el perÃÂ­odo seleccionado
              </div>
            ) : (
              <div className="space-y-6">
                {/* GrÃÂ¡fico 1: Cuentas por Cobrar */}
                <div>
                    ...
                    <AreaChart data={datosGrafico} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
                      ...
                      {fechaDivision && (
                        <ReferenceLine
                          ...
                          stroke="#ef4444"
                          strokeDasharray="5 5"
                          label={{ value: 'Inicio ProyecciÃÂ³n', position: 'top', fill: '#ef4444' }}
                        />
                      )}
                      <Area
                        type="monotone"
                ...
                {/* GrÃÂ¡fico 2: Cuotas en DÃÂ­as */}
                <div>
                  <h3 className="text-lg font-semibold mb-3">Cuotas en DÃÂ­as</h3>
                  <ResponsiveContainer width="100%" height={300}>
                    <LineChart data={datosGrafico} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
                      ...
                      <YAxis
                        stroke="#6b7280"
                        style={{ fontSize: '12px' }}
                        label={{ value: 'DÃÂ­as', angle: -90, position: 'insideLeft' }}
                      />
                      <Tooltip content={<CustomTooltip />} />
                      <Legend />
                      {fechaDivision && (
                      ...
                      <Line
                        ...
                        name="Cuotas en DÃÂ­as (Proyectado)"
                        stroke="#10b981"
                        strokeDasharray="5 5"
                        strokeWidth={2}
                        dot={{ r: 3 }}
