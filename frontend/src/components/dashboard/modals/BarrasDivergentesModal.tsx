...
export function BarrasDivergentesModal({
  isOpen,
  onClose,
}: BarrasDivergentesModalProps) {
  const [filtros, setFiltros] = useState<DashboardFiltros>({})

  const [tipoDistribucion, setTipoDistribucion] =
    useState<TipoDistribucion>('rango_monto')
  ...
  const datosGrafico = [...(distribucionData?.distribucion || [])].sort(
    (a, b) => a.monto_total - b.monto_total
  )
  ...
  const CustomTooltip = ({
    ...
    if (active && payload && payload.length) {
      ...
      const cantidad =
        typeof data.cantidad_prestamos === 'number'
          ? data.cantidad_prestamos
          : 0

      const montoTotal =
        typeof data.monto_total === 'number' ? data.monto_total : 0

      const porcentaje =
        typeof data.porcentaje_monto === 'number' ? data.porcentaje_monto : 0
        ...
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <Select
              ...
              onValueChange={v => setTipoDistribucion(v as TipoDistribucion)}
            >
              ...
              <div className="mt-6 grid grid-cols-2 gap-4 border-t pt-6">
                <div>
                  ...
                  <div className="text-2xl font-bold">
                    {distribucionData.total_prestamos}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="text-2xl font-bold">
                    {formatCurrency(distribucionData.total_monto)}
                  </div>
                </div>
              </div>
            )}
