...
export function TreemapMorosidadModal({
  isOpen,
  onClose,
}: TreemapMorosidadModalProps) {
  ...
  const CustomTooltip = ({
    ...
    if (active && payload && payload.length) {
      ...
      const cantidadCuotas =
        typeof data.cantidad_cuotas_vencidas === 'number'
          ? data.cantidad_cuotas_vencidas
          : 0

      return (
        <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-lg">
          <p className="mb-2 text-lg font-bold">
            {typeof data.name === 'string' ? data.name : 'N/A'}
          </p>
          ...
          <p className="mb-1 text-sm">
            <span className="font-semibold">Cuotas vencidas:</span>{' '}
            {cantidadCuotas}
          </p>
        </div>
      )
    ...
    >
      <div className="space-y-6">
        ...
        <Card>
          <CardHeader>
            <CardTitle>
              Pago vencido por Analista (cuotas vencidas y dólares vencidos)
            </CardTitle>
          </CardHeader>
          ...
          <CardContent>
            {loadingMorosidad ? (
              ...
              <div className="flex h-[500px] items-center justify-center text-gray-500">
                No hay datos de pago vencido disponibles para el período
                seleccionado
              </div>
            ) : (
