...
export interface ChartWithDateRangeSliderProps<T extends Record<string, unknown>> {
  /** Datos completos del grÃÂ¡fico (cada item debe tener la clave dataKey para etiquetas) */
  data: T[]
  /** Clave del objeto que contiene la etiqueta de perÃÂ­odo (ej. "mes", "nombre_fecha") */
  dataKey: string
  /** Contenido del grÃÂ¡fico: recibe solo los datos filtrados por el rango */
  children: (filteredData: T[]) => React.ReactNode
  /** Altura del contenedor del grÃÂ¡fico (sin contar la regleta) */
  chartHeight?: number
...
/**
 * Envuelve un grÃÂ¡fico y aÃÂ±ade una regleta (slider de rango) al pie para filtrar
 * los datos entre dos extremos. Cada grÃÂ¡fico muestra solo el segmento seleccionado.
 */
    ...
    <div className="w-full">
      ...
      {showSlider && (
        <div
          ...
          role="group"
          aria-label="Rango de fechas del grÃÂ¡fico"
        >
          <div className="flex items-center justify-between gap-2 mb-2">
            ...
            <span className="text-xs font-semibold text-gray-700 tabular-nums">
              {String(labelFrom)} Ã¢ÂÂ {String(labelTo)}
            </span>
          </div>
          {/* Una sola regleta: pista ÃÂºnica con dos thumbs que recorren los extremos */}
          <div className="relative h-8 flex items-center">
            ...
            />
            {/* Thumbs visibles posicionados segÃÂºn valores (maxIndex puede ser 0, evitar /0) */}
