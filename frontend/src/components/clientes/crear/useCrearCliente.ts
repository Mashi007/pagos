...
export function useCrearCliente({
  ...
  const validateNombres = (nombres: string): ValidationResult => {
    ...
    if (wordCount < 2 && textEndsWithSpace) return { field: 'nombres', isValid: false, message: 'MÃ­nimo 2 palabras requeridas (nombre + apellido)' }
    if (wordCount > 7) return { field: 'nombres', isValid: false, message: `MÃ¡ximo 7 palabras permitidas (tienes ${wordCount})` }
    if (wordCount >= 2 && wordCount <= 7) {
      const invalidWords = words.filter(w => w.length < 2)
      if (invalidWords.length > 0) return { field: 'nombres', isValid: false, message: 'Cada palabra debe tener mÃ­nimo 2 caracteres' }
      return { field: 'nombres', isValid: true, message: `${wordCount} palabra${wordCount > 1 ? 's' : ''} - VÃ¡lido` }
    }
    if (wordCount === 1 && !textEndsWithSpace) return { field: 'nombres', isValid: false, message: 'Agrega mÃ¡s palabras (mÃ­nimo 2, mÃ¡ximo 7)' }
  ...
  const validateOcupacion = (ocupacion: string): ValidationResult => {
    ...
    if (!ocupacion || ocupacion.trim() === '') return { field: 'ocupacion', isValid: false, message: 'OcupaciÃ³n requerida' }
    if (ocupacion.trim().length < 2) return { field: 'ocupacion', isValid: false, message: 'MÃ­nimo 2 caracteres' }
    const words = ocupacion.trim().split(/\s+/).filter(w => w.length > 0)
    if (words.length > 2) return { field: 'ocupacion', isValid: false, message: 'MÃ¡ximo 2 palabras permitidas en ocupaciÃ³n' }
  ...
  const validateDescripcion = (descripcion: string): ValidationResult => {
    ...
    if (words.length < 5) return { field: 'descripcion', isValid: false, message: `MÃ­nimo 5 palabras requeridas (tienes ${words.length})` }
    return { field: 'descripcion', isValid: true, message: `${words.length} palabras - VÃ¡lido` }
  }
  ...
  const validateTelefono = (telefono: string): ValidationResult => {
    ...
    if (!telefono || telefono.trim() === '') return { field: 'telefono', isValid: false, message: 'TelÃ©fono requerido' }
    const numeroLimpio = telefono.replace(/\D/g, '')
    if (numeroLimpio.length > 10) return { field: 'telefono', isValid: true, message: 'Se usarÃ¡ 9999999999 por defecto (>10 dÃ­gitos)' }
    if (numeroLimpio.length !== 10) return { field: 'telefono', isValid: false, message: 'El telÃ©fono debe tener exactamente 10 dÃ­gitos' }
    if (numeroLimpio.startsWith('0')) return { field: 'telefono', isValid: false, message: 'El telÃ©fono no puede empezar por 0' }
    if (!/^[1-9]\d{9}$/.test(numeroLimpio)) return { field: 'telefono', isValid: false, message: 'El telÃ©fono solo puede contener nÃºmeros (0-9)' }
  ...
  const validateFechaNacimiento = (fecha: string): ValidationResult => {
    ...
    if (!fechaFormatRegex.test(fecha.trim())) return { field: 'fechaNacimiento', isValid: false, message: 'Formato invÃ¡lido. Use: DD/MM/YYYY' }
    const [, dia, mes, ano] = fecha.trim().match(fechaFormatRegex)!
    const diaNum = parseInt(dia, 10)
    const mesNum = parseInt(mes, 10)
    const anoNum = parseInt(ano, 10)
    if (diaNum < 1 || diaNum > 31) return { field: 'fechaNacimiento', isValid: false, message: 'DÃ­a invÃ¡lido (1-31)' }
    if (mesNum < 1 || mesNum > 12) return { field: 'fechaNacimiento', isValid: false, message: 'Mes invÃ¡lido (1-12)' }
    if (anoNum < 1900 || anoNum > 2100) return { field: 'fechaNacimiento', isValid: false, message: 'AÃ±o invÃ¡lido (1900-2100)' }
    ...
    if (fecha21 > hoy) return { field: 'fechaNacimiento', isValid: false, message: 'Debe tener al menos 21 aÃ±os cumplidos' }
    const fecha60 = new Date(anoNum + 60, mesNum - 1, diaNum)
    if (fecha60 <= hoy) return { field: 'fechaNacimiento', isValid: false, message: 'No puede tener mÃ¡s de 60 aÃ±os cumplidos' }
  ...
  const validateEmail = (email: string): ValidationResult => {
    ...
    if (!emailPattern.test(emailTrimmed)) return { field: 'email', isValid: false, message: 'El email debe tener una extensiÃ³n vÃ¡lida (.com, .edu, .gob, etc.)' }
  ...
  const validateFieldWithBackend = async (field: string, value: string): Promise<ValidationResult> => {
    if (field === 'cedula' && !(value || '').trim()) return { field: 'cedula', isValid: true, message: 'Se usarÃ¡ Z999999999 por defecto' }
    ...
    try {
      ...
      if (resultado.validacion.valido) return { field, isValid: true, message: resultado.validacion.mensaje || 'Campo vÃ¡lido' }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if ((!formData.ocupacion || !formData.ocupacion.trim()) && !isNN(formData.ocupacion)) { alert('ERROR: Debe completar el campo OcupaciÃ³n'); return }
    ...
    try {
      ...
      if (isAxiosError(error)) {
        ...
        if (statusCode === 400 || statusCode === 409) {
          ...
          if (detailText.includes('misma cÃ©dula')) { tipoDuplicado = 'cedula'; _errorMessageUser = errorDetail || 'Ya existe un cliente con esa cÃ©dula.' }
          ...
          else if (detailText.includes('mismo telÃ©fono')) { tipoDuplicado = 'telefono'; _errorMessageUser = errorDetail || 'Ya existe un cliente con ese telÃ©fono.' }
          else if (detailText.includes('cÃ©dula') || detailText.includes('nombre') || detailText.includes('email') || detailText.includes('telÃ©fono')) { tipoDuplicado = 'datos'; _errorMessageUser = errorDetail || 'CÃ©dula, nombre, email o telÃ©fono ya pertenecen a otro cliente.' }
          else { _errorMessageUser = errorDetail || 'Datos duplicados (cÃ©dula, nombre, email o telÃ©fono).' }
      ...
      if (isAxiosError(error) && (error.response?.status === 400 || error.response?.status === 409)) {
        const mensajes: Record<string, string> = { cedula: 'la misma cÃ©dula', nombre: 'el mismo nombre completo', email: 'el mismo email', telefono: 'el mismo telÃ©fono', datos: 'la misma cÃ©dula, nombre, email o telÃ©fono' }
        const mensajeDuplicado = mensajes[tipoDuplicado] || 'la misma cÃ©dula, nombre, email o telÃ©fono'
