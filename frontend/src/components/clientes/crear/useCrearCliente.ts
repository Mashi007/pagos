...
export function useCrearCliente({
  ...
  const formatOcupacion = (text: string): string =>
    text?.trim() ? toTitleCase(text) : text
  ...
  const validateNombres = (nombres: string): ValidationResult => {
    ...
    if (wordCount < 2 && textEndsWithSpace)
      return {
        field: 'nombres',
        isValid: false,
        message: 'Mínimo 2 palabras requeridas (nombre + apellido)',
      }
    ...
    if (wordCount > 7)
      return {
        field: 'nombres',
        isValid: false,
        message: `Máximo 7 palabras permitidas (tienes ${wordCount})`,
      }
    ...
    if (wordCount >= 2 && wordCount <= 7) {
      ...
      if (invalidWords.length > 0)
        return {
          field: 'nombres',
          isValid: false,
          message: 'Cada palabra debe tener mínimo 2 caracteres',
        }

      return {
        field: 'nombres',
        isValid: true,
        message: `${wordCount} palabra${wordCount > 1 ? 's' : ''} - Válido`,
      }
    }
    ...
    if (wordCount === 1 && !textEndsWithSpace)
      return {
        field: 'nombres',
        isValid: false,
        message: 'Agrega más palabras (mínimo 2, máximo 7)',
      }
  ...
  const validateOcupacion = (ocupacion: string): ValidationResult => {
    ...
    if (words.length > 2)
      return {
        field: 'ocupacion',
        isValid: false,
        message: 'Máximo 2 palabras permitidas en ocupación',
      }
  ...
  const validateDescripcion = (descripcion: string): ValidationResult => {
    ...
    if (!descripcion || descripcion.trim() === '')
      return { field: 'descripcion', isValid: true, message: '' }
    ...
    if (words.length < 5)
      return {
        field: 'descripcion',
        isValid: false,
        message: `Mínimo 5 palabras requeridas (tienes ${words.length})`,
      }

    return {
      field: 'descripcion',
      isValid: true,
      message: `${words.length} palabras - Válido`,
    }
  }

  const validateDireccion = (): ValidationResult => {
    if (
      (!formData.callePrincipal || formData.callePrincipal.trim() === '') &&
      !isNN(formData.callePrincipal)
    )
      return {
        field: 'direccion',
        isValid: false,
        message: 'Calle Principal es requerida',
      }

    if (
      (!formData.parroquia || formData.parroquia.trim() === '') &&
      !isNN(formData.parroquia)
    )
      return {
        field: 'direccion',
        isValid: false,
        message: 'Parroquia es requerida',
      }

    if (
      (!formData.municipio || formData.municipio.trim() === '') &&
      !isNN(formData.municipio)
    )
      return {
        field: 'direccion',
        isValid: false,
    ...
    if (
      (!formData.ciudad || formData.ciudad.trim() === '') &&
      !isNN(formData.ciudad)
    )
      return {
        field: 'direccion',
        isValid: false,
    ...
    if (
      (!formData.estadoDireccion || formData.estadoDireccion.trim() === '') &&
      !isNN(formData.estadoDireccion)
    )
      return {
        field: 'direccion',
        isValid: false,
  ...
  const validateTelefono = (telefono: string): ValidationResult => {
    ...
    if (numeroLimpio.length > 10)
      return {
        field: 'telefono',
        isValid: true,
        message: 'Se usará 9999999999 por defecto (>10 dígitos)',
      }
    ...
    if (numeroLimpio.startsWith('0'))
      return {
        field: 'telefono',
        isValid: false,
        message: 'El teléfono no puede empezar por 0',
      }
    ...
    if (!/^[1-9]\d{9}$/.test(numeroLimpio))
      return {
        field: 'telefono',
        isValid: false,
        message: 'El teléfono solo puede contener números (0-9)',
      }
  ...
  const validateFechaNacimiento = (fecha: string): ValidationResult => {
    ...
    if (anoNum < 1900 || anoNum > 2100)
      return {
        field: 'fechaNacimiento',
        isValid: false,
        message: 'Año inválido (1900-2100)',
      }
    ...
    if (fecha21 > hoy)
      return {
        field: 'fechaNacimiento',
        isValid: false,
        message: 'Debe tener al menos 21 años cumplidos',
      }
    ...
    if (fecha60 <= hoy)
      return {
        field: 'fechaNacimiento',
        isValid: false,
        message: 'No puede tener más de 60 años cumplidos',
      }
  ...
  const validateEmail = (email: string): ValidationResult => {
    ...
    if (emailTrimmed.includes(','))
      return {
        field: 'email',
        isValid: false,
        message: 'El email no puede contener comas',
      }
    ...
    if (!emailTrimmed.includes('@'))
      return {
        field: 'email',
        isValid: false,
        message: 'El email debe contener un @',
      }
    ...
    if (!emailPattern.test(emailTrimmed))
      return {
        ...
        isValid: false,
        message:
          'El email debe tener una extensión válida (.com, .edu, .gob, etc.)',
      }
  ...
  const validateFieldWithBackend = async (
    ...
    if (field === 'cedula' && !(value || '').trim())
      return {
        field: 'cedula',
        isValid: true,
        message: 'Se usará Z999999999 por defecto',
      }
    ...
    try {
      ...
      return {
        field,
        isValid: false,
        message: sugerencia ? `${errorMsg}. ${sugerencia}` : errorMsg,
      }
    } catch {
      if (!value)
        return { field, isValid: false, message: `${field} es obligatorio` }
  ...
  const handleInputChange = async (field: keyof FormData, value: string) => {
    ...
    if (field === 'nombres') validation = validateNombres(formattedValue)
      ...
      setTimeout(() => {
        ...
        setValidations(prev => [
          ...prev.filter(v => v.field !== 'direccion'),
          direccionValidation,
        ])
      }, 100)
      ...
      validation = !cliente
        ? await validateFieldWithBackend(field, formattedValue)
  ...
  const isFormValid = () => {
    ...
    const fechaNacimientoValidation = validateFechaNacimiento(
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (
      ...
      alert('ERROR: Debe completar el campo Calle Principal')
      return
    }
    ...
    if (
      ...
      alert('ERROR: Debe completar el campo Parroquia')
      return
    }
    ...
    if (
      ...
      alert('ERROR: Debe completar el campo Municipio')
      return
    }
    ...
    if (
      ...
      alert('ERROR: Debe completar el campo Ciudad')
      return
    }
    ...
    if (
      ...
      alert('ERROR: Debe completar el campo Estado')
      return
    }

    if (
      (!formData.fechaNacimiento || !formData.fechaNacimiento.trim()) &&
      !isNN(formData.fechaNacimiento)
    ) {
      alert('ERROR: Debe completar el campo Fecha de Nacimiento')
      return
    }

    if (
      (!formData.ocupacion || !formData.ocupacion.trim()) &&
      !isNN(formData.ocupacion)
    ) {
      alert('ERROR: Debe completar el campo Ocupación')
      return
    }
    ...
    try {
      ...
      const direccionCompleta = JSON.stringify({
        ...
        calleTransversal: blankIfNN(formData.calleTransversal)
          ? toTitleCase(blankIfNN(formData.calleTransversal))
          : null,
      ...
      const todosLosDatos = {
        ...
        fecha_nacimiento: convertirFechaAISO(
          blankIfNN(formData.fechaNacimiento)
        ),
      ...
      if (cliente && typeof cliente.id === 'number') {
        ...
        if (datosOriginales) {
          const reconstruirDireccionOriginal = () => {
            try {
              return JSON.stringify({
                callePrincipal: toTitleCase(
                  blankIfNN(datosOriginales.callePrincipal || '')
                ),

                calleTransversal: blankIfNN(
                  datosOriginales.calleTransversal || ''
                )
                  ? toTitleCase(
                      blankIfNN(datosOriginales.calleTransversal || '')
                    )
                  : null,

                descripcion:
                  blankIfNN(datosOriginales.descripcion || '') || null,

                parroquia: toTitleCase(
                  blankIfNN(datosOriginales.parroquia || '')
                ),

                municipio: toTitleCase(
                  blankIfNN(datosOriginales.municipio || '')
                ),

                ciudad: toTitleCase(blankIfNN(datosOriginales.ciudad || '')),

                estado: toTitleCase(
                  blankIfNN(datosOriginales.estadoDireccion || '')
                ),
              })
            } catch {
              return ''
          ...
          const telefonoOriginal = datosOriginales.telefono
            ? `+58${extraerNumeroTelefono(datosOriginales.telefono)}`
            : ''
          ...
          if (
            !valoresIguales(
              todosLosDatos.fecha_nacimiento,
              convertirFechaAISO(datosOriginales.fechaNacimiento || '')
            )
          )
            clienteData.fecha_nacimiento = todosLosDatos.fecha_nacimiento

          if (
            !valoresIguales(todosLosDatos.ocupacion, datosOriginales.ocupacion)
          )
        ...
        await clienteService.updateCliente(
          String(cliente.id),
          clienteData as Partial<ClienteForm>
        )
      ...
      if (isAxiosError(error)) {
        ...
        if (statusCode === 400 || statusCode === 409) {
          ...
          if (detailText.includes('misma cédula')) {
            ...
            _errorMessageUser =
              errorDetail || 'Ya existe un cliente con esa cédula.'
            ...
            _errorMessageUser =
              errorDetail || 'Ya existe un cliente con ese teléfono.'
          } else if (
            ...
            _errorMessageUser =
              errorDetail ||
              'Datos duplicados (cédula, nombre, email o teléfono).'
          }
  ...
  const getFieldValidation = (field: string) =>
    cliente ? null : validations.find(v => v.field === field)
  ...
  useEffect(() => {
    if (cliente) {
      ...
      const parsearDireccion = (direccionCompleta: string) => {
        ...
        if (
          direccionLimpia.trim().startsWith('{') &&
          direccionLimpia.trim().endsWith('}')
        ) {
          try {
            ...
            return {
              ...
              estadoDireccion: String(
                parsed.estado || parsed.estadoDireccion || ''
              ).trim(),
            }
          } catch {
            const calleMatch = direccionLimpia.match(
              /"callePrincipal"\s*:\s*"([^"]*)"/i
            )

            const parroquiaMatch = direccionLimpia.match(
              /"parroquia"\s*:\s*"([^"]*)"/i
            )

            const municipioMatch = direccionLimpia.match(
              /"municipio"\s*:\s*"([^"]*)"/i
            )

            const ciudadMatch = direccionLimpia.match(
              /"ciudad"\s*:\s*"([^"]*)"/i
            )

            const estadoMatch = direccionLimpia.match(
              /"estado"\s*:\s*"([^"]*)"/i
            )
      ...
      const newFormData: FormData = {
        ...
        telefono: extraerNumeroTelefono(
          typeof cliente.telefono === 'string' ? cliente.telefono : ''
        ),
        ...
        fechaNacimiento: convertirFechaLocal(
          typeof cliente.fecha_nacimiento === 'string'
            ? cliente.fecha_nacimiento
            : ''
        ),

        ocupacion:
          typeof cliente.ocupacion === 'string' ? cliente.ocupacion : '',

        estado:
          typeof cliente.estado === 'string' && cliente.estado
            ? cliente.estado
            : 'ACTIVO',

        notas:
          typeof cliente.notas === 'string'
            ? cliente.notas
            : 'No hay observacion',
      }
