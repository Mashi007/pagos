...
export function useCrearCliente({
  ...
  const validateNombres = (nombres: string): ValidationResult => {
    ...
    if (wordCount < 2 && textEndsWithSpace) return { field: 'nombres', isValid: false, message: 'MÃÂ­nimo 2 palabras requeridas (nombre + apellido)' }
    if (wordCount > 7) return { field: 'nombres', isValid: false, message: `MÃÂ¡ximo 7 palabras permitidas (tienes ${wordCount})` }
    if (wordCount >= 2 && wordCount <= 7) {
      const invalidWords = words.filter(w => w.length < 2)
      if (invalidWords.length > 0) return { field: 'nombres', isValid: false, message: 'Cada palabra debe tener mÃÂ­nimo 2 caracteres' }
      return { field: 'nombres', isValid: true, message: `${wordCount} palabra${wordCount > 1 ? 's' : ''} - VÃÂ¡lido` }
    }
    if (wordCount === 1 && !textEndsWithSpace) return { field: 'nombres', isValid: false, message: 'Agrega mÃÂ¡s palabras (mÃÂ­nimo 2, mÃÂ¡ximo 7)' }
    return { field: 'nombres', isValid: false, message: 'MÃÂ­nimo 2 palabras requeridas (nombre + apellido)' }
  }
  ...
  const validateOcupacion = (ocupacion: string): ValidationResult => {
    ...
    if (!ocupacion || ocupacion.trim() === '') return { field: 'ocupacion', isValid: false, message: 'OcupaciÃÂ³n requerida' }
    if (ocupacion.trim().length < 2) return { field: 'ocupacion', isValid: false, message: 'MÃÂ­nimo 2 caracteres' }
    const words = ocupacion.trim().split(/\s+/).filter(w => w.length > 0)
    if (words.length > 2) return { field: 'ocupacion', isValid: false, message: 'MÃÂ¡ximo 2 palabras permitidas en ocupaciÃÂ³n' }
    return { field: 'ocupacion', isValid: true, message: 'OcupaciÃÂ³n vÃÂ¡lida' }
  }
  ...
  const validateDescripcion = (descripcion: string): ValidationResult => {
    ...
    if (words.length < 5) return { field: 'descripcion', isValid: false, message: `MÃÂ­nimo 5 palabras requeridas (tienes ${words.length})` }
    return { field: 'descripcion', isValid: true, message: `${words.length} palabras - VÃÂ¡lido` }
  }
  ...
  const validateDireccion = (): ValidationResult => {
    ...
    return { field: 'direccion', isValid: true, message: 'DirecciÃÂ³n vÃÂ¡lida' }
  }
  ...
  const validateTelefono = (telefono: string): ValidationResult => {
    ...
    if (!telefono || telefono.trim() === '') return { field: 'telefono', isValid: false, message: 'TelÃÂ©fono requerido' }
    const numeroLimpio = telefono.replace(/\D/g, '')
    if (numeroLimpio.length > 10) return { field: 'telefono', isValid: true, message: 'Se usarÃÂ¡ 9999999999 por defecto (>10 dÃÂ­gitos)' }
    if (numeroLimpio.length !== 10) return { field: 'telefono', isValid: false, message: 'El telÃÂ©fono debe tener exactamente 10 dÃÂ­gitos' }
    if (numeroLimpio.startsWith('0')) return { field: 'telefono', isValid: false, message: 'El telÃÂ©fono no puede empezar por 0' }
    if (!/^[1-9]\d{9}$/.test(numeroLimpio)) return { field: 'telefono', isValid: false, message: 'El telÃÂ©fono solo puede contener nÃÂºmeros (0-9)' }
    return { field: 'telefono', isValid: true, message: 'TelÃÂ©fono vÃÂ¡lido' }
  }
  ...
  const validateFechaNacimiento = (fecha: string): ValidationResult => {
    ...
    if (!fechaFormatRegex.test(fecha.trim())) return { field: 'fechaNacimiento', isValid: false, message: 'Formato invÃÂ¡lido. Use: DD/MM/YYYY' }
    const [, dia, mes, ano] = fecha.trim().match(fechaFormatRegex)!
    const diaNum = parseInt(dia, 10)
    const mesNum = parseInt(mes, 10)
    const anoNum = parseInt(ano, 10)
    if (diaNum < 1 || diaNum > 31) return { field: 'fechaNacimiento', isValid: false, message: 'DÃÂ­a invÃÂ¡lido (1-31)' }
    if (mesNum < 1 || mesNum > 12) return { field: 'fechaNacimiento', isValid: false, message: 'Mes invÃÂ¡lido (1-12)' }
    if (anoNum < 1900 || anoNum > 2100) return { field: 'fechaNacimiento', isValid: false, message: 'AÃÂ±o invÃÂ¡lido (1900-2100)' }
    ...
    if (fechaNac.getDate() !== diaNum || fechaNac.getMonth() !== mesNum - 1 || fechaNac.getFullYear() !== anoNum)
      return { field: 'fechaNacimiento', isValid: false, message: 'Fecha invÃÂ¡lida (ej: 31/02 no existe)' }
    const hoy = new Date()
    ...
    if (fecha21 > hoy) return { field: 'fechaNacimiento', isValid: false, message: 'Debe tener al menos 21 aÃÂ±os cumplidos' }
    const fecha60 = new Date(anoNum + 60, mesNum - 1, diaNum)
    if (fecha60 <= hoy) return { field: 'fechaNacimiento', isValid: false, message: 'No puede tener mÃÂ¡s de 60 aÃÂ±os cumplidos' }
    return { field: 'fechaNacimiento', isValid: true, message: 'Fecha vÃÂ¡lida' }
  }
  ...
  const validateEmail = (email: string): ValidationResult => {
    ...
    if (!emailPattern.test(emailTrimmed)) return { field: 'email', isValid: false, message: 'El email debe tener una extensiÃÂ³n vÃÂ¡lida (.com, .edu, .gob, etc.)' }
    return { field: 'email', isValid: true, message: 'Email vÃÂ¡lido' }
  }

  const validateFieldWithBackend = async (field: string, value: string): Promise<ValidationResult> => {
    if (field === 'cedula' && !(value || '').trim()) return { field: 'cedula', isValid: true, message: 'Se usarÃÂ¡ Z999999999 por defecto' }
    ...
    try {
      ...
      if (resultado.validacion.valido) return { field, isValid: true, message: resultado.validacion.mensaje || 'Campo vÃÂ¡lido' }
      const errorMsg = resultado.validacion.error || 'Campo invÃÂ¡lido'
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if ((!formData.ocupacion || !formData.ocupacion.trim()) && !isNN(formData.ocupacion)) { alert('ERROR: Debe completar el campo OcupaciÃÂ³n'); return }
    ...
    try {
      ...
      if (isAxiosError(error)) {
        ...
        if (statusCode === 400 || statusCode === 409) {
          ...
          if (detailText.includes('misma cÃÂ©dula')) { tipoDuplicado = 'cedula'; _errorMessageUser = errorDetail || 'Ya existe un cliente con esa cÃÂ©dula.' }
          ...
          else if (detailText.includes('mismo telÃÂ©fono')) { tipoDuplicado = 'telefono'; _errorMessageUser = errorDetail || 'Ya existe un cliente con ese telÃÂ©fono.' }
          else if (detailText.includes('cÃÂ©dula') || detailText.includes('nombre') || detailText.includes('email') || detailText.includes('telÃÂ©fono')) { tipoDuplicado = 'datos'; _errorMessageUser = errorDetail || 'CÃÂ©dula, nombre, email o telÃÂ©fono ya pertenecen a otro cliente.' }
          else { _errorMessageUser = errorDetail || 'Datos duplicados (cÃÂ©dula, nombre, email o telÃÂ©fono).' }
      ...
      if (isAxiosError(error) && (error.response?.status === 400 || error.response?.status === 409)) {
        const mensajes: Record<string, string> = { cedula: 'la misma cÃÂ©dula', nombre: 'el mismo nombre completo', email: 'el mismo email', telefono: 'el mismo telÃÂ©fono', datos: 'la misma cÃÂ©dula, nombre, email o telÃÂ©fono' }
        const mensajeDuplicado = mensajes[tipoDuplicado] || 'la misma cÃÂ©dula, nombre, email o telÃÂ©fono'
        const friendly = existingId
          ? `ADVERTENCIA: Ya existe un cliente con ${mensajeDuplicado}.\n\nCliente existente ID: ${existingId}\n\nÃÂ¿Deseas abrir el cliente existente para editarlo?`
          : `ADVERTENCIA: Ya existe un cliente con ${mensajeDuplicado}.\n\nÃÂ¿Deseas buscar el cliente existente?`
