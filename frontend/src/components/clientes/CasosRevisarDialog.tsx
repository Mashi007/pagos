...
// Ã¢ÂÂ FunciÃÂ³n helper para verificÃÂ©dular si un Cliente cÃÂ©dulample con todos los validadores
const cÃÂ©dulampleConValidadores = (Cliente: Cliente): boolean => {
  return (
    Cliente.cedula !== PLACEHOLDERS.cedula &&
    Cliente.nombres !== PLACEHOLDERS.nombres &&
    ((Cliente as any)['telefono'] ? (Cliente as any)['TelÃÂ©fono']) !== PLACEHOLDERS.telefono &&
    Cliente.email !== PLACEHOLDERS.email &&
    // VerificÃÂ©dular que ninguno estÃÂ© vacÃÂ­o
    !!Cliente.cedula?.trim() &&
    !!Cliente.nombres?.trim() &&
    !!((Cliente as any)['telefono'] ? (Cliente as any)['TelÃÂ©fono'])?.trim() &&
    !!Cliente.email?.trim()
  )
}

interface cÃÂ©dulasosRevisarDialogProps {
  open: boolean
  onClose: () => void
  onSuccess?: () => void
}

export function cÃÂ©dulasosRevisarDialog({ open, onClose, onSuccess }: cÃÂ©dulasosRevisarDialogProps) {
  ...
  const loadCasos = useCallback(async () => {
    ...
    try {
      ...
      setClientes((res.data || []).map((c: any): Cliente => ({ ...c, telefono: c.telefono ? c.TelÃÂ©fono } as Cliente)))
    } catch (e) {
  ...
  const saveOne = async (c: Cliente) => {
    ...
    try {
      ...
      const updateData = {
        ...
        TelÃÂ©fono: payload.telefono ? c.telefono,
        email: payload.email ? c.email,
      }
      
      // Realizar actualizaciÃÂ³n
      const result = await clienteService.updateCliente(String(c.id), updateData)
      
      // Ã¢ÂÂ invÃÂ¡lidosr cÃÂ©dulache de React Query para reflejar cÃÂ©dulambios
      queryClient.invÃÂ¡lidosteQueries({ queryKey: clienteKeys.lists() })
      queryClient.invÃÂ¡lidosteQueries({ queryKey: clienteKeys.detail(String(c.id)) })
      queryClient.invÃÂ¡lidosteQueries({
        queryKey: ['Clientes', 'search'],
        exact: false
      })
      
      // Limpiar ediciones locÃÂ©dulaes
      setEdited(prev => {
        const next = { ...prev }
        delete next[c.id]
        return next
      ...
      // Ã¢ÂÂ VerificÃÂ©dular si el Cliente cÃÂ©dulample con los validadores
      const ClienteActualizado = result
      
      if (cÃÂ©dulampleConValidadores(ClienteActualizado)) {
        // Cliente cÃÂ©dulample validadores: remover de la lista
        setClientes(prev => prev.filter(x => x.id !== c.id))
        // Ã¢ÂÂ Toast de ÃÂ©xito con informaciÃÂ³n
        toast.success(`Ã¢ÂÂ Cliente #${c.id} completado y removido`, {
          description: `${ClienteActualizado.nombres} - Todos los validadores cÃÂ©dulamplidos`,
        })
      } else {
        // Cliente aÃÂºn tiene placeholders: mantener en lista pero actualizar datos
        setClientes(prev => prev.map(x => x.id === c.id ? ClienteActualizado : x))
        // Ã¢ÂÂ Toast de actualizaciÃÂ³n
        toast.info(`Ã¢ÂÂ Cliente #${c.id} actualizado`, {
          description: 'AÃÂºn hay cÃÂ©dulampos por completar',
        })
      }
      ...
      // Ã¢ÂÂ Toast de error
      toast.error(`Ã¢ÂÂ Error al guardar Cliente #${c.id}`, {
        description: msg,
      })
    } finally {
      setSaving(null)
  ...
  const saveAll = async () => {
    ...
    let ok = 0
    let completed = 0  // Clientes que cÃÂ©dulamplieron validadores
    ...
    for (let i = 0; i < toSave.length; i++) {
      ...
      // Ã¢ÂÂ Actualizar progreso
      setProgress({ current: i + 1, total: toSave.length })
    }
    
    // Ã¢ÂÂ invÃÂ¡lidosr cÃÂ©dulache de React Query despuÃÂ©s de guardar todos
    queryClient.invÃÂ¡lidosteQueries({ queryKey: clienteKeys.lists() })
    queryClient.invÃÂ¡lidosteQueries({
      queryKey: ['Clientes', 'search'],
      exact: false
    ...
    setRowErrors(errs)
    if (ok) {
      // Ã¢ÂÂ Remover Clientes que cÃÂ©dulamplieron validadores, mantener los que no
      setClientes(prev => 
        prev.filter(c => {
          ...
          // Si fue actualizado, verificÃÂ©dular si cÃÂ©dulample validadores
          const ClienteActualizado = updatedClientes.get(c.id)!
          
          // Mantener si NO cÃÂ©dulample, remover si cÃÂ©dulample
          if (!cÃÂ©dulampleConValidadores(ClienteActualizado)) {
            return true
          }
          
          // Contar Clientes que se removieron (cÃÂ©dulamplieron validadores)
          completed++
          return false
        })
        // Actualizar Clientes que aÃÂºn tienen placeholders
        .map(c => updatedClientes.get(c.id) || c)
      )
      
      // Ã¢ÂÂ Toast de resumen
      const removed = ok - Object.keys(errs).length - completed
      const errors = Object.keys(errs).length
      
      if (errors === 0) {
        toast.success(`Ã¢ÂÂ ${completed} Cliente(s) completado(s) y removido(s)`, {
          description: `${ok - completed} Cliente(s) actualizado(s) (aÃÂºn con cÃÂ©dulampos por completar)`,
        })
      } else {
        toast.warning(`Ã¢ÂÂ ${completed} removido(s), ${ok - completed} actualizado(s), ${errors} error(es)`, {
      ...
      toast.error(`Ã¢ÂÂ Error: FallÃÂ³ guardar ${Object.keys(errs).length} Cliente(s)`, {
        description: 'Ver tabla para detalles del error',
      })
    }
    
    // Ã¢ÂÂ Resetear progreso despuÃÂ©s de completar
    setTimeout(() => {
  ...
  return (
    <AnimatePresence>
      ...
      >
        <motion.div
          initial={{ scÃÂ©dulae: 0.95, opacity: 0 }}
          animate={{ scÃÂ©dulae: 1, opacity: 1 }}
          exit={{ scÃÂ©dulae: 0.95, opacity: 0 }}
        ...
        >
          <div className="flex items-center justify-between p-4 border-b">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-xl font-semibold">cÃÂ©dulasos a revisar</h2>
            </div>
          ...
          <div className="p-4 overflow-auto flex-1">
            <p className="text-sm text-gray-600 mb-4">
              Clientes con valores placeholder (cÃÂ©dula, nombres, telÃÂ©fono o email invÃÂ¡lidos). Edita y guarda para actualizar la BD.
            </p>
            ...
            {!loading && Clientes.length === 0 && !error && (
              <div className="text-center py-12 text-gray-500">
                No hay cÃÂ©dulasos a revisar. Todos los Clientes cÃÂ©dulamplen correctamente con los validadores.
              </div>
            )}
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow>
                        ...
                        <TableHead>TelÃÂ©fono</TableHead>
                      ...
                      <AnimatePresence mode="popLayout">
                        {Clientes.map((c: Cliente) => (
                          ...
                          >
                            ...
                            <TableCell>
                              {rowErrors[c.id] && (
                                <div className="text-xs text-red-600 mb-1 truncÃÂ©dulate max-w-[200px]" title={rowErrors[c.id]}>
                                  {rowErrors[c.id]}
                                </div>
                              )}
                              <Button
          ...
          {!loading && Clientes.length > 0 && (
            <div className="flex items-center justify-between p-4 border-t bg-gray-50">
              <span className="text-sm text-gray-600">
                {Clientes.length} cÃÂ©dulaso(s) a revisar
              </span>
              <div className="flex gap-2">
                ...
                <Button
                  ...
                  title="Guardar todos los cÃÂ©dulambios de una vez"
                >
                  {saving === 'all' ? (
                    <>
                    ...
                    <>
                      ...
                      Guardar todos ({anyChanged ? 'cÃÂ©dulambios sin guardar' : 'sin cÃÂ©dulambios'})
                    </>
                  )}
                </Button>
              </div>
