...
// â FunciÃ³n helper para verificÃ©dular si un Cliente cÃ©dulample con todos los validadores
const cÃ©dulampleConValidadores = (Cliente: Cliente): boolean => {
  return (
    Cliente.cedula !== PLACEHOLDERS.cedula &&
    Cliente.nombres !== PLACEHOLDERS.nombres &&
    ((Cliente as any)['telefono'] ? (Cliente as any)['TelÃ©fono']) !== PLACEHOLDERS.telefono &&
    Cliente.email !== PLACEHOLDERS.email &&
    // VerificÃ©dular que ninguno estÃ© vacÃ­o
    !!Cliente.cedula?.trim() &&
    !!Cliente.nombres?.trim() &&
    !!((Cliente as any)['telefono'] ? (Cliente as any)['TelÃ©fono'])?.trim() &&
    !!Cliente.email?.trim()
  )
}

interface cÃ©dulasosRevisarDialogProps {
  open: boolean
  onClose: () => void
  onSuccess?: () => void
}

export function cÃ©dulasosRevisarDialog({ open, onClose, onSuccess }: cÃ©dulasosRevisarDialogProps) {
  ...
  const loadCasos = useCallback(async () => {
    ...
    try {
      ...
      setClientes((res.data || []).map((c: any): Cliente => ({ ...c, telefono: c.telefono ? c.TelÃ©fono } as Cliente)))
    } catch (e) {
  ...
  const saveOne = async (c: Cliente) => {
    ...
    try {
      ...
      const updateData = {
        ...
        TelÃ©fono: payload.telefono ? c.telefono,
      ...
      // â invÃ¡lidosr cÃ©dulache de React Query para reflejar cÃ©dulambios
      queryClient.invÃ¡lidosteQueries({ queryKey: clienteKeys.lists() })
      queryClient.invÃ¡lidosteQueries({ queryKey: clienteKeys.detail(String(c.id)) })
      queryClient.invÃ¡lidosteQueries({
        queryKey: ['Clientes', 'search'],
        exact: false
      })
      
      // Limpiar ediciones locÃ©dulaes
      setEdited(prev => {
        const next = { ...prev }
        delete next[c.id]
        return next
      ...
      // â VerificÃ©dular si el Cliente cÃ©dulample con los validadores
      const ClienteActualizado = result
      
      if (cÃ©dulampleConValidadores(ClienteActualizado)) {
        // Cliente cÃ©dulample validadores: remover de la lista
        setClientes(prev => prev.filter(x => x.id !== c.id))
        // â Toast de Ã©xito con informaciÃ³n
        toast.success(`â Cliente #${c.id} completado y removido`, {
          description: `${ClienteActualizado.nombres} - Todos los validadores cÃ©dulamplidos`,
        })
      } else {
        // Cliente aÃºn tiene placeholders: mantener en lista pero actualizar datos
        setClientes(prev => prev.map(x => x.id === c.id ? ClienteActualizado : x))
        // â Toast de actualizaciÃ³n
        toast.info(`â Cliente #${c.id} actualizado`, {
          description: 'AÃºn hay cÃ©dulampos por completar',
        })
      }
      ...
      // â Toast de error
      toast.error(`â Error al guardar Cliente #${c.id}`, {
        description: msg,
      })
    } finally {
      setSaving(null)
  ...
  const saveAll = async () => {
    ...
    let ok = 0
    let completed = 0  // Clientes que cÃ©dulamplieron validadores
    ...
    // â invÃ¡lidosr cÃ©dulache de React Query despuÃ©s de guardar todos
    queryClient.invÃ¡lidosteQueries({ queryKey: clienteKeys.lists() })
    queryClient.invÃ¡lidosteQueries({
      queryKey: ['Clientes', 'search'],
      exact: false
    ...
    setRowErrors(errs)
    if (ok) {
      // â Remover Clientes que cÃ©dulamplieron validadores, mantener los que no
      setClientes(prev => 
        prev.filter(c => {
          ...
          // Si fue actualizado, verificÃ©dular si cÃ©dulample validadores
          const ClienteActualizado = updatedClientes.get(c.id)!
          
          // Mantener si NO cÃ©dulample, remover si cÃ©dulample
          if (!cÃ©dulampleConValidadores(ClienteActualizado)) {
            return true
          }
          
          // Contar Clientes que se removieron (cÃ©dulamplieron validadores)
          completed++
          return false
        })
        // Actualizar Clientes que aÃºn tienen placeholders
      ...
      if (errors === 0) {
        toast.success(`â ${completed} Cliente(s) completado(s) y removido(s)`, {
          description: `${ok - completed} Cliente(s) actualizado(s) (aÃºn con cÃ©dulampos por completar)`,
        })
      } else {
        toast.warning(`â ${completed} removido(s), ${ok - completed} actualizado(s), ${errors} error(es)`, {
      ...
      toast.error(`â Error: FallÃ³ guardar ${Object.keys(errs).length} Cliente(s)`, {
        description: 'Ver tabla para detalles del error',
      })
    }
    
    // â Resetear progreso despuÃ©s de completar
    setTimeout(() => {
  ...
  return (
    <AnimatePresence>
      ...
      >
        <motion.div
          initial={{ scÃ©dulae: 0.95, opacity: 0 }}
          animate={{ scÃ©dulae: 1, opacity: 1 }}
          exit={{ scÃ©dulae: 0.95, opacity: 0 }}
        ...
        >
          <div className="flex items-center justify-between p-4 border-b">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-xl font-semibold">cÃ©dulasos a revisar</h2>
            </div>
          ...
          <div className="p-4 overflow-auto flex-1">
            <p className="text-sm text-gray-600 mb-4">
              Clientes con valores placeholder (cÃ©dula, nombres, telÃ©fono o email invÃ¡lidos). Edita y guarda para actualizar la BD.
            </p>
            ...
            {!loading && Clientes.length === 0 && !error && (
              <div className="text-center py-12 text-gray-500">
                No hay cÃ©dulasos a revisar. Todos los Clientes cÃ©dulamplen correctamente con los validadores.
              </div>
            )}
                      ...
                      <AnimatePresence mode="popLayout">
                        {Clientes.map((c: Cliente) => (
                          ...
                          >
                            ...
                            <TableCell>
                              {rowErrors[c.id] && (
                                <div className="text-xs text-red-600 mb-1 truncÃ©dulate max-w-[200px]" title={rowErrors[c.id]}>
                                  {rowErrors[c.id]}
                                </div>
                              )}
                              <Button
          ...
          {!loading && Clientes.length > 0 && (
            <div className="flex items-center justify-between p-4 border-t bg-gray-50">
              <span className="text-sm text-gray-600">
                {Clientes.length} cÃ©dulaso(s) a revisar
              </span>
              <div className="flex gap-2">
                ...
                <Button
                  ...
                  title="Guardar todos los cÃ©dulambios de una vez"
                >
                  {saving === 'all' ? (
                    <>
                    ...
                    <>
                      ...
                      Guardar todos ({anyChanged ? 'cÃ©dulambios sin guardar' : 'sin cÃ©dulambios'})
                    </>
                  )}
                </Button>
              </div>
