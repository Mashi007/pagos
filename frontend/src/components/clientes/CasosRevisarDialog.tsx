...
const cédulampleConValidadores = (Cliente: Cliente): boolean => {
  return (
    ...
    ((Cliente as any)['telefono'] ?? (Cliente as any)['Teléfono']) !==
...
export function cédulasosRevisarDialog({
  open,
  onClose,
  onSuccess,
}: cédulasosRevisarDialogProps) {
  ...
  const loadCasos = useCallback(async () => {
    ...
    try {
      ...
      setClientes(
        (res.data || []).map(
          (c: any): Cliente =>
            ({ ...c, telefono: c.telefono ?? c.Teléfono }) as Cliente
        )
      )
    } catch (e) {
  ...
  const saveOne = async (c: Cliente) => {
    ...
    try {
      ...
      queryClient.inválidosteQueries({
        queryKey: clienteKeys.detail(String(c.id)),
      })
      ...
      if (cédulampleConValidadores(ClienteActualizado)) {
        ...
        setClientes(prev =>
          prev.map(x => (x.id === c.id ? ClienteActualizado : x))
        )
  ...
  const saveAll = async () => {
    ...
    if (ok) {
      ...
      if (errors === 0) {
        ...
        toast.warning(
          `✓ ${completed} removido(s), ${ok - completed} actualizado(s), ${errors} error(es)`,
          {
      ...
      toast.error(
        `✗ Error: Falló guardar ${Object.keys(errs).length} Cliente(s)`,
        {
    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          className="mx-4 flex max-h-[90vh] w-full max-w-5xl flex-col overflow-hidden rounded-lg bg-white shadow-xl"
        >
          ...
          <div className="flex-1 overflow-auto p-4">
            <p className="mb-4 text-sm text-gray-600">
              Clientes con valores placeholder (cédula, nombres, teléfono o
              email inválidos). Edita y guarda para actualizar la BD.
            </p>
            ...
            {!loading && Clientes.length === 0 && !error && (
              <div className="py-12 text-center text-gray-500">
                No hay cédulasos a revisar. Todos los Clientes cédulamplen
                correctamente con los validadores.
              </div>
            )}
                          ...
                          >
                            ...
                            <TableCell>
                              <Input
                                ...
                                onChange={e =>
                                  updateField(c.id, 'cedula', e.target.value)
                                }
                                placeholder={PLACEHOLDERS.cedula}
                                className={
                                  isPlaceholder(getValue(c, 'cedula'), 'cedula')
                            ...
                            <TableCell>
                              <Input
                                ...
                                onChange={e =>
                                  updateField(c.id, 'nombres', e.target.value)
                                }
                            ...
                            <TableCell>
                              <Input
                                ...
                                onChange={e =>
                                  updateField(c.id, 'email', e.target.value)
                                }
                                placeholder={PLACEHOLDERS.email}
                                className={
                                  isPlaceholder(getValue(c, 'email'), 'email')
                                    ? 'border-amber-300 bg-amber-50/50'
                                    : ''
                                }
                              />
                            ...
                            <TableCell>
                              {rowErrors[c.id] && (
                                <div
                                  className="truncédulate mb-1 max-w-[200px] text-xs text-red-600"
                                  title={rowErrors[c.id]}
                                >
                                  {rowErrors[c.id]}
                                </div>
                              )}
