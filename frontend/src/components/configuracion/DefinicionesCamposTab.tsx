...
export function DefinicionesCamposTab() {
  ...
  const cargarTodo = async () => {
    ...
    try {
      ...
      console.log(`ÃÂ°ÃÂ¸Ã¢ÂÂÃÂ  Campos disponibles: ${totalCampos}, Definiciones existentes: ${definicionesCargadas.length}`)
      
      // Si no hay definiciones pero hay campos disponibles, sincronizar automÃÂ¡ticamente
      if (definicionesCargadas.length === 0 && totalCampos > 0 && !yaSincronizado) {
        console.log('ÃÂ°ÃÂ¸Ã¢ÂÂÃ¢ÂÂ No hay definiciones precargadas. Sincronizando automÃÂ¡ticamente...')
  ...
  const cargarDefiniciones = async () => {
    try {
      // Carga desde la tabla definiciones_campos (BD) vÃÂ­a API
  ...
  const cargarCamposDisponibles = async () => {
    try {
      ...
      // Log para debugging
      const totalCampos = Object.values(camposCargados).reduce((total, campos) => total + campos.length, 0)
      console.log(`ÃÂ°ÃÂ¸Ã¢ÂÂÃ¢ÂÂ¹ Campos disponibles cargados: ${totalCampos} campos en ${Object.keys(camposCargados).length} tablas`)
      
      return camposCargados
    } catch (error: any) {
      console.error('ÃÂ¢ÃÂÃÂ Error cargando campos disponibles:', error)
  ...
  const handleSincronizarAutomatico = async () => {
    // SincronizaciÃÂ³n automÃÂ¡tica sin confirmaciÃÂ³n
    ...
    try {
      ...
      toast.success(
        `ÃÂ¢ÃÂÃ¢ÂÂ¦ SincronizaciÃÂ³n automÃÂ¡tica completada: ${response.campos_creados} campos precargados`
      )
      // Recargar todo despuÃÂ©s de sincronizar
      await cargarCamposDisponibles()
      await cargarDefiniciones()
      await cargarTablas()
    } catch (error: any) {
      console.error('Error sincronizando automÃÂ¡ticamente:', error)
      toast.error(error?.response?.data?.detail || 'Error al sincronizar campos automÃÂ¡ticamente')
    } finally {
  ...
  const handleSincronizar = async () => {
    if (!confirm('ÃÂ¿Deseas sincronizar todos los campos de la base de datos? Esto crearÃÂ¡ definiciones para campos que no existen y actualizarÃÂ¡ informaciÃÂ³n tÃÂ©cnica de campos existentes sin sobrescribir definiciones personalizadas.')) {
      return
    }
    ...
    try {
      ...
      toast.success(
        `SincronizaciÃÂ³n completada: ${response.campos_creados} creados, ${response.campos_actualizados} actualizados`
      )
      cargarDefiniciones()
      cargarTablas()
  ...
  const handleSeleccionarCampo = (tabla: string, campo: CampoDisponible) => {
    // Verificar si ya existe definiciÃÂ³n
    const existe = definiciones.find(d => d.tabla === tabla && d.campo === campo.nombre)
    if (existe) {
      toast.info('Este campo ya tiene una definiciÃÂ³n. Puedes editarla.')
      handleEditar(existe)
      return
    }

    // Prellenar formulario con informaciÃÂ³n del campo
    setFormulario({
      tabla: tabla,
      campo: campo.nombre,
  ...
  const handleGuardar = async () => {
    if (!formulario.tabla.trim() || !formulario.campo.trim() || !formulario.definicion.trim()) {
      toast.error('Tabla, campo y definiciÃÂ³n son obligatorios')
      return
    }
    ...
    try {
      ...
      if (editandoId) {
        ...
        toast.success('DefiniciÃÂ³n actualizada exitosamente')
      } else {
        await apiClient.post('/api/v1/configuracion/ai/definiciones-campos', datos)
        toast.success('DefiniciÃÂ³n creada exitosamente')
      }
      ...
      toast.error(error?.response?.data?.detail || 'Error al guardar la definiciÃÂ³n')
    } finally {
      setGuardando(false)
    }
  }
  ...
  const handleEliminar = async (id: number) => {
    if (!confirm('ÃÂ¿EstÃÂ¡s seguro de eliminar esta definiciÃÂ³n?')) return

    try {
      await apiClient.delete(`/api/v1/configuracion/ai/definiciones-campos/${id}`)
      toast.success('DefiniciÃÂ³n eliminada exitosamente')
      cargarDefiniciones()
    } catch (error: any) {
      console.error('Error eliminando:', error)
      toast.error(error?.response?.data?.detail || 'Error al eliminar la definiciÃÂ³n')
    }
  }
  ...
  const handleToggleActivo = async (definicion: DefinicionCampo) => {
    try {
      ...
      toast.success(`DefiniciÃÂ³n ${!definicion.activo ? 'activada' : 'desactivada'}`)
      cargarDefiniciones()
    } catch (error: any) {
      ...
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-2xl font-bold flex items-center gap-2">
            ...
            CatÃÂ¡logo de Campos
          </h3>
          <p className="text-sm text-gray-600 mt-1">
            Define todos los campos de la base de datos con sus descripciones para entrenar acceso rÃÂ¡pido
          </p>
        </div>
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="text-xs text-gray-500 mt-1">
                            {campo.tipo}
                            {campo.es_obligatorio && ' Ã¢ÂÂ¢ NOT NULL'}
                            {campo.tiene_indice && ' Ã¢ÂÂ¢ Indexado'}
                            {campo.es_clave_foranea && ` Ã¢ÂÂ¢ FK Ã¢ÂÂ ${campo.tabla_referenciada}`}
                          </div>
                        </button>
                      )
                    })}
          ...
          <div className="flex items-center gap-4">
            <div className="flex-1">
              <div className="relative">
                ...
                <Input
                  placeholder="Buscar por tabla, campo o definiciÃÂ³n..."
                  value={busqueda}
        ...
        <Card className="border-purple-200">
          <CardContent className="pt-6">
            <h4 className="text-lg font-semibold mb-4">
              {editandoId ? 'Editar DefiniciÃÂ³n' : 'Nueva DefiniciÃÂ³n'}
            </h4>
            <div className="space-y-4">
              {/* Esencial: Tabla + Campo + DefiniciÃÂ³n + Tipo */}
              ...
              <div>
                <label className="text-sm font-medium block mb-1">DefiniciÃÂ³n <span className="text-red-500">*</span></label>
                <Textarea
                  value={formulario.definicion}
                  onChange={(e) => setFormulario({ ...formulario, definicion: e.target.value })}
                  placeholder="QuÃÂ© almacena este campo y cÃÂ³mo se usa..."
                  rows={2}
                />
              </div>
                  ...
                  <div className="mt-3 pl-2 border-l-2 border-gray-200 space-y-3">
                    <div className="flex flex-wrap gap-4">
                      ...
                      <label className="flex items-center gap-2">
                        ...
                        <span className="text-sm">Tiene ÃÂ­ndice</span>
                      </label>
                      <label className="flex items-center gap-2">
                        ...
                        <span className="text-sm">Clave forÃÂ¡nea</span>
                      </label>
                    </div>
                    ...
                    <div>
                      <label className="text-sm text-gray-600 block mb-1">Valores posibles (uno por lÃÂ­nea)</label>
                    ...
                    <div>
                      <label className="text-sm text-gray-600 block mb-1">Ejemplos (uno por lÃÂ­nea)</label>
                          ...
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            {def.es_clave_foranea && (
                              <Badge variant="outline" className="bg-purple-50 text-purple-700 border-purple-200">
                                <Key className="h-3 w-3 mr-1" />
                                FK Ã¢ÂÂ {def.tabla_referenciada}.{def.campo_referenciado}
                              </Badge>
                            )}
                          </div>
