...
export function DefinicionesCamposTab() {
  ...
  const cargarTodo = async () => {
    ...
    try {
      ...
      console.log(`Ã°Å¸âÅ  Campos disponibles: ${totalCampos}, Definiciones existentes: ${definicionesCargadas.length}`)
      ...
      if (definicionesCargadas.length === 0 && totalCampos > 0 && !yaSincronizado) {
        console.log('Ã°Å¸ââ No hay definiciones precargadas. Sincronizando automÃ¡ticamente...')
  ...
  const cargarDefiniciones = async () => {
    try {
      // Carga desde la tabla definiciones_campos (BD) vÃ­a API
  ...
  const cargarCamposDisponibles = async () => {
    try {
      ...
      // Log para debugging
      const totalCampos = Object.values(camposCargados).reduce((total, campos) => total + campos.length, 0)
      console.log(`Ã°Å¸ââ¹ Campos disponibles cargados: ${totalCampos} campos en ${Object.keys(camposCargados).length} tablas`)
      
      return camposCargados
    } catch (error: any) {
      console.error('Ã¢ÂÅ Error cargando campos disponibles:', error)
  ...
  const handleSincronizarAutomatico = async () => {
    ...
    try {
      ...
      toast.success(
        `Ã¢Åâ¦ SincronizaciÃ³n automÃ¡tica completada: ${response.campos_creados} campos precargados`
      )
        ...
        <Card className="border-purple-200">
          <CardContent className="pt-6">
            ...
            <div className="space-y-4">
              {/* Esencial: Tabla + Campo + DefiniciÃ³n + Tipo */}
              ...
              <div>
                <label className="text-sm font-medium block mb-1">DefiniciÃ³n <span className="text-red-500">*</span></label>
                <Textarea
                  value={formulario.definicion}
                  onChange={(e) => setFormulario({ ...formulario, definicion: e.target.value })}
                  placeholder="QuÃ© almacena este campo y cÃ³mo se usa..."
                  rows={2}
                />
              </div>
                  ...
                  <div className="mt-3 pl-2 border-l-2 border-gray-200 space-y-3">
                    <div className="flex flex-wrap gap-4">
                      ...
                      <label className="flex items-center gap-2">
                        ...
                        <span className="text-sm">Clave forÃ¡nea</span>
                      </label>
                    </div>
                    ...
                    <div>
                      <label className="text-sm text-gray-600 block mb-1">Valores posibles (uno por lÃ­nea)</label>
                    ...
                    <div>
                      <label className="text-sm text-gray-600 block mb-1">Ejemplos (uno por lÃ­nea)</label>
