...
export function DefinicionesCamposTab() {
  ...
  const [mostrarCamposDisponibles, setMostrarCamposDisponibles] =
    useState(false)
  ...
  const cargarTodo = async () => {
    ...
    try {
      ...
      const totalCampos = Object.values(camposCargados).reduce(
        (total, campos) => total + campos.length,
        0
      )

      console.log(
        `ðŸ"Š Campos disponibles: ${totalCampos}, Definiciones existentes: ${definicionesCargadas.length}`
      )

      // Si no hay definiciones pero hay campos disponibles, sincronizar automáticamente

      if (
        definicionesCargadas.length === 0 &&
        totalCampos > 0 &&
        !yaSincronizado
      ) {
        console.log(
          'ðŸ"„ No hay definiciones precargadas. Sincronizando automáticamente...'
        )
  ...
  const cargarCamposDisponibles = async () => {
    try {
      ...
      console.log(
        `ðŸ"‹ Campos disponibles cargados: ${totalCampos} campos en ${Object.keys(camposCargados).length} tablas`
      )
    ...
    try {
      ...
      toast.error(
        error?.response?.data?.detail ||
          'Error al sincronizar campos automáticamente'
      )
    } finally {
  ...
  const handleSincronizar = async () => {
    if (
      !confirm(
        '¿Deseas sincronizar todos los campos de la base de datos? Esto creará definiciones para campos que no existen y actualizará información técnica de campos existentes sin sobrescribir definiciones personalizadas.'
      )
    ) {
      return
    }
    ...
    try {
      ...
      toast.error(
        error?.response?.data?.detail || 'Error al sincronizar campos'
      )
    } finally {
      setSincronizando(false)
    }
  ...
  const handleSeleccionarCampo = (tabla: string, campo: CampoDisponible) => {
    ...
    const existe = definiciones.find(
      d => d.tabla === tabla && d.campo === campo.nombre
    )
  ...
  const handleGuardar = async () => {
    if (
      !formulario.tabla.trim() ||
      !formulario.campo.trim() ||
      !formulario.definicion.trim()
    ) {
    ...
    try {
      const datos = {
        ...
        valores_posibles: formulario.valores_posibles
          .split('\n')
          .filter(v => v.trim()),

        ejemplos_valores: formulario.ejemplos_valores
          .split('\n')
          .filter(e => e.trim()),
      ...
      if (editandoId) {
        await apiClient.put(
          `/api/v1/configuracion/ai/definiciones-campos/${editandoId}`,
          datos
        )
      ...
      toast.error(
        error?.response?.data?.detail || 'Error al guardar la definición'
      )
    } finally {
      setGuardando(false)
    }
  ...
  const handleEliminar = async (id: number) => {
    ...
    try {
      await apiClient.delete(
        `/api/v1/configuracion/ai/definiciones-campos/${id}`
      )
      ...
      toast.error(
        error?.response?.data?.detail || 'Error al eliminar la definición'
      )
    }
  }
  ...
  const handleToggleActivo = async (definicion: DefinicionCampo) => {
    try {
      await apiClient.put(
        `/api/v1/configuracion/ai/definiciones-campos/${definicion.id}`,
        {
          activo: !definicion.activo,
        }
      )
      ...
      toast.success(
        `Definición ${!definicion.activo ? 'activada' : 'desactivada'}`
      )
  ...
  const definicionesPorTabla = definicionesFiltradas.reduce(
    (acc, def) => {
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-600">
            Define todos los campos de la base de datos con sus descripciones
            para entrenar acceso rápido
          </p>
        </div>
        ...
        <div className="flex gap-2">
          ...
          <Button
            variant="outline"
            onClick={() =>
              setMostrarCamposDisponibles(!mostrarCamposDisponibles)
            }
          >
          ...
          <CardContent className="pt-6">
            ...
            <div className="max-h-96 space-y-4 overflow-y-auto">
              {Object.entries(camposDisponibles).map(([tabla, campos]) => (
                <div key={tabla} className="rounded-lg border bg-white p-3">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {campos.map(campo => {
                      const tieneDefinicion = definiciones.some(
                        d => d.tabla === tabla && d.campo === campo.nombre
                      )
                      ...
                      return (
                        <button
                          ...
                          className={`rounded border p-2 text-left text-sm transition-colors hover:bg-gray-50 ${
                            tieneDefinicion
                        ...
                        >
                          ...
                          <div className="mt-1 text-xs text-gray-500">
                            ...
                            {campo.es_clave_foranea &&
                              ` • FK → ${campo.tabla_referenciada}`}
                          </div>
                        </button>
                      )
                    })}
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={v =>
                      setFormulario({ ...formulario, tabla: v })
                    }
                  >
                    <SelectTrigger>
                    ...
                    <SelectContent>
                      {Object.keys(camposDisponibles).map(t => (
                ...
                <div>
                  ...
                  {formulario.tabla && camposDisponibles[formulario.tabla] ? (
                    <Select
                      ...
                      onValueChange={v => {
                        const c = camposDisponibles[formulario.tabla].find(
                          x => x.nombre === v
                        )
              ...
              <div>
                ...
                <Textarea
                  ...
                  onChange={e =>
                    setFormulario({ ...formulario, definicion: e.target.value })
                  }
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={e =>
                    setFormulario({ ...formulario, tipo_dato: e.target.value })
                  }
                  ...
                  <div className="mt-3 space-y-3 border-l-2 border-gray-200 pl-2">
                      ...
                      <label className="flex items-center gap-2">
                        <input
                          ...
                          onChange={e =>
                            setFormulario({
                              ...formulario,
                              es_clave_foranea: e.target.checked,
                            })
                          }
                          className="rounded"
                        />
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <Input
                          ...
                          onChange={e =>
                            setFormulario({
                              ...formulario,
                              tabla_referenciada: e.target.value,
                            })
                          }
                        ...
                        <Input
                          ...
                          onChange={e =>
                            setFormulario({
                              ...formulario,
                              campo_referenciado: e.target.value,
                            })
                          }
                          placeholder="Campo referenciado"
                        />
                      </div>
                    )}
                    ...
                    <div>
                      <label className="mb-1 block text-sm text-gray-600">
                        Valores posibles (uno por línea)
                      </label>
                      ...
                      <Textarea
                        ...
                        onChange={e =>
                          setFormulario({
                            ...formulario,
                            valores_posibles: e.target.value,
                          })
                        }
                    ...
                    <div>
                      ...
                      <Textarea
                        ...
                        onChange={e =>
                          setFormulario({
                            ...formulario,
                            ejemplos_valores: e.target.value,
                          })
                        }
                        ...
                        <div className="flex-1">
                          <div className="mb-2 flex items-center gap-2">
                            ...
                            {def.es_clave_foranea && (
                              ...
                              >
                                <Key className="mr-1 h-3 w-3" />
                                FK → {def.tabla_referenciada}.
                                {def.campo_referenciado}
                              </Badge>
                            )}
                          </div>

                          <p className="mb-2 text-gray-700">{def.definicion}</p>
                          ...
                          <div className="flex flex-wrap gap-2 text-sm text-gray-600">
                            ...
                            {def.ejemplos_valores.length > 0 && (
                              <Badge variant="outline">
                                Ejemplos:{' '}
                                {def.ejemplos_valores.slice(0, 2).join(', ')}
                              </Badge>
                            )}
                          </div>
