...
export function PromptEditor() {
  ...
  const [tienePromptPersonalizado, setTienePromptPersonalizado] =
    useState(false)
  ...
  const [variablesPersonalizadas, setVariablesPersonalizadas] = useState<
    Array<{
      id: number
  ...
  const [variableEditada, setVariableEditada] = useState({
    variable: '',
    descripcion: '',
  })
  ...
  const handleRestaurarDefault = async () => {
    if (
      !confirm(
        '¿Estás seguro de restaurar el prompt por defecto? Se perderá el prompt personalizado.'
      )
    ) {
      return
    }
  ...
  const handleIniciarEdicionVariable = (
    variable: (typeof variablesPersonalizadas)[0]
  ) => {
  ...
  const handleActualizarVariable = async (id: number) => {
    if (
      !variableEditada.variable.trim() ||
      !variableEditada.descripcion.trim()
    ) {
      toast.error('Variable y descripción son requeridos')
    ...
    try {
      ...
      toast.error(
        error?.response?.data?.detail || 'Error actualizando variable'
      )
    }
  }
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <Button onClick={handleGuardarPrompt} disabled={guardandoPrompt}>
            {guardandoPrompt ? (
              <>
          ...
          <div className="flex-1">
            <div className="mb-1 flex items-center justify-between">
              ...
              <Button
                ...
                size="sm"
                onClick={() =>
                  setMostrarGestionVariables(!mostrarGestionVariables)
                }
                className="text-xs"
              >
            ...
            <p className="mb-2 text-sm text-amber-800">
              El sistema reemplazará automáticamente estos placeholders con
              datos reales:
            </p>

            {/* Placeholders predeterminados */}

            <div className="mb-3 space-y-1 rounded bg-white p-3 font-mono text-xs">
              <div className="mb-1 font-semibold text-amber-900">
                Predeterminados:
              </div>

              <div>
                <code className="text-blue-600">{'{resumen_bd}'}</code> -
              ...
              <div>
                ...
                - Información del cliente si se busca por cédula
              </div>

              <div>
                <code className="text-blue-600">{'{datos_adicionales}'}</code> -
                Cálculos y análisis adicionales
              </div>

              <div>
                <code className="text-blue-600">{'{info_esquema}'}</code> -
                Esquema completo de la base de datos
              </div>

              <div>
                <code className="text-blue-600">{'{contexto_documentos}'}</code>{' '}
            ...
            {variablesPersonalizadas.length > 0 && (
              <div className="mb-3 space-y-1 rounded bg-white p-3 font-mono text-xs">
                ...
                {variablesPersonalizadas
                  ...
                  .map(varItem => (
                    <div key={varItem.id}>
                      <code className="text-green-600">{varItem.variable}</code>{' '}
                      - {varItem.descripcion}
                    </div>
                  ))}
              </div>
            ...
            {mostrarGestionVariables && (
              <div className="mt-3 rounded-lg border border-amber-300 bg-white p-4">
                ...
                <div className="mb-4 space-y-3 rounded-lg border p-3">
                  <h5 className="text-sm font-medium">Nueva Variable</h5>
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-xs text-gray-500">
                      Se agregarán llaves automáticamente si no las incluyes
                    </p>
                  </div>
                  ...
                  <Button
                    ...
                    disabled={
                      creandoVariable ||
                      !nuevaVariable.variable.trim() ||
                      !nuevaVariable.descripcion.trim()
                    }
                    size="sm"
                    className="w-full"
                  >
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="flex gap-2">
                              <Button
                                size="sm"
                                onClick={() =>
                                  handleActualizarVariable(varItem.id)
                                }
                                disabled={
                                  !variableEditada.variable.trim() ||
                                  !variableEditada.descripcion.trim()
                          ...
                          <div className="flex items-start justify-between">
                            <div className="flex-1">
                              <div className="mb-1 flex items-center gap-2">
                                ...
                                <Badge
                                  variant={
                                    varItem.activo ? 'default' : 'secondary'
                                  }
                                  className="text-xs"
                                >
                            ...
                            <div className="flex items-center gap-1">
                              <Button
                                ...
                                size="sm"
                                onClick={() =>
                                  handleIniciarEdicionVariable(varItem)
                                }
                              ...
                              <Button
                                ...
                                size="sm"
                                onClick={() =>
                                  handleEliminarVariable(varItem.id)
                                }
      ...
      <div>
        ...
        {mostrarPlaceholders && !tienePromptPersonalizado && (
          <div className="mb-4 rounded-lg border bg-gray-50 p-4">
            <p className="mb-2 text-sm font-medium">
              Template de Prompt (para referencia):
            </p>
        ...
        <p className="mt-2 text-xs text-gray-500">
          {promptPersonalizado.length} caracteres. El prompt debe incluir los
          placeholders mencionados arriba.
        </p>
      </div>

      <div className="rounded-lg border border-blue-200 bg-blue-50 p-4">
        <p className="text-sm text-blue-800">
          ðŸ'¡ <strong>Tip:</strong> Puedes personalizar el comportamiento del
          AI ajustando el prompt. Los placeholders se reemplazarán
          automáticamente con datos reales del sistema. Si dejas el prompt
          vacío, se usará el prompt por defecto.
        </p>
      </div>
    </div>
  )
