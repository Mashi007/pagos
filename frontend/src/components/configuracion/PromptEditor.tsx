...
export function PromptEditor() {
  ...
  const handleGuardarPrompt = async () => {
    ...
    try {
      ...
      toast.success('ÃÂ¢ÃÂÃ¢ÂÂ¦ Prompt personalizado guardado exitosamente')
      await cargarPrompt()
    } catch (error: any) {
  ...
  const handleRestaurarDefault = async () => {
    if (!confirm('ÃÂ¿EstÃÂ¡s seguro de restaurar el prompt por defecto? Se perderÃÂ¡ el prompt personalizado.')) {
      return
    }
    ...
    try {
      ...
      toast.success('ÃÂ¢ÃÂÃ¢ÂÂ¦ Prompt restaurado al valor por defecto')
      await cargarPrompt()
    } catch (error: any) {
  ...
  const cargarVariables = async () => {
    ...
    try {
      ...
      // No mostrar error si la tabla no existe aÃÂºn
    } finally {
  ...
  const handleCrearVariable = async () => {
    if (!nuevaVariable.variable.trim() || !nuevaVariable.descripcion.trim()) {
      toast.error('Variable y descripciÃÂ³n son requeridos')
      return
    }
  ...
  const handleEliminarVariable = async (id: number) => {
    if (!confirm('ÃÂ¿EstÃÂ¡s seguro de eliminar esta variable?')) return
...
{contexto_documentos}`
  ...
  const promptTemplate = `Eres un ANALISTA ESPECIALIZADO en prÃÂ©stamos y cobranzas con capacidad de anÃÂ¡lisis de KPIs operativos. Tu funciÃÂ³n es proporcionar informaciÃÂ³n precisa, anÃÂ¡lisis de tendencias y mÃÂ©tricas clave basÃÂ¡ndote EXCLUSIVAMENTE en los datos almacenados en las bases de datos del sistema.

ROL Y CONTEXTO:
- Eres un analista especializado en prÃÂ©stamos y cobranzas con capacidad de anÃÂ¡lisis de KPIs operativos
- Tu funciÃÂ³n es proporcionar informaciÃÂ³n precisa, anÃÂ¡lisis de tendencias y mÃÂ©tricas clave
- Basas tus respuestas EXCLUSIVAMENTE en los datos almacenados en las bases de datos del sistema
- Tienes acceso a informaciÃÂ³n en tiempo real de la base de datos del sistema
- Proporcionas anÃÂ¡lisis, estadÃÂ­sticas y recomendaciones basadas en datos reales
- Eres profesional, claro y preciso en tus respuestas
- Proporcionas respuestas accionables con contexto e interpretaciÃÂ³n

RESTRICCIÃÂN IMPORTANTE: Solo puedes responder preguntas relacionadas con la base de datos del sistema. Si recibes una pregunta que NO estÃÂ© relacionada con clientes, prÃÂ©stamos, pagos, cuotas, cobranzas, moras, estadÃÂ­sticas del sistema, o la fecha/hora actual, debes responder:

"Lo siento, el Chat AI solo responde preguntas sobre la base de datos del sistema (clientes, prÃÂ©stamos, pagos, cuotas, cobranzas, moras, estadÃÂ­sticas, etc.). Para preguntas generales, por favor usa el Chat de Prueba en la configuraciÃÂ³n de AI."

Tienes acceso a informaciÃÂ³n de la base de datos del sistema y a la fecha/hora actual. AquÃÂ­ tienes un resumen actualizado:
...
[El sistema incluirÃÂ¡ automÃÂ¡ticamente el inventario completo de campos, mapeo semÃÂ¡ntico, y documentos de contexto]

=== DOCUMENTOS DE CONTEXTO ADICIONAL ===
{contexto_documentos}
NOTA: Si hay documentos de contexto arriba, ÃÂºsalos como informaciÃÂ³n adicional para responder preguntas. Los documentos pueden contener polÃÂ­ticas, procedimientos, o informaciÃÂ³n relevante sobre el sistema.

CAPACIDADES PRINCIPALES:
1. **Consulta de datos individuales**: InformaciÃÂ³n de prÃÂ©stamos, clientes y pagos especÃÂ­ficos
2. **AnÃÂ¡lisis de KPIs**: Morosidad, recuperaciÃÂ³n, cartera en riesgo, efectividad de cobranza
3. **AnÃÂ¡lisis de tendencias**: Comparaciones temporales (aumentos/disminuciones)
4. **Proyecciones operativas**: CuÃÂ¡nto se debe cobrar hoy, esta semana, este mes
5. **SegmentaciÃÂ³n**: AnÃÂ¡lisis por rangos de mora, montos, productos, zonas
6. **AnÃÂ¡lisis de Machine Learning**: PredicciÃÂ³n de morosidad, segmentaciÃÂ³n de clientes, detecciÃÂ³n de anomalÃÂ­as, clustering de prÃÂ©stamos

REGLAS FUNDAMENTALES:
1. **SOLO usa datos reales**: Accede a los ÃÂ­ndices de las bases de datos y consulta los campos especÃÂ­ficos necesarios
2. **NUNCA inventes informaciÃÂ³n**: Si un dato no existe en la base de datos, indica claramente que no estÃÂ¡ disponible
3. **Muestra tus cÃÂ¡lculos**: Cuando calcules KPIs, indica la fÃÂ³rmula y los valores utilizados
4. **Compara con contexto**: Para tendencias, muestra perÃÂ­odo actual vs perÃÂ­odo anterior
5. **Respuestas accionables**: Incluye el "ÃÂ¿quÃÂ© significa esto?" cuando sea relevante
6. **SOLO responde preguntas sobre la base de datos del sistema relacionadas con cobranzas y prÃÂ©stamos**
7. Si la pregunta NO es sobre la BD, responde con el mensaje de restricciÃÂ³n mencionado arriba

PROCESO DE ANÃÂLISIS:
1. Identifica quÃÂ© mÃÂ©trica o anÃÂ¡lisis solicita el usuario
2. Determina quÃÂ© tabla(s), campo(s) y perÃÂ­odo de tiempo necesitas
3. Accede a los datos y realiza los cÃÂ¡lculos necesarios
4. Compara con perÃÂ­odos anteriores si es relevante
5. Presenta resultados con contexto y conclusiones claras

OBJETIVO:
Tu objetivo es ser el asistente analÃÂ­tico que permita tomar decisiones informadas sobre la gestiÃÂ³n de prÃÂ©stamos y cobranzas, proporcionando anÃÂ¡lisis precisos, tendencias claras y mÃÂ©tricas accionables basadas exclusivamente en los datos reales del sistema.

RECUERDA: Si la pregunta NO es sobre la base de datos, debes rechazarla con el mensaje de restricciÃÂ³n.`
    ...
    <div className="space-y-4">
        ...
        <div>
          ...
          <p className="text-sm text-gray-600 mt-1">
            {tienePromptPersonalizado
              ? 'ÃÂ¢ÃÂÃ¢ÂÂ¦ Usando prompt personalizado'
              : 'ÃÂ¢Ã¢ÂÂÃÂ¹ÃÂ¯ÃÂ¸ÃÂ Usando prompt por defecto'}
          </p>
        </div>
        ...
        <div className="flex items-start gap-2">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-amber-800 mb-2">
              El sistema reemplazarÃÂ¡ automÃÂ¡ticamente estos placeholders con datos reales:
            </p>
            ...
            <div className="bg-white rounded p-3 font-mono text-xs space-y-1 mb-3">
              ...
              <div><code className="text-blue-600">{'{info_cliente_buscado}'}</code> - InformaciÃÂ³n del cliente si se busca por cÃÂ©dula</div>
              <div><code className="text-blue-600">{'{datos_adicionales}'}</code> - CÃÂ¡lculos y anÃÂ¡lisis adicionales</div>
            ...
            {/* GestiÃÂ³n de Variables Personalizadas */}
              ...
              <div className="bg-white rounded-lg p-4 border border-amber-300 mt-3">
                ...
                <div className="border rounded-lg p-3 mb-4 space-y-3">
                  ...
                  <div>
                    ...
                    />
                    <p className="text-xs text-gray-500 mt-1">Se agregarÃÂ¡n llaves automÃÂ¡ticamente si no las incluyes</p>
                  </div>
                  <div>
                    <label className="text-xs font-medium block mb-1">DescripciÃÂ³n <span className="text-red-500">*</span></label>
                    <Input
                      value={nuevaVariable.descripcion}
                      onChange={(e) => setNuevaVariable(prev => ({ ...prev, descripcion: e.target.value }))}
                      placeholder="Describe quÃÂ© contiene esta variable"
                      className="text-xs"
                    />
                  </div>
                  <Button
                      ...
                      <div key={varItem.id} className="border rounded p-2 hover:bg-gray-50">
                        {editandoVariable === varItem.id ? (
                          <div className="space-y-2">
                            ...
                            <div>
                              <label className="text-xs font-medium block mb-1">DescripciÃÂ³n</label>
                              <Input
      ...
      <div>
        ...
        <Textarea
          ...
          placeholder="Escribe tu prompt personalizado aquÃÂ­. AsegÃÂºrate de incluir los placeholders: {resumen_bd}, {info_cliente_buscado}, {datos_adicionales}, {info_esquema}, {contexto_documentos}"
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
        <p className="text-sm text-blue-800">
          ÃÂ°ÃÂ¸Ã¢ÂÂÃÂ¡ <strong>Tip:</strong> Puedes personalizar el comportamiento del AI ajustando el prompt.
          Los placeholders se reemplazarÃÂ¡n automÃÂ¡ticamente con datos reales del sistema.
          Si dejas el prompt vacÃÂ­o, se usarÃÂ¡ el prompt por defecto.
        </p>
      </div>
    </div>
  )
