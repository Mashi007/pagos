...
export function RAGTab() {
  ...
  const [editandoDocumento, setEditandoDocumento] = useState<number | null>(
    null
  )
  ...
  const [procesandoDocumento, setProcesandoDocumento] = useState<number | null>(
    null
  )
  ...
  const cargarDocumentos = async () => {
    ...
    try {
      const resultado = await apiClient.get<{
        total: number
        documentos: DocumentoAI[]
      }>('/api/v1/configuracion/ai/documentos')
  ...
  const handleGenerarEmbeddings = async (todos: boolean = true) => {
    ...
    try {
      const result = await aiTrainingService.generarEmbeddings(
        todos ? undefined : []
      )
  ...
  const handleBuscar = async () => {
    ...
    try {
      const result = await aiTrainingService.buscarDocumentosRelevantes(
        preguntaBusqueda,
        topK
      )
  ...
  const handleProcesarDocumento = async (id: number) => {
    ...
    try {
      ...
      if (
        respuesta.mensaje?.includes('ya estaba procesado') ||
        respuesta.contenido_en_bd
      ) {
        toast.success(
          `âœ… ${respuesta.mensaje || 'Documento procesado'} (${respuesta.caracteres_extraidos || 0} caracteres)`,
          {
            description:
              'El contenido ya estaba disponible en la base de datos',
          }
        )
      } else {
        toast.success(
          `Documento procesado exitosamente (${respuesta.caracteres_extraidos || 0} caracteres extraídos)`
        )
      }
      ...
      if (mensajeError.includes('El archivo físico no existe')) {
        mensajeError =
          'El archivo físico no existe en el servidor. Por favor, elimina este documento y súbelo nuevamente.'
        ...
        mensajeError =
          partes[0] +
          (partes.length > 1 ? ' (Ver consola para más detalles)' : '')
      }
  ...
  const handleActualizarDocumento = async (id: number) => {
    ...
    try {
      ...
      toast.error(
        error?.response?.data?.detail || 'Error actualizando documento'
      )
    } finally {
  ...
  const handleActivarDesactivarDocumento = async (
    id: number,
    activo: boolean
  ) => {
    ...
    if (activo && documento && !documento.contenido_procesado) {
      if (
        !confirm(
          'Este documento no está procesado. ¿Deseas procesarlo ahora antes de activarlo?'
        )
      ) {
        return
      }
    ...
    try {
      await apiClient.patch(
        `/api/v1/configuracion/ai/documentos/${id}/activar`,
        { activo }
      )

      toast.success(
        `Documento ${activo ? 'activado' : 'desactivado'} exitosamente`
      )
      ...
      toast.error(
        error?.response?.data?.detail || 'Error cambiando estado del documento'
      )
    }
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            Gestiona documentos y búsqueda semántica mejorada con embeddings
            vectoriales
          </p>
        </div>
        ...
        <Button
          ...
          variant="outline"
          size="sm"
          disabled={cargando || cargandoDocumentos}
        >
      ...
      >
          ...
          <Card>
            <CardContent className="space-y-4 pt-6">
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div className="flex-1">
                    ...
                    <ul className="list-inside list-disc space-y-1 text-sm text-amber-800">
                      ...
                      <li>
                        Los documentos se procesarán para generar respuestas
                        contextualizadas
                      </li>
                    </ul>
                  </div>
              ...
              <div className="border-t pt-4">
                <h4 className="mb-4 font-semibold">Documentos Existentes</h4>
                          ...
                          <div className="space-y-3">
                            ...
                            <div className="flex items-center gap-2">
                              <Button
                                size="sm"
                                onClick={() =>
                                  handleActualizarDocumento(doc.id)
                                }
                                disabled={
                                  actualizandoDocumento ||
                                  !documentoEditado.titulo.trim()
                                }
                              >
                                {actualizandoDocumento ? (
                                  <>
                            ...
                            <div className="flex items-center gap-2">
                              {!doc.contenido_procesado && (
                                <Button
                                  ...
                                  size="sm"
                                  onClick={() =>
                                    handleProcesarDocumento(doc.id)
                                  }
                                  disabled={
                                    procesandoDocumento === doc.id ||
                                    procesandoDocumento !== null
                                  }
                              ...
                              <Button
                                ...
                                size="sm"
                                onClick={() =>
                                  handleActivarDesactivarDocumento(
                                    doc.id,
                                    !doc.activo
                                  )
                                }
                              ...
                              >
                                <Zap
                                  className={`h-4 w-4 ${doc.activo ? '' : 'opacity-50'}`}
                                />
                              </Button>
        ...
        <TabsContent value="embeddings" className="mt-6 space-y-4">
                ...
                <div className="space-y-4">
                  ...
                  {estado.ultima_actualizacion && (
                    <div className="text-sm text-gray-500">
                      ...
                      {new Date(estado.ultima_actualizacion).toLocaleString(
                        'es-ES'
                      )}
                    </div>
                  )}
                      ...
                      <div className="flex items-start gap-2">
                        ...
                        <div className="flex-1">
                          ...
                          <p className="mt-1 text-xs text-amber-700">
                            {estado.total_documentos -
                              estado.documentos_con_embeddings}{' '}
                            documentos aún no tienen embeddings. Genera
                            embeddings para mejorar la búsqueda semántica.
                          </p>
                        </div>
                      </div>
                    </div>
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <Button
                    ...
                    disabled={buscando || !preguntaBusqueda.trim()}
                  >
                    {buscando ? (
                      <>
                        <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                ...
                {resultados.length === 0 && preguntaBusqueda && !buscando && (
                  <div className="py-8 text-center text-gray-500">
                    ...
                    <p className="mt-1 text-xs">
                      Intenta con otra pregunta o genera embeddings para más
                      documentos
                    </p>
                  </div>
                )}
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-blue-800">
                      RAG (Retrieval-Augmented Generation) mejorado usa
                      embeddings vectoriales para encontrar documentos más
                      relevantes a cada pregunta. Esto mejora la precisión de
                      las respuestas del AI al incluir solo el contexto más
                      pertinente.
                    </p>
                    ...
                    <ul className="mt-2 list-inside list-disc space-y-1 text-xs text-blue-700">
                      ...
                      <li>
                        El sistema busca automáticamente documentos relevantes
                      </li>
