...
export function RAGTab() {
  // ÃÂ¢ÃÂÃ¢ÂÂ¦ Por defecto mostrar la pestaÃÂ±a de GestiÃÂ³n de Documentos para facilitar la carga
  ...
  // Funciones para gestiÃÂ³n de documentos
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      if (file.size > 10 * 1024 * 1024) {
        toast.error('El archivo es demasiado grande. MÃÂ¡ximo 10MB')
        return
      }
  ...
  const handleSubirDocumento = async () => {
    if (!nuevoDocumento.titulo.trim()) {
      toast.error('El tÃÂ­tulo es requerido')
      return
    }
  ...
  const handleProcesarDocumento = async (id: number) => {
    ...
    try {
      ...
      // Mensaje mejorado segÃÂºn el resultado
      if (respuesta.mensaje?.includes('ya estaba procesado') || respuesta.contenido_en_bd) {
        toast.success(`ÃÂ¢ÃÂÃ¢ÂÂ¦ ${respuesta.mensaje || 'Documento procesado'} (${respuesta.caracteres_extraidos || 0} caracteres)`, {
          description: 'El contenido ya estaba disponible en la base de datos'
        })
      } else {
        toast.success(`Documento procesado exitosamente (${respuesta.caracteres_extraidos || 0} caracteres extraÃÂ­dos)`)
      }
      ...
      // Intentar obtener el mensaje del backend de mÃÂºltiples fuentes
      let mensajeError = error?.response?.data?.detail || 
                        error?.response?.data?.message || 
                        error?.message || 
                        'Error procesando documento'

      // Simplificar mensajes largos de diagnÃÂ³stico
      if (mensajeError.includes('El archivo fÃÂ­sico no existe')) {
        mensajeError = 'El archivo fÃÂ­sico no existe en el servidor. Por favor, elimina este documento y sÃÂºbelo nuevamente.'
        ...
        mensajeError = partes[0] + (partes.length > 1 ? ' (Ver consola para mÃÂ¡s detalles)' : '')
      }

      toast.error(mensajeError, {
        duration: 8000, // Aumentar duraciÃÂ³n para mensajes importantes
      })
    } finally {
  ...
  const handleEliminarDocumento = async (id: number) => {
    if (!confirm('ÃÂ¿EstÃÂ¡ seguro de eliminar este documento?')) return
  ...
  const handleActivarDesactivarDocumento = async (id: number, activo: boolean) => {
    ...
    if (activo && documento && !documento.contenido_procesado) {
      if (!confirm('Este documento no estÃÂ¡ procesado. ÃÂ¿Deseas procesarlo ahora antes de activarlo?')) {
        return
      }
  ...
  const formatearTamaÃÂ±o = (bytes: number | null) => {
    if (!bytes) return '0 B'
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Gestiona documentos y bÃÂºsqueda semÃÂ¡ntica mejorada con embeddings vectoriales
          </p>
        </div>
      ...
      <Tabs value={activeSubTab} onValueChange={setActiveSubTab} className="w-full">
        <TabsList className="grid w-full grid-cols-2">
          <TabsTrigger value="documentos">
            ...
            GestiÃÂ³n de Documentos
          </TabsTrigger>
          <TabsTrigger value="embeddings">
            <Zap className="h-4 w-4 mr-2" />
            Embeddings y BÃÂºsqueda
          </TabsTrigger>
        </TabsList>

        {/* Tab: GestiÃÂ³n de Documentos */}
          ...
          <Card>
            <CardContent className="pt-6 space-y-4">
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div className="flex-1">
                    ...
                    <ul className="text-sm text-amber-800 space-y-1 list-disc list-inside">
                      ...
                      <li>TamaÃÂ±o mÃÂ¡ximo: 10MB por archivo</li>
                      <li>Los documentos se procesarÃÂ¡n para generar respuestas contextualizadas</li>
                    </ul>
                  </div>
                </div>
              </div>
              ...
              <div className="border rounded-lg p-4 space-y-4">
                ...
                <div>
                  <label className="text-sm font-medium block mb-2">TÃÂ­tulo <span className="text-red-500">*</span></label>
                  <Input
                    value={nuevoDocumento.titulo}
                    onChange={(e) => setNuevoDocumento(prev => ({ ...prev, titulo: e.target.value }))}
                    placeholder="Ej: PolÃÂ­ticas de PrÃÂ©stamos"
                  />
                </div>

                <div>
                  <label className="text-sm font-medium block mb-2">DescripciÃÂ³n</label>
                  <Textarea
                    value={nuevoDocumento.descripcion}
                    onChange={(e) => setNuevoDocumento(prev => ({ ...prev, descripcion: e.target.value }))}
                    placeholder="Describe de quÃÂ© trata este documento..."
                    rows={3}
                  />
                </div>
                    ...
                    {documentos.map((doc) => (
                      <div key={doc.id} className="border rounded-lg p-4 hover:bg-gray-50">
                        {editandoDocumento === doc.id ? (
                          <div className="space-y-3">
                            <div>
                              <label className="text-sm font-medium block mb-1">TÃÂ­tulo <span className="text-red-500">*</span></label>
                              <Input
                                value={documentoEditado.titulo}
                                onChange={(e) => setDocumentoEditado(prev => ({ ...prev, titulo: e.target.value }))}
                                placeholder="TÃÂ­tulo del documento"
                              />
                            </div>
                            <div>
                              <label className="text-sm font-medium block mb-1">DescripciÃÂ³n</label>
                              <Textarea
                                value={documentoEditado.descripcion}
                                onChange={(e) => setDocumentoEditado(prev => ({ ...prev, descripcion: e.target.value }))}
                                placeholder="DescripciÃÂ³n del documento"
                                rows={2}
                              />
                            </div>
                            <div className="flex items-center gap-2">
                          ...
                          <div className="flex items-start justify-between">
                            <div className="flex-1">
                              <div className="flex items-center gap-2 mb-2">
                                ...
                                {doc.activo && !doc.contenido_procesado && (
                                  <Badge variant="outline" className="bg-red-50 text-red-700 border-red-200">
                                    <AlertCircle className="h-3 w-3 mr-1" />
                                    ÃÂ¢ÃÂ¡ ÃÂ¯ÃÂ¸ÃÂ No disponible para AI
                                  </Badge>
                                )}
                                {doc.activo && doc.contenido_procesado && (
                                  <Badge variant="outline" className="bg-blue-50 text-blue-700 border-blue-200">
                                    <CheckCircle className="h-3 w-3 mr-1" />
                                    ÃÂ¢ÃÂÃ¢ÂÂ¦ Disponible para AI
                                  </Badge>
                                )}
                              </div>
                              {doc.descripcion && (
                              ...
                              <div className="flex items-center gap-4 text-xs text-gray-500">
                                ...
                                <span>{formatearTamaÃÂ±o(doc.tamaÃÂ±o_bytes)}</span>
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-amber-700 mt-1">
                        ...
                        aÃÂºn no tienen embeddings. Genera embeddings para mejorar la bÃÂºsqueda
                        semÃÂ¡ntica.
                      </p>
                    </div>
                  </div>
      ...
      <Card>
        <CardContent className="pt-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h4 className="font-semibold">BÃÂºsqueda SemÃÂ¡ntica de Prueba</h4>
          </div>
          ...
          <div className="space-y-4">
            <div>
              ...
              <Textarea
                placeholder="Ej: ÃÂ¿CuÃÂ¡les son las polÃÂ­ticas de prÃÂ©stamos para clientes nuevos?"
            ...
            {resultados.length === 0 && preguntaBusqueda && !buscando && (
              <div className="text-center py-8 text-gray-500">
                ...
                <p className="text-xs mt-1">
                  Intenta con otra pregunta o genera embeddings para mÃÂ¡s documentos
                </p>
              </div>
            )}
          </div>
            ...
            <CardContent className="pt-6">
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                <div className="flex items-start gap-2">
                  ...
                  <div className="flex-1">
                    <p className="font-semibold text-blue-900 mb-1">ÃÂ¿QuÃÂ© es RAG Mejorado?</p>
                    <p className="text-sm text-blue-800">
                      RAG (Retrieval-Augmented Generation) mejorado usa embeddings vectoriales para
                      encontrar documentos mÃÂ¡s relevantes a cada pregunta. Esto mejora la precisiÃÂ³n de
                      las respuestas del AI al incluir solo el contexto mÃÂ¡s pertinente.
                    </p>
                    <ul className="text-xs text-blue-700 mt-2 list-disc list-inside space-y-1">
                      <li>Genera embeddings para todos tus documentos</li>
                      <li>El sistema busca automÃÂ¡ticamente documentos relevantes</li>
                      <li>Mejora la precisiÃÂ³n y reduce costos de tokens</li>
                    </ul>
                  </div>
                </div>
              </div>
