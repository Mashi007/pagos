...
export default function UsuariosConfig() {
  ...
  // FunciÃÂ³n para validar email con el sistema
  const validateEmailWithSystem = async (email: string) => {
    try {
      ...
      if (result) {
        ...
        // Mostrar error y sugerencia si estÃÂ¡ disponible
        const errorMsg = result.validacion?.error || 'Formato de email invÃÂ¡lido'
      ...
      // Endpoint no disponible o error de red: usar validaciÃÂ³n local
    }

    // Fallback: validaciÃÂ³n local
  ...
  const [formData, setFormData] = useState<UserCreate>({
    ...
    nombre: '',
    apellido: '',
    rol: 'operativo',  // Cambio clave: rol Ã¢ÂÂ is_admin
    password: '',
    cargo: 'Usuario',
    is_active: true
  })

  // Hook para validar contraseÃÂ±a
  ...
  // ValidaciÃÂ³n del formulario
  const isFormValid = () => {
    if (!formData.email || !formData.nombre || !formData.apellido) {
      return false
    }

    // Si estamos creando un nuevo usuario, la contraseÃÂ±a es obligatoria
    if (!editingUser && !formData.password) {
      return false
    }

    // Si hay una contraseÃÂ±a (ya sea en creaciÃÂ³n o actualizaciÃÂ³n), debe cumplir todos los requisitos
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!emailValidation.isValid) {
        toast.error(emailValidation.message || 'Email invÃÂ¡lido')
        return
      }
      ...
      if (editingUser) {
        ...
        const updateData: UserUpdate = {
          ...
          rol: formData.rol,  // Cambio clave: rol Ã¢ÂÂ is_admin
          is_active: formData.is_active
        }

        // Solo incluir password si se proporcionÃÂ³
  ...
  const handleEdit = (usuario: User) => {
    ...
    setFormData({
      email: usuario.email.toLowerCase(), // Normalizar a minÃÂºsculas
      nombre: usuario.nombre,
      apellido: usuario.apellido,
      cargo: usuario.cargo || 'Usuario', // Preservar cargo existente
      rol: usuario.rol || 'operativo',  // Cambio clave: rol Ã¢ÂÂ is_admin
  ...
  const handleDelete = async (usuario: User) => {
    if (!confirm(`ÃÂ¿EstÃÂ¡s seguro de eliminar el usuario ${usuario.nombre} ${usuario.apellido} (${usuario.email})?`)) {
      return
    }
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
            ...
            GestiÃÂ³n de Usuarios
          </h2>
          <p className="text-sm text-gray-600 mt-1">
            Administra usuarios: Administrador (acceso total). Operativo: no puede realizar cambios en configuraciÃÂ³n ni auditorÃÂ­a.
          </p>
        </div>
        <Button
            ...
            <div className="p-6">
              ...
              <form onSubmit={handleSubmit} className="space-y-4" autoComplete="off">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    Administrador: acceso total. Operativo: no puede realizar cambios en mÃÂ³dulos de configuraciÃÂ³n ni auditorÃÂ­a.
                  </p>
                </div>
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
                    ContraseÃÂ±a {editingUser ? '(dejar en blanco para no cambiar)' : '*'}
                  </label>
                  <PasswordField
                    value={formData.password}
                    onChange={(password) => setFormData({ ...formData, password })}
                    placeholder={editingUser ? 'Nueva contraseÃÂ±a (opcional)' : 'MÃÂ­nimo 8 caracteres'}
                    required={!editingUser}
                    showGenerateButton={true}  // ÃÂ¢ÃÂÃ¢ÂÂ¦ SIEMPRE mostrar botÃÂ³n de generar
                    showCopyButton={true}      // ÃÂ¢ÃÂÃ¢ÂÂ¦ SIEMPRE mostrar botÃÂ³n de copiar
                  />
                </div>
            ...
            <div className="p-6">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <p className="text-sm font-medium text-gray-500">Fecha de creaciÃÂ³n</p>
                    <p className="text-sm text-gray-900 mt-1">
                  ...
                  <div>
                    <p className="text-sm font-medium text-gray-500">ÃÂltimo acceso</p>
                    <p className="text-sm text-gray-900 mt-1">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-green-700 mt-1">
                        {(viewingUser.rol || 'operativo') === 'administrador'
                          ? 'Acceso total: usuarios, configuraciÃÂ³n y todas las funcionalidades.'
                          : 'No puede realizar cambios en mÃÂ³dulos de configuraciÃÂ³n ni auditorÃÂ­a.'}
                      </p>
                    </div>
                  </div>
                </div>
