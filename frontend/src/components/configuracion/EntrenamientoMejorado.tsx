...
export function EntrenamientoMejorado() {
  ...
  const handleActivarRecoleccionAutomatica = async () => {
    ...
    try {
      ...
      if (nuevoEstado) {
        toast.success('Ã¢Åâ¦ RecolecciÃ³n automÃ¡tica activada. Las conversaciones se guardarÃ¡n automÃ¡ticamente.', {
          duration: 5000,
        })
      } else {
  ...
  const handleRecoleccionAutomatica = async () => {
    ...
    try {
      ...
      toast.success(`Ã¢Åâ¦ RecolecciÃ³n completada: ${resultado.total_recolectadas} conversaciones nuevas`)
      cargarMetricas()
    } catch (error: any) {
  ...
  const obtenerRecomendaciones = (): Recomendacion[] => {
    ...
    if ((conv.con_calificacion ? 0) > 0 && (conv.promedio_calificacion ? 0) < 3.5) {
      recomendaciones.push({
        ...
        descripcion: `El promedio de calificaciones es ${(conv.promedio_calificacion ? 0).toFixed(1)}/5. Revisa y mejora las conversaciones con baja calificaciÃ³n.`,
        accion: 'Revisar',
            ...
            <div className="flex items-start justify-between">
              <div className="flex-1">
                ...
                <div className="flex items-center gap-2 mt-2">
                  ...
                  >
                    {metricas?.fine_tuning?.modelo_activo ? 'â Activo' : 'Sin activar'}
                  </Badge>
                </div>
              </div>
            </div>
