/**
 * ConfiguraciÃÂ³n de 4 cuentas de correo con asignaciÃÂ³n por servicio.
 * Cuenta 1 = Cobros, 2 = Estado de cuenta, 3 y 4 = Notificaciones (por pestaÃÂ±a).
 */
...
export function EmailCuentasConfig() {
  ...
  const load = async () => {
    ...
    try {
      ...
      console.error(e)
      toast.error('Error cargando configuraciÃÂ³n de cuentas')
      setData({
        version: 2,
  ...
  const SERVICIOS_DISPONIBLES: { key: keyof EmailCuentasResponse; label: string; cuenta: number }[] = [
    { key: 'email_activo_cobros', label: 'Cobros (formulario pÃÂºblico, recibos)', cuenta: 1 },
    { key: 'email_activo_estado_cuenta', label: 'Estado de cuenta (cÃÂ³digo y envÃÂ­o PDF)', cuenta: 2 },
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.success('ConfiguraciÃÂ³n de 4 cuentas guardada')
      await load()
    } catch (e: unknown) {
...
if (loading) {
    return (
      <Card>
        <CardContent className="pt-6">Cargando configuraciÃÂ³n de cuentas...</CardContent>
      </Card>
    )
  }
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Active o desactive el envÃÂ­o de correo por servicio. Cada uno usa la cuenta indicada (Cuenta 1, 2 o 3/4).
          </CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <Card className="border-amber-200 bg-amber-50/50 dark:bg-amber-950/20">
        <CardHeader>
          ...
          <CardDescription>
            Si "Modo pruebas" estÃ¡ activo, todos los envÃ­os se redirigen al correo indicado. El correo de pruebas es obligatorio cuando el modo pruebas estÃ¡ activado.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="flex items-center justify-between rounded border p-3">
            <span className="text-sm font-medium">Modo pruebas (redirigir envÃ­os al correo de pruebas)</span>
            <label className="relative inline-flex items-center cursor-pointer">
              <input
                type="checkbox"
          ...
          <div>
            <Label>Correo de pruebas (obligatorio si modo pruebas estÃ¡ activo)</Label>
            <Input
              type="email"
        ...
        <CardHeader>
          ...
          <CardDescription>
            Configure hasta 4 cuentas. Cada servicio usa una cuenta: <strong>Cuenta 1</strong> = Cobros (formulario pÃÂºblico),
            <strong> Cuenta 2</strong> = Estado de cuenta, <strong>Cuentas 3 y 4</strong> = Notificaciones (puede elegir por pestaÃÂ±a).
          </CardDescription>
        </CardHeader>
      </Card>
        ...
        <Card key={i}>
          <CardHeader className="pb-2">
            <CardTitle className="text-base">
              Cuenta {i + 1} Ã¢ÂÂ {SERVICIO_POR_CUENTA[i + 1] ? `Cuenta ${i + 1}`}
            </CardTitle>
            <CardDescription>
              {i === 0 && 'Usada en: rapicredit-cobros (reporte de pago).'}
              {i === 1 && 'Usada en: rapicredit-estadocuenta (consulta y envÃÂ­o de PDF).'}
              {(i === 2 || i === 3) && `Usada en: Notificaciones (pestaÃÂ±as que elija como "Cuenta ${i + 1}" abajo).`}
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base flex items-center gap-2">
            ...
            AsignaciÃÂ³n Notificaciones: quÃÂ© cuenta usa cada pestaÃÂ±a
          </CardTitle>
          <CardDescription>
            Elija para cada pestaÃÂ±a de Notificaciones si usa <strong>Cuenta 3</strong> o <strong>Cuenta 4</strong>.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid gap-3 sm:grid-cols-2">
      ...
      <div className="flex justify-end">
        <Button onClick={handleSave} disabled={saving}>
          ...
          {saving ? 'GuardandoÃ¢ÂÂ¦' : 'Guardar configuraciÃÂ³n de 4 cuentas'}
        </Button>
      </div>
    </div>
  )
