...
export function MLImpagoCuotasTab() {
  ...
  const handleEntrenar = async () => {
    ...
    try {
      ...
      console.log('Ã¢Åâ¦ Modelo entrenado exitosamente:', resultado)
    } catch (error: any) {
      ...
      console.group('Ã¢ÂÅ Error entrenando modelo ML Impago')
      ...
      if (error?.response?.data) {
        console.error('Ã°Å¸ââ¹ Detalles del error del servidor:', JSON.stringify(error.response.data, null, 2))
      }
      console.groupEnd()
      ...
      console.error('Ã°Å¸âÂ Mensaje de error extraÃ­do para mostrar al usuario:', mensajeError)
  ...
  const handleActivarModelo = async (modeloId: number) => {
    ...
    try {
      console.log('Ã°Å¸ââ Activando modelo ML Impago:', modeloId)
      const resultado = await aiTrainingService.activarModeloImpago(modeloId)
      console.log('Ã¢Åâ¦ Modelo activado exitosamente:', resultado)
      toast.success(esDesactivacion ? 'Modelo desactivado' : 'Modelo activado exitosamente')
      await cargarModelos()
    } catch (error: any) {
      console.error('Ã¢ÂÅ Error activando modelo ML Impago:', error)
