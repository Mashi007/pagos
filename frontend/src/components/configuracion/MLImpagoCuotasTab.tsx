...
export function MLImpagoCuotasTab() {
  ...
  const cargarModelos = async () => {
    ...
    try {
      ...
      // Manejar timeout especÃÂ­ficamente
      let mensajeError = 'Error al cargar modelos'
      if (error?.code === 'ECONNABORTED' || error?.message?.includes('timeout')) {
        mensajeError = 'La peticiÃÂ³n estÃÂ¡ tardando mÃÂ¡s de lo esperado. El servidor puede estar procesando. Por favor, intenta nuevamente en unos momentos.'
      ...
      toast.error(mensajeError, {
        duration: 8000, // Mostrar por mÃÂ¡s tiempo para mensajes de timeout
      })
    } finally {
      setCargando(false)
    }
  ...
  const handleEntrenar = async () => {
    ...
    const intervalProgreso = setInterval(() => {
      setEstadoEntrenamiento((prev) => {
        ...
        if (prev.estado === 'iniciando' && prev.progreso < 10) {
          ...
          nuevoMensaje = 'Procesando prÃÂ©stamos y extrayendo features...'
    ...
    try {
      ...
      // Limpiar intervalo y mostrar ÃÂ©xito
      clearInterval(intervalProgreso)

      setEstadoEntrenamiento({
        estado: 'completado',
        progreso: 100,
        mensaje: 'ÃÂ¡Modelo entrenado exitosamente!',
        modelo: resultado.modelo,
      })

      // NotificaciÃÂ³n mejorada con mÃÂ©tricas
      ...
      // Recargar modelos despuÃÂ©s de 2 segundos
      setTimeout(async () => {
        await cargarModelos()
        setEstadoEntrenamiento(null)
      }, 2000)

      console.log('ÃÂ¢ÃÂÃ¢ÂÂ¦ Modelo entrenado exitosamente:', resultado)
    } catch (error: any) {
      ...
      console.group('ÃÂ¢ÃÂÃÂ Error entrenando modelo ML Impago')
      ...
      if (error?.response?.data) {
        console.error('ÃÂ°ÃÂ¸Ã¢ÂÂÃ¢ÂÂ¹ Detalles del error del servidor:', JSON.stringify(error.response.data, null, 2))
      }
      console.groupEnd()
      ...
      // Detectar timeout especÃÂ­ficamente
      if (error?.code === 'ECONNABORTED' || error?.message?.includes('timeout')) {
        mensajeError = 'El entrenamiento estÃÂ¡ tardando mÃÂ¡s de lo esperado. El proceso continÃÂºa en el servidor. Por favor, espera unos minutos y recarga la pÃÂ¡gina para ver el modelo entrenado.'
      } else if (error?.response?.data?.detail) {
      ...
      console.error('ÃÂ°ÃÂ¸Ã¢ÂÂÃÂ Mensaje de error extraÃÂ­do para mostrar al usuario:', mensajeError)
  ...
  const handleEliminarModelo = async (modeloId: number) => {
    if (!confirm('ÃÂ¿EstÃÂ¡s seguro de que deseas eliminar este modelo? Esta acciÃÂ³n no se puede deshacer.')) {
      return
    }
  ...
  const handleActivarModelo = async (modeloId: number) => {
    ...
    const mensajeConfirmacion = esDesactivacion
      ? `ÃÂ¿EstÃÂ¡s seguro de desactivar el modelo "${modelo.nombre}" v${modelo.version}?`
      : `ÃÂ¿EstÃÂ¡s seguro de ${modeloActivo ? 'cambiar y ' : ''}activar el modelo "${modelo.nombre}" v${modelo.version}?`
    ...
    try {
      console.log('ÃÂ°ÃÂ¸Ã¢ÂÂÃ¢ÂÂ Activando modelo ML Impago:', modeloId)
      const resultado = await aiTrainingService.activarModeloImpago(modeloId)
      console.log('ÃÂ¢ÃÂÃ¢ÂÂ¦ Modelo activado exitosamente:', resultado)
      toast.success(esDesactivacion ? 'Modelo desactivado' : 'Modelo activado exitosamente')
      await cargarModelos()
    } catch (error: any) {
      console.error('ÃÂ¢ÃÂÃÂ Error activando modelo ML Impago:', error)
  ...
  const handlePredecir = async () => {
    if (!prestamoId) {
      toast.error('Ingresa un ID de prÃÂ©stamo')
      return
    }
      ...
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-2xl font-bold flex items-center gap-2">
            ...
            ML - PredicciÃÂ³n de Impago de Cuotas
          </h3>
          <p className="text-sm text-gray-500 mt-1">
            Entrena y usa modelos de machine learning para predecir si un cliente pagarÃÂ¡ sus cuotas futuras
          </p>
        </div>
        ...
        <Card className="border-green-200 bg-green-50/50">
          <CardContent className="pt-6">
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
              ...
              <div>
                <div className="text-sm text-gray-500 mb-1">VersiÃÂ³n</div>
        ...
        <Card className="border-amber-200 bg-amber-50/50">
          <CardContent className="pt-6">
            ...
            <p className="text-sm text-gray-600 mb-4">
              Entrena un modelo y actÃÂ­valo para comenzar a predecir impago de cuotas.
            </p>
      ...
      {mostrarPrediccion && (
        <Card>
          <CardContent className="pt-6">
            ...
            <div className="space-y-4">
              <div>
                <label className="text-sm font-medium mb-2 block">ID del PrÃÂ©stamo</label>
                <Input
                  type="number"
                  placeholder="Ej: 123"
                  value={prestamoId}
                ...
                <p className="text-xs text-gray-500 mt-1">
                  Ingresa el ID del prÃÂ©stamo para predecir si el cliente pagarÃÂ¡ sus cuotas futuras
                </p>
              </div>
              <div className="flex gap-2">
              ...
              {/* Resultado de PredicciÃÂ³n */}
              {prediccion && (
                <Card className="mt-4 border-blue-200 bg-blue-50/50">
                  <CardContent className="pt-6">
                    <h5 className="font-semibold mb-4 flex items-center gap-2">
                      <TrendingUp className="h-5 w-5" />
                      Resultado de la PredicciÃÂ³n
                    </h5>
                    <div className="grid gap-4 md:grid-cols-2">
                      <div>
                        <div className="text-sm text-gray-500 mb-1">PredicciÃÂ³n</div>
                    ...
                    <div className="mt-4 pt-4 border-t">
                      <div className="text-sm text-gray-500 mb-1">RecomendaciÃÂ³n</div>
        ...
        <CardContent className="pt-6">
          ...
          {cargando ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-sm text-gray-500">No hay modelos entrenados aÃÂºn</p>
              <Button
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        <div className="grid gap-2 md:grid-cols-4 text-sm">
                          <div>
                            <span className="text-gray-500">VersiÃÂ³n:</span> {modelo.version}
                          </div>
                          <div>
