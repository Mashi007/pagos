...
interface EmailConfigData {
  ...
  /** On/Off por funcionalidad: si estÃ¡ en 'false', ese servicio no envÃ­a email (el resto sigue funcionando). */
  ...
  /** Modo prueba por funcionalidad: 'true' = redirigir envÃ­os de este servicio al correo de pruebas; 'false' = producciÃ³n (envÃ­o real). */
...
const [probando, setProbando] = useState(false)
  ...
  const [emailActivo, setEmailActivo] = useState<boolean>(true) // â Estado activo/inactivo
  ...
  useEffect(() => {
    ...
    verificarEstadoGoogle() // â Verificar estado de Google/Gmail al cargar
  }, [])

  // â Verificar estado de configuraciÃ³n con Google/Gmail
  const verificarEstadoGoogle = async () => {
    try {
      ...
      // â Actualizar estado de vinculaciÃ³n basado en la verificaciÃ³n REAL de Gmail
      ...
      if (estado.configurada && estado.conexion_smtp?.success === true) {
        ...
        setMensajeVinculacion('â Sistema vinculado correctamente con Google/Google Workspace')
        setRequiereAppPassword(false)
        if (process.env.NODE_ENV === 'development') {
          console.log('â Gmail confirmÃ³: ConfiguraciÃ³n correcta y conexiÃ³n aceptada')
        }
      } else {
        ...
        setMensajeVinculacion(estado.mensaje || 'â Gmail rechazÃ³ la conexiÃ³n. Verifica tus credenciales.')

        if (process.env.NODE_ENV === 'development') {
          console.log('â Gmail rechazÃ³:', {
  ...
  const cargarConfiguracion = async () => {
    try {
      ...
      // â CRÃTICO: Sincronizar from_email con smtp_user si estÃ¡ vacÃ­o
      ...
      // â Asegurar que from_email tenga un valor por defecto si smtp_user existe
      ...
      // â Asegurar valores por defecto para campos requeridos
      ...
      setConfig(prev => {
        ...
        // Si el API devuelve ***, mantener el valor que el usuario tenia (no borrar la contraseÃ±a del campo)
      ...
      // â Cargar estado activo/inactivo
  ...
  const handleChange = (campo: keyof EmailConfigData, valor: string) => {
    setConfig(prev => {
      ...
      // â Sincronizar from_email con smtp_user automÃ¡ticamente
  ...
  const puedeGuardar = useMemo((): boolean => {
    // â CONDICIÃN 1: Campos obligatorios bÃ¡sicos (siempre requeridos)
    ...
    // â CONDICIÃN 2: Puerto vÃ¡lido (nÃºmero entre 1 y 65535)
    const puerto = parseInt(config.smtp_port || '0')
    if (isNaN(puerto) || puerto < 1 || puerto > 65535) {
      return false
    }

    // â CONDICIÃN 3: Validaciones especÃ­ficas para Gmail/Google Workspace
    const esGmail = config.smtp_host?.toLowerCase().includes('gmail.com') || false
    if (esGmail) {
      // â 3.1: Gmail requiere contraseÃ±a siempre
      const tienePassword = config.smtp_password?.trim() || ''
      if (!tienePassword) {
        return false
      }

      // â 3.2: Gmail solo acepta puertos 587 (TLS) o 465 (SSL)
      if (puerto !== 587 && puerto !== 465) {
        return false
      }

      // â 3.3: Puerto 587 requiere TLS habilitado
      if (puerto === 587 && config.smtp_use_tls !== 'true') {
        return false
      }
    }

    // â CONDICIÃN 4: En modo Pruebas, el correo de pruebas es obligatorio y debe ser vÃ¡lido
    if (modoPruebas === 'true') {
  ...
  // Guardar: validaciÃ³n Ãºnica en backend (PUT); errores 400 se muestran en toast.
  const handleGuardar = async () => {
    ...
    try {
      ...
      // â Actualizar estado de vinculaciÃ³n INMEDIATAMENTE con la respuesta del guardado
      ...
      if (nuevaVinculacion) {
        toast.success(nuevoMensaje || 'â Sistema vinculado correctamente con Google', { duration: 10000 })
      } else if (nuevoRequiereAppPassword) {
        toast.warning(nuevoMensaje || 'ConfiguraciÃ³n guardada pero requiere App Password', { duration: 15000 })
      } else {
      ...
      if (resultado?.password_not_updated || passwordLimpia === '***') {
        toast.warning(resultado?.mensaje_password || 'La contraseÃ±a no se actualizÃ³ (el campo tenÃ­a ***). Para cambiarla, escrÃ­bela de nuevo y guarda.', { duration: 12000 })
      }

      await cargarConfiguracion()

      // â Verificar estado de Google despuÃ©s de guardar (prueba conexiÃ³n SMTP real con Gmail)
      ...
      // â Si el guardado indicÃ³ que requiere App Password, mantener ese estado
  ...
  const handleProbar = async () => {
    ...
    try {
      ...
      setEmailEnviadoExitoso(false) // â Resetear estado de Ã©xito
    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          {/* â Toggle Activar/Desactivar Email */}
            ...
            <div className="flex items-center justify-between">
              <div className="flex-1">
                ...
                <p className="text-xs text-gray-600">
                  {emailActivo
                    ? 'â El sistema estÃ¡ enviando emails automÃ¡ticamente'
                    : 'El sistema NO enviarÃ¡ emails. Activa el servicio para habilitar envÃ­os.'}
                </p>
              </div>
          ...
          {/* On/Off por funcionalidad: cada una tiene su interruptor; si apagas uno, los demÃ¡s siguen funcionando. */}
          <div className="mt-4 space-y-3">
            <p className="text-sm font-medium text-gray-700">Email por servicio</p>
            <p className="text-xs text-gray-600">Activa o desactiva el envÃ­o de email para cada tipo de uso. Si apagas uno, los demÃ¡s siguen funcionando.</p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {[
                ...
                { key: 'email_activo_estado_cuenta' as const, label: 'Estado de cuenta (cÃ³digo y PDF)' },
                { key: 'email_activo_cobros' as const, label: 'Cobros (recibos)' },
                { key: 'email_activo_campanas' as const, label: 'CampaÃ±as CRM' },
          ...
          {/* Modo prueba por funcionalidad: cada una puede ir a producciÃ³n o a correo de pruebas de forma independiente. */}
          <div className="mt-4 space-y-3 border-t pt-4">
            <p className="text-sm font-medium text-gray-700">Modo prueba por funcionalidad</p>
            <p className="text-xs text-gray-600">Para cada tipo de envÃ­o puedes elegir ProducciÃ³n (correos reales a clientes) o Pruebas (todos los correos de ese tipo van al Email de Pruebas). AsÃ­ puedes probar uno sin afectar al resto.</p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {[
                ...
                { key: 'modo_pruebas_estado_cuenta' as const, label: 'Estado de cuenta (cÃ³digo y PDF)' },
                { key: 'modo_pruebas_cobros' as const, label: 'Cobros (recibos)' },
                { key: 'modo_pruebas_campanas' as const, label: 'CampaÃ±as CRM' },
                ...
                <label key={key} className="flex items-center justify-between gap-2 p-2 rounded border border-amber-100 bg-amber-50/50">
                  ...
                  <span className="flex items-center gap-2">
                    <span className="text-xs text-gray-500">{(config[key] ? 'false') === 'true' ? 'Pruebas' : 'ProducciÃ³n'}</span>
                    <input
                      type="checkbox"
          ...
          {esGmail && (
            <>
              {/* â Estado: Configurado y vinculado correctamente */}
              ...
              {/* Â Estado: Requiere App Password (prioridad sobre otros estados) */}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-sm text-gray-600 space-y-1">
                        ...
                        <p>3. Pega la contraseÃ±a de 16 caracteres y guarda</p><p className="text-blue-700 font-medium mt-2">Cuentas corporativas / Google Workspace: puedes usar tu contraseÃ±a normal en el campo ContraseÃ±a.</p>
                      </div>
                      {mensajeVinculacion && (
              ...
              {/* â Estado: No configurado o con problemas (solo si no requiere App Password) */}
                ...
                <div className="bg-white border-2 border-amber-400 rounded-lg p-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-gray-600">
                        Usa &quot;Enviar Email de Prueba&quot; para verificar. Gmail personal: suele requerir ContraseÃ±a de aplicaciÃ³n. Cuentas corporativas / Google Workspace: puedes usar tu contraseÃ±a normal.
                      </p>
                    </div>
                  </div>
                </div>
              ...
              {/* â³ Estado: Pendiente de verificaciÃ³n (solo si no hay estado verificado) */}
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <div>
              <label className="text-sm font-medium block mb-2">ContraseÃ±a</label>
              <div className="relative">
                <Input
                  type={mostrarPassword ? 'text' : 'password'}
                  value={config.smtp_password || ''}
                  onChange={(e) => handleChange('smtp_password', e.target.value)}
                  placeholder="ContraseÃ±a normal o App Password (cuentas corporativas: contraseÃ±a normal)"
                  className="pr-10"
                />
                <button
                  type="button"
              ...
              <p className="text-xs text-gray-500 mt-1">
                <strong>Gmail personal:</strong> con 2FA activado usa una App Password (16 caracteres). <strong>Cuentas corporativas / Google Workspace:</strong> puedes usar tu contraseÃ±a normal.
              </p>
              <p className="text-amber-700 text-sm mt-1"><strong>Importante:</strong> Si cambiaste de cuenta o contraseÃ±a, escrÃ­bela aquÃ­. El valor *** no actualiza la contraseÃ±a en el servidor.</p>
            </div>
          </div>
          ...
          <div className="border-t pt-6 mt-6">
            <div className="bg-slate-50 border border-slate-200 rounded-lg p-4 mb-4">
              ...
              <p className="text-sm text-slate-700 mb-2">
                Configura IMAP para recibir correo (lectura de bandeja). Gmail: <strong>imap.gmail.com</strong>, puerto <strong>993</strong> (SSL) o <strong>143</strong> (STARTTLS). Usa la misma contraseÃ±a que SMTP (normal o de aplicaciÃ³n).
              </p>
            ...
            <div className="mt-4 flex gap-2">
                ...
                onClick={async () => {
                  ...
                  try {
                    ...
                    const mensajeImap = (r.mensaje && (r.mensaje.startsWith("b'") || /AUTHENTICATIONFAILED|Invalid credentials/i.test(r.mensaje)))
                      ? "Usuario o contraseÃ±a no aceptados. Gmail personal: usa ContraseÃ±a de aplicaciÃ³n. Cuentas corporativas/Google Workspace: prueba con tu contraseÃ±a normal."
                      : (r.mensaje || "")
              ...
              {resultadoImap && (
                <span className={`text-sm ${resultadoImap.success ? 'text-green-600' : 'text-red-600'}`}>
                  {resultadoImap.success ? 'OK' : (resultadoImap.mensaje?.startsWith("b'") || /AUTHENTICATIONFAILED|Invalid credentials/i.test(resultadoImap.mensaje || '')) ? "Usuario o contraseÃ±a no aceptados. Gmail personal: usa ContraseÃ±a de aplicaciÃ³n. Cuentas corporativas/Google Workspace: prueba con tu contraseÃ±a normal." : resultadoImap.mensaje}
                </span>
              )}
            </div>
          </div>
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
              ...
              {modoPruebas === 'false' && (
                <div className="bg-green-50 border border-green-200 rounded-lg p-3 mb-4">
                  <p className="text-sm text-green-800 font-semibold mb-1">â Modo ProducciÃ³n activo</p>
              ...
              <div className="space-y-4">
                ...
                <Button
                  ...
                  className={`flex items-center gap-2 transition-colors duration-300 ${
                    emailEnviadoExitoso
                      ? 'bg-green-600 hover:bg-green-700' // â Verde cuando fue enviado exitosamente
      ...
      <Card>
        ...
        <CardContent>
          <div className="flex justify-between items-center mb-4">
            <div className="text-sm text-gray-600">Ãltimos 10 envÃ­os de notificaciones</div>
            <Button
              variant="outline"
              size="sm"
