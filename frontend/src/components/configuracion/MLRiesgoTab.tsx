...
export function MLRiesgoTab() {
  ...
  const handleEntrenar = async () => {
    ...
    try {
      ...
      // El modelo se obtendrÃÂ¡ cuando se consulte el estado del job
      toast.success('Entrenamiento iniciado. El modelo se crearÃÂ¡ cuando el proceso termine.')
  ...
  const handleActivarModelo = async (modeloId: number) => {
    ...
    // Confirmar activaciÃÂ³n/desactivaciÃÂ³n
    ...
    const mensajeConfirmacion = esDesactivacion
      ? `ÃÂ¿EstÃÂ¡s seguro de desactivar el modelo "${modelo.nombre}" v${modelo.version}?\n\n` +
        `No habrÃÂ¡ modelo activo despuÃÂ©s de esto.`
      : `ÃÂ¿EstÃÂ¡s seguro de ${modeloActivo ? 'cambiar y ' : ''}activar el modelo "${modelo.nombre}" v${modelo.version}?\n\n` +
        `${modeloActivo ? `Esto desactivarÃÂ¡ el modelo actual "${modeloActivo.nombre}".\n\n` : ''}` +
        `El modelo estarÃÂ¡ disponible para predicciones.`
    ...
    try {
      ...
      // Si hay un modelo activo previo y se estÃÂ¡ cambiando, mostrar mensaje
      ...
      const mensajeError =
        ...
        error?.message ||
        `Error al ${esDesactivacion ? 'desactivar' : 'activar'} modelo. Verifica que el modelo existe y estÃÂ¡ disponible.`
      toast.error(mensajeError)
    }
  }
  ...
  return (
    <div className="space-y-6">
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between text-sm">
                <span className="text-gray-600">
                  {estadoJob.status === 'succeeded'
                    ? 'ÃÂ¡Modelo entrenado exitosamente!'
                    : estadoJob.status === 'failed'
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="text-sm font-medium mb-2 block">
                  TamaÃÂ±o de Test Set ({testSize * 100}%)
                </label>
                <Input
                  type="number"
                  min="0.1"
      ...
      {mostrarPrediccion && (
        <Card>
          <CardContent className="pt-6">
            <h4 className="font-semibold mb-4">Probar PredicciÃÂ³n de Riesgo</h4>
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <div>
                <label className="text-sm font-medium mb-2 block">DÃÂ­as desde ÃÂltimo PrÃÂ©stamo</label>
                <Input
                  type="number"
              ...
              <div>
                <label className="text-sm font-medium mb-2 block">NÃÂºmero de PrÃÂ©stamos Previos</label>
                <Input
                  type="number"
            ...
            {prediccion && (
              <div className="mt-6 border rounded-lg p-4 bg-gray-50">
                <h5 className="font-semibold mb-3">Resultado de la PredicciÃÂ³n</h5>
                <div className="space-y-3">
                  <div>
                  ...
                  {prediccion.features_used && (
                    <div>
                      <div className="text-sm text-gray-500 mb-1">CaracterÃÂ­sticas Usadas</div>
