...
export function ConfigGeneralTab() {
  ...
  const cargarConfiguracionGeneral = async (preserveLogoStateIfMissing = false) => {
    try {
      ...
      console.error('Error cargando configuraciÃÂ³n general:', err)
      setError('Error al cargar configuraciÃÂ³n general')
    } finally {
      setLoading(false)
    }
  }
  ...
  const actualizarConfiguracionGeneral = async (campo: string, valor: string) => {
    try {
      ...
      console.error('Error actualizando configuraciÃÂ³n:', err)
  ...
  const handleGuardar = async () => {
    try {
      ...
      console.error('Error guardando configuraciÃÂ³n:', err)
      toast.error(`Error al guardar configuraciÃÂ³n: ${getErrorMessage(err)}`)
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleCargarLogo = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!allowedTypes.includes(file.type)) {
      toast.error('Formato no vÃÂ¡lido. Use SVG, PNG o JPG')
      return
    }
    if (file.size > 2 * 1024 * 1024) {
      toast.error('El archivo es demasiado grande. MÃÂ¡ximo 2MB')
      return
    }
    try {
  ...
  if (loading && !configuracionGeneral) {
    return (
      <div className="flex items-center justify-center py-8">
        ...
        <span className="text-gray-600">Cargando configuraciÃÂ³n...</span>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <div>
          <label className="text-sm font-medium">VersiÃÂ³n del Sistema</label>
        ...
        <div>
          ...
          <Select value={formState.idioma} onValueChange={(v) => handleCambio('idioma', v)}>
            ...
            <SelectContent>
              <SelectItem value="ES">EspaÃÂ±ol</SelectItem>
        ...
        <div>
          ...
          <Select value={formState.moneda} onValueChange={(v) => handleCambio('moneda', v)}>
            <SelectTrigger><SelectValue /></SelectTrigger>
            <SelectContent>
              <SelectItem value="VES">BolÃÂ­var Soberano (VES)</SelectItem>
              <SelectItem value="USD">DÃÂ³lar Americano (USD)</SelectItem>
      ...
      <Card className="border-2 border-blue-200 bg-gradient-to-br from-blue-50 to-indigo-50/30">
        <CardHeader>
          ...
          <CardDescription>
            Suba un nuevo logo para la empresa. El logo se mostrarÃÂ¡ en el sidebar, login y otras pÃÂ¡ginas del sistema.
          </CardDescription>
        </CardHeader>
        <CardContent>
                ...
                <div className="flex flex-col items-center justify-center pt-5 pb-6">
                  {uploadingLogo ? (
                    ...
                    <>
                      ...
                      <p className="mb-2 text-sm text-gray-600"><span className="font-semibold">Haga clic para seleccionar</span> o arrastre el archivo aquÃÂ­</p>
