...
export function ConfigGeneralTab() {
  const [configuracionGeneral, setConfiguracionGeneral] =
    useState<ConfiguracionGeneral | null>(null)
  ...
  const cargarConfiguracionGeneral = async (
    preserveLogoStateIfMissing = false
  ) => {
    try {
      setLoading(true)
      ...
      const config =
        await configuracionGeneralService.obtenerConfiguracionGeneral()
  ...
  const actualizarConfiguracionGeneral = async (
    campo: string,
    valor: string
  ) => {
    try {
      ...
      await configuracionGeneralService.actualizarConfiguracionGeneral({
        [campo]: valor,
      })

      setConfiguracionGeneral(prev =>
        prev ? { ...prev, [campo]: valor } : null
      )
    ...
    try {
      ...
      if (configuracionGeneral && cambiosPendientes) {
        ...
        for (const [campoFrontend, campoBackend] of Object.entries(camposMap)) {
          const valorFrontend =
            formState[campoFrontend as keyof GeneralFormState]

          const valorBackend =
            configuracionGeneral[campoBackend as keyof ConfiguracionGeneral]

          if (valorFrontend && valorFrontend !== valorBackend) {
            await actualizarConfiguracionGeneral(
              campoBackend,
              String(valorFrontend)
            )
          }
        }
  ...
  const handleCargarLogo = async (
    ...
    try {
      ...
      const response = await fetch(
        `${env.API_URL}/api/v1/configuracion/upload-logo`,
        {
          method: 'POST',
    ...
    <div className="space-y-6">
      <div className="grid gap-4 md:grid-cols-2">
        <div>
          ...
          <Input
            ...
            className={
              erroresValidacion['general.nombreEmpresa'] ? 'border-red-500' : ''
            }
          />
        ...
        <div>
          ...
          <Select
            ...
            onValueChange={v => handleCambio('idioma', v)}
          >
            <SelectTrigger>
              <SelectValue />
            </SelectTrigger>
        ...
        <div>
          ...
          <Select
            value={formState.zonaHoraria}
            onValueChange={v => handleCambio('zonaHoraria', v)}
          >
            <SelectTrigger>
              <SelectValue />
            </SelectTrigger>
        ...
        <div>
          ...
          <Select
            ...
            onValueChange={v => handleCambio('moneda', v)}
          >
            <SelectTrigger>
              <SelectValue />
            </SelectTrigger>
      ...
      <Card className="border-2 border-blue-200 bg-gradient-to-br from-blue-50 to-indigo-50/30">
        <CardHeader>
          ...
          <CardDescription>
            Suba un nuevo logo para la empresa. El logo se mostrará en el
            sidebar, login y otras páginas del sistema.
          </CardDescription>
        </CardHeader>
        ...
        <CardContent>
          <div className="flex flex-col space-y-4">
            {logoPreview && (
              <div className="flex flex-col items-center space-y-3 rounded-lg border border-blue-200 bg-white p-4">
                <img
                  src={logoPreview}
                ...
                {hasCustomLogo && (
                  <p className="text-xs text-gray-500">
                    Logo personalizado actual:{' '}
                    {logoInfo?.filename || 'logo-custom'}
                  </p>
                )}
              </div>
            )}
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-red-700">
                      Puede eliminar el logo para restaurar el logo por defecto
                    </p>
                  </div>
                </div>
              ...
              <div className="flex items-center justify-center rounded-lg border border-blue-200 bg-blue-50 p-3">
                <div className="flex items-center space-x-2">
                  ...
                  <p className="text-sm text-blue-900">
                    Usando logo por defecto. Puede subir un logo personalizado
                    arriba.
                  </p>
                </div>
              </div>
            ...
            <div className="flex flex-col space-y-2">
              <label className="flex h-32 w-full cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed border-blue-300 bg-white/50 transition-colors hover:bg-blue-50/50">
