...
export function PagosListResumen() {
  ...
  // Query para detalle de pagos de un cliente (mÃ¡s reciente a mÃ¡s antiguo, paginado)
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="text-sm font-medium text-gray-700 mb-1 block">Buscar por cÃ©dula</label>
              <Input
                placeholder="Escriba cÃ©dula para filtrar..."
      ...
      {/* Tabla de Resumen: clientes con Ãºltimo pago; bÃºsqueda por cÃ©dula y paginaciÃ³n */}
      <Card>
        <CardHeader>
          <CardTitle>Detalle por Cliente (Ãºltimo pago y ver historial)</CardTitle>
        </CardHeader>
        <CardContent>
          {isLoading ? (
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b">
                      ...
                      <th className="px-4 py-3 text-left">ID Ãltimo Pago</th>
                      <th className="px-4 py-3 text-left">Estado</th>
                      <th className="px-4 py-3 text-right">Monto Ãltimo Pago</th>
                      <th className="px-4 py-3 text-left">Fecha Ãltimo Pago</th>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex gap-2">
                            <Button
                              ...
                              title="Ver todos los pagos del cliente (mÃ¡s reciente a mÃ¡s antiguo)"
                            >
      ...
      {/* Modal: detalle de pagos del cliente (mÃ¡s reciente a mÃ¡s antiguo, con paginaciÃ³n) */}
      <Dialog open={!!cedulaDetalle} onOpenChange={(open) => !open && setCedulaDetalle(null)}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          ...
          <p className="text-sm text-gray-600 mb-4">
            Orden: del mÃ¡s reciente al mÃ¡s antiguo. Use la paginaciÃ³n para ver mÃ¡s registros.
          </p>
          {loadingDetalle ? (
            <div className="py-8 text-center text-gray-500">Cargando pagos...</div>
          ) : !detalleData?.pagos?.length ? (
            <div className="py-8 text-center text-gray-500">No hay pagos para esta cÃ©dula.</div>
          ) : (
            <>
                  ...
                  <TableBody>
                    {detalleData.pagos.map((pago: Pago) => (
                      <TableRow key={pago.id}>
                        ...
                        <TableCell>{pago.numero_documento ? 'â'}</TableCell>
                        <TableCell>
                          {(pago.verificado_concordancia === 'SI' || pago.conciliado) ? (
                            <Badge className="bg-green-500 text-white">SÃ­</Badge>
                          ) : (
              ...
              {detalleData.total_pages > 1 && (
                <div className="flex justify-between items-center mt-4">
                  <div className="text-sm text-gray-600">
                    PÃ¡gina {detalleData.page} de {detalleData.total_pages} ({detalleData.total} pagos)
                  </div>
                  <div className="flex gap-2">
                    <Button
                      variant="outline"
