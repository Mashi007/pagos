...
export function PagosListResumen() {
  ...
  // Query para obtener ÃÂºltimos pagos por cÃÂ©dula
  ...
  // Query para detalle de pagos de un cliente (mÃÂ¡s reciente a mÃÂ¡s antiguo, paginado)
  ...
  const handleFilterChange = (key: string, value: string) => {
    // Convertir "all" a cadena vacÃÂ­a para que el servicio no incluya el filtro
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="text-sm font-medium text-gray-700 mb-1 block">Buscar por cÃÂ©dula</label>
              <Input
                placeholder="Escriba cÃÂ©dula para filtrar..."
      ...
      {/* Tabla de Resumen: clientes con ÃÂºltimo pago; bÃÂºsqueda por cÃÂ©dula y paginaciÃÂ³n */}
      <Card>
        <CardHeader>
          <CardTitle>Detalle por Cliente (ÃÂºltimo pago y ver historial)</CardTitle>
        </CardHeader>
        <CardContent>
          {isLoading ? (
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b">
                      <th className="px-4 py-3 text-left">CÃÂ©dula</th>
                      <th className="px-4 py-3 text-left">ID ÃÂltimo Pago</th>
                      <th className="px-4 py-3 text-left">Estado</th>
                      <th className="px-4 py-3 text-right">Monto ÃÂltimo Pago</th>
                      <th className="px-4 py-3 text-left">Fecha ÃÂltimo Pago</th>
                      <th className="px-4 py-3 text-right">Cuotas Atrasadas</th>
                      <th className="px-4 py-3 text-right">Saldo Vencido</th>
                      <th className="px-4 py-3 text-left">Total PrÃÂ©stamos</th>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex gap-2">
                            <Button
                              ...
                              title="Ver todos los pagos del cliente (mÃÂ¡s reciente a mÃÂ¡s antiguo)"
                            >
              ...
              {data && data.total_pages > 1 && (
                <div className="flex justify-between items-center mt-4">
                  <div className="text-sm text-gray-600">
                    PÃÂ¡gina {data.page} de {data.total_pages} ({data.total} registros)
                  </div>
                  <div className="flex gap-2">
                    <Button
                      variant="outline"
      ...
      {/* Modal: detalle de pagos del cliente (mÃÂ¡s reciente a mÃÂ¡s antiguo, con paginaciÃÂ³n) */}
      <Dialog open={!!cedulaDetalle} onOpenChange={(open) => !open && setCedulaDetalle(null)}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          ...
          <p className="text-sm text-gray-600 mb-4">
            Orden: del mÃÂ¡s reciente al mÃÂ¡s antiguo. Use la paginaciÃÂ³n para ver mÃÂ¡s registros.
          </p>
          {loadingDetalle ? (
            <div className="py-8 text-center text-gray-500">Cargando pagos...</div>
          ) : !detalleData?.pagos?.length ? (
            <div className="py-8 text-center text-gray-500">No hay pagos para esta cÃÂ©dula.</div>
          ) : (
            <>
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead>NÃÂº Documento</TableHead>
                  ...
                  <TableBody>
                    {detalleData.pagos.map((pago: Pago) => (
                      <TableRow key={pago.id}>
                        ...
                        <TableCell>{pago.numero_documento ? 'Ã¢ÂÂ'}</TableCell>
                        <TableCell>
                          {(pago.verificado_concordancia === 'SI' || pago.conciliado) ? (
                            <Badge className="bg-green-500 text-white">SÃÂ­</Badge>
                          ) : (
              ...
              {detalleData.total_pages > 1 && (
                <div className="flex justify-between items-center mt-4">
                  <div className="text-sm text-gray-600">
                    PÃÂ¡gina {detalleData.page} de {detalleData.total_pages} ({detalleData.total} pagos)
                  </div>
                  <div className="flex gap-2">
                    <Button
                      variant="outline"
