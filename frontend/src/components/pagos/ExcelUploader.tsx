  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (registrados === 0 && filasOmitidas > 0 && numErrores === 0) {
        toast.info(
          `${filasOmitidas} fila(s) omitida(s): cédula vacía o monto ≤ 0. Revisa el orden de columnas del Excel.`
        )
      }
    } catch (error: any) {
      toast.error(
              ...
              <CardContent className="pt-6">
                <div className="space-y-3">
                  ...
                  <p className="text-sm text-gray-600">
                    Primera fila: encabezados. Desde la segunda fila, datos. El
                    sistema detecta automáticamente el orden de columnas.
                    Formatos soportados:
                  </p>

                  <ul className="list-inside list-disc space-y-1 text-sm text-gray-600">
                    <li>
                      <strong>D (recomendado):</strong> Cédula | Monto | Fecha |
                      Nº documento
                    </li>

                    <li>
                      <strong>A:</strong> Documento | Cédula | Fecha | Monto
                    </li>

                    <li>
                      <strong>B:</strong> Fecha | Cédula | Monto | Documento
                    </li>

                    <li>
                      <strong>C:</strong> Cédula | ID Préstamo | Fecha | Monto |
                      Nº documento
                    </li>
                  </ul>

                  <p className="mt-2 text-xs font-medium text-amber-600">
                    Si una persona tiene varios préstamos, debe indicar el ID
                    del préstamo (formato C). Cédula: solo V, E o J + 6-11
                    dígitos. Nº documento: cualquier formato; no se aceptan
                    duplicados.
                  </p>

                  <p className="mt-1 text-xs text-gray-500">
                    Formatos aceptados: .xlsx o .xls. Las filas con cédula vacía
                    o monto ≤ 0 se envían a Revisar Pagos.
                  </p>

                  <p className="mt-1 text-xs font-medium text-amber-600">
                    Recomendado: hasta 2.500 filas para evitar sobrecarga y
                    timeouts. Máximo permitido: 10.000 filas.
                  </p>
                </div>
              </CardContent>
            </Card>
                  ...
                  <CardContent className="pt-6">
                    <div className="flex items-center gap-2 text-green-600">
                      ...
                      <span className="font-semibold">
                        {results.registros_procesados ?? 0} pago(s)
                        registrado(s)
                      </span>
                    </div>
                    ...
                    {(results.filas_omitidas ?? 0) > 0 && (
                      <p className="mt-1 text-sm text-amber-600">
                        {results.filas_omitidas} fila(s) omitida(s) (cédula
                        vacía o monto ≤ 0). Comprueba que la columna 1 sea
                        Cédula y la 4 sea Monto &gt; 0.
                      </p>
                    )}
                    ...
                    {(results.errores?.length ?? 0) > 0 && (
                      <p className="mt-1 text-sm text-red-600">
                        ...
                        fila(s) con error. Revisa la tabla inferior para ver
                        fila, cédula y descripción.
                        {results.errores_truncados && (
                          <span className="mt-0.5 block text-amber-600">
                            Se muestran los primeros 50 errores y 100 detalles.
                            El listado completo está en Revisar Pagos; puede
                            descargar el Excel desde allí.
                          </span>
                        )}
                      </p>
                    )}

                    {(results.registros_con_error ?? 0) > 0 &&
                      onGoToRevisarPagos && (
