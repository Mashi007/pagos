...
interface ExcelUploaderProps {
  onClose: () => void
  onSuccess: () => void
  /** Si se proporciona, cuando hay registros_con_error > 0 se muestra botÃÂ³n "Ver X en Revisar Pagos" que cierra el modal y ejecuta este callback. */
...
export function ExcelUploader({ onClose, onSuccess, onGoToRevisarPagos }: ExcelUploaderProps) {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (selectedFile) {
      // ÃÂ¢ÃÂÃ¢ÂÂ¦ VALIDACIÃÂN DE SEGURIDAD: Validar archivo antes de aceptarlo
      const { validateExcelFile } = await import('../../utils/excelValidation')
      const validation = validateExcelFile(selectedFile)

      if (!validation.isValid) {
        toast.error(validation.error || 'Archivo invÃÂ¡lido')
        return
      }
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (registrados === 0 && filasOmitidas > 0 && numErrores === 0) {
        toast.info(`${filasOmitidas} fila(s) omitida(s): cÃÂ©dula vacÃÂ­a o monto Ã¢ÂÂ¤ 0. Revisa el orden de columnas del Excel.`)
      }
    } catch (error: any) {
      ...
      >
        ...
        >
          ...
          <div className="flex-1 min-h-0 overflow-y-auto p-6 space-y-6">
            ...
            <Card>
              <CardContent className="pt-6">
                <div className="space-y-3">
                  ...
                  <p className="text-sm text-gray-600">Primera fila: encabezados. Desde la segunda fila, datos. El sistema detecta automÃÂ¡ticamente el orden de columnas. Formatos soportados:</p>
                  <ul className="list-disc list-inside space-y-1 text-sm text-gray-600">
                    <li><strong>D (recomendado):</strong> CÃÂ©dula | Monto | Fecha | NÃÂº documento</li>
                    <li><strong>A:</strong> Documento | CÃÂ©dula | Fecha | Monto</li>
                    <li><strong>B:</strong> Fecha | CÃÂ©dula | Monto | Documento</li>
                    <li><strong>C:</strong> CÃÂ©dula | ID PrÃÂ©stamo | Fecha | Monto | NÃÂº documento</li>
                  </ul>
                  <p className="text-xs text-amber-600 mt-2 font-medium">
                    Si una persona tiene varios prÃÂ©stamos, debe indicar el ID del prÃÂ©stamo (formato C). CÃÂ©dula: solo V, E o J + 6-11 dÃÂ­gitos. NÃÂº documento: cualquier formato; no se aceptan duplicados.
                  </p>
                  <p className="text-xs text-gray-500 mt-1">
                    Formatos aceptados: .xlsx o .xls. Las filas con cÃÂ©dula vacÃÂ­a o monto Ã¢ÂÂ¤ 0 se envÃÂ­an a Revisar Pagos.
                  </p>
                  <p className="text-xs text-amber-600 mt-1 font-medium">
                    Recomendado: hasta 2.500 filas para evitar sobrecarga y timeouts. MÃÂ¡ximo permitido: 10.000 filas.
                  </p>
                </div>
              </CardContent>
            </Card>
            ...
            {/* Resumen y errores detallados (misma interfaz que prÃÂ©stamos/clientes) */}
            {results && (
                  ...
                  <CardContent className="pt-6">
                    ...
                    {(results.filas_omitidas ? 0) > 0 && (
                      <p className="text-sm text-amber-600 mt-1">
                        {results.filas_omitidas} fila(s) omitida(s) (cÃÂ©dula vacÃÂ­a o monto Ã¢ÂÂ¤ 0). Comprueba que la columna 1 sea CÃÂ©dula y la 4 sea Monto &gt; 0.
                      </p>
                    )}
                    {(results.errores?.length ? 0) > 0 && (
                      <p className="text-sm text-red-600 mt-1">
                        {(results.errores_total ? results.errores?.length ? 0)} fila(s) con error. Revisa la tabla inferior para ver fila, cÃÂ©dula y descripciÃÂ³n.
                        {results.errores_truncados && (
                          <span className="block text-amber-600 mt-0.5">
                            Se muestran los primeros 50 errores y 100 detalles. El listado completo estÃÂ¡ en Revisar Pagos; puede descargar el Excel desde allÃÂ­.
                          </span>
                        )}
                      </p>
                    )}
