 ...
 * Muestra errores de validaciÃÂ³n por celda (cÃÂ©dula, fecha, monto, documento).
 * La columna CrÃÂ©dito se rellena automÃÂ¡ticamente cuando la cÃÂ©dula tiene un solo prÃÂ©stamo activo.
 */
...
export interface FilaEditableProps {
  ...
  /** Si se pasa, el botÃÂ³n Guardar muestra estado de carga y se deshabilita mientras guarda. */
  savingProgress?: Record<number, boolean>
  serviceStatus?: 'online' | 'offline' | 'checking' | 'unknown'
  /** EnvÃÂ­a esta fila a Revisar Pagos (para filas que no cumplen validadores). */
...
/**
 * CrÃÂ©dito: si hay 1 prÃÂ©stamo activo por cÃÂ©dula se carga automÃÂ¡ticamente;
 * si hay mÃÂ¡s de uno el usuario debe elegir en el selector;
 * si no hay (0) se muestra "Sin crÃÂ©dito" (solo aplica cuando el cliente estÃÂ¡ registrado pero sin crÃÂ©ditos activos; si no hay cliente, la cÃÂ©dula ya marca error).
 */
function CreditoCell({
  ...
  // 0 crÃÂ©ditos: cliente puede no existir (cedula invÃÂ¡lida) o existir sin crÃÂ©ditos activos
  if (prestamos.length === 0) {
    return (
      <span className="text-xs text-gray-500 italic">Sin crÃÂ©dito</span>
    )
  }

  // 1 crÃÂ©dito: se carga automÃÂ¡ticamente (ya asignado por useEffect), solo mostrar
  if (prestamos.length === 1) {
    ...
    return (
      <input
        ...
        placeholder="Ã¢ÂÂ"
        title="Cargado automÃÂ¡ticamente (un solo crÃÂ©dito por cÃÂ©dula)"
      />
    )
  }

  // 2+ crÃÂ©ditos: el usuario debe elegir
  ...
  return (
    ...
    >
      ...
      >
        <SelectValue placeholder="Elegir crÃÂ©dito" />
      </SelectTrigger>
      <SelectContent>
        <SelectItem value="none">Ã¢ÂÂ Elegir crÃÂ©dito Ã¢ÂÂ</SelectItem>
        {prestamos.map((p) => (
...
export function TablaEditablePagos({
  ...
  // Auto-rellenar prestamo_id cuando la cÃÂ©dula tiene exactamente un prÃÂ©stamo (persiste en estado del padre)
  useEffect(() => {
  ...
  return (
    <div className="space-y-4">
      {/* Indicadores: filas cargadas, vÃÂ¡lidas, invÃÂ¡lidas (varÃÂ­an al editar y validar) */}
      <div className="bg-blue-50 border border-blue-400 rounded-lg p-4">
        ...
        <div className="flex flex-wrap gap-6 text-sm">
          ...
          <span className="font-medium text-green-700">
            VÃÂ¡lidas: <strong className="tabular-nums">{validos}</strong>
          </span>
          <span className="font-medium text-red-700">
            InvÃÂ¡lidas: <strong className="tabular-nums">{invalidas}</strong>
            <span className="text-blue-600 ml-1">(van a Revisar Pagos)</span>
          </span>
        </div>
        <p className="text-xs text-blue-700 mt-2">
          <strong>Los que no cumplen validadores van a Revisar Pagos.</strong> Puede enviarlos con el botÃÂ³n &quot;Revisar Pagos&quot; por fila o con &quot;Revisar Pagos (N)&quot; para enviar todas las invÃÂ¡lidas.
        </p>
        <p className="text-xs text-blue-700 mt-1">
          Al guardar una fila (botÃÂ³n Guardar en cada fila), esta desaparece de la tabla y el pago se registra aplicando las mismas reglas de negocio: aplicaciÃÂ³n a cuotas (FIFO), conciliaciÃÂ³n y actualizaciÃÂ³n de pagos/cuotas.
        </p>
        <p className="text-xs text-blue-700 mt-1">
          <strong>CrÃÂ©dito:</strong> si hay un solo crÃÂ©dito activo por cÃÂ©dula se carga automÃÂ¡ticamente; si hay varios debe elegir en la lista. &quot;Sin crÃÂ©dito&quot; aplica cuando el cliente estÃÂ¡ registrado pero no tiene crÃÂ©ditos activos (si no hay cliente, la cÃÂ©dula marca error).
        </p>
      </div>
        ...
        <table className="w-full text-sm">
          <thead className="bg-gray-100 border-b">
            <tr>
              ...
              <th className="p-2 text-left font-semibold border-r min-w-[140px]">CÃÂ©dula</th>
              ...
              <th className="p-2 text-left font-semibold border-r min-w-[80px]">CrÃÂ©dito</th>
              <th className="p-2 text-center font-semibold min-w-[100px]">AcciÃÂ³n</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((row) => (
              ...
              >
                ...
                {/* CrÃÂ©dito: 1 = auto; varios = elegir; 0 = Sin crÃÂ©dito (solo cuando no hay cliente o sin crÃÂ©ditos activos) */}
                <td className="p-2 border-r">
                  <CreditoCell
                    row={row}
                ...
                {/* AcciÃÂ³n: Guardar solo si la fila pasa validadores y crÃÂ©dito (1 auto, varios elegido, 0 no guardar) */}
                <td className="p-2">
                  {saveRowIfValid ? (() => {
                    const lookup = cedulaLookupParaFila(row.cedula || '', row.numero_documento || '')
                    const sinGuion = lookup.replace(/-/g, '')
                    ...
                    const title = row._hasErrors
                      ...
                      : sinCreditosActivos
                        ? 'Sin crÃÂ©ditos activos para esta cÃÂ©dula; use Revisar Pagos'
                        : sinCreditoElegido
                          ? 'Elija un crÃÂ©dito en la lista (varios por cÃÂ©dula)'
                          : 'Guardar esta fila'
                    return (
      ...
      {invalidas > 0 && (
        <div className="bg-red-50 border border-red-200 rounded p-3 text-sm text-red-800">
          <strong>Ã¢ÂÂ  {invalidas} fila(s) con errores detectados.</strong>
          {' '}CorrÃÂ­gelas en la tabla; cuando pasen los validadores el botÃÂ³n Guardar se activarÃÂ¡. Al guardar, la fila desaparece y el pago se aplica a cuotas y reglas de negocio.
          <ul className="mt-1.5 list-disc list-inside space-y-0.5 text-xs text-red-700">
            <li><strong>CÃÂ©dula</strong>: debe existir en la base de datos de clientes (formato V/E/J + dÃÂ­gitos).</li>
            <li><strong>Fecha</strong>: formato DD/MM/YYYY y fecha vÃÂ¡lida.</li>
            <li><strong>Monto</strong>: nÃÂºmero mayor a 0.</li>
