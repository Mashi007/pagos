          ...
          {gmailStatus && (
            <p className="text-xs text-gray-600 px-2 py-1 mb-1 border-b border-gray-100">
              {gmailStatus.last_status === 'error' ? (
                <span className="text-amber-600">ÃÂltima sync fallÃÂ³</span>
              ) : gmailStatus.last_run ? (
                <>ÃÂltima sync: {formatLastSyncDate(gmailStatus.last_run)} Ã¢ÂÂ {gmailStatus.last_emails} correos, {gmailStatus.last_files} archivos</>
              ) : (
                <span className="text-gray-500">Sin sync aÃÂºn</span>
              )}
            </p>
          )}
<div className="space-y-1">
            <div className="px-2 py-1">
              ...
              <Select value={scanFilter} onValueChange={(v: 'unread' | 'read' | 'all') => setScanFilter(v)}>
                ...
                <SelectContent>
                  <SelectItem value="unread">No leÃÂ­dos</SelectItem>
                  <SelectItem value="read">LeÃÂ­dos</SelectItem>
                  <SelectItem value="all">Todos (leÃÂ­dos y no leÃÂ­dos)</SelectItem>
                </SelectContent>
              </Select>
            </div>
            <button
            ...
            <p className="text-xs text-gray-500 px-2 py-1 mt-1 border-t border-gray-100">
              {scanFilter === 'unread' ? 'Solo no leÃÂ­dos. Al terminar se vuelve a revisar la bandeja.' : scanFilter === 'read' ? 'Solo correos leÃÂ­dos.' : 'Con ÃÂ«TodosÃÂ»: leÃÂ­dos y no leÃÂ­dos de toda la bandeja.'}
            </p>
          </div>
        </PopoverContent>
      </Popover>
      ...
      <ConfirmarBorrarDiaDialog
        ...
        onElegir={async (borrar) => {
          ...
          if (result.confirmado) {
            toast.success(result.mensaje || 'InformaciÃÂ³n del dÃÂ­a borrada.')
          } else {
            toast(result.mensaje || 'InformaciÃÂ³n del dÃÂ­a se mantiene en BD.')
          }
          pagoService.getGmailStatus().then(setGmailStatus)
        }}
      />
