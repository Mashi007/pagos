          ...
          {gmailStatus && (
            <p className="text-xs text-gray-600 px-2 py-1 mb-1 border-b border-gray-100">
              {gmailStatus.last_status === 'error' ? (
                <span className="text-amber-600">Ãltima sync fallÃ³</span>
              ) : gmailStatus.last_run ? (
                <>Ãltima sync: {formatLastSyncDate(gmailStatus.last_run)} â {gmailStatus.last_emails} correos, {gmailStatus.last_files} archivos</>
              ) : (
                <span className="text-gray-500">Sin sync aÃºn</span>
              )}
            </p>
          )}
<div className="space-y-1">
            <div className="px-2 py-1">
              ...
              <Select value={scanFilter} onValueChange={(v: 'unread' | 'read' | 'all') => setScanFilter(v)}>
                ...
                <SelectContent>
                  <SelectItem value="unread">No leÃ­dos</SelectItem>
                  <SelectItem value="read">LeÃ­dos</SelectItem>
                  <SelectItem value="all">Todos (leÃ­dos y no leÃ­dos)</SelectItem>
                </SelectContent>
              </Select>
            </div>
            <button
            ...
            <p className="text-xs text-gray-500 px-2 py-1 mt-1 border-t border-gray-100">
              {scanFilter === 'unread' ? 'Solo no leÃ­dos. Al terminar se vuelve a revisar la bandeja.' : scanFilter === 'read' ? 'Solo correos leÃ­dos.' : 'Con Â«TodosÂ»: leÃ­dos y no leÃ­dos de toda la bandeja.'}
            </p>
          </div>
        </PopoverContent>
      </Popover>
      ...
      <ConfirmarBorrarDiaDialog
        ...
        onElegir={async (borrar) => {
          ...
          if (result.confirmado) {
            toast.success(result.mensaje || 'InformaciÃ³n del dÃ­a borrada.')
          } else {
            toast(result.mensaje || 'InformaciÃ³n del dÃ­a se mantiene en BD.')
          }
          pagoService.getGmailStatus().then(setGmailStatus)
        }}
      />
