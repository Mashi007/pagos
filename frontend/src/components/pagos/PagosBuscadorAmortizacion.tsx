...
export function PagosBuscadorAmortizacion() {
  ...
  const handleBuscar = () => {
    ...
    if (!ced) {
      toast.error('Escriba una cÃ©dula para buscar')
      return
    }
    setCedulaBuscar(ced)
  }
  ...
  const handleConciliarAmortizacionMasiva = async () => {
    if (!window.confirm(
      'Â¿Reconciliar amortizaciÃ³n? Se aplicarÃ¡n los pagos conciliados a las cuotas de todos los prÃ©stamos que tengan pagos pendientes de aplicar (por ejemplo, tras haber actualizado fechas de amortizaciÃ³n).'
    )) return
    setConciliando(true)
    try {
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          ...
          Buscar por cÃ©dula â Tabla de amortizaciÃ³n
        </CardTitle>
        <p className="text-sm text-gray-600 mt-1">
          Ingrese la cÃ©dula del cliente y pulse Buscar para desplegar la tabla de amortizaciÃ³n. Puede descargarla en PDF.
        </p>
        <div className="mt-3 pt-3 border-t">
          ...
          >
            ...
            Reconciliar amortizaciÃ³n (aplicar pagos a cuotas)
          </Button>
          <p className="text-xs text-muted-foreground mt-1">
            Ãtil tras actualizar fechas de amortizaciÃ³n: reasigna pagos conciliados a las cuotas correctas.
          </p>
        </div>
      </CardHeader>
          ...
          <>
            {loading ? (
              <div className="flex items-center justify-center py-12 gap-2 text-gray-500">
                ...
                <span>Cargando tabla de amortizaciÃ³n...</span>
              </div>
            ) : !tieneResultados ? (
              <div className="py-8 text-center text-gray-600">
                No se encontrÃ³ cliente con cÃ©dula <strong>{cedulaBuscar}</strong> o no tiene prÃ©stamos/cuotas.
              </div>
            ) : (
              <>
                <div className="flex items-center justify-between flex-wrap gap-2">
                  <p className="text-sm text-gray-600">
                    Cliente: cÃ©dula <strong>{cedulaBuscar}</strong> Â· {listaCuotas.length} cuota(s)
                  </p>
                ...
                <div className="border rounded-lg overflow-x-auto">
                  <Table>
                    <TableHeader>
                      <TableRow>
                        <TableHead>PrÃ©stamo</TableHead>
                        <TableHead>NÂº Cuota</TableHead>
                        ...
                        .map((c) => (
                          <TableRow key={`${c.prestamo_id}-${c.numero_cuota}`}>
                            ...
                            <TableCell className="text-right font-medium">
                              ${typeof (c as any).monto_cuota === 'number' ? (c as any).monto_cuota.toFixed(2) : (c as any).monto?.toFixed(2) ? 'â'}
                            </TableCell>
                            <TableCell>
                              <Badge
