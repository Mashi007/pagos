...
export function PagosBuscadorAmortizacion() {
  ...
  const { data: prestamos, isLoading: loadingPrestamos } = usePrestamosByCedula(
    cedulaBuscar || ''
  )
    ...
    <Card>
      <CardHeader>
        ...
        <p className="mt-1 text-sm text-gray-600">
          Ingrese la cédula del cliente y pulse Buscar para desplegar la tabla
          de amortización. Puede descargarla en PDF.
        </p>
        ...
        <div className="mt-3 border-t pt-3">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            Útil tras actualizar fechas de amortización: reasigna pagos
            conciliados a las cuotas correctas.
          </p>
        </div>
      </CardHeader>
          ...
          <>
            {loading ? (
              ...
              <div className="py-8 text-center text-gray-600">
                No se encontró cliente con cédula{' '}
                <strong>{cedulaBuscar}</strong> o no tiene préstamos/cuotas.
              </div>
            ) : (
              <>
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <p className="text-sm text-gray-600">
                    Cliente: cédula <strong>{cedulaBuscar}</strong> ·{' '}
                    {listaCuotas.length} cuota(s)
                  </p>
                        ...
                        .map(c => (
                          <TableRow key={`${c.prestamo_id}-${c.numero_cuota}`}>
                            ...
                            <TableCell className="text-right font-medium">
                              $
                              {typeof (c as any).monto_cuota === 'number'
                                ? (c as any).monto_cuota.toFixed(2)
                                : (c as any).monto != null
