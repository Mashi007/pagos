...
/** Tipo de configuraciÃÂ³n por criterio (habilitado, cco, plantilla opcional, programador) */
export type ConfigEnvioItem = {
  habilitado: boolean
  cco: string[]
  plantilla_id?: number | null
  programador?: string
  /** Incluir PDF anexo (pestaÃÂ±a 2: Plantilla anexo PDF). Si true, Carta_Cobranza.pdf se agrega OBLIGATORIAMENTE al email. */
  incluir_pdf_anexo?: boolean
  /** Incluir documentos PDF fijos (pestaÃÂ±a 3: Documentos PDF anexos). Si true, se agregan OBLIGATORIAMENTE al email. */
...
/** Criterios de notificaciÃÂ³n (tipo Ã¢ÂÂ label). Exportado para uso en Plantillas / vinculaciÃÂ³n PDF. */
export const CRITERIOS: { tipo: string; label: string; categoria: string; color: 'blue' | 'green' | 'orange' | 'red' | 'slate' }[] = [
  { tipo: 'PAGO_5_DIAS_ANTES', label: 'Faltan 5', categoria: 'NotificaciÃÂ³n previa', color: 'blue' },
  { tipo: 'PAGO_3_DIAS_ANTES', label: 'Faltan 3', categoria: 'NotificaciÃÂ³n previa', color: 'blue' },
  { tipo: 'PAGO_1_DIA_ANTES', label: 'Falta 1', categoria: 'NotificaciÃÂ³n previa', color: 'blue' },
  { tipo: 'PAGO_DIA_0', label: 'Hoy vence', categoria: 'DÃÂ­a de pago', color: 'green' },
  { tipo: 'PAGO_1_DIA_ATRASADO', label: '1 dÃÂ­a de retraso', categoria: 'Retrasada', color: 'orange' },
  { tipo: 'PAGO_3_DIAS_ATRASADO', label: '3 dÃÂ­as de retraso', categoria: 'Retrasada', color: 'orange' },
  { tipo: 'PAGO_5_DIAS_ATRASADO', label: '5 dÃÂ­as atrasado', categoria: 'Retrasada', color: 'orange' },
  { tipo: 'PREJUDICIAL', label: 'Prejudicial', categoria: 'Prejudicial', color: 'red' },
  { tipo: 'MORA_90', label: '90+ dÃÂ­as de mora (moroso)', categoria: 'Mora 90+', color: 'slate' },
...
/** Normaliza la respuesta de la API en estado listo para el componente (carga ÃÂºnica y clara). */
...
export function ConfiguracionNotificaciones() {
  ...
  useEffect(() => {
    if (errorEnvios) toast.error('Error al cargar la configuraciÃÂ³n de envÃÂ­os')
  }, [errorEnvios])
  ...
  const getConfig = (tipo: string): ConfigEnvioItem => {
    ...
    return {
      ...
      // PDF = Carta_Cobranza (pestaÃÂ±a 2). Por defecto no; marcar para incluir. Adj. = documentos subidos (pestaÃÂ±a 3).
  ...
  const guardarConfiguracionEnvios = async () => {
    ...
    try {
      ...
      toast.success('ConfiguraciÃÂ³n de envÃÂ­os guardada')
    } catch {
      toast.error('Error al guardar la configuraciÃÂ³n de envÃÂ­os')
    } finally {
  ...
  /** En modo prueba el envÃÂ­o usa solo plantilla predeterminada (ejemplo) y envÃÂ­a a correo de pruebas 1 y 2. */
  const ASUNTO_PLANTILLA_PREDETERMINADA = 'NotificaciÃÂ³n de cobranza - RapiCredit (ejemplo)'
  const MENSAJE_PLANTILLA_PREDETERMINADA =
    'Este es un correo de ejemplo con la plantilla predeterminada. Se envÃÂ­a a los correos de pruebas 1 y 2 para verificar la configuraciÃÂ³n de notificaciones.'
  ...
  const handleEnviarNotificacionesPrueba = async () => {
    ...
    if (invalidos.length > 0) {
      toast.error(`Correo(s) no vÃÂ¡lido(s): ${invalidos.join(', ')}. Usa formato usuario@dominio.com`)
      return
    }
    try {
      ...
      if (!estadoEmail?.configurada) {
        const problemas = estadoEmail?.problemas?.join('. ') || 'servidor SMTP, usuario y contraseÃÂ±a'
        toast.error(
          `Configura el email SMTP antes de enviar pruebas: ${problemas} Ve a ConfiguraciÃÂ³n Ã¢ÂÂ Email.`,
          { duration: 6000 }
        )
      ...
      if (enviados === destinos.length) {
        toast.success(`NotificaciÃÂ³n de ejemplo enviada a ${destinos.length} correo(s) de pruebas.`)
      } else if (enviados > 0) {
  ...
  /** EnvÃÂ­o masivo manual: mismo flujo que las 11PM pero en este momento; si modo prueba estÃÂ¡ activo, todos los correos van al correo de pruebas. Guarda la config antes de enviar para que el backend use los toggles actuales. */
    ...
    try {
      ...
      if (res?.en_proceso && res?.mensaje) {
        ...
        if ((enviados ? 0) + (fallidos ? 0) + (sin_email ? 0) === 0 && (omitidos_config ? 0) > 0) {
          toast.warning(`NingÃÂºn envÃÂ­o: ${omitidos_config} omitidos (activa EnvÃÂ­o en al menos una pestaÃÂ±a y vuelve a intentar).`)
        } else {
          toast.success(`EnvÃÂ­os masivos prueba: ${enviados ? 0} enviados, ${fallidos ? 0} fallidos, ${sin_email ? 0} sin email.`)
        }
      }
    } catch (error: unknown) {
      const detalle = getErrorDetail(error)
      toast.error(detalle || 'Error al ejecutar envÃÂ­os masivos.')
    } finally {
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-xl">
            ...
            ConfiguraciÃÂ³n por caso
          </CardTitle>
          <CardDescription>
            Asigna una plantilla a cada pestaÃÂ±a, activa el envÃÂ­o y define si incluir PDF anexo (carta cobranza) y documentos PDF fijos. Cuando marques ÃÂ«PDFÃÂ» (pestaÃÂ±a 2) o ÃÂ«Adj.ÃÂ» (pestaÃÂ±a 3), esos adjuntos se agregan OBLIGATORIAMENTE al email. Los documentos fijos se asignan en ConfiguraciÃÂ³n Ã¢ÂÂ Plantillas Ã¢ÂÂ Documentos PDF anexos.
          </CardDescription>
        </CardHeader>
      </Card>

      {/* Modo Prueba / ProducciÃÂ³n: un solo bloque, sin duplicar config */}
      <Card className={enModoPrueba ? 'border-amber-300 bg-amber-50/50' : 'border-emerald-200 bg-emerald-50/30'}>
        <CardHeader className="pb-3">
          ...
          <CardDescription>
            {enModoPrueba
              ? 'Todos los envÃÂ­os van al correo de pruebas. Puedes activar o desactivar cada caso (EnvÃÂ­o) para elegir a quÃÂ© pestaÃÂ±as enviar.'
              : 'Los emails se envÃÂ­an al correo de cada cliente segÃÂºn la opciÃÂ³n EnvÃÂ­o de cada caso.'}
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
            ...
            <div className="p-3 bg-red-50 border border-red-200 rounded-lg flex items-start gap-2">
              ...
              <div>
                ...
                <p className="text-sm text-red-700 mt-1">
                  ...
                  <Link to="/configuracion?tab=email" className="underline font-medium">ConfiguraciÃÂ³n Ã¢ÂÂ Email</Link>.
                </p>
              </div>
            </div>
          )}
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-gray-600">{modoPruebas ? 'Activado (solo correo de pruebas)' : 'Desactivado (envÃÂ­o a clientes)'}</span>
          </div>

          {/* Correos de Prueba: hasta 2 que reciben notificaciones en modo prueba */}
          <div className="space-y-3">
            <p className="text-xs text-gray-500">Hasta 2 correos que recibirÃÂ¡n las notificaciones en modo prueba.</p>
            <div className="flex items-center gap-2">
          ...
          {/* En modo prueba: envÃÂ­o manual plantilla predeterminada + envÃÂ­os masivos prueba */}
          {modoPruebas && (emailsPruebas[0]?.trim() || emailsPruebas[1]?.trim()) && (
            <div className="pt-4 border-t border-amber-200 space-y-3">
              <p className="text-sm text-gray-600">
                EnvÃÂ­a un correo de ejemplo con la <strong>plantilla predeterminada</strong> a los correos de pruebas 1 y 2.
              </p>
              <Button
              ...
              <p className="text-sm text-gray-600 mt-2">
                EnvÃÂ­os masivos: envÃÂ­a ahora un correo por cada cliente que requiera notificaciÃÂ³n; todos van al correo de pruebas (no a clientes).
              </p>
              <Button
              ...
              >
                <Mail className="h-5 w-5" />
                {enviandoMasivo ? 'Enviando...' : 'EnvÃÂ­os masivos prueba'}
              </Button>
            </div>
          )}
        </CardContent>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gray-50 border-b border-gray-200">
              ...
              <th className="text-center py-3 px-4 font-semibold text-gray-700 w-20">EnvÃÂ­o</th>
              <th className="text-center py-3 px-4 font-semibold text-gray-700 w-20" title="Incluir Carta_Cobranza.pdf (generada desde ConfiguraciÃÂ³n Ã¢ÂÂ Plantillas Ã¢ÂÂ Plantilla anexo PDF). Desmarca para usar solo tus PDFs subidos en Documentos PDF anexos." aria-label="Incluir carta cobranza PDF">PDF</th>
              <th className="text-center py-3 px-4 font-semibold text-gray-700 w-20" title="Incluir documentos PDF fijos asignados a esta pestaÃÂ±a (ConfiguraciÃÂ³n Ã¢ÂÂ Plantillas Ã¢ÂÂ Documentos PDF anexos)" aria-label="Incluir documentos PDF fijos de esta pestaÃÂ±a">Adj.</th>
                ...
                <tr key={tipo} className={`border-b border-gray-100 ${col.bg}`}>
                  ...
                  <td className="py-3 px-4">
                    ...
                    {listaPlantillas.length === 0 && (
                      <p className="text-xs text-gray-500 mt-1">Crea plantillas en <Link to="/configuracion?tab=plantillas" className="text-blue-600 hover:underline">ConfiguraciÃÂ³n Ã¢ÂÂ Plantillas</Link></p>
                    )}
                  </td>
                  <td className="py-3 px-4 text-center">
                    <button
                      ...
                      title={config.habilitado ? 'Desactivar envÃÂ­o' : 'Activar envÃÂ­o'}
                      className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-1 ${
                  ...
                  <td className="py-3 px-4 text-center">
                    <input
                      ...
                      disabled={!config.habilitado}
                      title="PestaÃÂ±a 2: agregar OBLIGATORIAMENTE Carta_Cobranza.pdf al email. Desmarca para no incluirla."
                  ...
                  <td className="py-3 px-4 text-center">
                    <input
                      ...
                      disabled={!config.habilitado}
                      title="PestaÃÂ±a 3: agregar OBLIGATORIAMENTE los documentos PDF fijos (Documentos PDF anexos) al email."
                      className="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                    />
                  </td>
                  <td className="py-3 px-4">
                    <details className="group">
                      ...
                      <div className="mt-2 space-y-2 pl-0">
                        <label className="text-xs text-gray-500 block">Hora envÃÂ­o</label>
                        <Input
                          type="time"
