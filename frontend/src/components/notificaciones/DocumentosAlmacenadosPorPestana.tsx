...
interface DocumentosAlmacenadosPorPestanaProps {
  /** Si true, muestra botÃÂ³n eliminar en cada documento. Por defecto true. */
  permitirEliminar?: boolean
  /** TÃÂ­tulo de la secciÃÂ³n. Por defecto "Documentos almacenados por pestaÃÂ±a". */
  titulo?: string
  /** Clase CSS adicional para el contenedor. */
  className?: string
  /** Si true, al mostrarse la pestaÃÂ±a se vuelve a cargar la lista. */
  tabActivo?: boolean
}

export function DocumentosAlmacenadosPorPestana({
  permitirEliminar = true,
  titulo = 'Documentos almacenados por pestaÃÂ±a',
  className = '',
  tabActivo = true,
    ...
    <Card className={className}>
      <CardContent className="pt-4">
        ...
        <p className="text-xs text-muted-foreground mb-3">
          Cada documento se adjunta solo a la notificaciÃÂ³n de la pestaÃÂ±a indicada (ej. los de ÃÂ«1 dÃÂ­a retrasoÃÂ» van con ese tipo de aviso).
        </p>
        <div className="space-y-3">
          {TIPOS_CASO_DOCS.map(({ value, label }) => {
            ...
            return (
              <div key={value} className="rounded-md border bg-gray-50/50 p-3">
                <span className="text-sm font-medium text-gray-600" title={`Se envÃÂ­an con la notificaciÃÂ³n: ${label}`}>
                  {label}
                </span>
          ...
          {!tieneAlguno && (
            <p className="text-sm text-gray-500">No hay documentos vinculados a pestaÃÂ±as. Sube un PDF en la pestaÃÂ±a ÃÂ«Documentos PDF anexosÃÂ», elige la pestaÃÂ±a de notificaciÃÂ³n y pulsa Subir; se guardarÃÂ¡n y aparecerÃÂ¡n aquÃÂ­.</p>
          )}
        </div>
      </CardContent>
    </Card>
