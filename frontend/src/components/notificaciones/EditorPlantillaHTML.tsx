...
export function EditorPlantillaHTML({ plantilla, onGuardado }: EditorPlantillaHTMLProps) {
  ...
  const tiposPorCategoria = {
    antes: [
      { valor: 'PAGO_5_DIAS_ANTES', label: '5 dÂas antes' },
      { valor: 'PAGO_3_DIAS_ANTES', label: '3 dÂas antes' },
      { valor: 'PAGO_1_DIA_ANTES', label: '1 dÂa antes' },
    ],
    diaPago: [
      { valor: 'PAGO_DIA_0', label: 'DÂa de pago' },
    ],
    retraso: [
      { valor: 'PAGO_1_DIA_ATRASADO', label: '1 dÂa de retraso' },
      { valor: 'PAGO_3_DIAS_ATRASADO', label: '3 dÂas de retraso' },
      { valor: 'PAGO_5_DIAS_ATRASADO', label: '5 dÂas de retraso' },
    ],
  }
  ...
  const handleEnviarPrueba = async () => {
    if (!emailPrueba.trim() || !emailPrueba.includes('@')) {
      toast.error('Ingresa un email vÂlido')
      return
    }
  ...
  return (
    <div className="space-y-4">
      {/* InformaciÂn General */}
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <FileText className="h-5 w-5" />
            InformaciÂn de la Plantilla
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <div>
            <label className="text-sm font-medium text-gray-700">DescripciÂn (opcional)</label>
            <Textarea
              placeholder="Describe el propÂsito de esta plantilla"
              value={descripcion}
          ...
          <div>
            ...
            <Input
              placeholder="Ej: Tu pago vence en 5 dÂas"
              value={asunto}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Ingresa el cÂdigo HTML del email. Puedes usar variables como {'{'} nombre {'}'}, {'{'} monto {'}'}, {'{'} dias_atraso {'}'}, etc.
          </CardDescription>
        </CardHeader>
        <CardContent>
          ...
          {!mostrarPreview && (
            <div className="space-y-2">
              <Textarea
                placeholder="<html>&#10;  <body>&#10;    <p>Hola {'{'}nombre{'}'},</p>&#10;    <p>Tu pago de ${'{'}monto{'}'} vence en {'{'}dias_atraso{'}'} dÂas.</p>&#10;  </body>&#10;</html>"
                value={cuerpoHTML}
            ...
            <div className="bg-white border border-gray-200 rounded-lg p-6 min-h-96">
              <div
                ...
                dangerouslySetInnerHTML={{
                  __html: cuerpoHTML || '<p style="color: #999; text-align: center;">El preview aparecerÂ aquÂ...</p>',
                }}
              />
            </div>
          )}
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          ...
          <p className="text-xs text-gray-500">
            Se enviarÂ un email con el asunto y contenido HTML que ingresaste. Las variables se mostrarÂn de forma literal.
          </p>
        </CardContent>
      </Card>
