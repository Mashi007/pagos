...
const TIPO_LABEL: Record<string, string> = {
  ...
  PAGO_1_DIA_ATRASADO: '1 dÃÂ­a retraso',
  PAGO_3_DIAS_ATRASADO: '3 dÃÂ­as retraso',
  PAGO_5_DIAS_ATRASADO: '5 dÃÂ­as atraso',
...
interface PlantillasGuardadasListaProps {
  ...
  className?: string
  /** MÃÂ¡ximo de plantillas a mostrar (0 = todas) */
  maxItems?: number
    ...
    <Card className={className}>
      <CardContent className="pt-4">
        ...
        <p className="text-xs text-muted-foreground mb-3">
          Plantillas de cuerpo de email creadas y vinculadas por tipo de notificaciÃÂ³n. Editar en la pestaÃÂ±a ÃÂ«Plantilla cuerpo emailÃÂ».
        </p>
        {list.length === 0 ? (
          <p className="text-sm text-gray-500">AÃÂºn no hay plantillas. Crea una en la pestaÃÂ±a ÃÂ«Plantilla cuerpo emailÃÂ».</p>
        ) : (
          <ul className="space-y-1.5">
            ...
            {hayMas && (
              <li className="text-xs text-gray-500 pt-1">
                y {plantillas.length - list.length} mÃÂ¡s. Ver todas en ÃÂ«Plantilla cuerpo emailÃÂ».
              </li>
            )}
          </ul>
        )}
