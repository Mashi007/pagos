...
export function GeneraVariables() {
  ...
  const inicializarVariablesPrecargadas = async () => {
    try {
      ...
      if (error?.response?.status !== 404) {
        console.warn(
          'No se pudieron inicializar variables precargadas en BD:',
  ...
  const cargarVariables = async () => {
    ...
    try {
      ...
      if (error?.response?.status !== 404) {
        toast.error(
          error?.response?.data?.detail || 'Error al cargar variables'
        )
      }
  ...
  const camposTabla = nuevaVariable.tabla
    ? CAMPOS_DISPONIBLES[
        nuevaVariable.tabla as keyof typeof CAMPOS_DISPONIBLES
      ] || []
    : []
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Configure variables personalizadas que se relacionan con todos los
            campos disponibles en la base de datos. Estas variables pueden
            usarse en las plantillas de notificaciones usando la sintaxis{' '}
            {'{{nombre_variable}}'}.
          </CardDescription>
        </CardHeader>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-4 rounded-lg border bg-gray-50 p-4">
            <h3 className="text-sm font-semibold">Nueva Variable</h3>
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={e =>
                    setNuevaVariable({
                      ...nuevaVariable,
                      nombre_variable: e.target.value,
                    })
                  }
                ...
                <p className="mt-1 text-xs text-gray-500">
                  Se usará como {'{{nombre_cliente}}'} en plantillas
                </p>
              </div>
                ...
                <select
                  ...
                  onChange={e => {
                    ...
                    setNuevaVariable({
                      ...
                      descripcion:
                        campoInfo?.descripcion ||
                        nuevaVariable.descripcion ||
                        '',
                    })
                  }}
                  className="w-full rounded border px-3 py-2 text-sm"
            ...
            <div className="flex gap-2">
              <Button
                ...
                disabled={
                  !nuevaVariable.nombre_variable || !nuevaVariable.campo_bd
                }
              >
                  ...
                  <TableRow>
                    ...
                    >
                      {busqueda
                        ...
                        : 'No hay variables configuradas. Cree una nueva variable.'}
                    </TableCell>
                  </TableRow>
                ) : (
                      ...
                      <TableCell className="text-right">
                        <div className="flex justify-end gap-2">
                          ...
                          <Button
                            ...
                            size="sm"
                            onClick={() =>
                              variable.id && eliminarVariable(variable.id)
                            }
                            title="Eliminar"
                          >
            ...
            <CardContent>
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
                {Object.entries(CAMPOS_DISPONIBLES).map(([tabla, campos]) => (
                  <div key={tabla} className="rounded border p-3">
