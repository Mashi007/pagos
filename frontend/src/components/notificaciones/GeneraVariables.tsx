...
const CAMPOS_DISPONIBLES = {
  ...
  cuotas: [
    ...
    { campo: 'monto_interes', descripcion: 'Monto de interÃÂ©s' },
    ...
    { campo: 'interes_pagado', descripcion: 'InterÃÂ©s pagado' },
    ...
    { campo: 'interes_pendiente', descripcion: 'InterÃÂ©s pendiente' },
...
export function GeneraVariables() {
  ...
  const inicializarVariablesPrecargadas = async () => {
    try {
      ...
      // Recargar variables despuÃÂ©s de inicializar
      await cargarVariables()
    } catch (error: any) {
      // Si falla, no es crÃÂ­tico - las variables precargadas del frontend seguirÃÂ¡n funcionando
  ...
  const cargarVariables = async () => {
    ...
    try {
      ...
      // Si el endpoint no existe aÃÂºn, usar lista vacÃÂ­a
      if (error?.response?.status !== 404) {
  ...
  useEffect(() => {
    // Inicializar variables precargadas automÃÂ¡ticamente al cargar el componente
  ...
  const eliminarVariable = async (id: number) => {
    if (!window.confirm('ÃÂ¿EstÃÂ¡ seguro de eliminar esta variable?')) return
    ...
    <div className="space-y-6">
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="border rounded-lg p-4 space-y-4 bg-gray-50">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                />
                <p className="text-xs text-gray-500 mt-1">Se usarÃÂ¡ como {'{{nombre_cliente}}'} en plantillas</p>
              </div>
              <div>
                ...
                >
                  ...
                  <option value="prestamos">PrÃÂ©stamos</option>
              ...
              <div>
                <label className="text-sm text-gray-600 mb-1 block">DescripciÃÂ³n</label>
                <Input
                  value={nuevaVariable.descripcion || ''}
                  onChange={e => setNuevaVariable({ ...nuevaVariable, descripcion: e.target.value })}
                  placeholder="DescripciÃÂ³n de la variable"
                />
              </div>
          ...
          {/* InformaciÃÂ³n de campos disponibles */}
          <Card>
            <CardHeader>
                    ...
                    <ul className="space-y-1 text-xs text-gray-600">
                      {campos.map(campo => (
                        <li key={campo.campo} className="flex items-start gap-1">
                          <span className="font-mono text-xs">Ã¢ÂÂ¢</span>
