...
export function clearLogoCache() {
  ...
  devLog('Caché del logo limpiado, se usará el logo por defecto')
}
...
if (typeof window !== 'undefined' && process.env.NODE_ENV === 'development') {
  ...
  devLog(
    'Función de debugging disponible: window.clearLogoCache() para limpiar el caché del logo'
  )
}
...
export function Logo({
  ...
  const [customLogoUrl, setCustomLogoUrl] = useState<string | null>(
    forceDefault ? null : logoCache.logoUrl
  )

  const [hasChecked, setHasChecked] = useState(
    forceDefault ? true : logoCache.hasChecked
  )
  ...
  useEffect(() => {
    ...
    const checkCustomLogo = async () => {
      ...
      try {
        ...
        try {
          const config = await apiClient.get<{ logo_filename?: string }>(
          ...
          if (config?.logo_filename) {
            ...
            try {
              ...
              if (headResponse.ok) {
                ...
                const logoChanged =
                  logoCache.logoFilename !== config.logo_filename
                ...
                if (logoChanged) {
                  ...
                  devDebug(
                    'Logo actualizado desde configuración:',
                    config.logo_filename
                  )
                } else {
                  devDebug(
                    'Logo verificado (sin cambios):',
                    config.logo_filename
                  )
                }
              ...
              if (error?.name !== 'AbortError') {
                devWarn(
                  'Error verificando logo (HEAD), asumiendo que no existe:',
                  getErrorMessage(headError)
                )
              }
          ...
          if (error?.name !== 'AbortError') {
            devWarn(
              'No se pudo obtener logo_filename desde configuración:',
    ...
    const handleCacheUpdate = (url: string | null, version: number) => {
      ...
      if (filenameChanged || hadNoLogo) {
        ...
        if (filenameChanged && currentFilename) {
          devDebug(
            'ðŸ"„ Actualizando logo desde caché compartido, versión:',
            version,
            'filename:',
            newFilename
          )
    ...
    const handleLogoUpdate = (event: CustomEvent) => {
      ...
      devDebug('ðŸ"¢ Evento logoUpdated recibido:', {
        filename,
        url,
        confirmed,
      })
      ...
      if (confirmed && !filename && !url) {
        devWarn(
          'Evento logoUpdated recibido con confirmed pero sin filename/url'
        )
      ...
      if (confirmed && (filename || url)) {
        devDebug(
          'ðŸ"„ Logo confirmado, invalidando caché y recargando desde configuración'
        )
      ...
      <div className={cn('relative', sizeMap[size])}>
        ...
        <img
          ...
          onError={e => {
            ...
            devWarn(
              'Error cargando logo (GET falló o imagen inválida), limpiando caché:',
              customLogoUrl
            )
