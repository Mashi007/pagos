...
const forgotSchema = z.object({
  email: z.string().min(1, 'Indique su correo').email('Correo invÃ¡lido'),
})
...
export function LoginForm() {
  ...
  const onSubmit = async (data: LoginFormData) => {
    try {
      ...
      // Asegurar que remember sea boolean (por defecto true = Recordarme activo) y email en minÃºsculas
      const loginData = {
        ...data,
        email: data.email.toLowerCase().trim(), // Convertir a minÃºsculas
        remember: data.remember !== false // true por defecto: persistir sesiÃ³n en localStorage
      }
      ...
      if (isAxiosError(error) && error.response?.status === 422) {
        ...
        setError('root', {
          message: 'Demasiados intentos de inicio de sesiÃ³n. Espere un minuto e intente de nuevo.'
        })
      } else {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 to-indigo-100 p-4 overflow-y-auto">
      ...
      >
        ...
        <motion.div
          ...
          className="mx-auto"
        >
          {/* Logo RapiCredit: forceDefault para no llamar a /api en login (pÃºblico) y branding consistente */}
          ...
          <CardContent className="space-y-6">
            ...
            {/* Modal OlvidÃ³ su contraseÃ±a: envÃ­a notificaciÃ³n a itmaster@rapicreditca.com */}
            <Dialog open={forgotOpen} onOpenChange={setForgotOpen}>
              <DialogContent className="max-w-md">
                <DialogHeader>
                  <DialogTitle>Restablecer contraseÃ±a</DialogTitle>
                  <DialogDescription>
                    Indique su correo electrÃ³nico. Se enviarÃ¡ una notificaciÃ³n a {FORGOT_EMAIL_DESTINO} para el envÃ­o de una nueva contraseÃ±a.
                  </DialogDescription>
                </DialogHeader>
                {forgotSuccess ? (
                  <p className="text-sm text-green-700 py-2">
                    Solicitud enviada. Se ha notificado al administrador. RecibirÃ¡ un correo en {FORGOT_EMAIL_DESTINO} para gestionar el restablecimiento de su contraseÃ±a.
                  </p>
                ) : (
                  <>
                    <div className="py-2">
        ...
        <div className="text-center text-sm text-muted-foreground">
          ...
          <p className="mt-1">ÃÂ© 2024 - Todos los derechos reservados</p>
