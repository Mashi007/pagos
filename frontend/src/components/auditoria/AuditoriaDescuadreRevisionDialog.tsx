...
import {
  auditoriaService,
  type RevisionDescuadrePagosCuotasResponse,
  ...
  return (
    <div className="flex flex-wrap items-center gap-3">
      ...
      <div
        ...
        title="Semaforo de cuadre contable"
      >
        <span
...
export function AuditoriaDescuadreRevisionDialog({
  ...
  const [data, setData] = useState<RevisionDescuadrePagosCuotasResponse | null>(
    null
  )
  ...
  const cargar = useCallback(async () => {
    ...
    try {
      const r =
        await auditoriaService.obtenerRevisionDescuadrePagosCuotas(prestamoId)
      setData(r)
    } catch (e: unknown) {
      const msg =
          ...
          {prestamoId != null ? (
            <p className="text-sm text-slate-600">
              <Link
                ...
                to={`/prestamos?prestamo_id=${prestamoId}`}
              >
            ...
            <div className="space-y-4">
              <div className="grid gap-4 lg:grid-cols-3">
                <Card className="border-slate-200 shadow-sm">
                  <CardHeader className="pb-2">
                    ...
                    <CardDescription>
                      Identidad y estructura del credito en base de datos
                    </CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-3 text-sm">
                    <div>
                      ...
                      <p className="font-medium text-slate-900">
                        {data.prestamo_nombres || '-'}
                      </p>
                      <p className="font-mono text-xs text-slate-600">
                        {data.prestamo_cedula || '-'}
                      </p>
                    </div>
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      <div className="rounded-md bg-slate-50 p-2">
                        ...
                        <div className="font-mono text-sm tabular-nums">
                          {data.sum_monto_cuotas_usd ?? '-'}
                        </div>
                      </div>
                      <div className="rounded-md bg-slate-50 p-2">
                        <div className="text-[10px] uppercase text-slate-500">
                          Suma total_pagado (col.)
                        </div>
                        <div className="font-mono text-sm tabular-nums">
                    ...
                    <div className="flex flex-wrap items-center gap-2 border-t border-slate-100 pt-2">
                      ...
                      <span className="text-xs text-slate-600">
                        ...
                        <span className="font-mono">
                          {data.fecha_liquidado || 'sin fecha'}
                        </span>
                      </span>
                    </div>
                  </CardContent>
                  ...
                  <CardHeader className="space-y-3 pb-2">
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div>
                        ...
                        <CardDescription>
                          Pagos operativos (excl. anulados/reversados/duplicados
                          declarados) frente a suma de{' '}
                          <code className="rounded bg-slate-100 px-1 text-[10px]">
                            cuota_pagos
                          </code>{' '}
                          vía pagos operativos
                        </CardDescription>
                      </div>
                    ...
                    <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-4">
                      <div className="rounded-lg border border-slate-200 bg-gradient-to-br from-white to-slate-50/80 p-3">
                        ...
                        <div className="mt-1 text-[11px] text-slate-500">
                          USD · misma regla que auditoria cartera
                        </div>
                      </div>
                      <div className="rounded-lg border border-slate-200 bg-gradient-to-br from-white to-slate-50/80 p-3">
                        <div className="text-[10px] font-medium uppercase tracking-wide text-slate-500">
                      ...
                      <div className="rounded-lg border border-orange-200 bg-orange-50/60 p-3">
                        ...
                        <div className="font-mono text-xl tabular-nums text-orange-900">
                          {data.diff_usd}
                        </div>
                        <div className="mt-1 text-[11px] text-orange-800/90">
                          Tol. {data.tolerancia_usd} USD · si mayor, revisar
                          BS/USD, duplicados o aplicacion
                        </div>
                      </div>
                      <div className="rounded-lg border border-slate-200 bg-white p-3">
                        ...
                        <ul className="mt-1 space-y-1 text-[11px] leading-snug text-slate-700">
                          <li>
                            Pagos en prestamo:{' '}
                            <strong>
                              {data.n_pagos_en_bd ?? data.pagos.length}
                            </strong>
                          </li>
                          <li>
                            Operativos cartera:{' '}
                            <strong>
                              {data.n_pagos_operativos_cartera ?? '-'}
                            </strong>
                          </li>
                          <li>
                          ...
                          <li>
                            Cuotas en BD:{' '}
                            <strong>
                              {data.n_cuotas_en_bd ?? data.cuotas.length}
                            </strong>
                          </li>
                        </ul>
                      </div>
              ...
              <Card className="border-slate-200 shadow-sm">
                <CardHeader className="pb-2">
                  ...
                  <CardDescription>
                    Resumen calculado con los mismos datos que la tabla
                    expandible
                  </CardDescription>
                </CardHeader>
                <CardContent className="grid gap-2 text-sm text-slate-700 sm:grid-cols-2">
                  <p className="rounded-md border border-slate-100 bg-slate-50/80 p-3">
                    Si <strong>diferencia</strong> es alta y{' '}
                    <strong>suma total_pagado en cuotas</strong> se aleja del
                    aplicado vía <code className="text-xs">cuota_pagos</code>,
                    priorice revisar filas de pago (BS mal como USD, duplicados,
                    estados no excluidos).
                  </p>
                  <p className="rounded-md border border-slate-100 bg-slate-50/80 p-3">
                    Si <strong>financiamiento</strong> y{' '}
                    <strong>suma monto cuotas</strong> no alinean, el problema
                    puede estar en la tabla{' '}
                    <code className="text-xs">cuotas</code> o en configuracion
                    del prestamo, no solo en pagos.
                  </p>
                </CardContent>
              </Card>
              ...
              {!esAdmin ? (
                <p className="rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-950">
                  Modo lectura: solo administradores pueden editar, eliminar o
                  aplicar cuotas desde esta herramienta.
                </p>
              ) : null}
              ...
              <Card className="border-slate-200 shadow-sm">
                <CardHeader className="pb-2">
                  ...
                  <CardDescription>
                    Acciones sobre el prestamo completo o recarga desde BD
                  </CardDescription>
                </CardHeader>
              ...
              <TarjetaExpandibleBd
                ...
                resumenColapsado={
                  <span>
                    ...
                    <strong>{data.n_pagos_operativos_cartera ?? '-'}</strong>{' '}
                    cuentan como operativos cartera ·{' '}
                    <strong className="text-orange-800">
                      {data.n_pagos_operativos_saldo_fuera_tol ?? '-'}
                    </strong>{' '}
                      ...
                      <TableBody>
                        {data.pagos.map(p => (
                          <TableRow key={p.pago_id}>
                            ...
                            <TableCell className="text-right font-mono text-xs">
                              {p.sum_aplicado_cuotas}
                            </TableCell>
                            <TableCell className="text-right font-mono text-xs">
                              {p.saldo_sin_aplicar_usd}
                            </TableCell>
                            <TableCell
                                ...
                                {esAdmin ? (
                                  <>
                                    ...
                                    <Button
                                      ...
                                      onClick={() =>
                                        void aplicarCuotasPago(p.pago_id)
                                      }
                                    >
                                      {aplicarBusy === p.pago_id ? (
              ...
              <TarjetaExpandibleBd
                ...
                resumenColapsado={
                  <span>
                    <strong>{data.cuotas.length}</strong> cuotas en BD · suma
                    montos{' '}
                    <span className="font-mono">
                      {data.sum_monto_cuotas_usd ?? '-'}
                    </span>{' '}
