...
export function Sidebar({ isOpen, onClose, onToggle }: SidebarProps) {
  ...
  const menuItems: MenuItem[] = [
    ...
    },
    {
      ...
      children: [
        ...
        { title: 'CampaÃ±as', href: '/crm/campanas', icon: Mail },
    ...
    },
    {
      ...
      children: [
        ...
        { title: 'HistÃ³rico por cliente', href: '/cobros/historico-cliente', icon: Clock },
      ],
    },
    {
      title: 'Reportes',
  ...
  // Operativo: no ve ConfiguraciÃ³n (ni auditorÃ­a); solo Administrador puede acceder a esos mÃ³dulos
  const filteredMenuItems = menuItems.filter((item) => {
    const isAdmin = (user?.rol || 'operativo') === 'administrador'
    if (item.title === 'ConfiguraciÃ³n') return isAdmin
    return true
  })

  const isActiveRoute = (href: string) => {
    // Ã¢Åâ¦ Manejar dashboard especial
    if (href === '/dashboard') {
      return location.pathname === '/' || location.pathname === '/dashboard'
    }

    // Ã¢Åâ¦ Para rutas con query parameters, comparar URL completa (EXACTA)
    if (href.includes('?')) {
    ...
    // Ã¢Åâ¦ Para rutas sin query params, verificar que sea exacta Y que no haya query params en la URL actual
    ...
    // Ã¢Åâ¦ ComparaciÃ³n exacta de pathname para rutas sin query params
                    ...
                    <div className="mt-2 bg-white rounded-lg border border-blue-200 shadow-xl shadow-blue-500/10">
                      <div className="py-2">
                        ...
                        {(user?.rol || 'operativo') !== 'administrador' && (
                          ...
                          >
                            <span>Ã°Å¸ââ Actualizar Rol</span>
                          </button>
                        )}
                      </div>
