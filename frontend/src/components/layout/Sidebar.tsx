...
export function Sidebar({ isOpen, onClose, onToggle }: SidebarProps) {
  ...
  const toggleCompact = () => {
    ...
    // Si se estÃÂ¡ compactando, cerrar submenÃÂºs
    if (!isCompact) {
      setOpenSubmenus([])
    }
  }
  ...
  const menuItems: MenuItem[] = [
    ...
    },
    {
      ...
      children: [
        ...
        { title: 'CampaÃÂ±as', href: '/crm/campanas', icon: Mail },
        { title: 'Tickets AtenciÃÂ³n', href: '/crm/tickets', icon: FileText },
    ...
    },
    {
      ...
      children: [
        ...
        { title: 'HistÃÂ³rico por cliente', href: '/cobros/historico-cliente', icon: Clock },
      ],
    },
    {
      title: 'Reportes',
    ...
    },
    {
      ...
      children: [
        ...
        { title: 'ConfiguraciÃÂ³n Email', href: '/configuracion?tab=email', icon: Mail },
        { title: 'ConfiguraciÃÂ³n WhatsApp', href: '/configuracion?tab=whatsapp', icon: MessageSquare },
        { title: 'ConfiguraciÃÂ³n AI', href: '/configuracion?tab=ai', icon: Brain },
        ...
        { title: 'Modelos de VehÃÂ­culos', href: '/modelos-vehiculos', icon: Car },
        { title: 'Usuarios', href: '/usuarios', icon: Shield },
      ],
    },
  ]

  // Abrir automÃÂ¡ticamente el submenÃÂº si alguna de sus rutas estÃÂ¡ activa
  useEffect(() => {
  ...
  // Operativo: no ve ConfiguraciÃÂ³n (ni auditorÃÂ­a); solo Administrador puede acceder a esos mÃÂ³dulos
  const filteredMenuItems = menuItems.filter((item) => {
    const isAdmin = (user?.rol || 'operativo') === 'administrador'
    if (item.title === 'ConfiguraciÃÂ³n') return isAdmin
    return true
  })

  const isActiveRoute = (href: string) => {
    // ÃÂ¢ÃÂÃ¢ÂÂ¦ Manejar dashboard especial
    if (href === '/dashboard') {
      return location.pathname === '/' || location.pathname === '/dashboard'
    }

    // ÃÂ¢ÃÂÃ¢ÂÂ¦ Para rutas con query parameters, comparar URL completa (EXACTA)
    if (href.includes('?')) {
    ...
    // ÃÂ¢ÃÂÃ¢ÂÂ¦ Para rutas sin query params, verificar que sea exacta Y que no haya query params en la URL actual
    // Esto evita que /configuracion resalte cuando estÃÂ¡s en /configuracion?tab=email
    if (location.search) {
      // Si la URL actual tiene query params pero el href no, NO resaltar
      return false
    }

    // ÃÂ¢ÃÂÃ¢ÂÂ¦ ComparaciÃÂ³n exacta de pathname para rutas sin query params
      ...
      >
        <div className="flex flex-col h-full">
          ...
          <div className={cn(
            ...
            {/* BotÃÂ³n toggle modo compacto - solo desktop */}
              ...
              {filteredMenuItems.map((item, index) => (
                ...
                >
                  {item.isSubmenu && item.children ? (
                    // Renderizar submenÃÂº con dropdown
                    <div>
                      <button
                    ...
                    <div className="relative">
                      <NavLink
                        to={item.href!}
                        onClick={() => {
                          // Cerrar sidebar en mÃÂ³vil al hacer click
                          if (window.innerWidth < 1024) {
                            onClose()
                          }
                        }}
          ...
          {/* Footer con informaciÃÂ³n de usuario */}
          <div className={cn(
            ...
            <div className="relative">
              ...
              {/* MenÃÂº desplegable del usuario */}
              <AnimatePresence>
                {showUserMenu && (
                  <motion.div
                    ...
                    <div className="mt-2 bg-white rounded-lg border border-blue-200 shadow-xl shadow-blue-500/10">
                      <div className="py-2">
                        ...
                        {(user?.rol || 'operativo') !== 'administrador' && (
                          ...
                          >
                            <span>ÃÂ°ÃÂ¸Ã¢ÂÂÃ¢ÂÂ Actualizar Rol</span>
                          </button>
                        )}
                      </div>
