...
export function PrestamoDetalleModal({ prestamo: prestamoInitial, onClose }: PrestamoDetalleModalProps) {
  ...
  // Recargar datos completos del prÃÂ©stamo
  const { data: prestamo, isLoading } = usePrestamo(prestamoInitial.id)

  // Obtener evaluaciÃÂ³n de riesgo
  const { data: evaluacionRiesgo, isLoading: loadingEvaluacion } = useQuery({
    ...
    retry: false, // No reintentar si no hay evaluaciÃÂ³n
    onError: () => {
      // Silenciar error si no hay evaluaciÃÂ³n
    }
  })

  // Cargar predicciÃÂ³n de impago si el prÃÂ©stamo estÃÂ¡ aprobado
  useEffect(() => {
  ...
  const cargarPrediccionImpago = async (prestamoId: number) => {
    ...
    try {
      ...
      // No mostrar error si no hay modelo activo, solo no mostrar la predicciÃÂ³n
      if (error?.response?.status !== 400) {
        console.error('Error cargando predicciÃÂ³n de impago:', error)
      }
    } finally {
      ...
      <AnimatePresence>
        ...
        >
          <div className="text-center bg-white p-8 rounded-lg">
            ...
            <p className="text-gray-600">Cargando detalles del prÃÂ©stamo...</p>
          </div>
        </motion.div>
      </AnimatePresence>
    )
      ...
      >
        ...
        >
          ...
          <div className="sticky top-0 bg-white border-b p-4 flex justify-between items-center z-10">
            <div>
              <h2 className="text-2xl font-bold">Detalles del PrÃÂ©stamo #{prestamoData.id}</h2>
          ...
          <div className="p-6">
            {activeTab === 'detalles' && (
              <div className="space-y-6">
                ...
                <Card>
                  ...
                  <CardContent className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="font-medium">{prestamoData.cedula ? 'Ã¢ÂÂ'}</p>
                    </div>
                    <div>
                      <label className="text-sm text-gray-600">Nombres</label>
                      <p className="font-medium">{prestamoData.nombres ? 'Ã¢ÂÂ'}</p>
                    </div>
                  </CardContent>
                </Card>

                {/* InformaciÃÂ³n del PrÃÂ©stamo */}
                <Card>
                  <CardHeader>
                    <CardTitle className="flex items-center gap-2">
                  ...
                  <CardContent className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div className="min-w-0">
                      <label className="text-sm text-gray-600">NÃÂºmero de Cuotas</label>
                      <p className="text-xl font-semibold">{prestamoData.numero_cuotas}</p>
                    </div>
                    <div className="min-w-0">
                      <label className="text-sm text-gray-600">Cuota por PerÃÂ­odo</label>
                    ...
                    <div className="min-w-0">
                      <label className="text-sm text-gray-600">Tasa de InterÃÂ©s</label>
                      <p className="font-medium">
                    ...
                    <div className="min-w-0">
                      <label className="text-sm text-gray-600">Fecha de AprobaciÃÂ³n</label>
                      <p className="font-medium">
                        {prestamoData.fecha_aprobacion ? formatDate(prestamoData.fecha_aprobacion) : '-'}
                      </p>
                      <p className="text-xs text-gray-500 mt-0.5">Base para vencimientos de la tabla de amortizaciÃÂ³n</p>
                    </div>
                  </CardContent>
                </Card>

                {/* EvaluaciÃÂ³n de Riesgo - Solo lectura */}
                {evaluacionRiesgo && (
                  <Card>
                    <CardHeader>
                      <CardTitle className="flex items-center gap-2">
                    ...
                    <CardContent className="space-y-4">
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <label className="text-sm text-gray-600">PuntuaciÃÂ³n Total</label>
                          <p className="text-2xl font-bold text-blue-600">
                            {evaluacionRiesgo.puntuacion_total?.toFixed(2) || '0.00'} / 100
                          </p>
                        </div>
                        <div>
                          <label className="text-sm text-gray-600">ClasificaciÃÂ³n de Riesgo</label>
                          <Badge className={
                        ...
                        <div>
                          <label className="text-sm text-gray-600">DecisiÃÂ³n Final</label>
                          <p className="font-medium">{evaluacionRiesgo.decision_final || 'N/A'}</p>
                        </div>
                        {evaluacionRiesgo.sugerencias?.plazo_maximo_sugerido && (
                          <div>
                            <label className="text-sm text-gray-600">Plazo MÃÂ¡ximo Sugerido</label>
                      ...
                      {evaluacionRiesgo.detalle_criterios && (
                        <div className="mt-4 pt-4 border-t">
                          ...
                          <div className="grid grid-cols-2 gap-2 text-sm">
                            ...
                            <div>Criterio 4 - Arraigo GeogrÃÂ¡fico: {(evaluacionRiesgo.detalle_criterios.arraigo_familiar || 0) + (evaluacionRiesgo.detalle_criterios.arraigo_laboral || 0)} pts</div>
                            <div>Criterio 5 - Perfil SociodemogrÃÂ¡fico: {evaluacionRiesgo.detalle_criterios.vivienda?.puntos || 0} pts</div>
                ...
                {/* PredicciÃÂ³n de Impago de Cuotas */}
                    ...
                    <CardContent>
                      {cargandoPrediccion ? (
                        <div className="text-center py-4">
                          <p className="text-sm text-gray-500">Cargando predicciÃÂ³n...</p>
                        </div>
                      ) : prediccionImpago ? (
                        <div className="space-y-4">
                          <div className="grid grid-cols-2 gap-4">
                            <div>
                              <div className="text-sm text-gray-600 mb-1">PredicciÃÂ³n</div>
                          ...
                          <div className="pt-4 border-t">
                            <div className="flex items-start gap-2">
                              ...
                              <div>
                                <div className="text-sm font-medium mb-1">RecomendaciÃÂ³n</div>
                        ...
                        <div className="text-center py-4">
                          <p className="text-sm text-gray-500">
                            No hay modelo activo para predecir impago. Entrena un modelo en ConfiguraciÃÂ³n ÃÂ¢Ã¢ÂÂ Ã¢ÂÂ AI ÃÂ¢Ã¢ÂÂ Ã¢ÂÂ ML Impago.
                          </p>
                        </div>
                      )}
                    </CardContent>
                ...
                <Card>
                  ...
                  <CardContent className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="text-sm text-gray-600">Modelo de VehÃÂ­culo</label>
