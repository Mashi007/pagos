...
export function TablaAmortizacionPrestamo({ prestamo }: TablaAmortizacionPrestamoProps) {
  ...
  // Cargar cuotas del prÃÂ©stamo (tabla de amortizaciÃÂ³n desde BD: pagos aplicados a cuotas)
  ...
  // FunciÃÂ³n para determinar el estado correcto basado en los datos
  const determinarEstadoReal = (cuota: Cuota): string => {
    ...
    // Respaldo si el API no envÃÂ­a estado ÃÂºtil (versiones antiguas)
  ...
  const getEstadoBadge = (estado: string) => {
    ...
    const badges = {
      ...
      VENCIDO: 'bg-orange-100 text-orange-800',  // [MORA] Nueva: vencida pero <= 90 dÃÂ­as
      MORA: 'bg-red-100 text-red-800',           // [MORA] Nueva: >= 91 dÃÂ­as
  ...
  const getEstadoLabel = (estado: string) => {
    ...
    const labels: Record<string, string> = {
      ...
      VENCIDO: 'Vencido (Ã¢ÂÂ¤90 d)',      // [MORA] Nueva
        ...
        <CardContent className="pt-6">
          <div className="flex items-center gap-3">
            ...
            <p className="text-sm text-yellow-800">
              La tabla de amortizaciÃÂ³n solo se puede ver para prÃÂ©stamos APROBADOS.
      ...
      <Card>
        <CardContent className="pt-6">
          <div className="text-center py-8">
            ...
            <p className="text-sm text-gray-600 mt-2">Cargando tabla de amortizaciÃÂ³n...</p>
          </div>
        </CardContent>
      </Card>
    )
      ...
      <Card className="border-red-200 bg-red-50">
        <CardContent className="pt-6">
          <div className="flex items-center gap-3">
            ...
            <p className="text-sm text-red-800">Error al cargar la tabla de amortizaciÃÂ³n</p>
          </div>
        </CardContent>
      </Card>
    )
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm text-yellow-800 mb-2">
                No hay tabla de amortizaciÃÂ³n generada para este prÃÂ©stamo.
              </p>
              {prestamo.fecha_base_calculo && (
                <Button size="sm" onClick={async () => {
                  try {
                    await prestamoService.generarAmortizacion(prestamo.id)
                    toast.success('Tabla de amortizaciÃÂ³n generada exitosamente')
                    // Refrescar datos
                    window.location.reload()
                  } catch (error: any) {
                    toast.error(error.response?.data?.detail || 'Error al generar amortizaciÃÂ³n')
                  }
                }}>
                  Generar Tabla
                </Button>
    ...
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <CardTitle>Tabla de AmortizaciÃÂ³n</CardTitle>
        ...
        <div className="flex gap-2">
          <Button
            ...
            onClick={() => refetch()}
            disabled={isFetching}
            title="Actualizar tabla con los ÃÂºltimos pagos aplicados"
          >
          ...
          <Table>
            ...
            <TableBody>
              {cuotasVisibles.map((cuota: Cuota) => {
                ...
                // Priorizar pago_monto_conciliado del backend (valores conciliados por prÃÂ©stamo), luego total_pagado, luego monto_cuota si estÃÂ¡ pagado
                ...
                return (
                  <TableRow key={cuota.id}>
                    ...
                    <TableCell>
                      ...
                      {/* ÃÂ°ÃÂ¸Ã¢ÂÂÃÂ DEBUG: Mostrar informaciÃÂ³n de depuraciÃÂ³n */}
                ...
                <TableRow>
                  <TableCell colSpan={9} className="text-center py-4">
                    <Button variant="ghost" onClick={() => setShowFullTable(true)}>
                      Ver {cuotas.length - 5} cuotas mÃÂ¡s...
                    </Button>
                  </TableCell>
                </TableRow>
              )}
