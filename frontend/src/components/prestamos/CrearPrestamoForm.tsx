...
export function CrearPrestamoForm({ prestamo, onClose, onSuccess, onAprobarManual }: CrearPrestamoFormProps) {
  ...
  const [formData, setFormData] = useState<Partial<PrestamoForm>>({
    ...
    fecha_requerimiento: prestamo?.fecha_requerimiento || '', // Fecha actual por defecto para nuevos prÃ©stamos
  ...
  // Errores de carga de configuraciÃ³n (sin bloquear renderizado; solo en desarrollo se puede loguear)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (numeroCuotas < 1 || numeroCuotas > 50 || !Number.isInteger(numeroCuotas)) {
      errors.push('El nÃºmero de cuotas debe ser un entero entre 1 y 50')
    }
    ...
    // Para nuevo prÃ©stamo: mostrar modal de confirmaciÃ³n accesible
    if (!prestamo) {
      setShowConfirmCreate(true)
      return
    }

    // EdiciÃ³n: proceder directamente
  ...
  const crearOActualizarPrestamo = async () => {
    try {
      ...
      if (prestamo) {
        ...
        // Backend exige cliente_id para crear; se toma del cliente buscado por cÃ©dula
    ...
    <AnimatePresence>
      ...
      >
        ...
        >
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-1">Seleccione el modelo. Si tiene precio configurado, se cargarÃ¡ en Valor Activo; si no, ingrÃ©selo manualmente.</p>
                </div>
                <div>
                  <label className="block text-sm font-medium mb-1">
              ...
              <CardContent className="space-y-4">
                ...
                {prestamo && prestamo.estado === 'APROBADO' && (
                  <div className="mt-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                    <h4 className="font-semibold text-blue-900 mb-2">Ã°Å¸ââ¦ Fecha de Desembolso (DÃ­a/Mes/AÃ±o)</h4>
          ...
          {/* Modal de confirmaciÃ³n antes de crear prÃ©stamo */}
          <Dialog open={showConfirmCreate} onOpenChange={setShowConfirmCreate}>
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Confirmar creaciÃ³n de prÃ©stamo</DialogTitle>
              </DialogHeader>
              <p className="text-sm text-gray-600">
                El prÃ©stamo se registrarÃ¡ con los datos ingresados. PodrÃ¡ editarlo mientras estÃ© en estado Borrador o En RevisiÃ³n.
                La tabla de amortizaciÃ³n se generarÃ¡ al aprobar el prÃ©stamo, usando Ãºnicamente la fecha de aprobaciÃ³n.
                Â¿Desea continuar?
              </p>
              <DialogFooter>
          ...
          {/* Modal de confirmaciÃ³n de rechazo */}
          <Dialog open={showRechazarDialog} onOpenChange={setShowRechazarDialog}>
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Confirmar rechazo del prÃ©stamo</DialogTitle>
              </DialogHeader>
              <p className="text-sm text-gray-600">
                Â¿EstÃ¡ seguro de rechazar este prÃ©stamo? El estado cambiarÃ¡ a Rechazado y no podrÃ¡ revertirse.
              </p>
              <DialogFooter>
