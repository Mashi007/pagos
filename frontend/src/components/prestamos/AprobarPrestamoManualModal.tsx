...
export function AprobarPrestamoManualModal({
  prestamo,
  onClose,
  onSuccess,
}: AprobarPrestamoManualModalProps) {
  ...
  const cuotaCalculada = useMemo(() => {
    ...
    const periodosPorAnio =
      mod === 'MENSUAL' ? 12 : mod === 'QUINCENAL' ? 24 : 52
  ...
  const handleAprobar = async () => {
    ...
    try {
      ...
      toast.success(
        'Préstamo aprobado correctamente. Tabla de amortización generada.'
      )
      ...
      toast.error(
        typeof msg === 'string' ? msg : 'Error al aprobar el préstamo'
      )
    } finally {
      setIsLoading(false)
    }
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              <div>
                ...
                <div className="relative">
                  ...
                  <Input
                    ...
                    onChange={e =>
                      setTotalFinanciamiento(parseNumInput(e.target.value))
                    }
                    className="pl-10"
                  />
                </div>
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={e =>
                    setNumeroCuotas(
                      Math.max(1, parseNumInput(e.target.value) || 0)
                    )
                  }
                />
              </div>
