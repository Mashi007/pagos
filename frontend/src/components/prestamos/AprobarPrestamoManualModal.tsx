...
const DECLARACION_FIJA =
  'Al aprobar, usted asegura que el cliente cumple las polÃÂ­ticas de RapiCredit y que su riesgo estÃÂ¡ dentro de parÃÂ¡metros normales.'

/** Parsea nÃÂºmero desde input (acepta coma o punto como decimal). */
...
export function AprobarPrestamoManualModal({ prestamo, onClose, onSuccess }: AprobarPrestamoManualModalProps) {
  ...
  // Calcular cuota por periodo automÃÂ¡ticamente: monto, nÃÂºmero de cuotas, modalidad y tasa
  ...
  const handleAprobar = async () => {
    ...
    try {
      ...
      toast.success('PrÃÂ©stamo aprobado correctamente. Tabla de amortizaciÃÂ³n generada.')
      ...
      toast.error(typeof msg === 'string' ? msg : 'Error al aprobar el prÃÂ©stamo')
    } finally {
      setIsLoading(false)
    }
  }
        ...
        <Card>
          <CardHeader className="bg-gradient-to-r from-green-600 to-green-700 text-white">
            <div className="flex justify-between items-center">
              <CardTitle className="flex items-center gap-2">
                ...
                Aprobar prÃÂ©stamo (riesgo manual) - #{prestamo.id}
              </CardTitle>
          ...
          <CardContent className="p-6 space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <label className="text-sm font-medium text-gray-700">NÃÂºmero de cuotas *</label>
                <Input
                  type="number"
                  min={1}
                  value={numeroCuotas || ''}
              ...
              <div>
                ...
                />
                <p className="text-xs text-gray-500 mt-1">Calculada segÃÂºn monto, nÃÂºmero de cuotas, modalidad y tasa de interÃÂ©s.</p>
              </div>
              <div>
                <label className="text-sm font-medium text-gray-700">Tasa de interÃÂ©s (%)</label>
                <Input
                  type="number"
                  min={0}
                  max={100}
                ...
                />
                <p className="text-xs text-gray-500 mt-1">Por defecto 0. Al cambiar, la cuota por periodo se actualiza automÃÂ¡ticamente.</p>
              </div>
              <div>
                <label className="text-sm font-medium text-gray-700">Fecha de aprobaciÃÂ³n *</label>
                <div className="relative">
                ...
                <p className="text-xs text-gray-500 mt-1">Base para la tabla de amortizaciÃÂ³n.</p>
              </div>
            </div>
