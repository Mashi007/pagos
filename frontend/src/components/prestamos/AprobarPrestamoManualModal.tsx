...
const DECLARACION_FIJA =
  'Al aprobar, usted asegura que el cliente cumple las polÃ­ticas de RapiCredit y que su riesgo estÃ¡ dentro de parÃ¡metros normales.'

/** Parsea nÃºmero desde input (acepta coma o punto como decimal). */
...
export function AprobarPrestamoManualModal({ prestamo, onClose, onSuccess }: AprobarPrestamoManualModalProps) {
  ...
  // Calcular cuota por periodo automÃ¡ticamente: monto, nÃºmero de cuotas, modalidad y tasa
  ...
  const handleAprobar = async () => {
    ...
    if (fechaRequerimientoStr && fechaAprobacion < fechaRequerimientoStr) {
      toast.error(`La fecha de aprobaciÃ³n debe ser igual o posterior a la fecha de requerimiento (${new Date(fechaRequerimientoStr).toLocaleDateString()})`)
      return
    }
    setIsLoading(true)
    try {
      ...
      toast.success('PrÃ©stamo aprobado correctamente. Tabla de amortizaciÃ³n generada.')
      ...
      toast.error(typeof msg === 'string' ? msg : 'Error al aprobar el prÃ©stamo')
    } finally {
      setIsLoading(false)
    }
  }
        ...
        <Card>
          <CardHeader className="bg-gradient-to-r from-green-600 to-green-700 text-white">
            <div className="flex justify-between items-center">
              <CardTitle className="flex items-center gap-2">
                ...
                Aprobar prÃ©stamo (riesgo manual) - #{prestamo.id}
              </CardTitle>
          ...
          <CardContent className="p-6 space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <label className="text-sm font-medium text-gray-700">NÃºmero de cuotas *</label>
                <Input
                  type="number"
                  min={1}
                  value={numeroCuotas || ''}
              ...
              <div>
                ...
                />
                <p className="text-xs text-gray-500 mt-1">Calculada segÃºn monto, nÃºmero de cuotas, modalidad y tasa de interÃ©s.</p>
              </div>
              <div>
                <label className="text-sm font-medium text-gray-700">Tasa de interÃ©s (%)</label>
                <Input
                  type="number"
                  min={0}
                  max={100}
                ...
                />
                <p className="text-xs text-gray-500 mt-1">Por defecto 0. Al cambiar, la cuota por periodo se actualiza automÃ¡ticamente.</p>
              </div>
              <div>
                <label className="text-sm font-medium text-gray-700">Fecha de aprobaciÃ³n *</label>
                <div className="relative">
                ...
                <p className="text-xs text-gray-500 mt-1">Base para la tabla de amortizaciÃ³n.</p>
              </div>
            </div>
