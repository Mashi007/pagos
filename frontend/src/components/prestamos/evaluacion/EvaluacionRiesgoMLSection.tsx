    ...
    <Card className="border-purple-200 bg-purple-50/50">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="bg-blue-50 p-3 rounded border border-blue-200">
          <div className="text-xs font-semibold text-blue-900 mb-2">â¹ï¸ ComparaciÃ³n de MÃ©todos:</div>
          ...
          {clasificacion_riesgo === 'A' && prediccion_ml.riesgo_level === 'Bajo' && (
            <div className="mt-2 text-xs text-green-700 bg-green-100 p-2 rounded">
              [OK] Ambos mÃ©todos coinciden: Cliente de bajo riesgo
            </div>
          )}
          {clasificacion_riesgo === 'E' && prediccion_ml.riesgo_level === 'Alto' && (
            <div className="mt-2 text-xs text-red-700 bg-red-100 p-2 rounded">
              â ï¸ Ambos mÃ©todos coinciden: Cliente de alto riesgo
            </div>
          )}
          {((clasificacion_riesgo === 'A' || clasificacion_riesgo === 'B') &&
            ...
            <div className="mt-2 text-xs text-amber-700 bg-amber-100 p-2 rounded">
              â ï¸ Discrepancia detectada: El ML sugiere mayor riesgo que el sistema de puntos
            </div>
          )}
          {((clasificacion_riesgo === 'D' || clasificacion_riesgo === 'E') &&
            prediccion_ml.riesgo_level === 'Bajo') && (
            <div className="mt-2 text-xs text-amber-700 bg-amber-100 p-2 rounded">
              â ï¸ Discrepancia detectada: El ML sugiere menor riesgo que el sistema de puntos
            </div>
          )}
        </div>
      </CardContent>
