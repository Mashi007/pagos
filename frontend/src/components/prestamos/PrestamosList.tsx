...
export function PrestamosList() {
  ...
  const clienteIdFromUrl = clienteIdParam
    ? parseInt(clienteIdParam, 10)
    : undefined

  const clienteIdValid =
    clienteIdFromUrl && !Number.isNaN(clienteIdFromUrl)
      ? clienteIdFromUrl
      : undefined
  ...
  useEffect(() => {
    const reqRev =
      searchParams.get('requiere_revision') === 'true' ? true : undefined
      ...
      >
        <DialogContent>
          ...
          <p className="text-sm text-gray-600">
            ¿Está seguro de eliminar este préstamo? Esta acción no se puede
            deshacer.
          </p>

          <DialogFooter>
            <Button variant="outline" onClick={() => setDeletePrestamoId(null)}>
              Cancelar
            </Button>
            ...
            <Button
              ...
              disabled={deletePrestamo.isPending}
            >
                ...
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={value =>
                      setFilters({
                        ...filters,
                        analista: value === 'ALL' ? undefined : value,
                      })
                    }
                  >
                    <SelectTrigger>
                ...
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={value =>
                      setFilters({
                        ...filters,
                        modelo: value === 'ALL' ? undefined : value,
                      })
                    }
                  >
                    <SelectTrigger>
            ...
            <>
              ...
              {process.env.NODE_ENV === 'development' && (
                <div className="mb-4 rounded bg-gray-100 p-2 text-xs">
                  ...
                  data.data existe: {data?.data ? 'Sí' : 'No'}, es array:{' '}
                  {Array.isArray(data?.data) ? 'Sí' : 'No'}, longitud:{' '}
                  {Array.isArray(data?.data) ? data.data.length : 'N/A'}, total:{' '}
                  {data?.total || 0}
                </div>
              )}
                    ...
                    {data.total > 0 ? (
                      <>
                        <p className="mb-2 text-lg font-semibold text-red-600">
                          Problema detectado: El sistema reporta {data.total}{' '}
                          préstamos, pero no se pudieron mostrar.
                        </p>

                        <p className="mb-4 text-sm">
                          Esto puede deberse a un problema en la respuesta del
                          servidor o en el formato de los datos.
                        </p>
                      ...
                      <p>
                        No se encontraron préstamos con los filtros
                        seleccionados.
                      </p>
                    )}
                  </div>
                    ...
                    <TableBody>
                      {data.data.map((prestamo: any) => (
                        <TableRow key={prestamo.id}>
                          ...
                          <TableCell>
                            {prestamo.cedula ?? prestamo.cedula_cliente ?? '-'}
                          </TableCell>
                          ...
                          <TableCell>
                            {prestamo.modalidad_pago === 'MENSUAL'
                              ? 'Mensual'
                              : prestamo.modalidad_pago === 'QUINCENAL'
                                ? 'Quincenal'
                                : prestamo.modalidad_pago === 'SEMANAL'
                                  ? 'Semanal'
                                  : '-'}
                          </TableCell>

                          <TableCell>
                            {prestamo.numero_cuotas != null
                          ...
                          <TableCell>
                            {(() => {
                              ...
                              return (
                                <div className="max-w-[14rem] text-left text-sm leading-snug text-slate-800">
                              ...
                              {canViewEvaluacionRiesgo() &&
                                (prestamo.estado === 'DRAFT' ||
                                  ...
                                  <Button
                                    ...
                                    size="sm"
                                    onClick={() =>
                                      handleAprobarPrestamo(prestamo)
                                    }
                ...
                <div className="mt-6 flex flex-col gap-4 border-t pt-4">
                  <div className="flex flex-wrap items-center justify-between gap-4">
                    <div className="text-sm text-gray-600">
                      ...
                      de <span className="font-semibold">{data.total}</span>{' '}
                      préstamos
                    </div>
