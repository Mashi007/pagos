...
export function PrestamosList() {
  ...
  // Vista "Revisar prÃ©stamos" (enviados desde carga masiva), misma lÃ³gica que Clientes/Pagos
  const showRevisarPrestamos = searchParams.get('revisar') === '1'
  // Leer requiere_revision y cliente_id de los parÃ¡metros de URL
  ...
  const handleExportRevisarExcel = async () => {
    ...
    try {
      ...
      await createAndDownloadExcel(allItems, 'Revisar PrÃ©stamos', nombre)
      if (allIds.length > 0) {
        await prestamoService.eliminarPorDescarga(allIds)
        queryClient.invalidateQueries({ queryKey: ['prestamos-con-errores'] })
        refetchRevisar()
      }
      toast.success(`${allItems.length} prÃ©stamo(s) exportados y eliminados de la lista`)
    } catch (err) {
      console.error('Error exportando Revisar PrÃ©stamos:', err)
  ...
  return (
    <div className="space-y-6">
      <Dialog open={deletePrestamoId !== null} onOpenChange={(open) => !open && setDeletePrestamoId(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Eliminar prÃ©stamo</DialogTitle>
          </DialogHeader>
          <p className="text-sm text-gray-600">Â¿EstÃ¡ seguro de eliminar este prÃ©stamo? Esta acciÃ³n no se puede deshacer.</p>
          <DialogFooter>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <div className="relative group">
            ...
            {/* Puente invisible para que el hover no se pierda al bajar el cursor al menÃº */}
      ...
      {/* SecciÃ³n Revisar prÃ©stamos (enviados desde carga masiva) */}
      {showRevisarPrestamos && (
        <Card className="border-amber-200 bg-amber-50/50">
          <CardContent className="p-4">
            <div className="flex flex-wrap items-center justify-between gap-4 mb-4">
              <div>
                ...
                <p className="text-sm text-gray-600 mt-1">
                  PrÃ©stamos enviados desde la carga masiva para revisiÃ³n manual. Descarga el Excel para corregir y reimportar.
                </p>
              </div>
              <div className="flex items-center gap-2">
                ...
                <Button
                  ...
                  onClick={handleExportRevisarExcel}
                  disabled={isExportingRevisar || !revisarData?.items?.length}
                  title="Descargar todos los prÃ©stamos a revisar en Excel"
                >
                  {isExportingRevisar ? (
            ...
            {revisarLoading ? (
              ...
              <p className="text-gray-500 text-center py-6">No hay prÃ©stamos pendientes de revisiÃ³n.</p>
            ) : (
              <>
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow>
                        ...
                        <TableHead className="w-28">AcciÃ³n</TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                ...
                {revisarData.total > perPageRevisar && (
                  <div className="flex items-center justify-between mt-3 text-sm text-gray-600">
                    <span>
                      {((pageRevisar - 1) * perPageRevisar) + 1}â{Math.min(pageRevisar * perPageRevisar, revisarData.total)} de {revisarData.total}
                    </span>
          ...
          {filters.cliente_id != null && (
            <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg flex items-center justify-between">
              <span className="text-sm text-blue-800">
                Mostrando prÃ©stamos del cliente #{filters.cliente_id}
              </span>
              <Button
                variant="ghost"
                size="sm"
        ...
        <CardContent>
          ...
          {!isLoading && !error && data && (
            <>
                    ...
                    {data.total > 0 ? (
                      <>
                        <p className="text-lg font-semibold text-red-600 mb-2">
                          Problema detectado: El sistema reporta {data.total} prÃ©stamos, pero no se pudieron mostrar.
                        </p>
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow>
                        ...
                        <TableHead>Estado</TableHead>
                        <TableHead title="Fecha de aprobaciÃ³n">Fecha</TableHead>
                      ...
                      <TableRow key={prestamo.id}>
                        ...
                        <TableCell>
                          {prestamo.modalidad_pago === 'MENSUAL' ? 'Mensual' :
                           ...
                           prestamo.modalidad_pago ? 'â'}
                        </TableCell>
                        <TableCell>{prestamo.numero_cuotas != null ? prestamo.numero_cuotas : 'â'}</TableCell>
                        <TableCell>
                        ...
                        <TableCell>
                          <div className="flex items-center gap-1 text-sm text-gray-600" title="Fecha de aprobaciÃ³n">
                            <Calendar className="h-4 w-4" />
                            {prestamo.fecha_aprobacion ? formatDate(prestamo.fecha_aprobacion) : 'â'}
                          </div>
                        </TableCell>
                        <TableCell className="text-right">
                          <div className="flex justify-end gap-2">
                            {/* ICONO REVISIÃN MANUAL: â  no revisado | ? en revisiÃ³n | ausencia = ya revisado */}
                             ...
                             prestamo.revision_manual_estado !== 'revisado' && (
                              prestamo.revision_manual_estado === 'revisando' ? (
                                <Button
                                  ...
                                  title="EstÃ¡ siendo revisado - Click para continuar"
                            ...
                            {/* BotÃ³n Ver Detalles - Visible cuando APROBADO o tiene fecha_aprobacion */}
                            {(prestamo.estado === 'APROBADO' || prestamo.fecha_aprobacion) && (
                              <Button
                                variant="ghost"
                                size="sm"
                            ...
                            {/* BotÃ³n Editar - Solo si tiene permisos Y estÃ¡ APROBADO con fecha_aprobacion */}
                            ...
                            {/* Aprobar prÃ©stamo (riesgo manual) - Reemplaza Evaluar riesgo + Aprobar crÃ©dito + Asignar fecha */}
                            {canViewEvaluacionRiesgo() && (prestamo.estado === 'DRAFT' || prestamo.estado === 'EN_REVISION') && (
                              <Button
                                variant="ghost"
                                size="sm"
                                onClick={() => handleAprobarPrestamo(prestamo)}
                                title="Aprobar prÃ©stamo (fecha, datos editables y declaraciÃ³n)"
              ...
              {/* PaginaciÃ³n - siempre visible cuando hay datos */}
              {data && data.total > 0 && (
                <div className="flex flex-col gap-4 mt-6 pt-4 border-t">
                  <div className="flex flex-wrap items-center justify-between gap-4">
                    <div className="text-sm text-gray-600">
                      Mostrando <span className="font-semibold">{(page - 1) * perPage + 1}</span> - <span className="font-semibold">{Math.min(page * perPage, data.total)}</span> de <span className="font-semibold">{data.total}</span> prÃ©stamos
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="text-sm text-gray-600">Filas por pÃ¡gina:</span>
