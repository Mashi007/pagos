...
export function EvaluacionRiesgoForm({
  ...
  const secciones = [
    ...
    {
      id: 'criterio5',
      label: 'Criterio 5: Perfil Sociodemográfico',
      puntos: '17',
    },
    ...
    {
      id: 'criterio7',
      label: 'Criterio 7: Capacidad de Maniobra',
      puntos: '5',
    },

    ...(resultado
      ? [{ id: 'resultado', label: 'Resultado de Evaluación', puntos: '' }]
      : []),
  ]
  ...
  return (
    ...
    >
      <motion.div
        ...
        animate={{ scale: 1, y: 0 }}
        exit={{ scale: 0.95 }}
        className="max-h-[95vh] w-full max-w-6xl overflow-hidden rounded-lg bg-white shadow-xl"
        onClick={e => e.stopPropagation()}
      >
        <EvaluacionRiesgoHeader prestamo={prestamo} onClose={onClose} />

        <div className="overflow-x-auto border-b bg-gray-50 px-4 py-2">
          <div className="flex gap-2">
            {secciones.map(seccion => (
              <button
                ...
                className={`flex items-center gap-2 whitespace-nowrap rounded-full px-3 py-1.5 text-sm font-medium transition-colors ${
                  showSection === seccion.id
                    ? 'bg-blue-600 text-white'
                    : 'bg-white text-gray-600 hover:bg-gray-100'
                }`}
              >
                {seccionCompleta(seccion.id) &&
                  seccion.id.startsWith('criterio') && (
                ...
                {seccion.label.split(':')[0]}{' '}
                {seccion.puntos ? `(${seccion.puntos} pts)` : ''}
              </button>
            ))}
          </div>
          ...
          {!todasSeccionesCompletas && (
            <div className="mt-2 rounded border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-700">
              <strong>
                ⚠️ Debe completar todos los 7 criterios antes de evaluar.
              </strong>{' '}
              Faltan:{' '}
              {[
        ...
        >
          {(showSection === 'situacion' ||
            showSection.startsWith('criterio')) && (
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-3 gap-4">
                  ...
                  <div>
                    ...
                    <Badge
                      variant={
                        resultado.decision_final?.includes('RECHAZADO')
                          ? 'destructive'
                          : 'default'
                      }
                      className="text-lg"
                  ...
                  <div className="rounded border border-blue-200 bg-blue-50 p-4">
                    <div className="mb-3 flex items-center justify-between">
                      ...
                      <Button
                        ...
                        size="sm"
                        onClick={() =>
                          setShowFormularioAprobacion(!showFormularioAprobacion)
                        }
                      >
                        {showFormularioAprobacion
                          ? 'Ocultar Edición'
                    ...
                    {!showFormularioAprobacion ? (
                      <div className="grid grid-cols-3 gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-lg font-bold text-blue-900">
                            {resultado.sugerencias.plazo_maximo_sugerido || 36}{' '}
                            meses
                          </p>
                        </div>
                        ...
                        <div>
                          ...
                          <p className="text-lg font-bold text-blue-900">
                            {resultado.sugerencias.enganche_minimo_sugerido ||
                              15.0}
                            %
                          </p>
                        </div>
                        ...
                        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                          ...
                          <div className="space-y-2">
                            ...
                            <Input
                              ...
                              placeholder={
                                resultado.sugerencias.plazo_maximo_sugerido?.toString() ||
                                '36'
                              }
                            />
                            ...
                            <p className="text-xs text-gray-500">
                              Sugerido:{' '}
                              {resultado.sugerencias.plazo_maximo_sugerido ||
                                36}{' '}
                              meses
                            </p>
                          </div>
                ...
                <div className="rounded border bg-white p-3">
                  <h5 className="mb-2 font-semibold">Detalle de Criterios:</h5>

                  <ul className="list-inside list-disc space-y-1 text-sm">
                    <li>
                      <strong>1. Capacidad de Pago:</strong>{' '}
                      {resultado.detalle_criterios?.ratio_endeudamiento?.puntos?.toFixed(
                        1
                      )}{' '}
                      pts (Endeudamiento) +{' '}
                      {resultado.detalle_criterios?.ratio_cobertura?.puntos?.toFixed(
                        1
                      )}{' '}
                      pts (Cobertura) (Total:{' '}
                      {(
                        (resultado.detalle_criterios?.ratio_endeudamiento
                          ?.puntos ?? 0) +
                        (resultado.detalle_criterios?.ratio_cobertura?.puntos ??
                          0)
                      ).toFixed(1)}
                      /29 pts)
                    </li>
                    ...
                    <li>
                      ...
                      {resultado.detalle_criterios?.antiguedad_trabajo?.puntos?.toFixed(
                        1
                      )}{' '}
                      pts (Antigüedad) +{' '}
                      {resultado.detalle_criterios?.tipo_empleo?.puntos?.toFixed(
                        1
                      )}{' '}
                      pts (Tipo Empleo) +{' '}
                      {resultado.detalle_criterios?.sector_economico?.puntos?.toFixed(
                        1
                      )}{' '}
                      pts (Sector) (Total:{' '}
                      {(
                        (resultado.detalle_criterios?.antiguedad_trabajo
                          ?.puntos ?? 0) +
                        (resultado.detalle_criterios?.tipo_empleo?.puntos ??
                          0) +
                        (resultado.detalle_criterios?.sector_economico
                          ?.puntos ?? 0)
                      ).toFixed(1)}
                      /23 pts)
                    </li>
                    ...
                    <li>
                      ...
                      {resultado.detalle_criterios?.referencias?.puntos?.toFixed(
                        1
                      )}
                      /9 pts (Ref1:{' '}
                      {
                        resultado.detalle_criterios?.referencias
                          ?.referencia1_calificacion
                      }
                      , Ref2:{' '}
                      {
                        resultado.detalle_criterios?.referencias
                          ?.referencia2_calificacion
                      }
                      , Ref3:{' '}
                      {
                        resultado.detalle_criterios?.referencias
                          ?.referencia3_calificacion
                      }
                      )
                    </li>
                    ...
                    <li>
                      ...
                      {resultado.detalle_criterios?.arraigo_vivienda?.toFixed(
                        1
                      )}{' '}
                      pts (Vivienda) +{' '}
                      {resultado.detalle_criterios?.arraigo_laboral?.toFixed(1)}{' '}
                      pts (Laboral) (Total:{' '}
                      {(
                    ...
                    <li>
                      ...
                      {resultado.detalle_criterios?.vivienda?.puntos?.toFixed(
                        1
                      )}{' '}
                      pts (Vivienda) +{' '}
                      {resultado.detalle_criterios?.estado_civil?.puntos?.toFixed(
                        1
                      )}{' '}
                      pts (Estado Civil) +{' '}
                      {resultado.detalle_criterios?.hijos?.puntos?.toFixed(1)}{' '}
                      pts (Hijos) (Total:{' '}
                      {(
                        (resultado.detalle_criterios?.vivienda?.puntos ?? 0) +
                        (resultado.detalle_criterios?.estado_civil?.puntos ??
                          0) +
                    ...
                    <li>
                      ...
                      {resultado.detalle_criterios?.edad?.puntos?.toFixed(1)}/10
                      pts ({resultado.detalle_criterios?.edad?.cliente} años)
                    </li>

                    <li>
                      <strong>7. Capacidad de Maniobra:</strong>{' '}
                      {resultado.detalle_criterios?.capacidad_maniobra?.puntos?.toFixed(
                        1
                      )}
                      /5 pts (
                      {resultado.detalle_criterios?.capacidad_maniobra?.porcentaje_residual?.toFixed(
                        2
                      )}
                      % residual)
                    </li>
