...
interface EvaluacionFormData {
  ...
  // Criterio 4: Arraigo GeogrÃ¡fico (7 puntos - sin tipo_vivienda)
  familia_cercana: boolean
  familia_pais: boolean
  minutos_trabajo: number

  // Criterio 5: Perfil SociodemogrÃ¡fico
...
export function EvaluacionRiesgoForm({ prestamo, onClose, onSuccess }: EvaluacionRiesgoFormProps) {
  ...
  // Estado para condiciones editables de aprobaciÃ³n
  ...
  useEffect(() => {
    const calcularEdad = async () => {
      try {
        // Obtener informaciÃ³n del cliente por su cÃ©dula usando el servicio
  ...
  // Cargar resumen por cÃ©dula para validar si existen prÃ©stamos y cuotas en mora
  useEffect(() => {
  ...
  // Actualizar condiciones de aprobaciÃ³n cuando hay resultado de evaluaciÃ³n
  useEffect(() => {
  ...
  const [formData, setFormData] = useState<EvaluacionFormData>({
    ...
    // Criterio 5: Perfil SociodemogrÃ¡fico (17 puntos)
  ...
  // Criterio 4: Arraigo GeogrÃ¡fico (7 puntos)
  ...
  // âœ… ValidaciÃ³n completa: todos los 7 criterios deben estar completos
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validar que todos los criterios estÃ©n completos
    if (!todasSeccionesCompletas) {
      ...
      if (!seccion4Completa) criteriosFaltantes.push('Criterio 4: Arraigo GeogrÃ¡fico')
      if (!seccion5Completa) criteriosFaltantes.push('Criterio 5: Perfil SociodemogrÃ¡fico')
    ...
    const confirmacion = window.confirm(
      ...
      'âœ“ La informaciÃ³n de empleo es correcta\n' +
      'âœ“ Los valores de anticipo son reales\n\n' +
      'Â¿Desea proceder con la evaluaciÃ³n?'
    )

    if (!confirmacion) {
      toast('EvaluaciÃ³n cancelada')
      return
    }
    ...
    referencia3_calificacion: formData.referencia3_calificacion,
      ...
      // IMPORTANTE: onSuccess actualiza el dashboard y cambia el icono de calculadora a aprobaciÃ³n
      // Esperar 2 segundos para que el usuario vea el resultado antes de cerrar
      setTimeout(() => {
        onSuccess() // Esto actualizarÃ¡ el dashboard y cambiarÃ¡ el icono
        onClose() // Cierra el formulario
        toast.info('ðŸ”„ Ahora puede usar el nuevo icono en el dashboard para proceder con la AprobaciÃ³n (Fase 2)')
      }, 2000)
    } catch (error: any) {
  ...
  const secciones = [
    { id: 'situacion', label: 'SituaciÃ³n del Cliente', puntos: '' },
    ...
    { id: 'criterio4', label: 'Criterio 4: Arraigo GeogrÃ¡fico', puntos: '7' },
    { id: 'criterio5', label: 'Criterio 5: Perfil SociodemogrÃ¡fico', puntos: '17' },
    { id: 'criterio6', label: 'Criterio 6: Edad', puntos: '10' },
    { id: 'criterio7', label: 'Criterio 7: Capacidad de Maniobra', puntos: '5' },
    ...(resultado ? [{ id: 'resultado', label: 'Resultado de EvaluaciÃ³n', puntos: '' }] : []),
  ]
      ...
      >
        <div className="sticky top-0 bg-gradient-to-r from-blue-600 to-purple-600 text-white p-4 flex justify-between items-center z-10">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold">EvaluaciÃ³n de Riesgo</h2>
              <p className="text-sm opacity-90">PrÃ©stamo #{prestamo.id} - Sistema 100 Puntos</p>
          </div>
          </div>
        ...
        <div className="bg-gray-50 px-4 py-2 border-b overflow-x-auto">
          <div className="flex gap-2">
            {secciones.map((seccion) => {
              // Determinar si la secciÃ³n estÃ¡ completa
        ...
        <form onSubmit={handleSubmit} className="p-6 overflow-y-auto max-h-[60vh]">
          {/* SITUACIÃ“N DEL CLIENTE (Resumen de prÃ©stamos) */}
              ...
              <CardContent className="pt-4 space-y-3">
                ...
                {resumenPrestamos && !resumenPrestamos.tiene_prestamos && (
                  <div className="p-3 rounded bg-green-50 border border-green-200 text-green-800">
                    No tiene prÃ©stamos vigentes. Puede continuar con el anÃ¡lisis.
                  </div>
                )}
                {resumenPrestamos && resumenPrestamos.tiene_prestamos && (
                  <div className="space-y-3">
                    <div className="p-3 rounded bg-yellow-50 border border-yellow-200 text-yellow-900">
                      PrÃ©stamos vigentes: {resumenPrestamos.total_prestamos}. Saldo pendiente total: ${Number(resumenPrestamos.total_saldo_pendiente || 0).toFixed(2)}.
                    </div>
                    ...
                    {bloqueadoPorMora ? (
                      <div className="p-3 rounded bg-red-50 border border-red-200 text-red-800">
                        Hay cuotas en mora. No se puede continuar hasta regularizar la situaciÃ³n.
                      </div>
                    ) : (
                      <div className="p-3 rounded bg-blue-50 border border-blue-200 text-blue-800">
                        No hay cuotas en mora. Puede continuar con el anÃ¡lisis.
                      </div>
                    )}
                  </div>
                )}
                    ...
                    <PopoverTrigger asChild>
                      ...
                      <PopoverContent className="w-80">
                        ...
                        <ul className="text-xs space-y-1">
                          <li>â€¢ &lt; 25% â†’ 14 puntos (Excelente)</li>
                          <li>â€¢ 25-35% â†’ 11 puntos (Bueno)</li>
                          <li>â€¢ 35-50% â†’ 6 puntos (Regular)</li>
                          <li>â€¢ &gt; 50% â†’ 2 puntos (Malo)</li>
                          </ul>
                        <p className="text-sm font-semibold mb-2 mt-4">Ratio de Cobertura:</p>
                        <ul className="text-xs space-y-1">
                          <li>â€¢ &gt; 2.5x â†’ 15 puntos (Excelente)</li>
                          <li>â€¢ 2.0-2.5x â†’ 12 puntos (Bueno)</li>
                          <li>â€¢ 1.5-2.0x â†’ 6 puntos (Regular)</li>
                          <li>â€¢ &lt; 1.5x â†’ 0 puntos (RECHAZO)</li>
                        </ul>
                    </PopoverContent>
                  </Popover>
                </div>
              ...
              <CardContent className="pt-4 space-y-4">
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    Escala: &gt; 24 meses â†’ 9 pts, 12-24 â†’ 7 pts, 6-12 â†’ 4 pts, &lt; 6 â†’ 0 pts
                  </p>
                        </div>
                <div>
                  <label className="block text-sm font-medium mb-1">
                  ...
                  >
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="informal_estable">Informal Estable (&gt;1 aÃ±o) (6 pts)</SelectItem>
                ...
                <div>
                  ...
                  >
                    ...
                    <SelectContent>
                      <SelectItem value="gobierno_publico">Gobierno/PÃºblico (6 pts)</SelectItem>
                      ...
                      <SelectItem value="construccion_manufactura">ConstrucciÃ³n/Manufactura (3 pts)</SelectItem>
            ...
            <CardHeader className="bg-purple-50">
                ...
                <div className="border rounded-lg p-4 bg-purple-50">
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <Input
                        type="text"
                        placeholder="Nombre, relaciÃ³n, etc."
              ...
              >
                ...
                <SelectContent>
                          ...
                          <SelectItem value="0">0 - No contestÃ³</SelectItem>
                </SelectContent>
              </Select>
                    </div>
                  </div>
          ...
          {showSection === 'criterio4' && (
            <Card className="border-green-200">
              ...
              <Input
                ...
                required
              />
                  <p className="text-xs text-gray-500 mt-1">&lt; 30 min â†’ 3 pts, 30-60 min â†’ 2 pts, &gt; 60 min â†’ 0 pts</p>
                </div>
          ...
          {showSection === 'criterio5' && (
            <Card className="border-indigo-200">
              ...
              <CardContent className="pt-4 space-y-4">
                <div>
                  <label className="block text-sm font-medium mb-1">
                    SituaciÃ³n de Vivienda Detallada *
                  </label>
              <Select
              ...
              >
                ...
                <div>
                  ...
                  <Input
                    ...
                    />
                    <p className="text-xs text-gray-500 mt-1">&gt; 5 personas â†’ -0.5 pts</p>
                  </div>
                </div>
          ...
          {showSection === 'criterio7' && (
            ...
            <CardContent className="pt-4 space-y-4">
              <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
                <h4 className="font-semibold text-blue-900 mb-2">ðŸ“Š Â¿QuÃ© es la Capacidad de Maniobra?</h4>
                <p className="text-sm text-blue-800 mb-3">
                  Es el saldo que le queda al cliente despuÃ©s de pagar todos sus gastos, deudas y la cuota del prÃ©stamo propuesto.
                </p>
                <div className="bg-white p-3 rounded border border-blue-100">
                  <p className="text-xs font-mono text-gray-700">
                    <strong>FÃ³rmula:</strong><br />
                  ...
                  <div className="bg-green-50 p-4 rounded-lg border border-green-200">
                    ...
                    <div className="bg-white p-3 rounded border border-green-100 mb-3">
                      <div className="grid grid-cols-2 gap-2 text-sm mb-2">
                        ...
                        <div className="text-gray-600">- Cuota PrÃ©stamo:</div>
                    ...
                    <div className="grid grid-cols-2 gap-3 mb-3">
                      ...
                      <div className="bg-white p-3 rounded border">
                        <div className="text-xs text-gray-600">CategorÃ­a</div>
                    ...
                    <div className="bg-white p-3 rounded border text-center">
                      <div className="text-xs text-gray-600 mb-1">PUNTOS QUE OBTENDRÃA:</div>
                    ...
                    {saldoResidual < 0 && (
                      <div className="bg-red-100 p-2 rounded border border-red-300 mt-3">
                        <p className="text-xs text-red-800 font-semibold">
                          âš ï¸ ALERTA: El cliente tendrÃ­a un dÃ©ficit de ${Math.abs(saldoResidual).toFixed(2)} USD mensual
                        </p>
                      </div>
                    )}
                  </div>
              ...
              <div className="bg-amber-50 p-3 rounded border border-amber-200">
                <p className="text-xs text-amber-800 font-semibold mb-2">âš¡ ESTE CRITERIO SE CALCULA AUTOMÃTICAMENTE</p>
                <p className="text-xs text-amber-700">
                  La previsualizaciÃ³n arriba usa los datos que ingresaste. Cuando hagas click en "Evaluar Riesgo", el sistema calcularÃ¡ la capacidad de maniobra usando estos valores.
                </p>
              </div>

              <div className="bg-red-50 p-3 rounded border border-red-200">
                <p className="text-xs text-red-700">
                  <strong>Escala de PuntuaciÃ³n (3 bandas):</strong><br />
                  âœ… â‰¥15% del ingreso â†’ 5 pts (Holgado) - Margen suficiente para imprevistos<br />
                  âš ï¸ 5%-14.9% del ingreso â†’ 3 pts (Ajustado) - Margen mÃ­nimo<br />
                  âŒ &lt;5% o dÃ©ficit â†’ 0 pts (Insuficiente) - Sin margen, alto riesgo
                </p>
              </div>
            </CardContent>
          </Card>
          ...
          {/* RESULTADO - Nueva PestaÃ±a */}
          {showSection === 'resultado' && resultado && (
            <Card className="border-green-300 bg-green-50">
                <CardHeader>
                <CardTitle className="text-green-700">Resultado de la EvaluaciÃ³n</CardTitle>
                </CardHeader>
              <CardContent className="space-y-4">
                ...
                {/* Mostrar y editar condiciones de aprobaciÃ³n */}
                {resultado.sugerencias && (
                  <div className="bg-blue-50 p-4 rounded border border-blue-200">
                    <div className="flex justify-between items-center mb-3">
                      <h5 className="font-semibold text-blue-900">ðŸ“‹ Condiciones para AprobaciÃ³n:</h5>
                      <Button
                        type="button"
                        variant="outline"
                        size="sm"
                      ...
                      >
                        {showFormularioAprobacion ? 'Ocultar EdiciÃ³n' : 'Editar Condiciones'}
                      </Button>
                    </div>
                    ...
                    {!showFormularioAprobacion ? (
                      ...
                      <div className="grid grid-cols-3 gap-4">
                        <div>
                          <label className="text-xs text-blue-700">Tasa de InterÃ©s Sugerida:</label>
                          <p className="text-lg font-bold text-blue-900">{resultado.sugerencias.tasa_interes_sugerida || 8.0}%</p>
                        </div>
                        <div>
                          <label className="text-xs text-blue-700">Plazo MÃ¡ximo Sugerido:</label>
                          <p className="text-lg font-bold text-blue-900">{resultado.sugerencias.plazo_maximo_sugerido || 36} meses</p>
                        </div>
                        <div>
                          <label className="text-xs text-blue-700">Enganche MÃ­nimo:</label>
                      ...
                      <div className="bg-white p-4 rounded border border-blue-300 space-y-4">
                        ...
                        <div className="space-y-2">
                          ...
                          <textarea
                            ...
                            rows={3}
                            placeholder="Aprobado despuÃ©s de evaluaciÃ³n de riesgo..."
                          />
                        </div>
                      </div>
                    )}
                    ...
                    <CardContent className="space-y-4">
                      ...
                      {resultado.prediccion_ml.recommendation && (
                        <div className="bg-white p-3 rounded border border-purple-200">
                          <div className="text-sm font-medium text-gray-700 mb-1">RecomendaciÃ³n ML:</div>
                      ...
                      {/* ComparaciÃ³n con evaluaciÃ³n tradicional */}
                      <div className="bg-blue-50 p-3 rounded border border-blue-200">
                        <div className="text-xs font-semibold text-blue-900 mb-2">ðŸ“Š ComparaciÃ³n de MÃ©todos:</div>
                        ...
                        {resultado.clasificacion_riesgo === 'A' && resultado.prediccion_ml.riesgo_level === 'Bajo' && (
                          <div className="mt-2 text-xs text-green-700 bg-green-100 p-2 rounded">
                            âœ… Ambos mÃ©todos coinciden: Cliente de bajo riesgo
                          </div>
                        )}
                        {resultado.clasificacion_riesgo === 'E' && resultado.prediccion_ml.riesgo_level === 'Alto' && (
                          <div className="mt-2 text-xs text-red-700 bg-red-100 p-2 rounded">
                            âš ï¸ Ambos mÃ©todos coinciden: Cliente de alto riesgo
                          </div>
                        )}
                ...
                <div className="bg-white p-3 rounded border">
                  ...
                  <ul className="list-disc list-inside text-sm space-y-1">
                    ...
                    <li>
                      ...
                      {resultado.detalle_criterios?.antiguedad_trabajo?.puntos?.toFixed(1)} pts (AntigÃ¼edad) +{' '}
                    ...
                    <li>
                      <strong>4. Arraigo GeogrÃ¡fico:</strong>{' '}
                    ...
                    <li>
                      <strong>5. Perfil SociodemogrÃ¡fico:</strong>{' '}
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <h3 className="text-base font-bold text-blue-900 mb-2">
                      âœ… FASE 1 COMPLETADA: EvaluaciÃ³n de Riesgo
                    </h3>
                    <div className="space-y-2 text-sm text-blue-800">
                      <p>
                        â€¢ <strong>Estado actualizado:</strong> El prÃ©stamo ahora estÃ¡ marcado como <span className="font-bold text-blue-600">EVALUADO</span>
                      </p>
                      <p>
                        â€¢ <strong>PuntuaciÃ³n obtenida:</strong> {resultado.puntuacion_total?.toFixed(2) || 'N/A'}/100 puntos
                      </p>
                      <p>
                        â€¢ <strong>ClasificaciÃ³n de riesgo:</strong> <span className="font-semibold">{resultado.clasificacion_riesgo || 'N/A'}</span>
                      </p>
                      <div className="mt-3 pt-3 border-t border-blue-300">
                        <p className="font-semibold text-purple-700 mb-1">ðŸ”„ SIGUIENTE PASO: Fase 2 - AprobaciÃ³n</p>
                        <p className="text-xs">
                          El icono de <strong>calculadora (Calculator)</strong> en el dashboard desaparecerÃ¡ y serÃ¡ reemplazado
                          por el icono de <strong>verde (CheckCircle2)</strong> para "Aprobar CrÃ©dito".
