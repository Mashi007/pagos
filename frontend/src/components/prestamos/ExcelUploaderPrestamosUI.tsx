/**
 * UI para carga masiva de prÃÂ©stamos desde Excel.
 * Columnas: CÃÂ©dula, Total financiamiento, Modalidad pago, Fecha requerimiento, Producto,
 * Concesionario, Analista, Modelo vehÃÂ­culo, NÃÂºmero cuotas, Cuota perÃÂ­odo, Tasa interÃÂ©s, Observaciones.
 */
...
import { useExcelUploadPrestamos, type ExcelUploaderPrestamosProps } from '../../hooks/useExcelUploadPrestamos'
    ...
    >
      ...
      >
        ...
        <div className="flex-shrink-0 bg-gradient-to-r from-green-600 to-green-700 text-white p-6 rounded-t-lg">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <h2 className="text-xl font-bold">CARGA MASIVA DE PRÃÂSTAMOS</h2>
              <div
        ...
        <div className="flex-1 min-h-0 overflow-y-auto p-6 space-y-6">
              ...
              <CardContent className="pt-6">
                ...
                >
                  ...
                  <p className="text-gray-600 mb-4 text-sm">
                    Columnas: CÃÂ©dula | Total financiamiento | Modalidad pago | Fecha requerimiento | Producto | Concesionario | Analista | Modelo | NÃÂº cuotas | Cuota perÃÂ­odo | Tasa interÃÂ©s | Observaciones
                  </p>
          ...
          {/* Tabla, progreso y estadÃÂ­sticas cuando hay datos (mismo orden que Pagos) */}
          {excelData.length > 0 && (
                    ...
                    <table className="w-full border-collapse min-w-[1200px]">
                      <thead>
                        <tr className="bg-gray-50">
                          ...
                          <th className="border p-2 text-left text-xs font-medium w-24">Total $</th>
                          ...
                          <th className="border p-2 text-left text-xs font-medium w-28">Modelo vehÃÂ­culo</th>
                          ...
                          <tr key={row._rowIndex} className={row._hasErrors ? 'bg-red-50' : 'bg-green-50'}>
                            ...
                            <td className="border p-2">
                              <input
                                ...
                                onChange={(e) => updateCellValue(row, 'total_financiamiento', e.target.value)}
                                className={inputClass(row._validation.total_financiamiento?.isValid ? true)}
                              />
                            </td>
                            <td className="border p-2">
                              <select
                                value={row.modalidad_pago}
                                onChange={(e) => updateCellValue(row, 'modalidad_pago', e.target.value)}
                                className={inputClass(row._validation.modalidad_pago?.isValid ? true)}
                              >
                            ...
                            <td className="border p-2">
                              <input
                                ...
                                onChange={(e) => updateCellValue(row, 'fecha_requerimiento', e.target.value)}
                                placeholder="DD/MM/YYYY"
                                className={inputClass(row._validation.fecha_requerimiento?.isValid ? true)}
                              />
                            </td>
                            <td className="border p-2">
                              <input
                                type="text"
                                value={row.producto}
                                onChange={(e) => updateCellValue(row, 'producto', e.target.value)}
                                className={inputClass(row._validation.producto?.isValid ? true)}
                              />
                            </td>
                            <td className="border p-2">
                              <input
                            ...
                            <td className="border p-2">
                              <input
                                ...
                                placeholder="Modelo vehÃÂ­culo"
                              />
                            </td>
                            <td className="border p-2">
                              <input
                                ...
                                onChange={(e) => updateCellValue(row, 'numero_cuotas', e.target.value)}
                                min={1}
                                max={12}
                                className={inputClass(row._validation.numero_cuotas?.isValid ? true)}
                              />
                            </td>
                            <td className="border p-2">
                              <div className="flex items-center gap-1 flex-wrap">
                                ...
                                {!savedRows.has(row._rowIndex) && (
                                  <Button
                                    ...
                                    disabled={savingProgress[row._rowIndex] || serviceStatus === 'offline'}
                                    className="text-amber-700 border-amber-400 text-xs hover:bg-amber-50"
                                    title="Enviar a Revisar PrÃÂ©stamos"
                                  >
                                    {savingProgress[row._rowIndex] ? (
                                      <Loader2 className="h-3 w-3 animate-spin" />
                                    ) : (
              ...
              {batchProgress && (
                <div className="rounded-lg border border-blue-200 bg-blue-50 px-4 py-3">
                  <div className="flex items-center justify-between mb-1 text-sm font-medium text-blue-800">
                    <span>Enviando a Revisar PrÃÂ©stamos...</span>
                ...
                <CardContent className="pt-4">
                  <div className="flex flex-wrap items-center justify-between gap-4">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <Badge variant="outline" className="text-green-700">VÃÂ¡lidos: {validCount}</Badge>
                      <Badge variant="outline">Guardados: {savedRows.size}</Badge>
                      {enviadosRevisar.size > 0 && (
                        <Badge variant="outline" className="text-amber-700 border-amber-300">
                          {enviadosRevisar.size} a Revisar PrÃÂ©stamos
                        </Badge>
                      )}
                      ...
                      {getRowsToRevisarPrestamos().length > 0 && (
                        <Button
                          ...
                          title="Enviar todas las filas pendientes a Revisar PrÃÂ©stamos"
                        >
                          {isSendingAllRevisar ? (
                            <>
                            ...
                            <>
                              <Search className="mr-2 h-4 w-4" />
                              ENVIAR REVISAR PRÃÂSTAMOS ({getRowsToRevisarPrestamos().length})
                            </>
                          )}
                        </Button>
                      )}
