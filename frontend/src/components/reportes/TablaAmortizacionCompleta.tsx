...
export function TablaAmortizacionCompleta() {
  ...
  // Obtener prÃÂ©stamos por cÃÂ©dula usando hook
  ...
  // Obtener cuotas de todos los prÃÂ©stamos (optimizado - una sola query)
  ...
  // Debug: Log para ver quÃÂ© estÃÂ¡ pasando con los prÃÂ©stamos (solo en desarrollo)
  useEffect(() => {
    if (process.env.NODE_ENV === 'development') {
      console.log('[TablaAmortizacion] Estado prÃÂ©stamos:', {
        cedulaSeleccionada,
        loadingPrestamos,
        errorPrestamos,
        prestamosData,
      ...
      console.log('[TablaAmortizacion] CondiciÃÂ³n para cargar cuotas:', {
  ...
  const { data: todasLasCuotas, isLoading: loadingCuotas, error: errorCuotas } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (!prestamosData || prestamosData.length === 0 || ids.length === 0) {
        if (process.env.NODE_ENV === 'development') {
          console.log('[TablaAmortizacion] No hay prÃÂ©stamos para cargar cuotas')
        }
        return []
      }
      try {
        // Usar endpoint optimizado para mÃÂºltiples prÃÂ©stamos
        if (process.env.NODE_ENV === 'development') {
          console.log('[TablaAmortizacion] Cargando cuotas para prÃÂ©stamos:', ids)
        }
  ...
  // Debug: Verificar cuando cambian los prÃÂ©stamos (solo en desarrollo)
  useEffect(() => {
    if (process.env.NODE_ENV === 'development') {
  ...
  // Obtener pagos por cÃÂ©dula (manejar errores para que no fallen los reportes)
  const { data: pagosData, isLoading: loadingPagos, error: errorPagos } = useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        // Retornar estructura vacÃÂ­a para que no falle el reporte
  ...
  // Filtrar pagos activos y manejar valores nulos/vacÃÂ­os en numero_documento
  ...
  const mutationActualizarCuota = useMutation({
    ...
    onSuccess: (data, variables) => {
      ...
      // Invalidar todas las queries relacionadas con cuotas para asegurar actualizaciÃÂ³n
      queryClient.invalidateQueries({ queryKey: ['cuotas-prestamos'] })
      queryClient.invalidateQueries({ queryKey: ['cuotas'] })
      // TambiÃÂ©n invalidar la query especÃÂ­fica de esta cuota si existe
  ...
  // FunciÃÂ³n para confirmar eliminaciÃÂ³n
  const handleEliminarCuota = (cuotaId: number) => {
    if (!confirm('ÃÂ¿EstÃÂ¡ seguro de eliminar esta cuota? Esta acciÃÂ³n no se puede deshacer.')) {
      return
    }
  ...
  // MutaciÃÂ³n para generar amortizaciÃÂ³n
  const mutationGenerarAmortizacion = useMutation({
    ...
    onSuccess: (data, prestamoId) => {
      toast.success(`Tabla de amortizaciÃÂ³n generada exitosamente para el prÃÂ©stamo ${prestamoId}`)
      // Invalidar queries para recargar cuotas
      queryClient.invalidateQueries({ queryKey: ['cuotas-prestamos'] })
      queryClient.invalidateQueries({ queryKey: ['prestamos-cedula', cedulaSeleccionada] })
    },
    onError: (error: any) => {
      toast.error(`Error al generar amortizaciÃÂ³n: ${error?.response?.data?.detail || error?.message || 'Error desconocido'}`)
    },
  })

  // MutaciÃÂ³n para actualizar prÃÂ©stamo
  ...
  // FunciÃÂ³n para abrir diÃÂ¡logo de fecha y generar cuotas
  const handleAbrirDialogoFecha = (prestamo: any) => {
    ...
    if (prestamo.fecha_base_calculo) {
      ...
      // Usar fecha de aprobaciÃÂ³n o fecha actual como sugerencia
  ...
  // FunciÃÂ³n para generar cuotas con fecha ingresada manualmente
  const handleGenerarCuotasConFecha = async () => {
    if (!prestamoParaGenerar || !fechaBaseCalculo) {
      toast.error('Por favor, ingrese una fecha vÃÂ¡lida')
      return
    }

    try {
      // Paso 1: Actualizar fecha_base_calculo en el prÃÂ©stamo
      if (!prestamoParaGenerar.fecha_base_calculo) {
        ...
        toast.success('Fecha base de cÃÂ¡lculo guardada')
      }
      ...
      // Cerrar diÃÂ¡logo y recargar datos
      ...
      toast.success('Tabla de amortizaciÃÂ³n generada y pagos aplicados exitosamente')
    } catch (error: any) {
      toast.error(`Error: ${error?.response?.data?.detail || error?.message || 'Error desconocido'}`)
    }
  }

  // FunciÃÂ³n para generar cuotas para todos los prÃÂ©stamos sin cuotas
  const handleGenerarCuotasParaTodos = async () => {
    if (!prestamos || prestamos.length === 0) {
      toast.error('No hay prÃÂ©stamos para procesar')
      return
    }

    // Filtrar prÃÂ©stamos aprobados sin cuotas
    ...
    if (prestamosSinCuotas.length === 0) {
      toast.info('Todos los prÃÂ©stamos aprobados ya tienen cuotas generadas')
      return
    }

    // Si hay mÃÂºltiples prÃÂ©stamos, procesar el primero y mostrar diÃÂ¡logo
    if (prestamosSinCuotas.length === 1) {
      handleAbrirDialogoFecha(prestamosSinCuotas[0])
    } else {
      // Para mÃÂºltiples prÃÂ©stamos, procesar el primero
      handleAbrirDialogoFecha(prestamosSinCuotas[0])
      toast.info(`Se procesarÃÂ¡ el prÃÂ©stamo ${prestamosSinCuotas[0].id}. Los demÃÂ¡s se pueden procesar despuÃÂ©s.`)
    }
  }

  // FunciÃÂ³n para confirmar eliminaciÃÂ³n de pago
  const handleEliminarPago = (pagoId: number) => {
    if (!confirm('ÃÂ¿EstÃÂ¡ seguro de eliminar este pago? Esta acciÃÂ³n no se puede deshacer.')) {
      return
    }
    mutationEliminarPago.mutate(pagoId)
  }

  // ValidaciÃÂ³n de cÃÂ©dula venezolana
  const validarCedula = (cedula: string): boolean => {
    if (!cedula || cedula.trim().length === 0) return false
    // Formato: E/V/J/Z seguido de 6-12 dÃÂ­gitos
  ...
  const handleBuscar = () => {
    ...
    if (!cedulaLimpia) {
      toast.error('Por favor, ingrese una cÃÂ©dula')
      return
    }
    if (!validarCedula(cedulaLimpia)) {
      toast.error('CÃÂ©dula invÃÂ¡lida. Debe tener el formato E/V/J/Z seguido de 6-12 dÃÂ­gitos')
      return
    }
  ...
  const validarMonto = (valor: string): { valido: boolean; mensaje?: string } => {
    ...
    if (isNaN(numero)) return { valido: false, mensaje: 'Debe ser un nÃÂºmero vÃÂ¡lido' }
    if (numero < 0) return { valido: false, mensaje: 'No puede ser negativo' }
    // Validar mÃÂ¡ximo 2 decimales
    const partes = valor.split('.')
    if (partes.length === 2 && partes[1].length > 2) {
      return { valido: false, mensaje: 'MÃÂ¡ximo 2 decimales permitidos' }
    }
    return { valido: true }
  }
  ...
  // FunciÃÂ³n para determinar el estado correcto basado en los datos (igual que en PrÃÂ©stamos)
  const determinarEstadoReal = (cuota: Cuota): string => {
    const totalPagado = cuota.total_pagado || 0
    const montoCuota = cuota.monto_cuota || 0
    
    // Si total_pagado >= monto_cuota, deberÃÂ­a ser PAGADO
    if (totalPagado >= montoCuota) {
      return 'PAGADO'
    }
    // Si tiene algÃÂºn pago pero no completo
    if (totalPagado > 0) {
      // Verificar si estÃÂ¡ vencida
      const hoy = new Date()
  ...
  const getEstadoBadge = (estado: string) => {
    // Normalizar estado a mayÃÂºsculas para comparaciÃÂ³n (igual que en PrÃÂ©stamos)
  ...
  return (
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            Tabla de AmortizaciÃÂ³n por CÃÂ©dula
          </CardTitle>
          <CardDescription>
            Busca una cÃÂ©dula para ver su tabla de amortizaciÃÂ³n completa con opciones de ediciÃÂ³n y eliminaciÃÂ³n
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="flex gap-2 mb-6">
            <Input
              placeholder="Ingresa la cÃÂ©dula del cliente"
              value={cedulaBuscar}
              onChange={(e) => setCedulaBuscar(e.target.value)}
              onKeyPress={(e) => e.key === 'Enter' && handleBuscar()}
              className="max-w-md"
            />
            <Button onClick={handleBuscar} disabled={loadingCliente || loadingPrestamos} aria-label="Buscar cliente por cÃÂ©dula">
          ...
          {cedulaSeleccionada && (
            <>
              {/* InformaciÃÂ³n del Cliente */}
              {(loadingCliente || loadingPrestamos) ? (
                <div className="flex items-center justify-center py-8">
                  <Loader2 className="w-6 h-6 animate-spin text-blue-600" />
                  <span className="ml-2">Cargando informaciÃÂ³n...</span>
                </div>
              ) : errorPrestamos ? (
                <Card className="mb-6">
                  <CardContent className="py-8 text-center">
                    <AlertCircle className="w-12 h-12 text-red-400 mx-auto mb-4" />
                    <p className="text-red-600 mb-2">Error al cargar los prÃÂ©stamos</p>
                ...
                <Card className="mb-6">
                  <CardContent className="py-8 text-center">
                    ...
                    <p className="text-gray-600 mb-2">No se encontraron prÃÂ©stamos para esta cÃÂ©dula</p>
                  </CardContent>
                </Card>
              ) : prestamos && prestamos.length > 0 ? (
                <>
                  {/* Tablas de AmortizaciÃÂ³n agrupadas por prÃÂ©stamo */}
                  {loadingCuotas ? (
                    <Card className="mb-6">
                      <CardContent className="py-8 text-center">
                        <Loader2 className="w-8 h-8 animate-spin text-blue-600 mx-auto mb-4" />
                        <p className="text-gray-600">Cargando tabla de amortizaciÃÂ³n...</p>
                      </CardContent>
                    </Card>
                  ) : errorCuotas ? (
                    <Card className="mb-6">
                    ...
                    // Agrupar cuotas por prÃÂ©stamo y mostrar cada prÃÂ©stamo en su propia tabla
                      ...
                      return (
                        <Card key={prestamo.id} className="mb-6 border-l-4 border-l-blue-500 shadow-md">
                          <CardHeader className="bg-gradient-to-r from-blue-50 to-transparent border-b">
                            <CardTitle className="flex items-center justify-between">
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  <span className="text-lg font-bold">PrÃÂ©stamo #{prestamo.id}</span>
                                  {clienteInfo?.nombres && (
                          ...
                          <CardContent className="pt-6">
                              ...
                              <Table>
                                <TableHeader>
                                  <TableRow>
                                    ...
                                    <TableHead className="text-right">InterÃÂ©s</TableHead>
                                ...
                                <TableBody>
                                  {cuotasDelPrestamo.map((cuota: Cuota) => {
                                    // Determinar el estado real basado en los datos (igual que en PrÃÂ©stamos)
                                    const estadoReal = determinarEstadoReal(cuota)
                                    
                                    // Calcular monto_capital y monto_interes si no existen (igual que en PrÃÂ©stamos)
                            ...
                            {/* Resumen (igual que en PrÃÂ©stamos) */}
                    ...
                    <Card className="mb-6">
                      <CardContent className="py-8 text-center">
                        <AlertCircle className="w-12 h-12 text-gray-400 mx-auto mb-4" />
                        <p className="text-gray-600 mb-2">No se encontraron cuotas para los prÃÂ©stamos de este cliente</p>
                        <p className="text-sm text-gray-500 mb-4">
                          Los prÃÂ©stamos pueden no tener tabla de amortizaciÃÂ³n generada.
                        </p>
                        ...
                        {prestamos && prestamos.length > 0 && !prestamos.some((p: any) => 
                          ...
                          <p className="text-sm text-yellow-600 mt-2">
                            Los prÃÂ©stamos necesitan estar aprobados para generar cuotas.
                          </p>
                        )}
                      </CardContent>
                    </Card>
      ...
      <Dialog open={mostrarDialogCuota} onOpenChange={setMostrarDialogCuota}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <p className="text-sm text-gray-600 mb-4">Modifica los datos de la cuota. Los cambios se guardarÃÂ¡n inmediatamente.</p>
          </DialogHeader>
          {cuotaEditando && (
            <form
              onSubmit={(e) => {
                ...
                if (fechaVencimiento && !validarFecha(fechaVencimiento)) {
                  toast.error('Fecha de vencimiento invÃÂ¡lida')
                  return
                }
                if (fechaPago && !validarFecha(fechaPago)) {
                  toast.error('Fecha de pago invÃÂ¡lida')
                  return
                }
                ...
                // Manejar fecha_pago: si estÃÂ¡ vacÃÂ­o, enviar null explÃÂ­citamente
                if (fechaPago && fechaPago.trim() !== '') {
                  data.fecha_pago = fechaPago
                } else {
                  // Enviar null explÃÂ­citamente para limpiar fecha_pago
                  data.fecha_pago = null
                }

                // Procesar montos con validaciÃÂ³n
            ...
            >
              ...
              <div className="grid grid-cols-3 gap-4 border-b pb-4">
                <div>
                  ...
                  <Input
                    ...
                    onBlur={(e) => {
                      ...
                      if (valor) {
                        const validacion = validarMonto(valor)
                        if (!validacion.valido) {
                          toast.error(validacion.mensaje || 'Valor invÃÂ¡lido')
                          e.target.focus()
                        } else {
                  ...
                  />
                  <p className="text-xs text-gray-500 mt-1">MÃÂ¡ximo 2 decimales</p>
                </div>
              </div>
      ...
      <Dialog open={mostrarDialogPago} onOpenChange={setMostrarDialogPago}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <p className="text-sm text-gray-600 mb-4">Modifica los datos del pago. Los nÃÂºmeros cientÃÂ­ficos se normalizarÃÂ¡n automÃÂ¡ticamente.</p>
          </DialogHeader>
          {pagoEditando && (
            <form
              onSubmit={(e) => {
                ...
                // Normalizar nÃÂºmero de documento si es cientÃÂ­fico o estÃÂ¡ vacÃÂ­o
                let numeroDocumento = (formData.get('numero_documento') as string)?.trim() || ''
                
                // Si estÃÂ¡ vacÃÂ­o, usar el valor actual o cadena vacÃÂ­a
                if (!numeroDocumento) {
                  numeroDocumento = pagoEditando.numero_documento || ''
                }
                
                // Normalizar formato cientÃÂ­fico si existe
                if (numeroDocumento && (/[eE]/.test(numeroDocumento))) {
                  try {
                    ...
                    if (!isNaN(numeroFloat)) {
                      ...
                      toast.info(`NÃÂºmero de documento normalizado: ${numeroDocumento}`)
                    }
                  } catch (e) {
                    console.error('Error normalizando nÃÂºmero:', e)
                    toast.warning('Error al normalizar nÃÂºmero cientÃÂ­fico. Se guardarÃÂ¡ tal como estÃÂ¡.')
                  }
                }
                
                // Permitir guardar incluso si estÃÂ¡ vacÃÂ­o (no requerido)
                if (!numeroDocumento) {
                  numeroDocumento = '' // Permitir vacÃÂ­o
                }
            ...
            >
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <Label htmlFor="numero_documento">NÃÂºmero de Documento</Label>
                  <Input
                    id="numero_documento"
                    name="numero_documento"
                    defaultValue={pagoEditando.numero_documento || ''}
                    placeholder="Ingrese nÃÂºmero de documento"
                  />
                  <p className="text-xs text-gray-500 mt-1">
                    {pagoEditando.numero_documento && /[eE]/.test(pagoEditando.numero_documento) ? (
                      <span className="text-yellow-600">
                        ÃÂ¢ÃÂ¡ ÃÂ¯ÃÂ¸ÃÂ Formato cientÃÂ­fico detectado. Se normalizarÃÂ¡ automÃÂ¡ticamente al guardar.
                      </span>
                    ) : (
                      'Los nÃÂºmeros cientÃÂ­ficos se normalizarÃÂ¡n automÃÂ¡ticamente'
                    )}
                    {!pagoEditando.numero_documento && (
                      <span className="text-gray-500"> Campo opcional. Puede dejarse vacÃÂ­o.</span>
                    )}
                  </p>
                </div>
                <div>
                  <Label htmlFor="institucion_bancaria">InstituciÃÂ³n Bancaria</Label>
                  <Input
      ...
      {/* Dialog Ingresar Fecha Base de CÃÂ¡lculo */}
      <Dialog open={mostrarDialogFecha} onOpenChange={setMostrarDialogFecha}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Ingresar Fecha Base de CÃÂ¡lculo</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-4">
            {prestamoParaGenerar && (
              <div className="bg-blue-50 p-3 rounded-md">
                <p className="text-sm text-gray-600">
                  <strong>PrÃÂ©stamo ID:</strong> {prestamoParaGenerar.id}
                </p>
                <p className="text-sm text-gray-600">
            ...
            <div>
              <Label htmlFor="fecha_base_calculo">Fecha Base de CÃÂ¡lculo *</Label>
              <Input
              ...
              />
              <p className="text-xs text-gray-500 mt-1">
                Esta fecha serÃÂ¡ guardada y usada para generar las cuotas del prÃÂ©stamo.
              </p>
            </div>
              ...
              <div className="bg-yellow-50 p-3 rounded-md">
                <p className="text-sm text-yellow-800">
                  <strong>Nota:</strong> Se encontraron {pagos.filter((p: Pago) => 
                    p.prestamo_id === prestamoParaGenerar.id && 
                    (p.conciliado || p.verificado_concordancia === 'SI')
                  ).length} pago(s) conciliado(s) que se aplicarÃÂ¡n automÃÂ¡ticamente a las cuotas generadas.
                </p>
              </div>
            )}
          </div>
