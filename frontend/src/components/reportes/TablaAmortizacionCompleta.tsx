...
export function TablaAmortizacionCompleta() {
  ...
  useEffect(() => {
    if (process.env.NODE_ENV === 'development') {
      console.log('[TablaAmortizacion] Estado prÃ©stamos:', {
        cedulaSeleccionada,
        loadingPrestamos,
        errorPrestamos,
        prestamosData,
      ...
      console.log('[TablaAmortizacion] CondiciÃ³n para cargar cuotas:', {
  ...
  const { data: todasLasCuotas, isLoading: loadingCuotas, error: errorCuotas } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (!prestamosData || prestamosData.length === 0 || ids.length === 0) {
        if (process.env.NODE_ENV === 'development') {
          console.log('[TablaAmortizacion] No hay prÃ©stamos para cargar cuotas')
        }
        return []
      }
      try {
        ...
        if (process.env.NODE_ENV === 'development') {
          console.log('[TablaAmortizacion] Cargando cuotas para prÃ©stamos:', ids)
        }
  ...
  const validarCedula = (cedula: string): boolean => {
    ...
    // Formato: E/V/J/Z seguido de 6-12 dÃ­gitos
  ...
  const handleBuscar = () => {
    ...
    if (!validarCedula(cedulaLimpia)) {
      toast.error('CÃ©dula invÃ¡lida. Debe tener el formato E/V/J/Z seguido de 6-12 dÃ­gitos')
      return
    }
        ...
        <CardContent>
          <div className="flex gap-2 mb-6">
            <Input
              ...
              className="max-w-md"
            />
            <Button onClick={handleBuscar} disabled={loadingCliente || loadingPrestamos} aria-label="Buscar cliente por cÃ©dula">
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    {pagoEditando.numero_documento && /[eE]/.test(pagoEditando.numero_documento) ? (
                      <span className="text-yellow-600">
                        Ã¢Å¡ Ã¯Â¸Â Formato cientÃ­fico detectado. Se normalizarÃ¡ automÃ¡ticamente al guardar.
                      </span>
                    ) : (
