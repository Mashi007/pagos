...
export function ReporteDiferenciasAbonos() {
  ...
  const {
    ...
    queryFn: async (): Promise<DiferenciaAbono[]> => {
      const response = await apiClient.get<DiferenciaAbono[]>(
        '/api/v1/reportes/diferencias-abonos'
      )
  ...
  const ajustarAbono = useMutation({
    mutationFn: async ({
      ...
      const response = await apiClient.put(
        `/api/v1/reportes/diferencias-abonos/${prestamoId}/ajustar`,
        {
    ...
    onSuccess: () => {
      ...
      toast.success(
        'Total de abonos ajustado exitosamente. Los pagos han sido redistribuidos.'
      )
    },

    onError: (error: any) => {
      toast.error(
        error?.response?.data?.detail || 'Error al ajustar total de abonos'
      )
    },
  })
  ...
  const actualizarValorImagen = useMutation({
    mutationFn: async ({
      ...
      const response = await apiClient.put(
        '/api/v1/reportes/diferencias-abonos/actualizar-valor-imagen',
        {
          cedula,
    ...
    onError: (error: any) => {
      toast.error(
        error?.response?.data?.detail || 'Error al actualizar valor de imagen'
      )
    },
  })
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Préstamos marcados para revisión con diferencias entre BD y valores
            de referencia
          </CardDescription>
        </CardHeader>
        ...
        <CardContent className="py-8">
          <div className="text-center text-gray-500">
            ...
            <p className="mt-2 text-sm">
              Marca préstamos para revisar en la página de Préstamos para que
              aparezcan aquí.
            </p>
          </div>
        </CardContent>
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          {diferencias.length} préstamo(s) con diferencias pendientes de
          revisión. Puedes ajustar el total de abonos y el sistema redistribuirá
          los pagos uniformemente.
        </CardDescription>
      </CardHeader>
                  ...
                  <TableCell className="text-right">
                    {editandoId === diferencia.prestamo_id ? (
                      ...
                      <span className="font-semibold">
                        {formatCurrency(diferencia.total_abonos_bd)}
                      </span>
                    )}
                  </TableCell>
                    ...
                    {editandoImagenId === diferencia.prestamo_id ? (
                      ...
                      <div className="flex items-center justify-end gap-2">
                        <span className="text-gray-600">
                          {formatCurrency(diferencia.total_abonos_imagen)}
                        </span>
                  ...
                  <TableCell className="text-center">
                    {editandoImagenId === diferencia.prestamo_id ? (
                      ...
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={() => handleEditar(diferencia)}
                      >
