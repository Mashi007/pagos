...
export function DialogConciliacion({
  open,
  onOpenChange,
  onGuardar,
}: DialogConciliacionProps) {
  ...
  const [formatoDescarga, setFormatoDescarga] = useState<'excel' | 'pdf'>(
    'excel'
  )
  ...
  const validar = useCallback((filasToValidate: FilaConciliacion[]) => {
    ...
    filasToValidate.forEach((f, i) => {
      ...
      if (!validarNumero(f.total_financiamiento))
        err.push(`Fila ${i + 1}: total financiamiento debe ser un numero >= 0`)

      if (!validarNumero(f.total_abonos))
        err.push(`Fila ${i + 1}: total abonos debe ser un numero >= 0`)
    })
      ...
      reader.onload = () => {
        try {
          ...
          import('xlsx').then(XLSX => {
            ...
            if (json.length < 2) {
              toast.error(
                'El Excel debe tener al menos encabezado y una fila. Columnas: A=Cedula, B=Total financiamiento, C=Total abonos.'
              )
          ...
          toast.error(
            'Error al leer el Excel. Asegurese de que sea .xlsx y que la columna A sea cedula, B total financiamiento, C total abonos.'
          )
        }
      }
  ...
  const handleGuardar = async () => {
    ...
    try {
      ...
      const msg =
        getErrorDetail(error) || getErrorMessage(error) || 'Error al guardar'
  ...
  const handleDescargar = async () => {
    ...
    try {
      ...
      toast.success(
        `Reporte descargado en ${formatoDescarga.toUpperCase()}. Los datos temporales se han eliminado.`
      )

      onOpenChange(false)
    } catch (error: unknown) {
      toast.error(
        getErrorDetail(error) || getErrorMessage(error) || 'Error al descargar'
      )
    } finally {
      ...
      <DialogContent className="flex max-h-[90vh] max-w-3xl flex-col overflow-hidden">
        ...
        <div className="space-y-4 overflow-auto">
          ...
          {tab === 'carga' && (
            <>
              <p className="text-sm text-muted-foreground">
                Cargue un Excel con columnas: A = Cedula, B = Total
                financiamiento, C = Total abonos. Opcionales: columnas 5 y 6
                para E y F. Corrija errores en la tabla y pulse Guardar e
                integrar.
              </p>
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <Button
                  ...
                  variant="outline"
                  onClick={() =>
                    document.getElementById('conciliacion-file')?.click()
                  }
                >
                      ...
                      {filas.slice(0, 10).map((f, i) => (
                        <tr key={i}>
                          ...
                          <td className="p-2 text-right">
                            {Number(f.total_financiamiento)}
                          </td>

                          <td className="p-2 text-right">
                            {Number(f.total_abonos)}
                          </td>
                        </tr>
                      ))}
          ...
          {tab === 'resumen' && (
            <>
              <p className="text-sm text-muted-foreground">
                Ver resumen de conciliacion o descargar reporte en Excel/PDF con
                filtros opcionales.
              </p>
                ...
                <div className="rounded border border-blue-200 bg-blue-50 p-4">
                  ...
                  <div className="grid grid-cols-2 gap-3 text-sm">
                    ...
                    <div>
                      ...
                      <p className="font-semibold">
                        $
                        {resumen?.monto_total_financiamiento.toLocaleString(
                          'es-ES'
                        )}
                      </p>
                    </div>
                    ...
                    <div>
                      ...
                      <p className="font-semibold text-green-600">
                        ${resumen?.monto_total_abonos.toLocaleString('es-ES')}
                      </p>
                    </div>
                    ...
                    <div>
                      ...
                      <p
                        className={`font-semibold ${(resumen?.diferencia_total ?? 0) < 0 ? 'text-red-600' : 'text-green-600'}`}
                      >
                        $
                        {(resumen?.diferencia_total ?? 0).toLocaleString(
                          'es-ES'
                        )}
                      </p>
                    </div>
                    ...
                    <div>
                      <span className="text-gray-600">
                        Filas con Discrepancia:
                      </span>

                      <p
                        className={`font-semibold ${(resumen?.filas_con_discrepancia ?? 0) > 0 ? 'text-orange-600' : 'text-green-600'}`}
                      >
              ...
              <div>
                ...
                <select
                  ...
                  onChange={e =>
                    setFormatoDescarga(e.target.value as 'excel' | 'pdf')
                  }
                  className="w-full rounded border px-2 py-1 text-sm"
                >
