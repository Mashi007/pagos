...
function ViÃÂ±etaPagoVencido({ item }: { item: MorosidadPorRangosItem }) {
  return (
      ...
      <CardContent className="pt-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
          ...
          <div className="flex items-center gap-2">
            ...
            <span>ÃÂltimo pago: {item.ultimo_pago_fecha ? formatDate(item.ultimo_pago_fecha) : 'Ã¢ÂÂ'}</span>
          </div>
          <div className="flex items-center gap-2 md:col-span-2">
            <Calendar className="h-4 w-4 text-gray-500" />
            <span>PrÃÂ³ximo pago: {item.proximo_pago_fecha ? formatDate(item.proximo_pago_fecha) : 'Ã¢ÂÂ'}</span>
          </div>
        </div>
        <div className="mt-2">
          <Badge variant="secondary" className="text-xs">
            {item.dias_atraso} dÃÂ­as atraso ÃÂ· PrÃÂ©stamo #{item.prestamo_id}
          </Badge>
        </div>
      </CardContent>
    </Card>
    ...
    <Card className="shadow-sm">
      <CardHeader className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between pb-4">
        <div>
          ...
          <CardDescription>
            Clientes con cuotas vencidas por rango de dÃÂ­as. Fecha de corte: {data?.fecha_corte ? formatDate(data.fecha_corte) : 'Ã¢ÂÂ'}
          </CardDescription>
        </div>
              ...
              <TabsContent key={key} value={key} className="space-y-4 mt-4">
                {items.length === 0 ? (
                  ...
                  <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                    {items.map((item) => (
                      <ViÃÂ±etaPagoVencido key={item.prestamo_id} item={item} />
                    ))}
                  </div>
                )}
              </TabsContent>
