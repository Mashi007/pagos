...
function TablaPagosMes({
  ...
  return (
    <Card className="border-l-4 border-l-blue-500">
      ...
      <CardContent>
        {items.length === 0 ? (
          <p className="text-gray-500 text-sm py-4">No hay pagos en este perÃÂ­odo.</p>
        ) : (
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>ID PrÃÂ©stamo</TableHead>
              ...
              <TableBody>
                {items.map((item) => (
                  <TableRow key={item.pago_id ? `${item.fecha}-${item.cedula}-${item.documento}`}>
                    ...
                    <TableCell>{item.prestamo_id ? 'Ã¢ÂÂ'}</TableCell>
                    ...
                    <TableCell className="font-mono text-sm">{item.documento || 'Ã¢ÂÂ'}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
    ...
    <Card className="shadow-sm">
      <CardHeader className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between pb-4">
        <div>
          ...
          <CardDescription>
            Pagos por mes/aÃÂ±o. Columnas: Fecha, ID prÃÂ©stamo, CÃÂ©dula, Nombre, Monto pago, Documento. Orden descendente por fecha.
          </CardDescription>
        </div>
        ...
        {meses.length === 0 ? (
          ...
          <Tabs value={valorTabs} onValueChange={setTabActivo} className="w-full">
            <TabsList className="flex flex-wrap gap-1 mb-4 h-auto">
              {meses.map((m) => (
                <TabsTrigger key={`${m.aÃÂ±o}-${m.mes}`} value={m.label}>
                  {m.label}
                </TabsTrigger>
              ))}
            </TabsList>
            {meses.map((m) => (
              <TabsContent key={`${m.aÃÂ±o}-${m.mes}`} value={m.label} className="mt-4">
