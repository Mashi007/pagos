...
export function DialogReporteContableFiltros({
  ...
  const [cedulasDisponibles, setCedulasDisponibles] = useState<
    Array<{ cedula: string; nombre: string }>
  >([])

  const [cedulasSeleccionadas, setCedulasSeleccionadas] = useState<Set<string>>(
    new Set()
  )
      ...
      <DialogContent className="max-h-[90vh] max-w-md overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            ...
            {paso === 2 &&
              'Selecciona uno o varios meses (solo meses pasados tendrán datos de pagos)'}
