...
export function DialogReporteContableFiltros({
  ...
  const aÃ±osOpciones = [aÃ±oActual, aÃ±oActual - 1, aÃ±oActual - 2, aÃ±oActual - 3, aÃ±oActual - 4].filter((a) => a !== 2022 && a !== 2023)
  ...
  const seleccionarTodosAÃ±os = () => setAÃ±osSeleccionados(new Set(aÃ±osOpciones))
  ...
  const handleSiguiente = () => {
    if (paso === 1 && aÃ±osSeleccionados.size > 0) setPaso(2)
  ...
  const handleDescargar = () => {
    const aÃ±os = Array.from(aÃ±osSeleccionados).sort((a, b) => b - a)
    ...
    onConfirm({ aÃ±os, meses, cedulas })
    handleAbrir(false)
  }

  const puedeContinuar =
    (paso === 1 && aÃ±osSeleccionados.size > 0) ||
    ...
    <Dialog open={open} onOpenChange={handleAbrir}>
      <DialogContent className="max-w-md max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            {paso === 1 && 'Selecciona uno o varios aÃ±os'}
            {paso === 2 && 'Selecciona uno o varios meses (solo meses pasados tendrÃ¡n datos de pagos)'}
            {paso === 3 && 'Busca y selecciona cÃ©dulas, o marca "Todas"'}
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="space-y-3">
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                ...
                className="rounded"
              />
              <span className="text-sm font-medium">Todas las cÃ©dulas</span>
            </label>
                ...
                <div className="max-h-48 overflow-y-auto border rounded-md p-2 space-y-1">
                  {loadingCedulas ? (
                    ...
                    cedulasDisponibles.map((c) => (
                      ...
                      >
                        <span>
                          {c.cedula} â {c.nombre || '(sin nombre)'}
                        </span>
                        {cedulasSeleccionadas.has(c.cedula) && (
