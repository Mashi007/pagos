      ...
      <CardContent>
        {items.length === 0 ? (
          <p className="py-4 text-sm text-gray-500">
            No hay vencimiento en este período.
          </p>
        ) : (
        ...
        <CardContent className="py-8">
          ...
          <div className="mt-4 flex justify-center">
            <Button
              variant="outline"
              onClick={() => refetch()}
              aria-label="Reintentar cargar reporte de asesores"
            >
    ...
    <Card className="shadow-sm">
      <CardHeader className="flex flex-col gap-4 pb-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <CardDescription>
            Una pestaña por mes. Solo datos del mes reportado: cuotas que
            vencieron en ese mes sin pagar. Columnas: Analista, Total
            vencimiento, Total préstamos.
          </CardDescription>
        </div>

        <div className="flex items-center gap-2">
          <Select
            value={String(mesesAtras)}
            onValueChange={v => setMesesAtras(Number(v))}
          >
          ...
          <Button
            ...
            size="sm"
            onClick={() => refetch()}
            aria-label="Actualizar reporte de asesores"
          >
            <RefreshCw className="mr-2 h-4 w-4" aria-hidden />
            Actualizar
          </Button>
