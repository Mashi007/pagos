...
function TablaAsesoresMes({ label, items }: { label: string; items: AsesorPorMesItem[] }) {
  return (
    <Card className="border-l-4 border-l-indigo-500">
      ...
      <CardContent>
        {items.length === 0 ? (
          <p className="text-gray-500 text-sm py-4">No hay vencimiento en este perÃÂ­odo.</p>
        ) : (
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="text-right">Total prÃÂ©stamos</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {items.map((item) => (
    ...
    <Card className="shadow-sm">
      <CardHeader className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between pb-4">
        <div>
          ...
          <CardDescription>
            Una pestaÃÂ±a por mes. Solo datos del mes reportado: cuotas que vencieron en ese mes sin pagar. Columnas: Analista, Total vencimiento, Total prÃÂ©stamos.
          </CardDescription>
        </div>
