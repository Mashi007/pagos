...
function TablaProductosMes({ label, items }: { label: string; items: ProductoPorMesItem[] }) {
  return (
    <Card className="border-l-4 border-l-purple-500">
      ...
      <CardContent>
        {items.length === 0 ? (
          <p className="text-gray-500 text-sm py-4">No hay productos en este perÃÂ­odo.</p>
        ) : (
          <div className="rounded-md border overflow-x-auto">
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead>Modelo de vehÃÂ­culo</TableHead>
    ...
    <Card className="shadow-sm">
      <CardHeader className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between pb-4">
        <div>
          ...
          <CardDescription>
            Una pestaÃÂ±a por mes. Columnas: Modelo vehÃÂ­culo, Total financiamiento, Valor del activo (70%).
          </CardDescription>
        </div>
