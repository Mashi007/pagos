...
class ValidadoresService {
  ...
  async validarCampo(
    ...
    return await apiClient.post<ValidacionResponse>(
      `${this.baseUrl}/validar-campo`,
      body
    )
  }
  ...
  async obtenerConfiguracion(): Promise<ConfiguracionValidadores> {
    return await apiClient.get<ConfiguracionValidadores>(
      `${this.baseUrl}/configuracion-validadores`
    )
  }
  ...
  async corregirCliente(
    clienteId: number,
    correcciones: Record<string, string>,
    pais: string = 'VENEZUELA'
  ): Promise<any> {
    return await apiClient.post(
      `${this.baseUrl}/corregir-cliente/${clienteId}`,
      {
        correcciones,
