...
export const userService = {
  ...
  actualizarUsuario: async (userId: number, userData: UserUpdate): Promise<User> => {
    // Ã¢ÂÂ CRÃÂTICO: Validar que userId sea vÃÂ¡lido
    if (!userId || userId <= 0 || !Number.isInteger(userId)) {
      throw new Error(`ID de usuario invÃÂ¡lido: ${userId}`)
    }

    // Ã¢ÂÂ CRÃÂTICO: Logging del endpoint que se estÃÂ¡ llamando
    const endpoint = `/api/v1/usuarios/${userId}`
    console.log(`Ã°ÂÂÂ¤ [userService] Actualizando usuario - userId: ${userId}, endpoint: ${endpoint}`)
