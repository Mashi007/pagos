...
export const userService = {
  ...
  actualizarUsuario: async (userId: number, userData: UserUpdate): Promise<User> => {
    // â CRÃTICO: Validar que userId sea vÃ¡lido
    if (!userId || userId <= 0 || !Number.isInteger(userId)) {
      throw new Error(`ID de usuario invÃ¡lido: ${userId}`)
    }

    // â CRÃTICO: Logging del endpoint que se estÃ¡ llamando
    const endpoint = `/api/v1/usuarios/${userId}`
    console.log(`ð¤ [userService] Actualizando usuario - userId: ${userId}, endpoint: ${endpoint}`)
