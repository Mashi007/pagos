...
export interface LoginResponse {
  ...
  token_type: string
  expires_in?: number  // Tiempo de expiraciÃ³n en segundos (opcional para compatibilidad)
  user: User
}
...
export class AuthService {
  ...
  async login(credentials: LoginForm): Promise<LoginResponse> {
    try {
      ...
      if (!accessToken || typeof accessToken !== 'string') {
        console.error('Login: el servidor no devolviÃ³ access_token.', { hasUser: !!user })
        throw new Error('El servidor no devolviÃ³ un token de sesiÃ³n. Intenta de nuevo.')
      }

      // Limpiar sesiÃ³n anterior para no mezclar localStorage y sessionStorage
      clearAuthStorage()
      ...
      // Resetear el flag de refresh token expirado despuÃ©s de login exitoso
      apiClient.resetRefreshTokenExpired()

      return response
    } catch (error: any) {
      if (error.code === 'NETWORK_ERROR' || !error.response) {
        const networkError = new Error('Error de conexiÃ³n con el servidor') as any
  ...
  // Olvido de contraseÃ±a: notifica a itmaster@rapicreditca.com para envÃ­o de nueva contraseÃ±a
  ...
  async logout(): Promise<void> {
    try {
      ...
      // Limpiar todo el almacenamiento de autenticaciÃ³n
      clearAuthStorage()
    }
  }
  ...
  // Obtener informaciÃ³n del usuario actual - CON PERSISTENCIA SEGURA
  ...
  async changePassword(data: ChangePasswordRequest): Promise<{ requires_reauth: boolean; message?: string }> {
    ...
    // â Si el backend indica que se requiere reautenticaciÃ³n, cerrar sesiÃ³n y redirigir
    if (response.requires_reauth) {
      // Limpiar almacenamiento de autenticaciÃ³n
      clearAuthStorage()
