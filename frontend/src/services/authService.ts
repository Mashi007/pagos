...
export interface LoginResponse {
  ...
  expires_in?: number // Tiempo de expiración en segundos (opcional para compatibilidad)
...
export class AuthService {
  ...
  async login(credentials: LoginForm): Promise<LoginResponse> {
    try {
      ...
      const response: LoginResponse =
        (raw as any)?.data != null ? (raw as any).data : raw
  ...
  async refreshToken(): Promise<AuthTokens> {
    try {
      const refreshToken =
        safeGetItem('refresh_token') || safeGetSessionItem('refresh_token')
  ...
  ): Promise<{ requires_reauth: boolean; message?: string }> {
  ...
  isAuthenticated(): boolean {
    const token =
      safeGetItem('access_token') || safeGetSessionItem('access_token')
