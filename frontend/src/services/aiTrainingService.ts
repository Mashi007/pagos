...
class AITrainingService {
  ...
  /**
   * Entrenar modelo de predicciÃ³n de impago de cuotas
   */
  async entrenarModeloImpago(params?: {
    ...
    try {
      console.log('ð [aiTrainingService] Iniciando entrenamiento modelo impago:', params)
      // Usar timeout extendido (5 minutos) para entrenamiento ML que puede tardar mucho
      ...
      console.log('â [aiTrainingService] Modelo entrenado exitosamente:', resultado)
      return resultado
    } catch (error: any) {
      console.error('â [aiTrainingService] Error entrenando modelo impago:', error)
  ...
  async listarModelosImpago(): Promise<ModeloImpagoCuotas[] | { modelos: ModeloImpagoCuotas[]; error?: string }> {
    try {
      ...
      if (error?.code === 'ECONNABORTED' || error?.message?.includes('timeout')) {
        console.warn('â ï¸ [aiTrainingService] Timeout al listar modelos. El servidor puede estar procesando.')
        // Retornar lista vacÃ­a con mensaje de error en lugar de lanzar excepciÃ³n
        return { 
          modelos: [], 
          error: 'La peticiÃ³n estÃ¡ tardando mÃ¡s de lo esperado. Por favor, intenta nuevamente en unos momentos.' 
        }
      }
      throw error
    }
  ...
  async getModeloImpagoActivo(): Promise<ModeloImpagoCuotas | null> {
    try {
      ...
      if (error?.code === 'ECONNABORTED' || error?.message?.includes('timeout')) {
        console.warn('â ï¸ [aiTrainingService] Timeout al obtener modelo activo.')
      }
      return null
    }
  }
  ...
  async activarModeloImpago(modeloId: number): Promise<{ mensaje: string; modelo_activo: ModeloImpagoCuotas }> {
    try {
      console.log('ð [aiTrainingService] Activando modelo impago:', modeloId)
      const resultado = await apiClient.post<{ mensaje: string; modelo_activo: ModeloImpagoCuotas }>(
        `${this.baseUrl}/ml-impago/activar`,
        { modelo_id: modeloId }
      )
      console.log('â [aiTrainingService] Modelo activado:', resultado)
      return resultado
    } catch (error: any) {
      console.error('â [aiTrainingService] Error activando modelo impago:', error)
      console.error('Error response:', error?.response)
  ...
  async eliminarModeloImpago(modeloId: number, eliminarArchivo: boolean = false): Promise<{ mensaje: string; modelo_id: number; archivo_eliminado?: boolean }> {
    try {
      console.log('ð [aiTrainingService] Eliminando modelo impago:', modeloId, 'eliminarArchivo:', eliminarArchivo)
      ...
      console.log('â [aiTrainingService] Modelo eliminado:', resultado)
      return resultado
    } catch (error: any) {
      console.error('â [aiTrainingService] Error eliminando modelo impago:', error)
      console.error('Error response:', error?.response)
      throw error
    }
  }
  ...
  /**
   * Predecir impago de cuotas para un prÃ©stamo
   */
