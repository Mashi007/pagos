...
class AITrainingService {
  ...
  async getConversaciones(params?: {
    ...
    if (params?.con_calificacion !== undefined)
      queryParams.append('con_calificacion', params.con_calificacion.toString())
  ...
  async guardarConversacion(conversacion: {
    ...
    const response = await apiClient.post<{ conversacion: ConversacionAI }>(
      `${this.baseUrl}/conversaciones`,
      conversacion
    )
  ...
  async calificarConversacion(
    ...
    const response = await apiClient.post<{ conversacion: ConversacionAI }>(
      `${this.baseUrl}/conversaciones/${conversacionId}/calificar`,
      {
        calificacion,
  ...
  async actualizarConversacion(
    ...
    const response = await apiClient.put<{ conversacion: ConversacionAI }>(
      `${this.baseUrl}/conversaciones/${conversacionId}`,
      conversacion
    )
  ...
  async mejorarConversacion(params: {
    ...
    return await apiClient.post(
      `${this.baseUrl}/conversaciones/mejorar`,
      params
    )
  }
  ...
  async getEstadisticasFeedback(): Promise<{
    ...
    return await apiClient.get(
      `${this.baseUrl}/conversaciones/estadisticas-feedback`
    )
  }
  ...
  async iniciarFineTuning(params: {
    ...
    const response = await apiClient.post<{ job: FineTuningJob }>(
      `${this.baseUrl}/fine-tuning/iniciar`,
      params
    )
  ...
  async getEstadoFineTuning(jobId: string): Promise<FineTuningJob> {
    const response = await apiClient.get<{ job: FineTuningJob }>(
      `${this.baseUrl}/fine-tuning/jobs/${jobId}`
    )
  ...
  async listarFineTuningJobs(): Promise<FineTuningJob[]> {
    const response = await apiClient.get<{ jobs: FineTuningJob[] }>(
      `${this.baseUrl}/fine-tuning/jobs`
    )
  ...
  async activarModeloFineTuned(
    modeloId: string
  ): Promise<{ mensaje: string; modelo_activo: string }> {
    return await apiClient.post(`${this.baseUrl}/fine-tuning/activar`, {
      modelo_id: modeloId,
    })
  }
  ...
  async cancelarFineTuningJob(
    jobId: string
  ): Promise<{ job: FineTuningJob; mensaje: string }> {
    return await apiClient.post(
      `${this.baseUrl}/fine-tuning/jobs/${jobId}/cancelar`
    )
  }
  ...
  async eliminarTodosFineTuningJobs(
    soloFallidos: boolean = false
  ): Promise<{ mensaje: string; eliminados: number }> {
    return await apiClient.delete(
      `${this.baseUrl}/fine-tuning/jobs?solo_fallidos=${soloFallidos}`
    )
  }
  ...
  async generarEmbeddings(
    documentoIds?: number[]
  ): Promise<{ documentos_procesados: number; total_embeddings: number }> {
    const response = await apiClient.post<{
      documentos_procesados: number
      total_embeddings: number
    }>(`${this.baseUrl}/rag/generar-embeddings`, {
  ...
  async buscarDocumentosRelevantes(
    ...
    const response = await apiClient.post<{
      documentos: DocumentoEmbedding[]
      query_embedding?: number[]
    }>(`${this.baseUrl}/rag/buscar`, {
      pregunta,
  ...
  async actualizarEmbeddingsDocumento(
    documentoId: number
  ): Promise<{ embeddings_generados: number }> {
    const response = await apiClient.post<{ embeddings_generados: number }>(
      `${this.baseUrl}/rag/documentos/${documentoId}/embeddings`
    )
  ...
  async listarModelosRiesgo(): Promise<
    ModeloRiesgo[] | { modelos: ModeloRiesgo[]; error?: string }
  > {
    const response = await apiClient.get<{
      modelos: ModeloRiesgo[]
      error?: string
    }>(`${this.baseUrl}/ml-riesgo/modelos`)
  ...
  async getModeloRiesgoActivo(): Promise<ModeloRiesgo | null> {
    try {
      const response = await apiClient.get<{ modelo: ModeloRiesgo }>(
        `${this.baseUrl}/ml-riesgo/modelo-activo`
      )
  ...
  async activarModeloRiesgo(
    modeloId: number
  ): Promise<{ mensaje: string; modelo_activo: ModeloRiesgo }> {
    const response = await apiClient.post<{
      mensaje: string
      modelo_activo: ModeloRiesgo
    }>(`${this.baseUrl}/ml-riesgo/activar`, { modelo_id: modeloId })
  ...
  async predecirRiesgo(datosCliente: {
    ...
    return await apiClient.post(
      `${this.baseUrl}/ml-riesgo/predecir`,
      datosCliente
    )
  }
  ...
  async getMetricasEntrenamiento(): Promise<MetricasEntrenamiento> {
    return await apiClient.get<MetricasEntrenamiento>(
  ...
  async entrenarModeloImpago(params?: {
    ...
    try {
      console.log(
        '🔄 [aiTrainingService] Iniciando entrenamiento modelo impago:',
        params
      )
      ...
      console.log(
        '✅ [aiTrainingService] Modelo entrenado exitosamente:',
        resultado
      )
      ...
      console.error(
        '❌ [aiTrainingService] Error entrenando modelo impago:',
        error
      )
  ...
  async listarModelosImpago(): Promise<
    ModeloImpagoCuotas[] | { modelos: ModeloImpagoCuotas[]; error?: string }
  > {
    try {
      const response = await apiClient.get<{
        modelos: ModeloImpagoCuotas[]
        modelo_activo: ModeloImpagoCuotas | null
        total: number
        error?: string
      }>(
      ...
      if (
        ...
        console.warn(
          '⚠️ [aiTrainingService] Timeout al listar modelos. El servidor puede estar procesando.'
        )
        ...
        return {
          ...
          error:
            'La petición está tardando más de lo esperado. Por favor, intenta nuevamente en unos momentos.',
        }
      }
  ...
  async activarModeloImpago(
    modeloId: number
  ): Promise<{ mensaje: string; modelo_activo: ModeloImpagoCuotas }> {
    try {
      console.log('🔄 [aiTrainingService] Activando modelo impago:', modeloId)

      const resultado = await apiClient.post<{
        mensaje: string
        modelo_activo: ModeloImpagoCuotas
      }>(
      ...
      console.error(
        '❌ [aiTrainingService] Error activando modelo impago:',
        error
      )
  ...
  async eliminarModeloImpago(
    modeloId: number,
    eliminarArchivo: boolean = false
  ): Promise<{
    mensaje: string
    modelo_id: number
    ...
    try {
      console.log(
        '🔄 [aiTrainingService] Eliminando modelo impago:',
        modeloId,
        'eliminarArchivo:',
        eliminarArchivo
      )
      ...
      console.error(
        '❌ [aiTrainingService] Error eliminando modelo impago:',
        error
      )
