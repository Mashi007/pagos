...
class AITrainingService {
  ...
  /**
   * Guardar conversaciÃÂ³n para entrenamiento
   */
  ...
  /**
   * Actualizar conversaciÃÂ³n existente
   */
  ...
  /**
   * Obtener estadÃÂ­sticas de feedback
   */
  ...
  // ============================================
  // MÃÂTRICAS Y DASHBOARD
  // ============================================

  /**
   * Obtener mÃÂ©tricas de entrenamiento
   */
  ...
  /**
   * Entrenar modelo de predicciÃÂ³n de impago de cuotas
   */
  async entrenarModeloImpago(params?: {
    ...
    try {
      console.log('Ã°ÂÂÂ [aiTrainingService] Iniciando entrenamiento modelo impago:', params)
      // Usar timeout extendido (5 minutos) para entrenamiento ML que puede tardar mucho
      ...
      console.log('Ã¢ÂÂ [aiTrainingService] Modelo entrenado exitosamente:', resultado)
      return resultado
    } catch (error: any) {
      console.error('Ã¢ÂÂ [aiTrainingService] Error entrenando modelo impago:', error)
  ...
  async listarModelosImpago(): Promise<ModeloImpagoCuotas[] | { modelos: ModeloImpagoCuotas[]; error?: string }> {
    try {
      ...
      if (error?.code === 'ECONNABORTED' || error?.message?.includes('timeout')) {
        console.warn('Ã¢ÂÂ Ã¯Â¸Â [aiTrainingService] Timeout al listar modelos. El servidor puede estar procesando.')
        // Retornar lista vacÃÂ­a con mensaje de error en lugar de lanzar excepciÃÂ³n
        return { 
          modelos: [], 
          error: 'La peticiÃÂ³n estÃÂ¡ tardando mÃÂ¡s de lo esperado. Por favor, intenta nuevamente en unos momentos.' 
        }
      }
      throw error
    }
  ...
  async getModeloImpagoActivo(): Promise<ModeloImpagoCuotas | null> {
    try {
      ...
      if (error?.code === 'ECONNABORTED' || error?.message?.includes('timeout')) {
        console.warn('Ã¢ÂÂ Ã¯Â¸Â [aiTrainingService] Timeout al obtener modelo activo.')
      }
      return null
    }
  }
  ...
  async activarModeloImpago(modeloId: number): Promise<{ mensaje: string; modelo_activo: ModeloImpagoCuotas }> {
    try {
      console.log('Ã°ÂÂÂ [aiTrainingService] Activando modelo impago:', modeloId)
      const resultado = await apiClient.post<{ mensaje: string; modelo_activo: ModeloImpagoCuotas }>(
        `${this.baseUrl}/ml-impago/activar`,
        { modelo_id: modeloId }
      )
      console.log('Ã¢ÂÂ [aiTrainingService] Modelo activado:', resultado)
      return resultado
    } catch (error: any) {
      console.error('Ã¢ÂÂ [aiTrainingService] Error activando modelo impago:', error)
      console.error('Error response:', error?.response)
  ...
  async eliminarModeloImpago(modeloId: number, eliminarArchivo: boolean = false): Promise<{ mensaje: string; modelo_id: number; archivo_eliminado?: boolean }> {
    try {
      console.log('Ã°ÂÂÂ [aiTrainingService] Eliminando modelo impago:', modeloId, 'eliminarArchivo:', eliminarArchivo)
      ...
      console.log('Ã¢ÂÂ [aiTrainingService] Modelo eliminado:', resultado)
      return resultado
    } catch (error: any) {
      console.error('Ã¢ÂÂ [aiTrainingService] Error eliminando modelo impago:', error)
      console.error('Error response:', error?.response)
      throw error
    }
  }
  ...
  /**
   * Predecir impago de cuotas para un prÃÂ©stamo
   */
