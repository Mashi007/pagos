/**
 * Servicio pÃÂºblico de consulta de estado de cuenta por cÃÂ©dula.
 * Sin autenticaciÃÂ³n. Solo validar cÃÂ©dula y solicitar PDF (envÃÂ­o al email del cliente).
 */
...
export interface SolicitarCodigoResponse {
  ...
  mensaje?: string
  error?: string
  /** ISO 8601 (ej. "2025-03-11T16:30:00Z") para mostrar "CÃÂ³digo vÃÂ¡lido hasta las HH:MM" */
  expira_en?: string
}
...
/** PÃÂºblico: validar cÃÂ©dula (formato + existe en clientes). Sin auth. */
...
/** PÃÂºblico: solicitar cÃÂ³digo por email. Sin auth. Rate limit 5/hora por IP. */
export async function solicitarCodigo(cedula: string): Promise<SolicitarCodigoResponse> {
  ...
  if (res.status === 429) {
    return { ok: false, error: 'Ha alcanzado el lÃÂ­mite de consultas por hora. Intente mÃÂ¡s tarde.' }
  }
...
/** PÃÂºblico: verificar cÃÂ³digo y obtener PDF. Sin auth. Rate limit 15 intentos/15 min por IP. */
...
/** PÃÂºblico: solicitar estado de cuenta (genera PDF, envÃÂ­a al email, devuelve PDF en base64). Sin auth. */
export async function solicitarEstadoCuenta(cedula: string, opts?: { origen?: string }): Promise<SolicitarEstadoCuentaResponse> {
  ...
  try {
    ...
    return { ok: false, error: (text || `Error ${res.status}. Intente mÃÂ¡s tarde.`).slice(0, 200) }
  }
