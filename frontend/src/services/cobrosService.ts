...
export async function enviarReportePublico(formData: FormData): Promise<EnviarReporteResponse> {
  ...
  if (res.status === 503) {
    return {
      ok: false,
      error: 'Servicio temporalmente no disponible. Intente m├â┬ís tarde o contacte por WhatsApp 424-4579934.',
    }
  }
  ...
  try {
    ...
    return {
      ok: false,
      error: (text || `Error ${res.status}. Intente m├â┬ís tarde o contacte por WhatsApp 424-4579934.`).slice(0, 200),
    }
  }
