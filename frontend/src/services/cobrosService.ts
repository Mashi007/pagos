ï»¿/**
 * Servicio para el mâÃ¢â¬âdulo Cobros.
 * - PâÃ¢â¬âblico: validar câÃ¢â¬âdula, enviar reporte (sin auth).
 * - Admin: listado, detalle, aprobar, rechazar, histâÃ¢â¬ârico (con auth).
 */
...
export interface ValidarCedulaResponse {
  ...
  error?: string
  /** True si esta câÃ¢â¬âdula puede reportar pagos en BolâÃ¢â¬Â¡vares (Bs) en cobros/infopagos. */
...
/** PâÃ¢â¬âblico: validar câÃ¢â¬âdula (formato + tiene prâÃ¢â¬âstamo). Sin auth. Sin envâÃ¢â¬Â¡o de token. */
...
/** PâÃ¢â¬âblico: enviar reporte de pago (multipart). Sin auth. Sin envâÃ¢â¬Â¡o de token. */
export async function enviarReportePublico(formData: FormData): Promise<EnviarReporteResponse> {
  ...
  if (res.status === 429) {
    return { ok: false, error: 'Ha alcanzado el lâÃ¢â¬Â¡mite de envâÃ¢â¬Â¡os por hora. Intente mâÃ¢â¬Ã­s tarde.' }
  }
  if (res.status === 503) {
    return {
      ok: false,
      error: 'Servicio temporalmente no disponible. Intente mâÃ¢â¬Ã­s tarde o contacte por WhatsApp 424-4579934.',
    }
  }
  ...
  try {
    ...
    return {
      ok: false,
      error: (text || `Error ${res.status}. Intente mâÃ¢â¬Ã­s tarde o contacte por WhatsApp 424-4579934.`).slice(0, 200),
    }
  }
  if (!res.ok && data && typeof data === 'object') {
    return { ok: false, error: (data as EnviarReporteResponse).error || `Error ${res.status}. Intente mâÃ¢â¬Ã­s tarde o contacte por WhatsApp 424-4579934.` }
  }
  return data
}
...
export async function enviarReporteInfopagos(formData: FormData): Promise<EnviarReporteInfopagosResponse> {
  ...
  try {
    ...
    return {
      ok: false,
      error: (text || `Error ${res.status}. Intente mâÃ¢â¬Ã­s tarde.`).slice(0, 200),
    }
  }
  if (!res.ok && data && typeof data === 'object') {
...
export interface PagoReportadoItem {
  ...
  /** Divergencias de Gemini con lo ingresado (para columna ObservaciâÃ¢â¬ân y revisiâÃ¢â¬ân manual) */
  observacion?: string
  /** Correo al que se enviâÃ¢â¬â (o se enviarâÃ¢â¬Â¡a) el recibo; para icono estado en Acciones */
...
/** Abre el comprobante (imagen/PDF) en nueva pestaâÃ¢â¬âa. Usa auth del apiClient. */
...
/** EnvâÃ¢â¬Â¡a por correo el recibo PDF (manual). Genera PDF si no existe. */
