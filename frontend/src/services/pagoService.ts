...
export interface Pago {
  ...
  cuotas_atrasadas?: number  // â Campo calculado: cuotas vencidas con pago incompleto
}
...
class PagoService {
   ...
   * Los datos estÃ¡n en datos_importados_conerrores; el backend vacÃ­a la tabla tras generar el archivo.
   */
  ...
  /** CÃ©dulas que pueden reportar en Bs (rapicredit-cobros / infopagos). */
  async getCedulasReportarBs(): Promise<{ total: number }> {
    return await apiClient.get(`${this.baseUrl}/cedulas-reportar-bs`)
  }

  /** Agrega una cÃ©dula a la lista (nuevo cliente que paga en bolÃ­vares). */
  ...
  /** Carga Excel con columna 'cedula' para definir quiÃ©nes pueden reportar en Bs. */
