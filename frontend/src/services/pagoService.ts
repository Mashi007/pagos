...
export interface Pago {
  ...
  conciliado: boolean
  verificado_concordancia?: string | null  // SI/NO - VerificaciÃ¯Â¿Â½n de concordancia con mÃ¯Â¿Â½dulo de pagos
  usuario_registro: string
  notas: string | null
  documento_nombre: string | null
  documento_tipo: string | null
  documento_ruta: string | null
  cuotas_atrasadas?: number  // Ã¢ÂÂ Campo calculado: cuotas vencidas con pago incompleto
}
...
class PagoService {
  ...
  /** Obtiene el 100% de los pagos para exportar (paginaciÃ¯Â¿Â½n automÃ¯Â¿Â½tica sin lÃ¯Â¿Â½mite) */
  ...
  /**
   * Crea varios pagos en una sola peticiÃ¯Â¿Â½n (Guardar todos). MÃ¯Â¿Â½x. 500.
   * Devuelve resultados por Ã¯Â¿Â½ndice (Ã¯Â¿Â½xito/error) para actualizar la tabla sin mÃ¯Â¿Â½ltiples rondas.
   */
  ...
  /** Actualiza solo el estado de conciliaciÃ¯Â¿Â½n (SÃ¯Â¿Â½/No) en BD */
  ...
  /**
   * Importa pagos reportados aprobados (mÃ¯Â¿Â½dulo Cobros) a la tabla pagos.
   ...
   * Los datos estÃÂ¡n en datos_importados_conerrores; el backend vacÃÂ­a la tabla tras generar el archivo.
   */
  ...
  // Cargar Excel de conciliaciÃ¯Â¿Â½n (2 columnas: Fecha de DepÃ¯Â¿Â½sito, NÃ¯Â¿Â½mero de Documento)
  ...
  /** CÃÂ©dulas que pueden reportar en Bs (rapicredit-cobros / infopagos). */
  async getCedulasReportarBs(): Promise<{ total: number }> {
    return await apiClient.get(`${this.baseUrl}/cedulas-reportar-bs`)
  }

  /** Agrega una cÃÂ©dula a la lista (nuevo cliente que paga en bolÃÂ­vares). */
  ...
  /** Carga Excel con columna 'cedula' para definir quiÃÂ©nes pueden reportar en Bs. */
  ...
  // Obtener Ã¯Â¿Â½ltimos pagos por cÃ¯Â¿Â½dula (resumen)
  ...
  // Descargar PDF tabla de amortizaciÃ¯Â¿Â½n completa por cÃ¯Â¿Â½dula
...
/** Pagos Gmail: ejecutar pipeline (Gmail -> Drive -> Gemini -> BD). force=true permite ejecutar aunque la Ã¯Â¿Â½ltima sync fue hace poco. scan_filter: "unread" | "read" | "all". */
  ...
  /** Pagos Gmail: estado Ã¯Â¿Â½ltima ejecuciÃ¯Â¿Â½n, prÃ¯Â¿Â½xima y Ã¯Â¿Â½ltima fecha con datos disponibles. */
  ...
  /** Pagos Gmail: confirmar dÃ¯Â¿Â½a (sÃ¯Â¿Â½/no). Si confirmado=true, se borran los datos del dÃ¯Â¿Â½a en el servidor. */
  ...
  /** Pagos Gmail: descargar Excel del dÃ¯Â¿Â½a (datos del Sheet). Solo descarga si status 200; si no, lanza error (evita guardar HTML/JSON como .xlsx). */
  async downloadGmailExcel(fecha?: string): Promise<void> {
    ...
    if (response.status !== 200) {
      ...
      try {
        ...
        throw new Error(response.status === 404 ? 'Sin datos para descargar. Ejecute Ã¯Â¿Â½Generar Excel desde GmailÃ¯Â¿Â½ primero.' : `No se pudo descargar el archivo (${response.status}).`)
      }
    }
    const blob = response.data as Blob
    const disposition = response.headers?.['content-disposition']
  ...
  async downloadGmailExcelTemporal(): Promise<void> {
    ...
    if (response.status !== 200) {
      try {
        ...
        if (e instanceof Error) throw e
        throw new Error(response.status === 404 ? 'No hay datos en la tabla temporal.' : 'No se pudo descargar (' + response.status + ').')
      }
    }
    const blob = response.data as Blob
    const disposition = response.headers?.['content-disposition']
