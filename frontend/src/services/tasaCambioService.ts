...
/**
 * Obtiene la tasa de cambio del dÃÂ­a actual
 */
 ...
 */
export async function guardarTasa(tasa_oficial: number): Promise<TasaCambioResponse> {
  try {
...
/**
 * Obtiene la tasa de cambio para una fecha especÃÂ­fica
 */
export async function getTasaPorFecha(fecha: string): Promise<TasaCambioResponse | null> {
  try {
...
/**
 * Obtiene el historial de tasas (ÃÂºltimas N fechas)
 */
export async function getHistorialTasas(limite: number = 30): Promise<TasaCambioHistorial[]> {
  try {
    const res = await fetch(
