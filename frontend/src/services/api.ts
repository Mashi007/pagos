  ...
  async post<T>(url: string, data?: unknown, config?: AxiosRequestConfig): Promise<T> {
    try {
      ...
      const isSlowEndpoint = url.includes('/ml-riesgo/entrenar') ||
                            ...
                            url.includes('/clientes/check-cedulas') // Pipeline Gmail: puede tardar si el backend es sï¿½ncrono (credenciales OAuth)

      const defaultTimeout = isSlowEndpoint
        ? (url.includes('/prestamos/cedula/batch') ? 60000
          : url.includes('/pagos/upload') ? 120000
          : url.includes('/pagos/batch') ? 180000  // 3 min: Render frÃ­o + muchas filas
