...
// Constantes de configuraciÃ¯Â¿Â½n
const DEFAULT_TIMEOUT_MS = 30000
const SLOW_ENDPOINT_TIMEOUT_MS = 60000 // Para endpoints que pueden tardar mÃ¯Â¿Â½s

// Base URL de la API. CSP permite same-origin y orÃ¯Â¿Â½genes Render; usar VITE_API_URL cuando front y back estÃ¯Â¿Â½n en servicios distintos.
...
class ApiClient {
  ...
  private refreshTokenExpired = false // ? Flag para evitar requests cuando el refresh token estÃ¯Â¿Â½ expirado
  ...
  private setupInterceptors() {
    // Request interceptor - agregar token de autenticaciÃ¯Â¿Â½n
    this.client.interceptors.request.use(
      (config) => {
        // ? Si el refresh token estÃ¯Â¿Â½ expirado, cancelar el request inmediatamente
        if (this.refreshTokenExpired && !config.url?.includes('/auth/login')) {
          const error = new Error('SesiÃ¯Â¿Â½n expirada. Por favor, inicia sesiÃ¯Â¿Â½n nuevamente.')
          ;(error as any).isCancelled = true
          return Promise.reject(error)
        }

        // NO agregar token a endpoints de autenticaciÃ¯Â¿Â½n ni olvido de contraseÃ¯Â¿Â½a
        ...
        if (!isAuthEndpoint) {
          ...
          // ? Limpiar token: remover espacios, saltos de lÃ¯Â¿Â½nea, y prefijo "Bearer " si existe
          if (token) {
            token = token.trim()
            // Remover prefijo "Bearer " si estÃ¯Â¿Â½ presente
            ...
            if (parts.length !== 3) {
              ...
              const error = new Error('Token invÃ¯Â¿Â½lido. Por favor, inicia sesiÃ¯Â¿Â½n nuevamente.')
              ;(error as any).isCancelled = true
              return Promise.reject(error)
            }

            // ? Verificar si el token estÃ¯Â¿Â½ expirado ANTES de enviar el request
    ...
    this.client.interceptors.response.use(
      ...
      async (error) => {
        // Auto-retry para errores 500 transitorios (conexiÃ¯Â¿Â½n SSL, timeouts, etc.)
        ...
        if (
          ...
          requestConfigForRetry.method !== 'get' // No reintentar GET en teorÃ¯Â¿Â½a, pero POST sÃ¯Â¿Â½ es seguro para carga
        ) {
          (requestConfigForRetry as any)._retryCount = retryCount + 1
          const delayMs = 500 * Math.pow(2, retryCount) // 500ms, 1s, 2s
          
          console.warn(`? [ApiClient] Error ${error.response?.status} (intento ${retryCount + 1}/${maxRetries}), reintentando en ${delayMs}ms:`, {
        ...
        // Auto-refresh de tokens con protecciÃ¯Â¿Â½n contra loops infinitos y race conditions
        if (error.response?.status === 401 && !originalRequest?._retry) {
          // No intentar refresh en endpoints de autenticaciÃ¯Â¿Â½n
          ...
          // Si ya hay un refresh en progreso, encolar esta peticiÃ¯Â¿Â½n
          if (this.isRefreshing) {
            return new Promise<string>((resolve, reject) => {
              ...
              .then((token) => {
                if (!token) {
                  throw new Error('Token no disponible despuÃ¯Â¿Â½s del refresh')
                }
          ...
          try {
            ...
            // Hacer la peticiÃ¯Â¿Â½n de refresh con validaciÃ¯Â¿Â½n estricta (lanzar error para 4xx)
            let response
            try {
              ...
              // Si el refresh token estÃ¯Â¿Â½ expirado o es invÃ¯Â¿Â½lido, el servidor devuelve 401
              if (error.response?.status === 401 || error.response?.status === 400) {
                throw new Error('Refresh token invÃ¯Â¿Â½lido o expirado')
              }
              // Para otros errores (red, timeout, etc.), propagar el error
              throw error
            }

            // Verificar si la respuesta es vÃ¯Â¿Â½lida
            ...
            // Reintentar la peticiÃ¯Â¿Â½n original
            originalRequest.headers.Authorization = `Bearer ${access_token}`
            return this.client(originalRequest)
          } catch (refreshError: any) {
            // ? Marcar que el refresh token estÃ¯Â¿Â½ expirado para cancelar requests futuros
            this.refreshTokenExpired = true
            ...
            if (!isAlreadyRedirecting) {
              ...
              if (process.env.NODE_ENV === 'development') {
                console.warn('? Refresh token fallÃ¯Â¿Â½. Redirigiendo al login...', refreshError)
              }

              // ? Redirigir inmediatamente sin delay para evitar mÃ¯Â¿Â½s requests (respeta BASE_PATH)
              window.location.replace(LOGIN_PATH)
            }
  ...
  private processQueue(error: any, token: string | null) {
    this.failedQueue.forEach((prom) => {
      if (error) {
        ...
        prom.reject(new Error('Token no disponible despuÃ¯Â¿Â½s del refresh'))
      }
    })
    this.failedQueue = []
  }
  ...
  private handleError(error: unknown) {
    ...
    if (error.response) {
      ...
      // Evitar mostrar toast de 401 cuando estÃ¯Â¿Â½ siendo manejado por el interceptor
      const isBeingHandledByInterceptor = (error.config as { _retry?: boolean } | undefined)?._retry !== undefined

      switch (status) {
        case 400:
          // Error de validaciÃ¯Â¿Â½n o cliente duplicado (misma cÃ¯Â¿Â½dula y mismo nombre)
          if (typeof responseData?.detail === 'string') {
            toast.error(responseData.detail)
          } else {
            toast.error(responseData?.message || 'Error de validaciÃ¯Â¿Â½n')
          }
          break
        case 401:
          // No mostrar toast si estÃ¯Â¿Â½ siendo manejado por el interceptor de refresh
          if (!isBeingHandledByInterceptor) {
            toast.error('SesiÃ¯Â¿Â½n expirada. Redirigiendo al inicio de sesiÃ¯Â¿Â½n...')
          }
          break
        case 403:
          toast.error('Sin permisos para esta acciÃ¯Â¿Â½n')
          break
        case 404:
          toast.error('Recurso no encontrado')
          break
        case 409:
          toast.error(responseData?.message || 'Conflicto de datos. Verifica la informaciÃ¯Â¿Â½n.')
          break
        case 422:
          // Errores de validaciÃ¯Â¿Â½n
          if (responseData?.detail && Array.isArray(responseData.detail)) {
            responseData.detail.forEach((err: { loc?: string[]; msg?: string }) => {
              toast.error(`${err.loc?.join(' ') || 'Campo'}: ${err.msg || 'Error de validaciÃ¯Â¿Â½n'}`)
            })
          } else {
            toast.error(responseData?.message || 'Error de validaciÃ¯Â¿Â½n')
          }
          break
        case 500:
          // Mostrar el mensaje de error especÃ¯Â¿Â½fico del backend si estÃ¯Â¿Â½ disponible
          const errorDetail = typeof responseData?.detail === 'string'
            ? responseData.detail
            : responseData?.message || 'Error interno del servidor'

          // Logging detallado para diagnÃ¯Â¿Â½stico
          ...
          // Mostrar toast con el mensaje del backend (ahora mÃ¯Â¿Â½s especÃ¯Â¿Â½fico)
          toast.error(errorDetail, { duration: 10000 })
          break
        case 503:
          // NO mostrar toast genÃ¯Â¿Â½rico para errores 503 de duplicados
          // Permitir que el componente maneje el error especÃ¯Â¿Â½fico
          ...
          if (detailStr.includes('duplicate key') || detailStr.includes('already exists') ||
              detailStr.includes('violates unique constraint') || detailStr.includes('cÃ¯Â¿Â½dula') ||
            ...
            return Promise.reject(error) // ? CORRECCIÃ¯Â¿Â½N: Asegurar que se propague el error
          } else {
        ...
        default:
          if (error.code === 'ECONNABORTED' || error.message?.includes('timeout')) {
            toast.error('Tiempo de espera agotado. Verifica tu conexiÃ¯Â¿Â½n.')
          } else if (error.message?.includes('Network Error') || error.code === 'ERR_NETWORK') {
            toast.error('Error de conexiÃ¯Â¿Â½n. Verifica que el servidor estÃ¯Â¿Â½ funcionando.')
          } else {
            toast.error(responseData?.message || 'Error desconocido')
          }
      }
    } else if (error.request) {
      // Error de red - puede ser que el servidor estÃ¯Â¿Â½ reiniciando
      ...
      // Log detallado para diagnÃ¯Â¿Â½stico
      console.error('? [ApiClient] Error de conexiÃ¯Â¿Â½n:', {
        url: error.config?.url,
        method: error.config?.method,
        baseURL: API_BASE_URL,
        code: errorCode,
      ...
      // No mostrar toast para errores de conexiÃ¯Â¿Â½n durante el inicio (servidor reiniciando)
      // Estos errores son temporales y se resuelven automÃ¯Â¿Â½ticamente
      if (
        ...
        // Solo loggear en consola, no mostrar toast (el usuario verÃ¯Â¿Â½ el error si persiste)
        console.warn('? Servidor no disponible temporalmente. Esto es normal durante reinicios.')
        console.warn('   Verifica que el backend estÃ¯Â¿Â½ funcionando en:', API_BASE_URL || 'NO CONFIGURADO')
        return
      }

      // Mensaje mÃ¯Â¿Â½s descriptivo segÃ¯Â¿Â½n el tipo de error
      let errorMsg = 'Error de conexiÃ¯Â¿Â½n. Verifique su conexiÃ¯Â¿Â½n a internet.'
      if (errorCode === 'ECONNABORTED' || errorMessage.includes('timeout')) {
        errorMsg = 'Tiempo de espera agotado. El servidor estÃ¯Â¿Â½ tardando demasiado en responder.'
      } else if (errorCode === 'ERR_NETWORK') {
        errorMsg = 'Error de red. Verifique que el servidor estÃ¯Â¿Â½ funcionando y su conexiÃ¯Â¿Â½n a internet.'
      }
      
      toast.error(errorMsg)
    } else {
      // Error de configuraciÃ¯Â¿Â½n
      toast.error('Error en la configuraciÃ¯Â¿Â½n de la peticiÃ¯Â¿Â½n')
    }
  }

  // MÃ¯Â¿Â½todos HTTP
  async get<T>(url: string, config?: AxiosRequestConfig): Promise<T> {
    // Detectar endpoints lentos y usar timeout extendido (p. ej. Render tarda 3Ã¯Â¿Â½5 s en frÃ¯Â¿Â½o)
    ...
    // ? Priorizar timeout explÃ¯Â¿Â½cito si se proporciona, sino usar el calculado
    const timeout = config?.timeout ? defaultTimeout
    // ? Asegurar que el timeout se aplique correctamente
    // Axios respeta el timeout en la configuraciÃ¯Â¿Â½n del request sobre el del cliente
    const finalConfig = { 
      ...config, 
      timeout,
    }
  ...
  async post<T>(url: string, data?: unknown, config?: AxiosRequestConfig): Promise<T> {
    try {
      ...
      const isSlowEndpoint = url.includes('/ml-riesgo/entrenar') ||
                            ...
                            url.includes('/prestamos/cedula/batch') || // Batch carga masiva: muchas cÃ¯Â¿Â½dulas
                            ...
                            url.includes('/clientes/check-cedulas') // Pipeline Gmail: puede tardar si el backend es sÃ¯Â¿Â½ncrono (credenciales OAuth)

      const defaultTimeout = isSlowEndpoint
        ? (url.includes('/prestamos/cedula/batch') ? 60000
          : url.includes('/pagos/upload') ? 120000
          : url.includes('/pagos/batch') ? 180000  // 3 min: Render frÃÂ­o + muchas filas
          : url.includes('/pagos/gmail/run-now') ? 90000  // 90s: cubre credenciales OAuth + margen para backend sÃ¯Â¿Â½ncrono viejo
      ...
      if (response.status >= 400 && response.status < 500) {
        ...
        if (isAuthFailure && process.env.NODE_ENV === 'development') {
          ...
          if (!is409Pagos) {
            console.error('? [ApiClient] POST recibiÃ¯Â¿Â½ error 4xx:', { url, status: response.status, data: response.data })
          }
  ...
  async put<T>(url: string, data?: unknown, config?: AxiosRequestConfig): Promise<T> {
    console.log('? [ApiClient] PUT request:', { url, data: data ? '***' : '(vacÃ¯Â¿Â½o)', config })
    try {
      ...
      // Verificar si la respuesta es un error 4xx (validateStatus permite 4xx pero debemos manejarlos)
      if (response.status >= 400 && response.status < 500) {
        console.error('? [ApiClient] PUT recibiÃ¯Â¿Â½ error 4xx:', { url, status: response.status, data: response.data })
  ...
  // MÃ¯Â¿Â½todo para subir archivos
  async uploadFile<T>(
    url: string,
    file: File,
  ...
  // MÃ¯Â¿Â½todo para descargar archivos
  ...
  // FUNCIÃ¯Â¿Â½N DE EMERGENCIA: Limpiar completamente el storage
  emergencyClearStorage(): void {
    try {
      ...
      safeClearSession()
      // Limpiar especÃ¯Â¿Â½ficamente tokens de auth
      clearAuthStorage()
    } catch (error) {
...
// FUNCIÃ¯Â¿Â½N GLOBAL DE EMERGENCIA: Limpiar storage desde consola del navegador
...
// Funciones de utilidad para construir URLs con parÃ¯Â¿Â½metros
