...
// Tipo para el resumen de prÃ©stamos
...
class PrestamoService {
  ...
  // Obtener lista de prÃ©stamos con filtros y paginaciÃ³n
  async getPrestamos(
    filters?: {
      search?: string
      estado?: string
  ...
  // Obtener prÃ©stamo por ID (backend devuelve el objeto directamente, no { data })
  async getPrestamo(id: number): Promise<Prestamo> {
    return await apiClient.get<Prestamo>(`${this.baseUrl}/${id}`)
  }

  // Crear nuevo prÃ©stamo (backend devuelve el prÃ©stamo creado directamente)
  async createPrestamo(data: PrestamoForm): Promise<Prestamo> {
    return await apiClient.post<Prestamo>(this.baseUrl, data)
  }

  // --- Revisar PrÃ©stamos (prestamos_con_errores) ---

  /** Lista de prÃ©stamos enviados a revisiÃ³n (paginado) */
  ...
  /** Enviar una fila a Revisar PrÃ©stamos (desde carga masiva). */
  ...
  /** Eliminar de la lista Revisar PrÃ©stamos (marcar como resuelto) */
  async resolverPrestamoError(errorId: number): Promise<void> {
    await apiClient.delete(`${this.baseUrl}/revisar/${errorId}`)
  }

  /** Elimina de prestamos_con_errores tras descargar Excel (misma regla que Pagos). La lista se vacÃ­a y se rellena al enviar desde Carga Masiva. */
  ...
  // Actualizar prÃ©stamo (backend devuelve el prÃ©stamo actualizado directamente)
  ...
  // Buscar prÃ©stamos por cÃ©dula (normaliza: sin guiones, trim)
  ...
  /**
   * Batch: obtener prÃ©stamos para mÃºltiples cÃ©dulas en una sola peticiÃ³n.
   * Evita timeouts cuando hay muchas cÃ©dulas en carga masiva.
   */
  ...
  // Obtener resumen de prÃ©stamos por cÃ©dula (saldo, mora, etc.)
  ...
  // Evaluar riesgo de un prÃ©stamo
  ...
  // Obtener cuotas (tabla de amortizaciÃ³n) de un prÃ©stamo
  ...
  /**
   * Descarga la tabla de amortizaciÃ³n en Excel (vÃ­a API, sin depender de exceljs en el frontend).
   */
  ...
  /**
   * Reconciliar amortizaciÃ³n masiva: aplica pagos conciliados a cuotas
   * (Ãºtil tras regenerar tabla de amortizaciÃ³n). Si prestamoIds es vacÃ­o, procesa todos los que tengan pagos pendientes de aplicar.
   */
  ...
  // Aplicar condiciones de aprobaciÃ³n
  ...
  // Obtener KPIs de prÃ©stamos mensuales desde /prestamos/stats (mes/aÃ±o por defecto = actual)
  async getKPIs(filters?: {
    ...
    try {
      ...
      logger.error('getKPIs prestamos/stats fallÃ³', { error: err, params: { mes: filters?.mes, anio: filters?.anio } })
      const now = new Date()
      return {
  ...
  // Marcar/desmarcar prÃ©stamo como requiere revisiÃ³n
  ...
  // Asignar fecha de aprobaciÃ³n y recalcular tabla de amortizaciÃ³n
  ...
  // AprobaciÃ³n manual de riesgo (reemplaza evaluaciÃ³n 7 criterios + aplicar condiciones + asignar fecha)
  async aprobarManual(
    prestamoId: number,
    body: {
  ...
  // Obtener evaluaciÃ³n de riesgo de un prÃ©stamo
...
logger.info('Servicio de prÃ©stamos inicializado')
