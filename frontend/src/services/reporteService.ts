...
export interface ResumenDashboard {
  ...
  /** PrÃÂ©stamos con cuotas vencidas 90+ dÃÂ­as (backend: pagos_vencidos) */
...
class ReporteService {
  ...
  /**
   * Obtiene cuentas por cobrar por mes: por dÃÂ­a cuÃÂ¡ndo debe cobrar
   */
  ...
  /**
   * Obtiene pagos agrupados por mes/aÃÂ±o. Cada mes tiene lista ordenada descendente por fecha.
   */
   ...
   * @param filtros aÃÂ±os y meses seleccionados (para Excel)
   */
  async exportarReporteCartera(
    formato: 'excel' | 'pdf',
    fechaCorte?: string,
    filtros?: { aÃÂ±os: number[]; meses: number[] }
  ): Promise<Blob> {
    const params = new URLSearchParams({ formato })
    if (fechaCorte) params.set('fecha_corte', fechaCorte)
    if (filtros?.aÃÂ±os?.length) params.set('anos', filtros.aÃÂ±os.join(','))
  ...
  /**
   * Obtiene informe pago vencido por rangos de dÃÂ­as (1 dÃÂ­a, 15 dÃÂ­as, 30 dÃÂ­as, 2 meses, 90+ moroso)
   */
  ...
  /**
   * Obtiene asesores por mes: una pestaÃÂ±a por mes, orden descendente por morosidad
   */
  ...
  /**
   * Exporta reporte de pagos. Excel: una pestaÃÂ±a por mes (DÃÂ­a | Cantidad pagos | Cantidad cÃÂ©dulas | Monto).
   */
  ...
  /**
   * Exporta reporte Morosidad (clientes con cuotas 90+ dÃÂ­as): Nombre, CÃÂ©dula, Cant cuotas, Total USD
   */
  ...
  /**
   * Busca cÃÂ©dulas para filtrar reporte contable.
   */
   ...
   * Filtros: aÃÂ±os, meses, cedulas (opcional). PolÃÂ­tica: ÃÂºltimos 7 dÃÂ­as se actualizan.
   */
  async exportarReporteContable(
    ...
    const params = new URLSearchParams()
    if (aÃÂ±os.length) params.set('anos', aÃÂ±os.join(','))
  ...
  /**
   * Exporta reporte por cÃÂ©dula en Excel.
   * Columnas: ID prÃÂ©stamo | CÃÂ©dula | Nombre | Total financiamiento | Total abono | Cuotas totales | Cuotas pagadas | Cuotas atrasadas | Cuotas atrasadas ($).
   */
  ...
  /**
   * EnvÃ¯Â¿Â½a filas de conciliaciÃ¯Â¿Â½n (cedula, total_financiamiento, total_abonos, columna_e, columna_f) para guardar en BD temporal.
   */
  ...
  /**
   * Exporta reporte ConciliaciÃ¯Â¿Â½n en Excel. Al descargar se eliminan los datos temporales.
   */
