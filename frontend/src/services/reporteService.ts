...
export interface ResumenDashboard {
  ...
  /** PrÃ©stamos con cuotas vencidas 90+ dÃ­as (backend: pagos_vencidos) */
...
class ReporteService {
  ...
  /**
   * Obtiene cuentas por cobrar por mes: por dÃ­a cuÃ¡ndo debe cobrar
   */
  ...
  /**
   * Obtiene pagos agrupados por mes/aÃ±o. Cada mes tiene lista ordenada descendente por fecha.
   */
   ...
   * @param filtros aÃ±os y meses seleccionados (para Excel)
   */
  async exportarReporteCartera(
    formato: 'excel' | 'pdf',
    fechaCorte?: string,
    filtros?: { aÃ±os: number[]; meses: number[] }
  ): Promise<Blob> {
    const params = new URLSearchParams({ formato })
    if (fechaCorte) params.set('fecha_corte', fechaCorte)
    if (filtros?.aÃ±os?.length) params.set('anos', filtros.aÃ±os.join(','))
  ...
  /**
   * Obtiene informe pago vencido por rangos de dÃ­as (1 dÃ­a, 15 dÃ­as, 30 dÃ­as, 2 meses, 90+ moroso)
   */
  ...
  /**
   * Obtiene asesores por mes: una pestaÃ±a por mes, orden descendente por morosidad
   */
  ...
  /**
   * Exporta reporte de pagos. Excel: una pestaÃ±a por mes (DÃ­a | Cantidad pagos | Cantidad cÃ©dulas | Monto).
   */
  ...
  /**
   * Exporta reporte Morosidad (clientes con cuotas 90+ dÃ­as): Nombre, CÃ©dula, Cant cuotas, Total USD
   */
   ...
   * Filtros: aÃ±os, meses, cedulas (opcional). PolÃ­tica: Ãºltimos 7 dÃ­as se actualizan.
   */
  async exportarReporteContable(
    ...
    const params = new URLSearchParams()
    if (aÃ±os.length) params.set('anos', aÃ±os.join(','))
  ...
  /**
   * Exporta reporte por cÃ©dula en Excel.
   * Columnas: ID prÃ©stamo | CÃ©dula | Nombre | Total financiamiento | Total abono | Cuotas totales | Cuotas pagadas | Cuotas atrasadas | Cuotas atrasadas ($).
   */
  ...
  /**
   * Envï¿½a filas de conciliaciï¿½n (cedula, total_financiamiento, total_abonos, columna_e, columna_f) para guardar en BD temporal.
   */
  ...
  /**
   * Exporta reporte Conciliaciï¿½n en Excel. Al descargar se eliminan los datos temporales.
   */
