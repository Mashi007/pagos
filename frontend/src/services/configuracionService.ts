...
class ConfiguracionService {
  ...
  async obtenerValidadores(): Promise<ValidadoresConfig> {
    const response = await apiClient.get(
      '/api/v1/validadores/configuracion-validadores'
    )
  ...
  async probarValidadores(
    datosPrueba: PruebaValidadores
  ): Promise<ResultadoPrueba> {
    const response = await apiClient.post(
      `${this.baseUrl}/validadores/probar`,
      datosPrueba
    )
  ...
  async obtenerConfiguracionCompleta(categoria?: string): Promise<any> {
    ...
    const response = await apiClient.get(`${this.baseUrl}/sistema/completa`, {
      params,
    })
  ...
  async obtenerConfiguracionCategoria(categoria: string): Promise<any> {
    const response = await apiClient.get(
      `${this.baseUrl}/sistema/categoria/${categoria}`
    )
