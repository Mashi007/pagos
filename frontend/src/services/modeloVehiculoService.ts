...
export interface ModeloVehiculo {
  id: number
  modelo: string  // Ã¢ÂÂ CORREGIDO: campo 'modelo', no 'nombre'
  activo: boolean
  precio: number | null
  created_at: string
  updated_at?: string
...
export interface ModeloVehiculoCreate {
  modelo: string
  activo?: boolean
  precio?: number | null  // Opcional; si no se envÃÂ­a o es null, no se usa para Valor Activo hasta editarlo
}
...
class ModeloVehiculoService {
  ...
  // Listar modelos con paginaciÃÂ³n y filtros
  async listarModelos(params?: {
    ...
    try {
      console.log('Ã°ÂÂÂ ParÃÂ¡metros enviados:', params)
      const response = await apiClient.get<ModeloVehiculoListResponse>(this.baseUrl, { params })
      console.log('Ã°ÂÂÂ¦ Respuesta recibida:', response)
      return response
    } catch (error) {
      console.error('Ã¢ÂÂ Error en listarModelos:', error)
      throw error
    }
  }
  ...
  // MÃÂ©todo alias para compatibilidad
  ...
  // ImportaciÃÂ³n masiva desde Excel
