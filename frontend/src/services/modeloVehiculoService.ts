...
export interface ModeloVehiculo {
  id: number
  modelo: string  // â CORREGIDO: campo 'modelo', no 'nombre'
  activo: boolean
  precio: number | null
  created_at: string
  updated_at?: string
...
export interface ModeloVehiculoCreate {
  modelo: string
  activo?: boolean
  precio?: number | null  // Opcional; si no se envÃ­a o es null, no se usa para Valor Activo hasta editarlo
}
...
class ModeloVehiculoService {
  ...
  // Listar modelos con paginaciÃ³n y filtros
  async listarModelos(params?: {
    ...
    try {
      console.log('ð ParÃ¡metros enviados:', params)
      const response = await apiClient.get<ModeloVehiculoListResponse>(this.baseUrl, { params })
      console.log('ð¦ Respuesta recibida:', response)
      return response
    } catch (error) {
      console.error('â Error en listarModelos:', error)
      throw error
    }
  }
