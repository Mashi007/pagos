...
/** PrÃ©stamo con total financiamiento = total abonos (liquidado). */
...
/** Un registro del historial de envÃ­os por cÃ©dula (para reportes/legales). */
export interface HistorialEnvioItem {
  ...
  cedula: string
  exito: boolean
  /** Indicador de entrega: "entregado" (enviado correctamente) o "rebotado" (fallo en envÃ­o). */
...
class NotificacionService {
  ...
  /** Clientes retrasados por reglas: 5 dÃ­as, 3 dÃ­as, 1 dÃ­a, hoy, mora 90+. Datos reales desde BD. */
  ...
  /** KPIs por pestaÃ±a: enviados y rebotados (dias_5, dias_3, dias_1, hoy, mora_90). */
  ...
  /** Historial de notificaciones enviadas/fallidas por cÃ©dula (reportes y fines legales). */
  ...
  /** Descarga Excel con historial de notificaciones para una cÃ©dula. */
  ...
  /** Obtiene el HTML del comprobante de un envÃ­o (abrir en nueva ventana o guardar como PDF). */
  ...
  /** EnvÃ­a correos a clientes con 90+ dÃ­as de mora (moroso, email desde tabla clientes). */
   ...
   * El servidor responde 202 de inmediato; el envÃ­o continÃºa en background (evita timeout).
   * Respeta modo_pruebas: si estÃ¡ activo, todos los correos van al correo de pruebas.
   */
