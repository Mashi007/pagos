...
/** PrÃÂ©stamo con total financiamiento = total abonos (liquidado). */
...
/** Un registro del historial de envÃÂ­os por cÃÂ©dula (para reportes/legales). */
export interface HistorialEnvioItem {
  ...
  cedula: string
  exito: boolean
  /** Indicador de entrega: "entregado" (enviado correctamente) o "rebotado" (fallo en envÃÂ­o). */
...
export interface EmailConfig {
  ...
  imap_use_ssl?: string
  /** Contactos prestablecidos: emails separados por coma para notificaciÃÂ³n automÃÂ¡tica de tickets CRM */
...
/** Prefijo API v1; pestaÃÂ±as de notificaciones usan rutas propias (notificaciones-previas, etc.). */
const API_V1 = '/api/v1'
...
class NotificacionService {
  ...
  /** Clientes retrasados por reglas: 5 dÃÂ­as, 3 dÃÂ­as, 1 dÃÂ­a, hoy, mora 90+. Datos reales desde BD. */
  ...
  /** KPIs por pestaÃÂ±a: enviados y rebotados (dias_5, dias_3, dias_1, hoy, mora_90). */
  ...
  /** Lista de correos no entregados (rebotados) por tipo de pestaÃÂ±a. */
  ...
  /** Historial de notificaciones enviadas/fallidas por cÃÂ©dula (reportes y fines legales). */
  ...
  /** Descarga Excel con historial de notificaciones para una cÃÂ©dula. */
  ...
  /** Obtiene el HTML del comprobante de un envÃÂ­o (abrir en nueva ventana o guardar como PDF). */
  ...
  /** Ejecutar actualizaciÃÂ³n de notificaciones (dias_mora en clientes). Llamar desde cron a las 2am. */
  ...
  async listarNotificacionesPrevias(estado?: string): Promise<{ items: any[], total: number, dias_5: number, dias_3: number, dias_1: number }> {
    ...
    // Usar timeout extendido para este endpoint que puede tardar mÃÂ¡s
  ...
  /** EnvÃÂ­a correo a cada cliente en la clasificaciÃÂ³n indicada (email desde tabla clientes). */
  ...
  /** EnvÃÂ­a correos a clientes con 90+ dÃÂ­as de mora (moroso, email desde tabla clientes). */
  ...
  /**
   * Inicia el envÃÂ­o de todas las notificaciones en segundo plano.
   * El servidor responde 202 de inmediato; el envÃÂ­o continÃÂºa en background (evita timeout).
   * Respeta modo_pruebas: si estÃÂ¡ activo, todos los correos van al correo de pruebas.
   */
...
class EmailConfigService {
  ...
  async actualizarConfiguracionEmail(config: Partial<EmailConfig>): Promise<any> {
    ...
    try {
      // Usar timeout extendido para validaciÃÂ³n SMTP (puede tardar hasta 10-15 segundos)
      const resultado = await apiClient.put(
        `${this.baseUrl}/email/configuracion`,
        config,
        { timeout: 60000 } // 60 segundos para permitir validaciÃÂ³n SMTP
      )
      return resultado
    } catch (error) {
      throw error
