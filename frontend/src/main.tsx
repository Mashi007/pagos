...
if (!rootElement) {
  ...
  document.body.innerHTML =
    '<div style="padding: 20px; font-family: sans-serif;"><h1>Error de inicialización</h1><p>No se pudo encontrar el elemento raíz de la aplicación.</p></div>'
} else {
...
window.addEventListener(
  ...
  event => {
    ...
    if (event.error && !event.error.name?.includes('NS_ERROR_FAILURE')) {
      ...
      const isKnownError =
        errorMessage.includes('radix-ui') ||
    ...
    if (event.error && event.error.name === 'NS_ERROR_FAILURE') {
      ...
      const isRadixUIError =
        stackTrace.includes('radix-ui') ||
      ...
      const isUseStateError =
        stackTrace.includes('useState') ||
    ...
    if (
      event.error &&
      (event.error.message === 'The operation is insecure' ||
  ...
  true
) // ✅ Usar capture phase para interceptar errores antes de que se propaguen
...
window.addEventListener('unhandledrejection', event => {
  ...
  if (event.reason && event.reason.name === 'NS_ERROR_FAILURE') {
    ...
    const isRadixUIError =
      reasonMessage.includes('radix-ui') ||
