/**
 * Store de autenticaciÃ³n simplificado usando Zustand
 * Gestiona el estado de autenticaciÃ³n del usuario con persistencia segura
 */
...
// FunciÃ³n para verificar si hay datos de autenticaciÃ³n en almacenamiento
...
export const useSimpleAuthStore = create<SimpleAuthState>((set) => {
  ...
  return {
    ...
    isLoading: hasAuth, // â Si hay datos de auth, empezar como loading
    error: null,

  // Inicializar autenticaciÃ³n desde almacenamiento seguro CON VERIFICACIÃN AUTOMÃTICA
  initializeAuth: async () => {
    // â CRÃTICO: Marcar como loading al inicio para evitar redirecciones durante la verificaciÃ³n
    set({ isLoading: true })
    ...
    try {
      ...
      // Verificar que tambiÃ©n existe un token de acceso
      const token = rememberMe
        ? safeGetItem('access_token', null)
        : safeGetSessionItem('access_token', null)
      ...
      if (user && token) {
        // CRÃTICO: Siempre verificar con el backend al inicializar
        // â Agregar timeout para evitar bloqueos si el backend no responde
        try {
          const timeoutPromise = new Promise((_, reject) => {
            setTimeout(() => reject(new Error('Timeout: El servidor no respondiÃ³')), 8000) // 8 segundos
          })
          ...
          console.warn('Error al verificar autenticaciÃ³n:', error)
          clearAuthStorage()
          set({
            user: null,
            isAuthenticated: false,
      ...
      console.warn('Error al inicializar autenticaciÃ³n:', error)
      set({
        user: null,
        isAuthenticated: false,
        isLoading: false,
  ...
  login: async (credentials: LoginForm): Promise<void> => {
    ...
    try {
      ...
      if (error.response?.data) {
        ...
        // Si es un array (errores de validaciÃ³n), tomar el primer mensaje
  ...
  // REFRESCAR USUARIO DESDE BACKEND - SOLUCIÃN DEFINITIVA AL CACHE
  refreshUser: async () => {
    try {
      ...
      // NO hacer logout automÃ¡tico, solo mostrar error
      set({
