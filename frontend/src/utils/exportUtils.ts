...
/** Mensaje cuando falla la carga dinÃ¡mica del mÃ³dulo (chunk 404 tras deploy) */
const MSJ_RECARGA = 'Recarga la pÃ¡gina (F5) e intenta de nuevo. Si el problema persiste, contacta al administrador.'
...
export const exportarAPDF = async (cuotas: Cuota[], prestamo: PrestamoInfo) => {
  try {
    ...
    doc.text('INFORMACIÃN DEL PRÃâ°STAMO', 15, 55)
    ...
    doc.setFont('helvetica', 'bold')
    doc.setTextColor(255, 255, 255)
    doc.text('RESUMEN DE PRÃâ°STAMO', 15, resumenY + 7)
