...
const MSJ_RECARGA =
  'Recarga la página (F5) e intenta de nuevo. Si el problema persiste, contacta al administrador.'
...
export const exportarAExcel = async (
  ...
  try {
    ...
    const datos = cuotas.map(cuota => {
      ...
      const montoCuota =
        typeof cuota.monto_cuota === 'number' ? cuota.monto_cuota : 0

      const montoCapital =
        typeof cuota.monto_capital === 'number' && !isNaN(cuota.monto_capital)
          ? cuota.monto_capital
          : Math.max(0, saldoInicial - saldoFinal)

      const montoInteres =
        typeof cuota.monto_interes === 'number' && !isNaN(cuota.monto_interes)
          ? cuota.monto_interes
    ...
    const totalCapital = datos.reduce(
      (sum, d) => sum + (d.Capital as number),
      0
    )

    const totalInteres = datos.reduce(
      (sum, d) => sum + (d['Interés'] as number),
      0
    )
