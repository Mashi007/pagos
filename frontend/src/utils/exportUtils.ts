/**
 * Utilidades para exportar tablas de amortizaciÃÂ³n
 */

import { toast } from 'sonner'

/** Mensaje cuando falla la carga dinÃÂ¡mica del mÃÂ³dulo (chunk 404 tras deploy) */
const MSJ_RECARGA = 'Recarga la pÃÂ¡gina (F5) e intenta de nuevo. Si el problema persiste, contacta al administrador.'
...
/**
 * Exporta tabla de amortizaciÃÂ³n a Excel
 */
export const exportarAExcel = async (cuotas: Cuota[], prestamo: PrestamoInfo) => {
  try {
    // Importar dinÃÂ¡micamente exceljs
    ...
    const totalInteres = datos.reduce((sum, d) => sum + (d.InterÃÂ©s as number), 0)
    const totalGeneral = datos.reduce((sum, d) => sum + (d.Total as number), 0)

    const resumen = [
      { 'Cuota': 'RESUMEN', 'Fecha Vencimiento': '', 'Capital': '', 'InterÃÂ©s': '', 'Total': '', 'Saldo Pendiente': '', 'Estado': '' },
      { 'Cuota': '', 'Fecha Vencimiento': '', 'Capital': totalCapital, 'InterÃÂ©s': totalInteres, 'Total': totalGeneral, 'Saldo Pendiente': '', 'Estado': '' },
    ]
    ...
    await createAndDownloadExcel(todosLosDatos, 'Tabla de AmortizaciÃÂ³n', nombreArchivo)
  } catch (error) {
...
/**
 * Exporta tabla de amortizaciÃÂ³n a PDF (formato empresarial)
 */
export const exportarAPDF = async (cuotas: Cuota[], prestamo: PrestamoInfo) => {
  try {
    // Importar dinÃÂ¡micamente jsPDF y jspdf-autotable
    ...
    doc.text('Sistema de GestiÃÂ³n de PrÃÂ©stamos', 20, 28)
    ...
    doc.text('INFORMACIÃÂN DEL PRÃÂÃ¢ÂÂ°STAMO', 15, 55)

    doc.setFontSize(10)
    doc.setFont('helvetica', 'normal')
    doc.text(`Cliente: ${prestamo.nombres}`, 15, 65)
    doc.text(`CÃÂ©dula: ${prestamo.cedula}`, 15, 70)
    doc.text(`PrÃÂ©stamo #${prestamo.id}`, 110, 65)
    ...
    doc.setFontSize(12)
    doc.setFont('helvetica', 'bold')
    doc.text('TABLA DE AMORTIZACIÃÂN', 15, 92)
    ...
    autoTable(doc, {
      startY: 95,
      head: [['Cuota', 'Fecha Vencimiento', 'Capital', 'InterÃÂ©s', 'Total', 'Saldo Pendiente', 'Estado']],
      body: datosTabla,
      theme: 'striped',
      headStyles: {
    ...
    doc.setFont('helvetica', 'bold')
    doc.setTextColor(255, 255, 255)
    doc.text('RESUMEN DE PRÃÂÃ¢ÂÂ°STAMO', 15, resumenY + 7)
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.text(
        `RapiCredit - Sistema de PrÃÂ©stamos - PÃÂ¡gina ${i} de ${pageCount}`,
        105,
        290,
        { align: 'center' }
      )
