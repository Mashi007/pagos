/**
 * ValidaciÃÂ³n para carga masiva de prÃÂ©stamos desde Excel.
...
export function validatePrestamoField(
  ...
  switch (field) {
    case 'cedula':
      if (!strVal) return { isValid: false, message: 'CÃÂ©dula requerida' }
      const c = strVal.replace(/[:$]/g, '')
      return /^[VEJZ]\d{6,11}$/i.test(c) ? { isValid: true } : { isValid: false, message: 'Formato E/V/J/Z + 6-11 dÃÂ­gitos' }
    ...
    case 'fecha_requerimiento':
      ...
      if (dn < 1 || dn > 31) return { isValid: false, message: 'DÃÂ­a 1-31' }
      if (mn < 1 || mn > 12) return { isValid: false, message: 'Mes 1-12' }
      if (yn < 2020 || yn > 2030) return { isValid: false, message: 'AÃÂ±o 2020-2030' }
      return { isValid: true }
    ...
    case 'cuota_periodo':
      ...
      return Number.isNaN(cp) || cp < 0 ? { isValid: false, message: 'NÃÂºmero >= 0' } : { isValid: true }

    case 'tasa_interes':
      if (!strVal) return { isValid: true }
      const ti = parseFloat(strVal)
      return Number.isNaN(ti) || ti < 0 ? { isValid: false, message: 'NÃÂºmero >= 0' } : { isValid: true }
