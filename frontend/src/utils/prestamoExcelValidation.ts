/**
 * ValidaciÃ³n para carga masiva de prÃ©stamos desde Excel.
...
export function validatePrestamoField(
  ...
  switch (field) {
    ...
    case 'cuota_periodo':
      ...
      return Number.isNaN(cp) || cp < 0 ? { isValid: false, message: 'NÃºmero >= 0' } : { isValid: true }

    case 'tasa_interes':
      if (!strVal) return { isValid: true }
      const ti = parseFloat(strVal)
      return Number.isNaN(ti) || ti < 0 ? { isValid: false, message: 'NÃºmero >= 0' } : { isValid: true }
