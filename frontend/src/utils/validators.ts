...
export function formatearTelefonoParaMostrar(telefono: string): string {
  ...
  return (
    (telefono.startsWith('+') ? '' : '+') +
    digits.replace(/(\d{1,3})(?=\d)/g, '$1 ').trim()
  )
}
...
const PREFIJOS_NUMERO_TELEFONO = [
  '58',
  '57',
  '593',
  '52',
...
export function pareceNumeroTelefonoParaMeta(phoneNumberId: string): boolean {
  ...
  if (
    PREFIJOS_NUMERO_TELEFONO.some(
      prefix => digits === prefix || digits.startsWith(prefix)
    )
  )
    return true
...
export function validarPhoneNumberID(phoneNumberId: string): {
  valido: boolean
  error?: string
} {
  ...
  if (!/^\d+$/.test(limpio)) {
    return {
      valido: false,
      error:
        'Phone Number ID debe contener solo n├â┬â├é┬║meros (sin espacios ni caracteres especiales)',
    }
  }
  ...
  if (pareceNumeroTelefonoParaMeta(limpio)) {
    return {
      ...
      error:
        'Parece un n├â┬â├é┬║mero de tel├â┬â├é┬⌐fono (+58, +57, etc.). Debes usar el ID num├â┬â├é┬⌐rico de Meta (Business Suite ├â┬ó├é┬å├é┬Æ WhatsApp ├â┬ó├é┬å├é┬Æ tu n├â┬â├é┬║mero ├â┬ó├é┬å├é┬Æ ID), no el n├â┬â├é┬║mero con c├â┬â├é┬│digo de pa├â┬â├é┬¡s.',
    }
  }
...
export function validarRangoNumerico(
  ...
  if (isNaN(valorNum)) {
    return {
      valido: false,
      error: `${nombreCampo} debe ser un n├â┬â├é┬║mero v├â┬â├é┬ílido`,
    }
  }
...
export function validarConfiguracionGmail(config: {
  ...
  if (esGmail) {
    if (!config.smtp_password?.trim()) {
      errores.push(
        'Contrase├â┬â├é┬▒a es requerida (normal o de aplicaci├â┬â├é┬│n; en cuentas corporativas suele bastar la contrase├â┬â├é┬▒a normal)'
      )
    }
    ...
    if (puerto !== 587 && puerto !== 465) {
      errores.push(
        'Gmail/Google Workspace requiere puerto 587 (STARTTLS) o 465 (SSL). El puerto 587 es recomendado.'
      )
    }

    if (puerto === 587 && config.smtp_use_tls !== 'true') {
      errores.push(
        'Para puerto 587, TLS debe estar habilitado (requerido por Gmail/Google Workspace)'
      )
    }
...
export function validarConfiguracionImapGmail(config: {
  ...
  if (esGmailImap) {
    if (!config.imap_password?.trim()) {
      errores.push(
        'Contrase├â┬â├é┬▒a es requerida para IMAP (normal o de aplicaci├â┬â├é┬│n; en cuentas corporativas usa la contrase├â┬â├é┬▒a normal)'
      )
    }
    ...
    if (puerto !== 993 && puerto !== 143) {
      errores.push(
        'Gmail IMAP requiere puerto 993 (SSL) o 143 (STARTTLS). El puerto 993 es recomendado.'
      )
    }

    if (puerto === 143 && config.imap_use_ssl !== 'true') {
      errores.push(
        'Para puerto 143, TLS/STARTTLS debe estar habilitado (requerido por Gmail/Google Workspace)'
      )
    }
  }
...
export function validarConfiguracionWhatsApp(config: {
  ...
  if (!config.api_url?.trim()) {
    ...
    errores.push(
      'API URL debe ser la base de Meta Graph API (ej: https://graph.facebook.com/v18.0)'
    )
  }
  ...
  const phoneNumberIdValidacion = validarPhoneNumberID(
    config.phone_number_id || ''
  )

  if (!phoneNumberIdValidacion.valido) {
    errores.push(
      phoneNumberIdValidacion.error ||
        'Phone Number ID es requerido (solo n├â┬â├é┬║meros, desde Meta Business)'
    )
  }
