/**
 * Utilidades de validaciÃÂ³n comunes para el frontend
 * Centraliza validaciones para evitar cÃÂ³digo duplicado
 */
...
// ValidaciÃÂ³n de email
export function validarEmail(email: string): boolean {
  if (!email || typeof email !== 'string') return false
  return emailRegex.test(email.trim())
}

// ValidaciÃÂ³n de telÃÂ©fono
...
/**
 * Normaliza el valor mientras el usuario escribe un telÃÂ©fono.
 * - Solo permite dÃÂ­gitos y un + al inicio.
 * - Venezuela: 10 dÃÂ­gitos que empiezan por 4 (ej. 4241234567) o 0+10 dÃÂ­gitos (04241234567) Ã¢ÂÂ se antepone +58.
 * - Otros: cÃÂ³digo paÃÂ­s + nÃÂºmero (mÃÂ¡x. 15 dÃÂ­gitos). Resultado siempre con + al inicio.
 */
export function normalizarTelefonoParaIngreso(raw: string): string {
  if (raw == null) return ''
  let digits = String(raw).replace(/\D/g, '')
  if (digits.length === 0) return ''
  // Venezuela: 0 + 10 dÃÂ­gitos (04241234567) Ã¢ÂÂ quitar el 0
  if (digits.startsWith('0') && digits.length === 11) digits = digits.slice(1)
  // Venezuela: 10 dÃÂ­gitos empezando por 4 (mÃÂ³vil) Ã¢ÂÂ anteponer 58
...
/**
 * Formato legible para mostrar (ej. +584241234567 Ã¢ÂÂ +58 424 1234567).
 * ÃÂtil para placeholder o resumen; el input puede mostrar el valor normalizado sin espacios.
 */
...
// ValidaciÃÂ³n de nombre de empresa
...
// ValidaciÃÂ³n de idioma
export function validarIdioma(idioma: string): boolean {
  const idiomasValidos = ['ES', 'EN']
  return idiomasValidos.includes(idioma)
}

// CÃÂ³digos de paÃÂ­s comunes (sin +). Si el "Phone Number ID" empieza por estos y tiene 10-14 dÃÂ­gitos, es un nÃÂºmero de telÃÂ©fono, no el ID de Meta.
const PREFIJOS_NUMERO_TELEFONO = ['58', '57', '593', '52', '54', '51', '56', '598', '595', '55', '59']

/** True si el valor parece un nÃÂºmero de telÃÂ©fono y NO el Phone Number ID de Meta (ej. 1038026026054793). */
export function pareceNumeroTelefonoParaMeta(phoneNumberId: string): boolean {
  ...
  // NÃÂºmero con cÃÂ³digo de paÃÂ­s (58..., 57..., etc.)
  if (PREFIJOS_NUMERO_TELEFONO.some(prefix => digits === prefix || digits.startsWith(prefix))) return true
  // NÃÂºmero local sin cÃÂ³digo: 10 dÃÂ­gitos que empiezan por 4 (Venezuela mÃÂ³vil 424..., 414...) o 04 (0424...)
  if (digits.length === 10 && digits.startsWith('4')) return true
  if (digits.length === 11 && digits.startsWith('04')) return true
  return false
}

// ValidaciÃÂ³n de Phone Number ID (solo nÃÂºmeros; avisar si parece nÃÂºmero de telÃÂ©fono)
export function validarPhoneNumberID(phoneNumberId: string): { valido: boolean; error?: string } {
  ...
  if (!/^\d+$/.test(limpio)) {
    return { valido: false, error: 'Phone Number ID debe contener solo nÃÂºmeros (sin espacios ni caracteres especiales)' }
  }
  if (pareceNumeroTelefonoParaMeta(limpio)) {
    return {
      valido: false,
      error: 'Parece un nÃÂºmero de telÃÂ©fono (+58, +57, etc.). Debes usar el ID numÃÂ©rico de Meta (Business Suite Ã¢ÂÂ WhatsApp Ã¢ÂÂ tu nÃÂºmero Ã¢ÂÂ ID), no el nÃÂºmero con cÃÂ³digo de paÃÂ­s.'
    }
  }
  return { valido: true }
}

// ValidaciÃÂ³n de rango numÃÂ©rico
export function validarRangoNumerico(
  ...
  if (isNaN(valorNum)) {
    return { valido: false, error: `${nombreCampo} debe ser un nÃÂºmero vÃÂ¡lido` }
  }
...
// ValidaciÃÂ³n de configuraciÃÂ³n de Gmail (SMTP - envÃÂ­o de correo)
export function validarConfiguracionGmail(config: {
  ...
  if (!config.smtp_port?.trim()) {
    ...
    if (isNaN(puerto) || puerto < 1 || puerto > 65535) {
      errores.push('Puerto SMTP debe ser un nÃÂºmero entre 1 y 65535')
    }
  }
  if (!config.smtp_user?.trim()) {
    errores.push('Email de usuario es requerido')
  } else if (!validarEmail(config.smtp_user)) {
    errores.push('Email de usuario no es vÃÂ¡lido')
  }
  if (!config.from_email?.trim()) {
    errores.push('Email del remitente es requerido')
  } else if (!validarEmail(config.from_email)) {
    errores.push('Email del remitente no es vÃÂ¡lido')
  }

  // Validaciones especÃÂ­ficas para Gmail/Google (normas SMTP oficiales)
  if (esGmail) {
    if (!config.smtp_password?.trim()) {
      errores.push('ContraseÃÂ±a es requerida (normal o de aplicaciÃÂ³n; en cuentas corporativas suele bastar la contraseÃÂ±a normal)')
    }
    ...
    // Puerto 465 usa SSL implÃÂ­cito; no requiere checkbox TLS adicional
  }
...
// ValidaciÃÂ³n de configuraciÃÂ³n IMAP para Google (recibir correo)
...
export function validarConfiguracionImapGmail(config: {
  ...
  if (!config.imap_port?.trim()) {
    ...
    if (isNaN(puerto) || puerto < 1 || puerto > 65535) {
      errores.push('Puerto IMAP debe ser un nÃÂºmero entre 1 y 65535')
    }
  }
  if (!config.imap_user?.trim()) {
    errores.push('Email de usuario IMAP es requerido')
  } else if (!validarEmail(config.imap_user)) {
    errores.push('Email de usuario IMAP no es vÃÂ¡lido')
  }

  if (esGmailImap) {
    if (!config.imap_password?.trim()) {
      errores.push('ContraseÃÂ±a es requerida para IMAP (normal o de aplicaciÃÂ³n; en cuentas corporativas usa la contraseÃÂ±a normal)')
    }
...
// - EnvÃÂ­o: POST {api_url}/{phone_number_id}/messages | Auth: Bearer {access_token}
// - Phone Number ID: solo dÃÂ­gitos (ID numÃÂ©rico de Meta)
...
// ValidaciÃÂ³n de configuraciÃÂ³n de WhatsApp (Meta Cloud API)
export function validarConfiguracionWhatsApp(config: {
  ...
  if (!config.api_url?.trim()) {
    ...
    errores.push('API URL no es vÃÂ¡lida')
  ...
  if (!phoneNumberIdValidacion.valido) {
    errores.push(phoneNumberIdValidacion.error || 'Phone Number ID es requerido (solo nÃÂºmeros, desde Meta Business)')
  }
