...
export function validateExcelFile(file: File): ExcelValidationResult {
  ...
  // 1. Validar extensiÃ³n del archivo
  const fileExtension = file.name.toLowerCase().substring(file.name.lastIndexOf('.'))
  if (!ALLOWED_EXTENSIONS.includes(fileExtension)) {
    return {
      isValid: false,
      error: `ExtensiÃ³n de archivo no permitida. Solo se permiten: ${ALLOWED_EXTENSIONS.join(', ')}`
    }
  }

  // 2. Validar tipo MIME (si estÃ¡ disponible)
  if (file.type && !ALLOWED_MIME_TYPES.includes(file.type)) {
    warnings.push(`Tipo MIME inesperado: ${file.type}. Continuando con validaciÃ³n de extensiÃ³n.`)
  }
  ...
  if (file.size > MAX_FILE_SIZE) {
    return {
      isValid: false,
      error: `El archivo es demasiado grande. TamaÃ±o mÃ¡ximo: ${(MAX_FILE_SIZE / 1024 / 1024).toFixed(0)} MB. TamaÃ±o actual: ${(file.size / 1024 / 1024).toFixed(2)} MB`
    }
  }

  // 4. Validar que el archivo no estÃ© vacÃ­o
  if (file.size === 0) {
    return {
      isValid: false,
...
/**
 * Valida la estructura del workbook despuÃ©s de leerlo
 */
export function validateWorkbookStructure(workbook: any): ExcelValidationResult {
  ...
  if (!workbook.SheetNames || workbook.SheetNames.length === 0) {
    return {
      isValid: false,
      error: 'El archivo Excel no contiene hojas de cÃ¡lculo'
    }
  }

  // 2. Validar nÃºmero de hojas (limitar a 10)
  if (workbook.SheetNames.length > 10) {
    warnings.push(`El archivo tiene ${workbook.SheetNames.length} hojas. Solo se procesarÃ¡ la primera.`)
  }
  ...
  if (range) {
    ...
    if (startCell && endCell) {
      ...
      if (numRows > MAX_ROWS) {
        return {
          isValid: false,
          error: `El archivo tiene demasiadas filas (${numRows}). MÃ¡ximo permitido: ${MAX_ROWS}`
        }
      }
      ...
      if (numCols > MAX_COLUMNS) {
        return {
          isValid: false,
          error: `El archivo tiene demasiadas columnas (${numCols}). MÃ¡ximo permitido: ${MAX_COLUMNS}`
        }
      }
...
/**
 * Valida los datos extraÃ­dos antes de procesarlos
 */
export function validateExcelData(data: any[]): ExcelValidationResult {
  ...
  // 2. Validar nÃºmero de filas
  if (data.length > MAX_ROWS) {
    return {
      isValid: false,
      error: `El archivo contiene demasiadas filas de datos (${data.length}). MÃ¡ximo permitido: ${MAX_ROWS}. Recomendado hasta ${MAX_ROWS_RECOMENDADO_CARGA_MASIVA} para evitar sobrecarga.`
    }
  }
  ...
  if (maxRowLength > MAX_COLUMNS) {
    warnings.push(`Algunas filas tienen mÃ¡s de ${MAX_COLUMNS} columnas. Solo se procesarÃ¡n las primeras ${MAX_COLUMNS}.`)
  }
...
export function sanitizeCellValue(value: any): string {
  ...
  // Remover caracteres de control (excepto saltos de lÃ­nea y tabs)
...
/** Normaliza cÃ©dula para validaciÃ³n y comparaciÃ³n: quita espacios, dos puntos, caracteres de ancho completo (ï¼-ï¼â0-9) y caracteres de ancho cero. Evita que reescribir el mismo valor cambie el resultado. */
...
/** Normaliza telÃ©fono al subir archivo: quita guiones/espacios, 0 inicial y prefijo 58. Devuelve 10 dÃ­gitos para Venezuela. */
...
export function validateField(field: string, value: string, options?: ValidateFieldOptions): ValidationResult {
  ...
  switch (field) {
    case 'cedula': if (!value.trim()) return { isValid: true }; const c = normalizeCedulaInput(value); return /^[VEJ]\d{6,11}$/.test(c) ? { isValid: true } : { isValid: false, message: 'Formato V/E/J + 6-11 dÃ­gitos (no se admite Z)' }
    ...
    case 'telefono': if (!value?.trim()) return { isValid: false, message: 'TelÃ©fono requerido' }; let d=(value||'').replace(/\D/g,''); if (d.startsWith('58')&&d.length>=11) d=d.slice(2); if (d.length>10) return { isValid: true }; if (d.length!==10) return { isValid: false, message: '10 dÃ­gitos' }; return /^[1-9]\d{9}$/.test(d) ? { isValid: true } : { isValid: false, message: '10 dÃ­gitos sin 0 inicial' }
    ...
    case 'direccion': if (!value.trim()) return { isValid: false, message: 'DirecciÃ³n requerida' }; return value.trim().length>=5 ? { isValid: true } : { isValid: false, message: 'MÃ­nimo 5 caracteres' }
    ...
    case 'fecha_nacimiento': if (!value.trim()) return { isValid: false, message: 'Fecha requerida' }; const fm = value.trim().match(/^(\d{1,2})\/(\d{1,2})\/(\d{4})$/); if (!fm) return { isValid: false, message: 'Formato DD/MM/YYYY' }; const dn=parseInt(fm[1],10), mnn=parseInt(fm[2],10), yn=parseInt(fm[3],10); if (dn<1||dn>31) return { isValid: false, message: 'DÃ­a 1-31' }; if (mnn<1||mnn>12) return { isValid: false, message: 'Mes 1-12' }; if (yn<1900||yn>2100) return { isValid: false, message: 'AÃ±o 1900-2100' }; const fd = new Date(yn, mnn-1, dn); if (fd.getDate()!==dn||fd.getMonth()!==mnn-1||fd.getFullYear()!==yn) return { isValid: false, message: 'Fecha invÃ¡lida' }; const hoy = new Date(); hoy.setHours(0,0,0,0); if (fd>=hoy) return { isValid: false, message: 'No futura' }; if (new Date(yn+18, mnn-1, dn) > hoy) return { isValid: false, message: 'MÃ­nimo 18 aÃ±os' }; return { isValid: true }
    case 'ocupacion': if (!value.trim()) return { isValid: false, message: 'OcupaciÃ³n requerida' }; return value.trim().length>=2 ? { isValid: true } : { isValid: false, message: 'MÃ­nimo 2 caracteres' }
