...
import {
  ...
  parsePrestamoIdFromNumeroCredito } from '../utils/pagoExcelValidation'
...
function observacionesDesdeError(msg: string): string {
  ...
  if (/monto|amount|debe ser|greater|positive/i.test(msg)) partes.push(OBSERVACIONES_POR_CAMPO.monto_pagado)

  if (/prestamo_id|prestamo.*entre|entre 1 y|id fuera|numero/i.test(msg)) {
    partes.push(OBSERVACIONES_POR_CAMPO.prestamo_id)
  } else if (/prestamo|credito|credito\/id/i.test(m)) partes.push(OBSERVACIONES_POR_CAMPO.prestamo_id)
  ...
  return partes.length > 0 ? partes.join(' / ') : 'Revisar (error de validacion)';
...
export function useExcelUploadPagos({ onClose, onSuccess }: ExcelUploaderPagosProps) {
    ...
    const timer = setTimeout(() => {
      ...
      const cedulasFaltantes = cedulasUnicas.filter((cedula) => {
        ...
        const hasData =
          ...
          (prestamosPorCedula[cedula] || prestamosPorCedula[cedula?.replace(/-/g, '')] || []).length > 0
      ...
      prestamoService
        ...
        .then((batch) => {
          ...
          cedulasFaltantes.forEach((cedula) => {
            ...
            const prestamos = (batch[cedula] || batch[cedula.replace(/-/g, '')] || []).filter(





              (p: any) => p && ESTADOS_PRESTAMO_ACTIVO.includes((p.estado || '').toUpperCase())
  ...
  const saveRowIfValid = useCallback(
    ...
    async (row: PagoExcelRow): Promise<boolean> => {
      ...
      const currentRow = excelData.find((r) => r._rowIndex === row._rowIndex) ? row
      ...
      try {
        ...
        if (prestamoId != null && prestamosActivos.length > 0 && !prestamosActivos.some((p) => p.id === prestamoId)) {





          prestamoId = prestamosActivos.length === 1 ? prestamosActivos[0].id : null
  ...
  const saveIndividualPago = useCallback(
    ...
    async (row: PagoExcelRow, opts?: { skipToast?: boolean; skipRefresh?: boolean }): Promise<{ ok: boolean; was409?: boolean }> => {
      ...
      try {
        ...
        if (prestamoId != null && (prestamoId < 1 || prestamoId > PRESTAMO_ID_MAX)) {
          ...
          addToast('error', 'Fila ' + row._rowIndex + ': El valor del credito no es valido.')
        ...
        if (is409) {
          ...
          const cedulaR = cedulaLookupParaFila(row.cedula || '', row.numero_documento || '')





          const prestamosR = prestamosPorCedula[cedulaR] || prestamosPorCedula[cedulaR?.replace(/-/g, '')] || []





          const prestamoIdRevisar = row.prestamo_id != null && prestamosR.some((p) => p.id === row.prestamo_id) ? row.prestamo_id : null
    ...
    [excelData, prestamosPorCedula, addToast, refreshPagos, onSuccess, onClose, getValidRows, queryClient]
  ...
  const sendAllToRevisarPagos = useCallback(async () => {
    ...
    else if (ok > 0 && fail > 0) addToast('warning', `${ok} enviado(s), ${fail} con errores.`)
  ...
  const saveAllValid = useCallback(async () => {
    ...
    const buildPagoData = (row: PagoExcelRow) => {
      ...
      if (!prestamoId && prestamosActivos.length === 1) prestamoId = prestamosActivos[0].id
    ...
    if (toSave.length >= 2) {
      ...
      try {
        ...
        for (const row of toSave) {
          ...
          if (result.ok) { ok++; indicesGuardadosEstaRonda.add(row._rowIndex) }





          else { fail++; if (result.was409) duplicados++ }
    ...
    if (fail === 0 && ok > 0 && !quedanConErrores && !quedanSinGuardar) {
      ...
      addToast('warning', 'Quedan filas pendientes. Use "Revisar Pagos" o envie a revisar.')
      ...
      try {
        ...
        const cols = (() => {
          ...
          for (let i = 0; i < Math.max(headerRow.length, 8); i++) {
            ...
            if (match(i, 'documento', 'numero_documento', 'numero documento', 'n documento', 'doc', 'referencia', 'numero doc', 'zelle')) documento = i





            if (match(i, 'prestamo', 'credito', 'prestamo_id', 'credito id', 'crédito')) prestamo = i
        ...
        for (let i = 1; i < jsonData.length; i++) {
          ...
          let prestamoId: number | null =
            ...
            !isConciliacionCol4 && prestamoIdRaw != null && String(prestamoIdRaw).trim() !== ''
              ...
              ? (parsePrestamoIdFromNumeroCredito(prestamoIdRaw) ? parseInt(String(prestamoIdRaw).trim(), 10))
    ...
    setExcelData((prev) =>
      ...
      prev.map((r) => {
        ...
        if (field === 'prestamo_id') {
          ...
          const parsed = value === '' || value === 'none' ? null : (parsePrestamoIdFromNumeroCredito(value) ? (Number(value) || null))
        ...
        if (field === 'numero_documento') {
          ...
          const validation = validatePagoField('numero_documento', (updated as any).numero_documento, { documentosEnArchivo, documentosDuplicadosBD: documentosDuplicadosBDRef.current })
          ...
          prev.forEach((other) => {
            ...
            const ced = (other._rowIndex === row._rowIndex ? String(value ? '').trim().replace(/-/g, '').toUpperCase() : (other.cedula || '').trim().replace(/-/g, '').toUpperCase())
          ...
          const vCedula = validatePagoField('cedula', (updated as any).cedula, { cedulasInvalidas })
          ...
          updated._hasErrors = !vCedula.isValid || !r._validation.fecha_pago?.isValid || !r._validation.monto_pagado?.isValid || !r._validation.numero_documento?.isValid
          ...
          const vFecha = validatePagoField('fecha_pago', (updated as any).fecha_pago)
          ...
          updated._hasErrors = !r._validation.cedula?.isValid || !vFecha.isValid || !r._validation.monto_pagado?.isValid || !r._validation.numero_documento?.isValid
          ...
          const vMonto = validatePagoField('monto_pagado', (updated as any).monto_pagado)
          ...
          updated._hasErrors = !r._validation.cedula?.isValid || !r._validation.fecha_pago?.isValid || !vMonto.isValid || !r._validation.numero_documento?.isValid
          ...
          updated._hasErrors = !updated._validation.cedula?.isValid || !updated._validation.fecha_pago?.isValid || !updated._validation.monto_pagado?.isValid || !updated._validation.numero_documento?.isValid
