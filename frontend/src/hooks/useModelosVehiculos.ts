...
// Hook para obtener modelos de vehÃ­culos activos (solo cuando hay sesiÃ³n vÃ¡lida; evita "SesiÃ³n expirada" en login)
  ...
  return useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        if (error?.message?.includes?.('SesiÃ³n expirada')) return []
