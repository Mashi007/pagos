/**
 * Hook para carga masiva de prÃÂ©stamos desde Excel.
 * La cÃÂ©dula debe existir en la tabla clientes; si no existe, la fila se marca como error y no se puede guardar.
 */
...
export function useExcelUploadPrestamos({ onClose, onSuccess }: ExcelUploaderPrestamosProps) {
  ...
  const sendToRevisarPrestamos = useCallback(
    async (row: PrestamoExcelRow, skipToast = false, skipStateUpdate = false): Promise<boolean> => {
      ...
      try {
        ...
        if (!skipToast) addToast(row._hasErrors ? 'warning' : 'success', `Fila ${row._rowIndex} enviada a Revisar PrÃÂ©stamos`)
  ...
  const sendAllToRevisarPrestamos = useCallback(async () => {
    ...
    if (rows.length === 0) {
      addToast('warning', 'No hay filas para enviar a Revisar PrÃÂ©stamos')
      return
    }
    ...
    if (ok > 0 && fail === 0) addToast('success', `Ã¢ÂÂ ${ok} fila(s) enviada(s) a Revisar PrÃÂ©stamos`)
  ...
  const saveIndividualPrestamo = useCallback(
    async (row: PrestamoExcelRow): Promise<boolean> => {
      if (row._hasErrors) {
        addToast('error', 'Hay errores en esta fila. CorrÃÂ­gelos antes de guardar.')
        return false
      }
      setSavingProgress((prev) => ({ ...prev, [row._rowIndex]: true }))
      try {
        ...
        if (!cliente) {
          addToast('error', `CÃÂ©dula ${cedulaNorm} no existe en tabla clientes. Registre al cliente primero.`)
          return false
        }
            ...
            prev.map((r) =>
              r._rowIndex === row._rowIndex
                ? {
                    ...
                    _validation: {
                      ...r._validation,
                      fecha_requerimiento: { isValid: false, message: 'fecha_requerimiento invÃÂ¡lida o vacÃÂ­a' },
                    },
                  }
                : r
            )
          )
          addToast('error', `Fila ${row._rowIndex}: fecha_requerimiento invÃÂ¡lida o vacÃÂ­a`)
          return false
        }
        ...
        setExcelData((prev) => prev.filter((r) => r._rowIndex !== row._rowIndex))
        refreshPrestamos()
        addToast('success', `PrÃÂ©stamo ${row.cedula} guardado`)
  ...
  const saveAllValid = useCallback(async () => {
    const valid = getValidRows()
    if (valid.length === 0) {
      addToast('warning', 'No hay prÃÂ©stamos vÃÂ¡lidos para guardar')
      return
    }
    if (serviceStatus === 'offline') {
      addToast('error', 'Sin conexiÃÂ³n')
    ...
    if (ok > 0) addToast('success', `${ok} prÃÂ©stamos guardados`)
    if (fail > 0) addToast('error', `${fail} fallaron`)
    setIsSavingIndividual(false)
    // Si se guardaron todos sin fallos y no hay problemas con validadores Ã¢ÂÂ volver a /pagos/prestamos
    if (fail === 0 && ok > 0) {
      onSuccess?.()
      onClose()
    }
    ...
    async (file: File) => {
      ...
      try {
        ...
        if (data.byteLength > 10 * 1024 * 1024) {
          alert('Archivo demasiado grande. MÃÂ¡ximo 10 MB')
          return
        }
        ...
        for (let i = 1; i < jsonData.length; i++) {
          ...
          if (!/^\d{4}-\d{2}-\d{2}$/.test(fechaBackend)) {
            rowData._validation.fecha_requerimiento = {
              isValid: false,
              message: 'fecha_requerimiento invÃÂ¡lida o vacÃÂ­a',
            }
            hasErrors = true
          }
        ...
        // Validar que cada cÃÂ©dula exista en tabla clientes (solo aceptar si estÃÂ¡ en BD)
          ...
          try {
            ...
            for (const row of processed) {
              ...
              if (cedNorm && !setExistentes.has(cedNorm)) {
                row._validation.cedula = { isValid: false, message: 'CÃÂ©dula no existe en tabla clientes' }
                row._hasErrors = true
              }
            }
          } catch (_e) {
            // Si falla la API, no marcar todas como error; el guardado individual validarÃÂ¡
          }
        }
