  ...
  const saveIndividualPrestamo = useCallback(
    async (row: PrestamoExcelRow): Promise<boolean> => {
      ...
      try {
        ...
        const fechaRequerimientoBackend = restarUnDiaYmd(fechaAprobacionBackend)
        ...
        const prestamoData = {
          ...
          // Regla única: fecha_requerimiento se deriva desde fecha_aprobacion.
    ...
    (row: PrestamoExcelRow, field: string, value: string | number) => {
      setExcelData(prev =>
        prev.map(r => {
          ...
          updated._validation = {
            ...updated._validation,
            fecha_aprobacion: validatePrestamoField(
              'fecha_aprobacion',
