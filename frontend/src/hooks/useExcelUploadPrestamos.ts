/**
 * Hook para carga masiva de prÃ©stamos desde Excel.
 * La cÃ©dula debe existir en la tabla clientes; si no existe, la fila se marca como error y no se puede guardar.
 */
...
export function useExcelUploadPrestamos({ onClose, onSuccess }: ExcelUploaderPrestamosProps) {
  ...
  const sendToRevisarPrestamos = useCallback(
    async (row: PrestamoExcelRow, skipToast = false, skipStateUpdate = false): Promise<boolean> => {
      ...
      try {
        ...
        if (!skipToast) addToast(row._hasErrors ? 'warning' : 'success', `Fila ${row._rowIndex} enviada a Revisar PrÃ©stamos`)
  ...
  const sendAllToRevisarPrestamos = useCallback(async () => {
    ...
    if (rows.length === 0) {
      addToast('warning', 'No hay filas para enviar a Revisar PrÃ©stamos')
      return
    }
    ...
    if (ok > 0 && fail === 0) addToast('success', `â ${ok} fila(s) enviada(s) a Revisar PrÃ©stamos`)
  ...
  const saveIndividualPrestamo = useCallback(
    async (row: PrestamoExcelRow): Promise<boolean> => {
      ...
      try {
        ...
        if (!cliente) {
          addToast('error', `CÃ©dula ${cedulaNorm} no existe en tabla clientes. Registre al cliente primero.`)
          return false
        }
            ...
            prev.map((r) =>
              r._rowIndex === row._rowIndex
                ? {
                    ...
                    _validation: {
                      ...r._validation,
                      fecha_requerimiento: { isValid: false, message: 'fecha_requerimiento invÃ¡lida o vacÃ­a' },
                    },
                  }
                : r
            )
          )
          addToast('error', `Fila ${row._rowIndex}: fecha_requerimiento invÃ¡lida o vacÃ­a`)
          return false
        }
        ...
        setExcelData((prev) => prev.filter((r) => r._rowIndex !== row._rowIndex))
        refreshPrestamos()
        addToast('success', `PrÃ©stamo ${row.cedula} guardado`)
  ...
  const saveAllValid = useCallback(async () => {
    const valid = getValidRows()
    if (valid.length === 0) {
      addToast('warning', 'No hay prÃ©stamos vÃ¡lidos para guardar')
      return
    }
    if (serviceStatus === 'offline') {
      addToast('error', 'Sin conexiÃ³n')
    ...
    if (ok > 0) addToast('success', `${ok} prÃ©stamos guardados`)
    if (fail > 0) addToast('error', `${fail} fallaron`)
    setIsSavingIndividual(false)
    // Si se guardaron todos sin fallos y no hay problemas con validadores â volver a /pagos/prestamos
    if (fail === 0 && ok > 0) {
      onSuccess?.()
      onClose()
    }
  ...
  const processExcelFile = useCallback(
    async (file: File) => {
      ...
      try {
        ...
        for (let i = 1; i < jsonData.length; i++) {
          ...
          if (!/^\d{4}-\d{2}-\d{2}$/.test(fechaBackend)) {
            rowData._validation.fecha_requerimiento = {
              isValid: false,
              message: 'fecha_requerimiento invÃ¡lida o vacÃ­a',
            }
            hasErrors = true
          }
        ...
        // Validar que cada cÃ©dula exista en tabla clientes (solo aceptar si estÃ¡ en BD)
          ...
          try {
            ...
            for (const row of processed) {
              ...
              if (cedNorm && !setExistentes.has(cedNorm)) {
                row._validation.cedula = { isValid: false, message: 'CÃ©dula no existe en tabla clientes' }
                row._hasErrors = true
              }
            }
          } catch (_e) {
            // Si falla la API, no marcar todas como error; el guardado individual validarÃ¡
          }
        }
