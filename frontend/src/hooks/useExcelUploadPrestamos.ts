...
export function useExcelUploadPrestamos({
  onClose,
  onSuccess,
}: ExcelUploaderPrestamosProps) {
      ...
      try {
        ...
        if (!skipToast)
          addToast(
            ...
            `Fila ${row._rowIndex} enviada a Revisar Préstamos`
          )
  ...
  const sendAllToRevisarPrestamos = useCallback(async () => {
    ...
    for (let i = 0; i < rows.length; i += CHUNK) {
      ...
      const results = await Promise.allSettled(
        chunk.map(row => sendToRevisarPrestamos(row, true, true))
      )
    ...
    if (ok > 0 && fail === 0)
      addToast('success', `✓ ${ok} fila(s) enviada(s) a Revisar Préstamos`)
    else if (ok > 0 && fail > 0)
  ...
  const saveIndividualPrestamo = useCallback(
    async (row: PrestamoExcelRow): Promise<boolean> => {
      if (row._hasErrors) {
        addToast(
          'error',
          'Hay errores en esta fila. Corrígelos antes de guardar.'
        )
      ...
      try {
        ...
        if (!cliente) {
          addToast(
            'error',
            `Cédula ${cedulaNorm} no existe en tabla clientes. Registre al cliente primero.`
          )

          return false
        }

        const fechaRequerimientoBackend = convertirFechaParaBackendPrestamo(
          row.fecha_requerimiento
        )

        const fechaValida = /^\d{4}-\d{2}-\d{2}$/.test(
          fechaRequerimientoBackend
        )
        ...
        if (!fechaValida) {
          ...
          addToast(
            'error',
            `Fila ${row._rowIndex}: fecha_requerimiento inválida o vacía`
          )
        ...
        const numCuotas = Math.min(
          50,
          Math.max(1, Math.round(Number(row.numero_cuotas) || 12))
        )

        const cuotaPeriodo =
          row.cuota_periodo != null && Number(row.cuota_periodo) > 0
        ...
        const prestamoData = {
          ...
          modalidad_pago: (row.modalidad_pago || 'MENSUAL').toUpperCase() as
            | 'MENSUAL'
            | 'QUINCENAL'
            | 'SEMANAL',
      ...
      try {
        ...
        for (let i = 1; i < jsonData.length; i++) {
          ...
          for (const field of requiredFields) {
            ...
            const validation = validatePrestamoField(
              field,
          ...
          const fechaBackend = convertirFechaParaBackendPrestamo(
            rowData.fecha_requerimiento
          )
          ...
          rowData._validation.cuota_periodo = validatePrestamoField(
            'cuota_periodo',
            rowData.cuota_periodo
          )

          rowData._validation.tasa_interes = validatePrestamoField(
            'tasa_interes',
            rowData.tasa_interes
          )
            ...
            for (const row of processed) {
              ...
              if (cedNorm && !setExistentes.has(cedNorm)) {
                row._validation.cedula = {
                  isValid: false,
                  message: 'Cédula no existe en tabla clientes',
                }
  ...
  const updateCellValue = useCallback(
    (row: PrestamoExcelRow, field: string, value: string | number) => {
      setExcelData(prev =>
        prev.map(r => {
