...
export function useExcelUpload({
  ...
  const [showModalCedulasExistentes, setShowModalCedulasExistentes] =
    useState(false)
  ...
  const addToast = useCallback(
    (
      ...
      setToasts(prev => [
        ...prev,
        { id, type, message, suggestion, field, rowIndex },
      ])
  ...
  const getDuplicadoMotivo = useCallback(
    (row: ExcelRow): string[] => {
      ...
      if (
        (row.nombres || '').trim() &&
        nombresDuplicadosEnArchivo.has((row.nombres || '').trim())
      )
  ...
  const getValidClients = useCallback((): ExcelRow[] => {
    return excelData.filter(
      row => isClientValid(row) && !savedClients.has(row._rowIndex)
    )
  ...
  const getSavedClientsCount = useCallback(
    () => savedClients.size,
    [savedClients]
  )
      ...
      try {
        ...
        try {
          ...
          if (err.response?.status === 400 || err.response?.status === 409) {
            ...
            addToast(
              'error',
              '⚠️ SERVICIO NO DISPONIBLE: El backend está caído. Contacta al administrador.'
            )
          } else if (
            ...
            addToast(
              'error',
              '⚠️ ERROR DE RED: No se puede conectar al servidor. Verifica tu conexión.'
            )
          } else if (
            err.response &&
            err.response.status &&
            ...
            addToast(
              'error',
              `Error guardando cliente en fila ${row._rowIndex}: ${msg}`
            )
          }
        ...
        setExcelData(prev => {
          ...
          if (remaining.length === 0) {
            addToast(
              'success',
              '¡Todos los clientes han sido guardados exitosamente!'
            )
            ...
            addToast(
              'success',
              'Redirigiendo al Dashboard de Clientes en 2 segundos...'
            )
        ...
        if (err.response?.status === 503) {
          ...
          addToast(
            'error',
            `Error de validación: ${err.response?.data?.detail || err.message}`
          )
        } else if (
          err.response &&
          err.response.status &&
          ...
          addToast(
            'error',
            `Error guardando cliente: ${err.response?.data?.detail || err.message}`
          )
        }
    ...
    async (clientsToSave: ExcelRow[]) => {
        ...
        for (const client of chunk) {
          try {
            ...
            if (err.response?.status === 409) {
              addToast(
                'warning',
                `Cliente ${client.cedula} (${client.nombres}) duplicado - se omitió`
              )
            } else {
              addToast(
                'error',
                `Error guardando ${client.cedula}: ${err.message || 'Error desconocido'}`
              )
            }
          }
        }
        ...
        if (chunkOk.length > 0) {
          setExcelData(prev => {
            ...
            if (remaining.length === 0) {
              addToast(
                'success',
                '¡Todos los clientes guardados! Cerrando en 2 segundos...'
              )
      ...
      if (successful > 0) {
        ...
        if (failed === 0)
          ...
          addToast(
            'warning',
            `✓ ${successful} guardados | ✗ ${failed} fallo(s)`
          )
      }

      if (failed > 0)
        addToast('error', `${failed} cliente(s) fallaron al guardar`)
    },
  ...
  const saveAllValidClients = useCallback(async () => {
    ...
    if (serviceStatus === 'offline') {
      addToast(
        'error',
        'Sin conexión. No se puede comprobar cédulas ni guardar.'
      )
    ...
    try {
      ...
      const { existing_cedulas } =
        await clienteService.checkCedulas(cedulasToCheck)
      ...
      if (err.response?.status === 503) {
        addToast(
          'error',
          'SERVICIO NO DISPONIBLE: El backend está caído. Contacta al administrador.'
        )
      } else if (
        err.code === 'ERR_NETWORK' ||
        err.message?.includes('Network Error')
  ...
  const sendAllToRevisarClientes = useCallback(async () => {
    ...
    for (let i = 0; i < rows.length; i += CHUNK) {
      ...
      setBatchProgress({
        sent: Math.min(i + CHUNK, rows.length),
        total: rows.length,
      })
    }
    ...
    if (ok > 0 && fail === 0)
      addToast('success', `✓ ${ok} fila(s) enviada(s) a Revisar Clientes`)
    else if (ok > 0 && fail > 0)
  ...
  const sendAllErrorsToRevisarClientes = useCallback(async () => {
    ...
    for (let i = 0; i < errorRows.length; i += CHUNK) {
      ...
      setBatchProgress({
        sent: Math.min(i + CHUNK, errorRows.length),
    ...
    async (file: File) => {
      ...
      try {
        ...
        if (uniqueCedulas.length > 0) {
          try {
            const { existing_cedulas } =
              await clienteService.checkCedulas(uniqueCedulas)
        ...
        if (uniqueEmails.length > 0) {
          try {
            const { existing_emails } =
              await clienteService.checkEmails(uniqueEmails)
  ...
  const updateCellValue = useCallback(
    async (
      ...
      if (
        field in row &&
        !['_rowIndex', '_validation', '_hasErrors'].includes(field)
      ) {
      ...
      if (
        field === 'email' &&
        emNorm &&
        emailsExistentesEnBD.includes(emNorm)
      ) {
        row._validation.email = {
          isValid: false,
          message: 'Email ya existe en el sistema',
      ...
      const fieldsToCheck = Object.keys(row._validation).filter(
        f => f !== 'notas'
      )
    ...
    [
      excelData,
      validateFieldWithOptions,
      handleRowValidationNotification,
      cedulasExistentesEnBD,
      emailsExistentesEnBD,
    ]
  )
