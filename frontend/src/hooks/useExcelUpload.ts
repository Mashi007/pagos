...
export function useExcelUpload({ onClose, onDataProcessed, onSuccess }: ExcelUploaderProps) {
  ...
  const getDuplicadoMotivo = useCallback(
    (row: ExcelRow): string[] => {
      ...
      const ced = (row.cedula || '').trim().toUpperCase() || 'Z999999999'
      if (ced !== 'Z999999999' && cedulasDuplicadasEnArchivo.has(ced)) motivos.push('cÃ©dula en archivo')
      if (ced !== 'Z999999999' && cedulasExistentesEnBD.includes(ced)) motivos.push('cÃ©dula ya existe en sistema')
      const em = (row.email || '').trim().toLowerCase()
      ...
      if (telDig.startsWith('58') && telDig.length > 10) telDig = telDig.slice(2)
      if (telDig.length > 10) telDig = '9999999999'
      if (telDig.length >= 10 && telefonoDuplicadosEnArchivo.has(telDig)) motivos.push('telÃ©fono')
      return motivos
    },
    [
      cedulasDuplicadasEnArchivo,
  ...
  const saveIndividualClient = useCallback(
    async (row: ExcelRow): Promise<boolean> => {
      try {
        if (row._hasErrors) {
          alert(
            'â ï¸ NO SE PUEDE GUARDAR: Hay campos vacÃ­os o con errores en esta fila.\n\nPor favor, complete todos los campos obligatorios en la tabla antes de guardar.'
          )
          return false
        }
        // CÃ©dula: comparar con tabla clientes â NUNCA permitir guardar si ya estÃ¡ en BD (100% igual)
        ...
        if (cedNorm !== 'Z999999999' && cedulasExistentesEnBD.includes(cedNorm)) {
          addToast('error', 'CÃ©dula ya existe en tabla clientes. No se puede guardar.')
          return false
        }
        ...
        try {
          ...
          if (err.response?.status === 400 || err.response?.status === 409) {
            const msg =
              ...
              'No se puede crear un cliente: ya existe uno con la misma cÃ©dula, nombre, email o telÃ©fono.'
            addToast('error', `Error en fila ${row._rowIndex}: ${msg}`)
          } else if (err.response?.status === 503) {
            addToast('error', 'â ï¸ SERVICIO NO DISPONIBLE: El backend estÃ¡ caÃ­do. Contacta al administrador.')
          } else if (err.code === 'ERR_NETWORK' || err.message?.includes('Network Error')) {
            addToast('error', 'â ï¸ ERROR DE RED: No se puede conectar al servidor. Verifica tu conexiÃ³n.')
        ...
        setExcelData((prev) => {
          ...
          if (remaining.length === 0) {
            addToast('success', 'Â¡Todos los clientes han sido guardados exitosamente!')
        ...
        if (err.response?.status === 503) {
          ...
          addToast('error', `Error de validaciÃ³n: ${err.response?.data?.detail || err.message}`)
        } else if (err.response && err.response.status && err.response.status >= 500) {
          addToast('error', 'Error del servidor. Contacta al administrador.')
        } else {
  ...
  const doSaveClientesList = useCallback(
    async (clientsToSave: ExcelRow[]) => {
      ...
      for (let i = 0; i < clientsToSave.length; i += CHUNK) {
        ...
        for (const client of chunk) {
          try {
            ...
            if (err.response?.status === 409) {
              addToast('warning', `Cliente ${client.cedula} (${client.nombres}) duplicado - se omitiÃ³`)
            } else {
        ...
        if (chunkOk.length > 0) {
          setExcelData((prev) => {
            ...
            if (remaining.length === 0) {
              addToast('success', 'Â¡Todos los clientes guardados! Cerrando en 2 segundos...')
              setTimeout(() => {
                onClose()
                navigate('/clientes')
              }, 2000)
      ...
      if (successful > 0) {
        ...
        else addToast('warning', `â ${successful} guardados | â ${failed} fallo(s)`)
      }
  ...
  const saveAllValidClients = useCallback(async () => {
    ...
    if (serviceStatus === 'offline') {
      addToast('error', 'Sin conexiÃ³n. No se puede comprobar cÃ©dulas ni guardar.')
      return
    }
    ...
    try {
      ...
      clearContradictoryToasts()
      if (err.response?.status === 503) {
        addToast('error', 'SERVICIO NO DISPONIBLE: El backend estÃ¡ caÃ­do. Contacta al administrador.')
      } else if (err.code === 'ERR_NETWORK' || err.message?.includes('Network Error')) {
        addToast('error', 'ERROR DE RED: No se puede conectar al servidor.')
      } else if (err.response && err.response.status && err.response.status >= 500) {
        addToast('error', 'Error del servidor. Contacta al administrador.')
      } else {
        addToast('error', 'Error al comprobar cÃ©dulas o guardar.')
      }
    } finally {
      setIsSavingIndividual(false)
    }
      ...
      const erroresDesc =
        row._hasErrors && row._validation
          ? Object.entries(row._validation)
              .filter(([, v]) => !v?.isValid)
              .map(([k]) => k)
              .join('; ') || 'Enviado a revisiÃ³n'
          : 'Enviado a revisiÃ³n desde carga masiva'
      try {
  ...
  const sendAllToRevisarClientes = useCallback(async () => {
    ...
    if (ok > 0 && fail === 0) addToast('success', `â ${ok} fila(s) enviada(s) a Revisar Clientes`)
    else if (ok > 0 && fail > 0) addToast('warning', `â ${ok} enviada(s) | â ${fail} fallo(s)`)
    else if (fail > 0) addToast('error', `â ${fail} fila(s) no se pudieron enviar`)
  ...
  const processExcelFile = useCallback(
    async (file: File) => {
      ...
      try {
        ...
        // CÃ©dula: comparar con tabla clientes (100% igual = duplicado). NUNCA permitir guardar si ya estÃ¡ en BD.
        const uniqueCedulas = [
          ...new Set(
            processedData
        ...
        if (uniqueCedulas.length > 0) {
          try {
              ...
              for (const row of processedData) {
                ...
                if (ced !== 'Z999999999' && existing_cedulas.includes(ced)) {
                  row._validation.cedula = {
                    isValid: false,
                    message: 'CÃ©dula ya existe en tabla clientes (no se puede guardar)',
                  }
                  row._hasErrors = true
                }
              }
            ...
            // Si falla la verificaciÃ³n (red/backend), no bloquear; el backend rechazarÃ¡ 409 al guardar
          }
        }
        ...
        if (uniqueEmails.length > 0) {
          try {
            ...
            // Si falla la verificaciÃ³n, el backend rechazarÃ¡ 409 al guardar
          }
        }
  ...
  const updateCellValue = useCallback(
    async (rowOrIndex: ExcelRow | number, field: string, value: string | null) => {
      ...
      // Re-aplicar regla: si cÃ©dula/email ya estÃ¡n en tabla clientes, no se puede guardar (no se borra al editar)
      const cedNorm = (row.cedula || '').trim().toUpperCase()
      if (field === 'cedula' && cedNorm !== 'Z999999999' && cedulasExistentesEnBD.includes(cedNorm)) {
        row._validation.cedula = { isValid: false, message: 'CÃ©dula ya existe en tabla clientes (no se puede guardar)' }
      }
