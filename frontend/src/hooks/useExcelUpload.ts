...
export function useExcelUpload({ onClose, onDataProcessed, onSuccess }: ExcelUploaderProps) {
  ...
  const getDuplicadoMotivo = useCallback(
    (row: ExcelRow): string[] => {
      ...
      const ced = (row.cedula || '').trim().toUpperCase() || 'Z999999999'
      if (ced !== 'Z999999999' && cedulasDuplicadasEnArchivo.has(ced)) motivos.push('cÃÂ©dula en archivo')
      if (ced !== 'Z999999999' && cedulasExistentesEnBD.includes(ced)) motivos.push('cÃÂ©dula ya existe en sistema')
      const em = (row.email || '').trim().toLowerCase()
      ...
      if (telDig.startsWith('58') && telDig.length > 10) telDig = telDig.slice(2)
      if (telDig.length > 10) telDig = '9999999999'
      if (telDig.length >= 10 && telefonoDuplicadosEnArchivo.has(telDig)) motivos.push('telÃÂ©fono')
      return motivos
    },
    [
      cedulasDuplicadasEnArchivo,
  ...
  const notifyDashboardUpdate = useCallback(
    (clientCount: number) => {
      addToast(
        ...
        'Los clientes ya estÃÂ¡n disponibles en la lista principal'
      )
    },
    [addToast]
  )
    ...
    (rowIndex: number, row: ExcelRow) => {
      ...
      if (!hasErrors) {
        const successToast: ToastItem = {
          ...
          type: 'success',
          message: `Fila ${rowIndex + 1} es vÃÂ¡lida`,
          field: 'fila_completa',
          rowIndex,
        }
  ...
  const saveIndividualClient = useCallback(
    async (row: ExcelRow): Promise<boolean> => {
      try {
        if (row._hasErrors) {
          alert(
            'Ã¢ÂÂ Ã¯Â¸Â NO SE PUEDE GUARDAR: Hay campos vacÃÂ­os o con errores en esta fila.\n\nPor favor, complete todos los campos obligatorios en la tabla antes de guardar.'
          )
          return false
        }
        // CÃÂ©dula: comparar con tabla clientes Ã¢ÂÂ NUNCA permitir guardar si ya estÃÂ¡ en BD (100% igual)
        ...
        if (cedNorm !== 'Z999999999' && cedulasExistentesEnBD.includes(cedNorm)) {
          addToast('error', 'CÃÂ©dula ya existe en tabla clientes. No se puede guardar.')
          return false
        }
        ...
        try {
          ...
          if (err.response?.status === 400 || err.response?.status === 409) {
            const msg =
              ...
              'No se puede crear un cliente: ya existe uno con la misma cÃÂ©dula, nombre, email o telÃÂ©fono.'
            addToast('error', `Error en fila ${row._rowIndex}: ${msg}`)
          } else if (err.response?.status === 503) {
            addToast('error', 'Ã¢ÂÂ Ã¯Â¸Â SERVICIO NO DISPONIBLE: El backend estÃÂ¡ caÃÂ­do. Contacta al administrador.')
          } else if (err.code === 'ERR_NETWORK' || err.message?.includes('Network Error')) {
            addToast('error', 'Ã¢ÂÂ Ã¯Â¸Â ERROR DE RED: No se puede conectar al servidor. Verifica tu conexiÃÂ³n.')
        ...
        setExcelData((prev) => {
          ...
          if (remaining.length === 0) {
            addToast('success', 'ÃÂ¡Todos los clientes han sido guardados exitosamente!')
        ...
        if (err.response?.status === 503) {
          ...
          addToast('error', `Error de validaciÃÂ³n: ${err.response?.data?.detail || err.message}`)
        } else if (err.response && err.response.status && err.response.status >= 500) {
          addToast('error', 'Error del servidor. Contacta al administrador.')
        } else {
  ...
  const doSaveClientesList = useCallback(
    async (clientsToSave: ExcelRow[]) => {
      ...
      for (let i = 0; i < clientsToSave.length; i += CHUNK) {
        ...
        for (const client of chunk) {
          try {
            ...
            if (err.response?.status === 409) {
              addToast('warning', `Cliente ${client.cedula} (${client.nombres}) duplicado - se omitiÃÂ³`)
            } else {
        ...
        if (chunkOk.length > 0) {
          setExcelData((prev) => {
            ...
            if (remaining.length === 0) {
              addToast('success', 'ÃÂ¡Todos los clientes guardados! Cerrando en 2 segundos...')
              setTimeout(() => {
                onClose()
                navigate('/clientes')
              }, 2000)
      ...
      if (successful > 0) {
        ...
        else addToast('warning', `Ã¢ÂÂ ${successful} guardados | Ã¢ÂÂ ${failed} fallo(s)`)
      }
  ...
  const saveAllValidClients = useCallback(async () => {
    ...
    if (validClients.length === 0) {
      addToast('warning', 'No hay clientes vÃÂ¡lidos para guardar')
      return
    }
    if (serviceStatus === 'offline') {
      addToast('error', 'Sin conexiÃÂ³n. No se puede comprobar cÃÂ©dulas ni guardar.')
      return
    }
    ...
    try {
      ...
      clearContradictoryToasts()
      if (err.response?.status === 503) {
        addToast('error', 'SERVICIO NO DISPONIBLE: El backend estÃÂ¡ caÃÂ­do. Contacta al administrador.')
      } else if (err.code === 'ERR_NETWORK' || err.message?.includes('Network Error')) {
        addToast('error', 'ERROR DE RED: No se puede conectar al servidor.')
      } else if (err.response && err.response.status && err.response.status >= 500) {
        addToast('error', 'Error del servidor. Contacta al administrador.')
      } else {
        addToast('error', 'Error al comprobar cÃÂ©dulas o guardar.')
      }
    } finally {
      setIsSavingIndividual(false)
    }
      ...
      const erroresDesc =
        row._hasErrors && row._validation
          ? Object.entries(row._validation)
              .filter(([, v]) => !v?.isValid)
              .map(([k]) => k)
              .join('; ') || 'Enviado a revisiÃÂ³n'
          : 'Enviado a revisiÃÂ³n desde carga masiva'
      try {
  ...
  const sendAllToRevisarClientes = useCallback(async () => {
    ...
    if (serviceStatus === 'offline') {
      addToast('error', 'Sin conexiÃÂ³n')
      return
    }
    ...
    if (ok > 0 && fail === 0) addToast('success', `Ã¢ÂÂ ${ok} fila(s) enviada(s) a Revisar Clientes`)
    else if (ok > 0 && fail > 0) addToast('warning', `Ã¢ÂÂ ${ok} enviada(s) | Ã¢ÂÂ ${fail} fallo(s)`)
    else if (fail > 0) addToast('error', `Ã¢ÂÂ ${fail} fila(s) no se pudieron enviar`)
    ...
    async (file: File) => {
      ...
      try {
        ...
        if (!fileValidation.isValid) {
          alert(`Error de validaciÃÂ³n: ${fileValidation.error}`)
          if (isMounted()) setIsProcessing(false)
          return
        }
        if (fileValidation.warnings?.length) {
          console.warn('Advertencias de validaciÃÂ³n:', fileValidation.warnings)
        }
        ...
        if (data.byteLength > 10 * 1024 * 1024) {
          alert('El archivo es demasiado grande. TamaÃÂ±o mÃÂ¡ximo: 10 MB')
          if (isMounted()) setIsProcessing(false)
          return
        }
        ...
        // CÃÂ©dula: comparar con tabla clientes (100% igual = duplicado). NUNCA permitir guardar si ya estÃÂ¡ en BD.
        const uniqueCedulas = [
          ...new Set(
            processedData
        ...
        if (uniqueCedulas.length > 0) {
          try {
              ...
              for (const row of processedData) {
                ...
                if (ced !== 'Z999999999' && existing_cedulas.includes(ced)) {
                  row._validation.cedula = {
                    isValid: false,
                    message: 'CÃÂ©dula ya existe en tabla clientes (no se puede guardar)',
                  }
                  row._hasErrors = true
                }
              }
            ...
            // Si falla la verificaciÃÂ³n (red/backend), no bloquear; el backend rechazarÃÂ¡ 409 al guardar
          }
        }
        ...
        if (uniqueEmails.length > 0) {
          try {
            ...
            // Si falla la verificaciÃÂ³n, el backend rechazarÃÂ¡ 409 al guardar
          }
        }
  ...
  const updateCellValue = useCallback(
    async (rowOrIndex: ExcelRow | number, field: string, value: string | null) => {
      ...
      // Re-aplicar regla: si cÃÂ©dula/email ya estÃÂ¡n en tabla clientes, no se puede guardar (no se borra al editar)
      const cedNorm = (row.cedula || '').trim().toUpperCase()
      if (field === 'cedula' && cedNorm !== 'Z999999999' && cedulasExistentesEnBD.includes(cedNorm)) {
        row._validation.cedula = { isValid: false, message: 'CÃÂ©dula ya existe en tabla clientes (no se puede guardar)' }
      }
