...
// Hook para obtener analistas activos (solo cuando hay sesión válida; evita "Sesión expirada" en login)
...
export function useUpdateAnalista() {
  ...
  return useMutation({
    ...
    onError: (error: any) => {
      toast.error(
        error.response?.data?.message || 'Error al actualizar analista'
      )
    },
  })
}
