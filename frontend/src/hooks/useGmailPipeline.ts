...
interface GmailStatus {
  ...
  latest_data_date?: string | null  // fecha mÃÂ¡s reciente con datos disponibles para descargar
}
...
const POLL_MAX_ATTEMPTS = 300 // 300 ÃÂ 5s = 25 min mÃÂ¡ximo de espera (pipeline con muchos correos)
        ...
        try {
          ...
          if (s.last_status && s.last_status !== 'running') {
            ...
            if (s.last_status === 'error') {
              ...
              // No abrir diÃÂ¡logo de descarga en caso de error
            } else if (emails === 0 && files === 0) {
              if (hasData) {
                // Datos de una ejecuciÃÂ³n anterior disponibles para descargar
                toast(
                  `Sin correos nuevos (solo se procesan no leÃÂ­dos; al terminar se vuelve a revisar la bandeja). Hay datos del ${s.latest_data_date} listos para descargar.`,
                  { duration: 8000 }
                )
                onDoneRef.current?.(s)
              } else {
                toast(
                  'No se encontraron correos para procesar. Regla: solo mensajes NO LEÃÂDOS (con o sin adjuntos). Marque como no leÃÂ­do si quiere reprocesar.',
                  { duration: 7000 }
                )
                // Sin datos: no abrir el diÃÂ¡logo de descarga
              }
            } else {
