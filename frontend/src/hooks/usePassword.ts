/**
 * Hook personalizado para manejar funcionalidades de contraseÃ±as
 * Centraliza la lÃ³gica de generaciÃ³n, copiado y visibilidad de contraseÃ±as
 */
...
export const usePassword = (options: UsePasswordOptions = {}) => {
  ...
  // Generar contraseÃ±a automÃ¡tica segura
  const generatePassword = () => {
    ...
    toast.success('ContraseÃ±a generada automÃ¡ticamente')
  ...
  const copyPassword = async () => {
    if (!password) {
      toast.error('No hay contraseÃ±a para copiar')
      return false
    }
    ...
    try {
      ...
      try {
        ...
        toast.error('No se pudo copiar la contraseÃ±a')
        return false
      }
    }
  }
  ...
  const validatePassword = (pwd: string = password) => {
    ...
    // SÃ­mbolos permitidos: debe coincidir con el backend [!@#$%^&*()_+\-=\[\]{};:"\\|,.<>\/?]
