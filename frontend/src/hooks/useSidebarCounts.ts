...
export function useSidebarCounts() {
  // Ã¢Åâ¦ OPTIMIZACIÃN: Usar React Query para compartir datos con otros componentes
  ...
  const { data: kpisData, isLoading: loadingKPIs } = useQuery({
    queryKey: ['kpis-pagos'], // Ã¢Åâ¦ Misma queryKey que DashboardPagos usa (sin filtros)
    queryFn: async () => {
    ...
    refetchOnWindowFocus: false, // Ã¢Åâ¦ No recargar automÃ¡ticamente al enfocar ventana
    refetchInterval: 5 * 60 * 1000, // Ã¢Åâ¦ Actualizar cada 5 minutos automÃ¡ticamente
