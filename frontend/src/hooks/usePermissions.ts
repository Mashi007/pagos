...
export function usePermissions() {
   ...
   * - ADMIN: acceso total
   * - OPERATIVO: puede ver y descargar reportes no financieros (Pagos, Morosidad, Vencimiento, Por cÃ©dula)
   */
  const canViewReports = (): boolean => {
    return true // Todos los usuarios autenticados pueden ver la pÃ¡gina de reportes
  }

  /**
   * Verifica si el usuario puede descargar reportes (en general).
   * La descarga especÃ­fica por tipo se valida con canAccessReport.
   */
  ...
  /**
   * Verifica si el usuario puede acceder a reportes especÃ­ficos por tipo
  ...
  const canAccessReport = (reportType: string): boolean => {
    ...
    // Operativos pueden ver: Pagos, Morosidad, Vencimiento, Por cÃ©dula. Contable solo admin.
