...
export function useEvaluacionRiesgo({
  prestamo,
  onSuccess,
  onClose,
}: UseEvaluacionRiesgoOptions) {
  const { user } = useSimpleAuth()

  const [formData, setFormData] =
    useState<EvaluacionFormData>(INITIAL_FORM_DATA)
  ...
  const [showFormularioAprobacion, setShowFormularioAprobacion] =
    useState(false)
  ...
  const loadPrestamoData = async () => {
    try {
      ...
      setResumenPrestamos(
        resumenRes ?? { error: true, tiene_prestamos: false, prestamos: [] }
      )
  ...
  if (!seccion1Completa)
    criteriosFaltantes.push('Criterio 1: Capacidad de Pago')

  if (!seccion2Completa)
    criteriosFaltantes.push('Criterio 2: Estabilidad Laboral')

  if (!seccion3Completa)
    criteriosFaltantes.push('Criterio 3: Referencias Personales')

  if (!seccion4Completa)
    criteriosFaltantes.push('Criterio 4: Arraigo Geográfico')

  if (!seccion5Completa)
    criteriosFaltantes.push('Criterio 5: Perfil Sociodemográfico')

  if (!seccion6Completa) criteriosFaltantes.push('Criterio 6: Edad del Cliente')

  if (!seccion7Completa)
    criteriosFaltantes.push('Criterio 7: Capacidad de Maniobra')
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await prestamoService.evaluarRiesgo(
        prestamo.id,
        datosEvaluacion
      )
      ...
      toast.success(
        '[OK] Fase 1 Completada: Evaluación de Riesgo guardada. El préstamo ahora está en estado EVALUADO.'
      )
      ...
      setTimeout(() => {
        ...
        toast.info(
          '✓ Ahora puede usar el nuevo icono en el dashboard para proceder con la Aprobación (Fase 2)'
        )
      }, 2000)
    } catch (error: any) {
  ...
  const seccionCompleta = (id: string) => {
    switch (id) {
      case 'criterio1':
        return seccion1Completa

      case 'criterio2':
        return seccion2Completa

      case 'criterio3':
        return seccion3Completa

      case 'criterio4':
        return seccion4Completa

      case 'criterio5':
        return seccion5Completa

      case 'criterio6':
        return seccion6Completa

      case 'criterio7':
        return seccion7Completa
