...
export interface EvaluacionFormData {
  ...
  // Criterio 4: Arraigo GeogrÃ¡fico
  ...
  // Criterio 7: Capacidad de Maniobra (calculado automÃ¡ticamente)
}
...
export function useEvaluacionRiesgo({ prestamo, onSuccess, onClose }: UseEvaluacionRiesgoOptions) {
  ...
  const loadPrestamoData = async () => {
    try {
      ...
      console.error('Error cargando datos del prÃ©stamo:', e)
    }
  }
  ...
  const loadEvaluacionExistente = async () => {
    try {
      ...
      // No hay evaluaciÃ³n previa, ignorar
    }
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const confirmacion = window.confirm(
      'â ï¸ CONFIRMACIÃN IMPORTANTE\n\n' +
        `Yo (correo: ${correo}) declaro que he revisado los documentos y que los mismos respaldan:\n` +
        'â¢ Los ingresos coinciden con documentos\n' +
        'â¢ Los gastos son verificables\n' +
        'â¢ La informaciÃ³n de empleo es correcta\n' +
        'â¢ Los valores de anticipo son reales\n\n' +
    ...
    try {
      ...
      toast.success('[OK] Fase 1 Completada: EvaluaciÃ³n de Riesgo guardada. El prÃ©stamo ahora estÃ¡ en estado EVALUADO.')

      setTimeout(() => {
        onSuccess()
        onClose()
        toast.info('â Ahora puede usar el nuevo icono en el dashboard para proceder con la AprobaciÃ³n (Fase 2)')
      }, 2000)
    } catch (error: any) {
