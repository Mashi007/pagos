...
export interface EvaluacionFormData {
  ...
  // Criterio 4: Arraigo GeogrÃÂ¡fico
  familia_cercana: boolean
  familia_pais: boolean
  minutos_trabajo: number

  // Criterio 5: Perfil SociodemogrÃÂ¡fico
  ...
  // Criterio 7: Capacidad de Maniobra (calculado automÃÂ¡ticamente)
}
...
export function useEvaluacionRiesgo({ prestamo, onSuccess, onClose }: UseEvaluacionRiesgoOptions) {
  ...
  const loadPrestamoData = async () => {
    try {
      ...
      if (clienteRes?.data?.length > 0 && clienteRes.data[0].fecha_nacimiento) {
        ...
        let aÃÂ±os = hoy.getFullYear() - nacimiento.getFullYear()
        ...
        setClienteEdad(aÃÂ±os + meses / 12)
      }
      ...
      console.error('Error cargando datos del prÃÂ©stamo:', e)
    }
  }
  ...
  const loadEvaluacionExistente = async () => {
    try {
      ...
      // No hay evaluaciÃÂ³n previa, ignorar
    }
  }
  ...
  useEffect(() => {
    if (resultado?.sugerencias) {
      setCondicionesAprobacion({
        ...
        fecha_base_calculo: new Date().toISOString().split('T')[0],
        observaciones: `Aprobado despuÃÂ©s de evaluaciÃÂ³n de riesgo. PuntuaciÃÂ³n: ${resultado.puntuacion_total?.toFixed(2)}/100, Riesgo: ${resultado.clasificacion_riesgo}`,
      })
  ...
  if (!seccion4Completa) criteriosFaltantes.push('Criterio 4: Arraigo GeogrÃÂ¡fico')
  if (!seccion5Completa) criteriosFaltantes.push('Criterio 5: Perfil SociodemogrÃÂ¡fico')
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const confirmacion = window.confirm(
      'Ã¢ÂÂ Ã¯Â¸Â CONFIRMACIÃÂN IMPORTANTE\n\n' +
        `Yo (correo: ${correo}) declaro que he revisado los documentos y que los mismos respaldan:\n` +
        'Ã¢ÂÂ¢ Los ingresos coinciden con documentos\n' +
        'Ã¢ÂÂ¢ Los gastos son verificables\n' +
        'Ã¢ÂÂ¢ La informaciÃÂ³n de empleo es correcta\n' +
        'Ã¢ÂÂ¢ Los valores de anticipo son reales\n\n' +
        'ÃÂ¿Desea proceder con la evaluaciÃÂ³n?'
    )
    if (!confirmacion) {
      toast('EvaluaciÃÂ³n cancelada')
      return
    }
    ...
    try {
      ...
      toast.success('[OK] Fase 1 Completada: EvaluaciÃÂ³n de Riesgo guardada. El prÃÂ©stamo ahora estÃÂ¡ en estado EVALUADO.')

      setTimeout(() => {
        onSuccess()
        onClose()
        toast.info('Ã¢ÂÂ Ahora puede usar el nuevo icono en el dashboard para proceder con la AprobaciÃÂ³n (Fase 2)')
      }, 2000)
    } catch (error: any) {
