...
export function useAIConfig() {
  ...
  const cargarConfiguracion = useCallback(async () => {
    try {
      ...
      setConfiguracionCorrecta(
        !!data.configured && (data.activo || 'true').toLowerCase() === 'true'
      )
    } catch (error) {
  ...
  const handleChange = useCallback(
    (campo: keyof AIConfigState, valor: string) => {
  ...
  const verificarConfiguracion = useCallback(
    async (guardarAutomaticamente: boolean = false) => {
  ...
  const handleProcesarDocumento = useCallback(
    async (id: number) => {
      try {
        ...
        if (
          ...
          toast.success(
            `📄 ${respuesta.mensaje || 'Documento procesado'} (${respuesta.caracteres_extraidos || 0} caracteres)`,
            {
              description:
  ...
  const handleProbar = useCallback(async () => {
    ...
    if (!config.configured) {
      toast.error(
        'Ingresa tu API Key de OpenRouter en el campo de arriba y guarda, o configura OPENROUTER_API_KEY en variables de entorno del servidor.'
      )
