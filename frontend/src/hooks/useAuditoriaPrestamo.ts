...
function mapAuditoriaItemToEntry(item: AuditoriaItemBackend): AuditoriaEntry {
  const valorAnt =
    item.datos_anteriores != null
      ? typeof item.datos_anteriores === 'string'
        ? item.datos_anteriores
        : JSON.stringify(item.datos_anteriores)
      : null

  const valorNuevo =
    item.datos_nuevos != null
      ? typeof item.datos_nuevos === 'string'
        ? item.datos_nuevos
        : JSON.stringify(item.datos_nuevos)
      : ''

  const datosAnt =
    item.datos_anteriores &&
    typeof item.datos_anteriores === 'object' &&
    item.datos_anteriores !== null
      ? (item.datos_anteriores as Record<string, unknown>)
      : null

  const datosNuevos =
    item.datos_nuevos &&
    typeof item.datos_nuevos === 'object' &&
    item.datos_nuevos !== null
      ? (item.datos_nuevos as Record<string, unknown>)
      : null

  const estadoAnterior =
    datosAnt && 'estado' in datosAnt ? String(datosAnt.estado) : null

  const estadoNuevo =
    datosNuevos && 'estado' in datosNuevos ? String(datosNuevos.estado) : null
...
export const useAuditoriaPrestamo = (prestamoId: number | null) => {
  return useQuery({
    ...
    queryFn: async (): Promise<AuditoriaEntry[]> => {
      ...
      const response = await apiClient.get<{ items: AuditoriaItemBackend[] }>(
        url
      )
