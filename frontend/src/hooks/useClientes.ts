...
export function useClientes(
  ...
  return useQuery({
    ...
    staleTime: STALE_TIME_SHORT, // ÃÂ¢ÃâÃ¢â¬Â¦ Reducido a 2 minutos para datos mÃ¡s frescos
    refetchOnMount: true, // ÃÂ¢ÃâÃ¢â¬Â¦ Refrescar cuando el componente se monta
    refetchOnWindowFocus: true, // ÃÂ¢ÃâÃ¢â¬Â¦ Refrescar cuando el usuario vuelve a la ventana
    refetchInterval: 3 * 60 * 1000, // ÃÂ¢ÃâÃ¢â¬Â¦ Auto-refresh cada 3 minutos
  })
}
...
export function useCreateCliente() {
  ...
  return useMutation({
    ...
    onSuccess: (newCliente) => {
      ...
      // ÃÂ¢ÃâÃ¢â¬Â¦ Invalidar tambiÃ©n bÃºsquedas de clientes (usadas en formularios de prÃ©stamos)
  ...
  return useMutation({
    ...
    onSuccess: (result) => {
      ...
      if (result.errors.length > 0) {
        toast.success(
          `ImportaciiÃ³n completada: ${result.success} exitosos, ${result.errors.length} errores`
        )
      } else {
