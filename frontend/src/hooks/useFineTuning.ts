...
export function useFineTuning(): UseFineTuningReturn {
  ...
  const cargarEstadisticasFeedback = useCallback(async () => {
    ...
    try {
      ...
      console.error('Error cargando estadÃÂ­sticas:', error)
      toast.error('Error al cargar estadÃÂ­sticas')
    } finally {
  ...
  const calificar = useCallback(
    async (conversacionId: number, calificacion: number, feedback: string) => {
      try {
        ...
        toast.success('ConversaciÃÂ³n calificada exitosamente')
        ...
        const mensaje = error?.response?.data?.detail || error?.message || 'Error al calificar conversaciÃÂ³n'
        toast.error(mensaje)
      }
    },
  ...
  const crearConversacion = useCallback(
    async (pregunta: string, respuesta: string) => {
      try {
        ...
        toast.success('ConversaciÃÂ³n creada exitosamente')
        cargarConversaciones()
      } catch (error: any) {
        console.error('Error creando conversaciÃÂ³n:', error)
        const mensaje = error?.response?.data?.detail || error?.message || 'Error al crear conversaciÃÂ³n'
        toast.error(mensaje)
      }
    },
    [cargarConversaciones]
  ...
  const actualizarConversacion = useCallback(
    async (conversacionId: number, pregunta: string, respuesta: string) => {
      try {
        ...
        toast.success('ConversaciÃÂ³n actualizada exitosamente')
        cargarConversaciones()
      } catch (error: any) {
        console.error('Error actualizando conversaciÃÂ³n:', error)
        const mensaje = error?.response?.data?.detail || error?.message || 'Error al actualizar conversaciÃÂ³n'
        toast.error(mensaje)
      }
    },
    [cargarConversaciones]
  ...
  const eliminarConversacion = useCallback(
    async (conversacionId: number) => {
      try {
        ...
        toast.success('ConversaciÃÂ³n eliminada exitosamente')
        cargarConversaciones()
      } catch (error: any) {
        console.error('Error eliminando conversaciÃÂ³n:', error)
        const mensaje = error?.response?.data?.detail || error?.message || 'Error al eliminar conversaciÃÂ³n'
        toast.error(mensaje)
      }
    },
    [cargarConversaciones]
  ...
  const mejorarConversacionCompleta = useCallback(
    async (pregunta: string, respuesta: string): Promise<{ pregunta: string; respuesta: string }> => {
      try {
        ...
        console.error('Error mejorando conversaciÃÂ³n:', error)
        const mensaje = error?.response?.data?.detail || error?.message || 'Error al mejorar conversaciÃÂ³n'
        toast.error(mensaje)
        throw error
      }
    },
  ...
  const importarConversaciones = useCallback(
    async (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
        ...
        toast.error('Error al leer el archivo. Verifica que sea un JSON vÃÂ¡lido.')
      }
    },
    [cargarConversaciones]
  )
