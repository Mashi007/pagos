...
import {
  MINIMO_CONVERSACIONES_ENTRENAMIENTO,
  detectarFeedbackNegativo,
} from '../constants/fineTuning'
...
export function useFineTuning(): UseFineTuningReturn {
  ...
  const calificar = useCallback(
    async (conversacionId: number, calificacion: number, feedback: string) => {
      try {
        await aiTrainingService.calificarConversacion(
          conversacionId,
          calificacion,
          feedback
        )
  ...
  const mejorarPregunta = useCallback(
    async (pregunta: string): Promise<string> => {
      try {
  ...
  const mejorarRespuesta = useCallback(
    async (respuesta: string): Promise<string> => {
      try {
        const resultado = await aiTrainingService.mejorarConversacion({
          tipo: 'respuesta',
    ...
    async (conversacionesIds: number[], soloCalificadas: boolean) => {
      try {
        ...
        const mensajeError =
          error?.response?.data?.detail ||
          error?.message ||
          'Error preparando los datos para entrenamiento'
  ...
  const activarModelo = useCallback(
  ...
  const eliminarTodosJobs = useCallback(
    async (soloFallidos: boolean = false) => {
      try {
        const resultado =
          await aiTrainingService.eliminarTodosFineTuningJobs(soloFallidos)
  ...
  const importarConversaciones = useCallback(
    async (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
        ...
        toast.error(
          'Error al leer el archivo. Verifica que sea un JSON válido.'
        )
      }
    },
