...
const DEBUG_LOGS =
  ...
  import.meta.env.VITE_DEBUG_FILTROS === 'true'
...
export function useDashboardFiltros(filtros: DashboardFiltros) {
  ...
  const construirParams = useMemo(() => {
    return (periodo: string = 'mes') => {
      ...
      if (filtros.analista && filtros.analista !== '__ALL__')
        params.append('analista', normalizarValor(filtros.analista)!)

      if (filtros.concesionario && filtros.concesionario !== '__ALL__')
        params.append('concesionario', normalizarValor(filtros.concesionario)!)

      if (filtros.modelo && filtros.modelo !== '__ALL__')
        params.append('modelo', normalizarValor(filtros.modelo)!)
      ...
      if (DEBUG_LOGS) {
        console.log('🔧 [useDashboardFiltros] Construyendo params:', {
          filtrosOriginales: filtros,
          periodo,
          paramsConstruidos: result,
        })
      }
  ...
  const calcularFechasPorPeriodo = (
    ...
    switch (periodo) {
      ...
      case 'semana':
        ...
        const diff =
          fecha_inicio.getDate() - diaSemana + (diaSemana === 0 ? -6 : 1) // Ajustar para que lunes = 1
  ...
  const construirFiltrosObject = useMemo(() => {
    return (
      ...
      if (filtros.analista && filtros.analista !== '__ALL__')
        obj.analista = normalizarValor(filtros.analista)

      if (filtros.concesionario && filtros.concesionario !== '__ALL__')
        obj.concesionario = normalizarValor(filtros.concesionario)

      if (filtros.modelo && filtros.modelo !== '__ALL__')
        obj.modelo = normalizarValor(filtros.modelo)
      ...
      if (DEBUG_LOGS) {
        console.log(
          '🔧 [useDashboardFiltros] Construyendo objeto de filtros:',
          { filtrosOriginales: filtros, periodo, objetoConstruido: obj }
        )
      }
  ...
  const cantidadFiltrosActivos = useMemo(() => {
    return Object.values(filtros).filter(v => v !== undefined && v !== '')
      .length
  }, [filtros])
