...
export function useCreateConcesionario() {
  ...
  return useMutation({
    mutationFn: (data: ConcesionarioCreate) =>
      concesionarioService.crearConcesionario(data),
    ...
    onError: (error: any) => {
      toast.error(
        error.response?.data?.detail ||
          error.response?.data?.message ||
          error.message ||
          'Error al actualizar concesionario'
      )
    },
  })
}
    ...
    onError: (error: any) => {
      toast.error(
        error.response?.data?.detail ||
          error.response?.data?.message ||
          error.message ||
          'Error al eliminar concesionario'
      )
    },
  })
}
