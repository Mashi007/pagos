...
export const prestamoKeys = {
  ...
  list: (filters?: PrestamoFilters, page?: number, perPage?: number) =>
    [...prestamoKeys.lists(), filters, page, perPage] as const,
  ...
  byCedula: (cedula: string) =>
    [...prestamoKeys.all, 'cedula', cedula] as const,

  auditoria: (prestamoId: number) =>
...
export function usePrestamos(
  ...
  return useQuery({
    ...
    staleTime: 0, // Siempre refetch cuando se invalida (mejor para actualizaciión inmediata de estado)
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({
        queryKey: [...prestamoKeys.all, 'kpis'],
        exact: false,
      })
...
export function useDeletePrestamo() {
  ...
  return useMutation({
    ...
    onError: (error: any) => {
      const errorMessage =
        error.response?.data?.detail || 'Error al eliminar préstamo'
...
// Hook para obtener cuotas de un préstamo (tabla de amortizaciión)
...
// Hook para generar tabla de amortizaciión
...
export function useGenerarAmortizacion() {
  ...
  return useMutation({
    ...
    onSuccess: (data, prestamoId) => {
      ...
      queryClient.invalidateQueries({
        queryKey: prestamoKeys.detail(prestamoId),
      })
      ...
      toast.success('Tabla de amortizaciión generada exitosamente')
    },

    onError: (error: any) => {
      const errorMessage =
        error.response?.data?.detail ||
        'Error al generar tabla de amortizaciión'
...
// Hook para aplicar condiciones de aprobaciión
...
export function useAplicarCondicionesAprobacion() {
  ...
  return useMutation({
    ...
    onSuccess: async (data, variables) => {
      ...
      toast.success(
        'Préstamo aprobado exitosamente. La tabla de amortizaciión ha sido generada. El dashboard se ha actualizado.'
      )
    },

    onError: (error: any) => {
      const errorMessage =
        error.response?.data?.detail || 'Error al aplicar condiciones'
