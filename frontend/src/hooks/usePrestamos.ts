...
export function usePrestamos(
  ...
  return useQuery({
    ...
    staleTime: 0, // Siempre refetch cuando se invalida (mejor para actualizaciiÃ³n inmediata de estado)
...
// Hook para obtener cuotas de un prÃ©stamo (tabla de amortizaciiÃ³n)
...
// Hook para generar tabla de amortizaciiÃ³n
export function useGenerarAmortizacion() {
  ...
  return useMutation({
    ...
    onSuccess: (data, prestamoId) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['kpis-principales-menu'], exact: false })
      queryClient.invalidateQueries({ queryKey: ['dashboard-menu'], exact: false })
      toast.success('Tabla de amortizaciiÃ³n generada exitosamente')
    },
    onError: (error: any) => {
      const errorMessage = error.response?.data?.detail || 'Error al generar tabla de amortizaciiÃ³n'
      toast.error(errorMessage)
    },
  })
}

// Hook para aplicar condiciones de aprobaciiÃ³n
export function useAplicarCondicionesAprobacion() {
  ...
  return useMutation({
    ...
    onSuccess: async (data, variables) => {
      ...
      toast.success('PrÃ©stamo aprobado exitosamente. La tabla de amortizaciiÃ³n ha sido generada. El dashboard se ha actualizado.')
    },
    onError: (error: any) => {
