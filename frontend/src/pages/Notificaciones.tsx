...
const TABS: { id: TabId; label: string; icon: typeof Calendar }[] = [
  ...
  { id: 'dias_5_atraso', label: '5 dÃÂ­as atrasado', icon: Clock },
  { id: 'dias_30_atraso', label: '30 dÃÂ­as atrasado', icon: Clock },
  { id: 'mora_90', label: '90+ dÃÂ­as de mora (moroso)', icon: FileText },
  { id: 'liquidados', label: 'CrÃÂ©dito pagado', icon: FileText },
  { id: 'configuracion', label: 'ConfiguraciÃÂ³n', icon: Settings },
]
...
export function Notificaciones() {
  ...
  const handleRefresh = () => {
    refetch()
    toast.success('Datos actualizados. Se recomienda ejecutar actualizaciÃÂ³n a las 2am (cron).')
  }
      ...
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            Recordatorios (Faltan 5, 3, 1), vence hoy, atrasos (5 y 30 dÃÂ­as), mora 90+ y crÃÂ©dito pagado. Datos desde BD. Se recomienda actualizar a las 2:00.
          </p>
          {data?.actualizado_en && (
            <p className="text-xs text-gray-500 mt-1">ÃÂltima actualizaciÃÂ³n: {new Date(data.actualizado_en).toLocaleString('es-ES')}</p>
          )}
        </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              {activeTab === 'mora_90'
                ? 'Informe: cuotas con 90 o mÃÂ¡s dÃÂ­as de mora - moroso (una a una)'
                : activeTab === 'liquidados'
                ? 'CrÃÂ©dito pagado (Total financiamiento = Total abonos)'
                : activeTab === 'dias_5_atraso'
                ? 'Cuotas con 5 dÃÂ­as de atraso'
                : activeTab === 'dias_30_atraso'
                ? 'Cuotas con 30 dÃÂ­as de atraso'
                : `Clientes con cuota no pagada ${activeTab === 'dias_5' ? 'y faltan 5 dÃÂ­as' : activeTab === 'dias_3' ? 'y faltan 3 dÃÂ­as' : activeTab === 'dias_1' ? 'y falta 1 dÃÂ­a' : 'y vence hoy'}`}
            </CardTitle>
            <CardDescription>
              {activeTab === 'mora_90'
                ? 'Listado de cada cuota atrasada 90+ dÃÂ­as (moroso) con nombre, cÃÂ©dula, nÃÂºmero de cuota, fecha de vencimiento, dÃÂ­as de atraso y monto.'
                : activeTab === 'liquidados'
                ? 'PrÃÂ©stamos donde total financiamiento (tabla prÃÂ©stamo) menos total abonos (tabla cuotas) es cero. Por cÃÂ©dula/cliente.'
                : activeTab === 'dias_5_atraso' || activeTab === 'dias_30_atraso'
                ? 'Cuotas vencidas no pagadas con 5 o 30 dÃÂ­as de atraso (nombre, cÃÂ©dula, nÃÂº cuota, fecha venc., dÃÂ­as atraso, monto).'
                : 'Nombre y cÃÂ©dula de clientes a notificar.'}
            </CardDescription>
          </CardHeader>
          <CardContent>
            {/* KPIs por pestaÃÂ±a: correos enviados y rebotados */}
            ...
            {/* BotÃÂ³n descargar informe Excel de no entregados (rebotados) */}
            {(activeTab as TabId) !== 'configuracion' && (activeTab as TabId) !== 'liquidados' && (activeTab as TabId) !== 'dias_5_atraso' && (activeTab as TabId) !== 'dias_30_atraso' && (
              <div className="mb-4">
                ...
                <p className="text-xs text-gray-500 mt-1">
                  Si todos los correos se enviaron correctamente se mostrarÃÂ¡ una notificaciÃÂ³n.
                </p>
              </div>
            )}
            {isError && (
                ...
                <table className="w-full text-sm min-w-[640px]">
                  <thead>
                    <tr className="border-b bg-gray-50">
                      ...
                      <th className="text-left py-2 px-3 font-semibold whitespace-nowrap">CÃÂ©dula</th>
                  ...
                  <tbody>
                    {(data?.liquidados ? []).length === 0 ? (
                      <tr><td colSpan={5} className="py-8 text-center text-gray-500">No hay prÃÂ©stamos con crÃÂ©dito pagado (Total financiamiento = Total abonos).</td></tr>
                    ) : (
                ...
                <table className="w-full text-sm min-w-[640px]">
                  <thead>
                    <tr className="border-b bg-gray-50">
                      ...
                      <th className="text-left py-2 px-3 font-semibold whitespace-nowrap">Nombre</th>
                      <th className="text-left py-2 px-3 font-semibold whitespace-nowrap">CÃÂ©dula</th>
                      <th className="text-left py-2 px-3 font-semibold whitespace-nowrap">NÃÂº cuota</th>
                      <th className="text-left py-2 px-3 font-semibold whitespace-nowrap">Fecha venc.</th>
                      <th className="text-right py-2 px-3 font-semibold whitespace-nowrap">DÃÂ­as atraso</th>
                      <th className="text-right py-2 px-3 font-semibold whitespace-nowrap">Monto</th>
                    </tr>
                  </thead>
                  <tbody>
                    {list.length === 0 ? (
                      <tr><td colSpan={7} className="py-8 text-center text-gray-500">{activeTab === 'mora_90' ? 'No hay cuotas con 90+ dÃÂ­as de mora (moroso).' : 'NingÃÂºn registro en este criterio.'}</td></tr>
                    ) : (
                      list.map((row, idx) => (
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b bg-gray-50">
                      ...
                      <th className="text-left py-2 px-3 font-semibold">CÃÂ©dula</th>
                    </tr>
                  </thead>
                  <tbody>
                    {list.length === 0 ? (
                      <tr><td colSpan={3} className="py-8 text-center text-gray-500">NingÃÂºn cliente en este criterio.</td></tr>
                    ) : (
                      list.map((row, idx) => (
                        <tr key={`${row.cliente_id}-${row.numero_cuota ? idx}`} className="border-b hover:bg-gray-50">
                          <td className="py-2 px-3">{idx + 1}</td>
