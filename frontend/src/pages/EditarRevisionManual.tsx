...
export function EditarRevisionManual() {
  ...
  const { isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      if (!prestamoId) throw new Error('ID de prÃÂ©stamo requerido')
  ...
  // Incluir estado actual del cliente si no estÃÂ¡ en la lista (valor legacy en BD)
  ...
  const handleGuardarParciales = async () => {
    ...
    if (!cambios.cliente && !cambios.prestamo && !cambios.cuotas) {
      toast.info('Ã¢ÂÂ¹Ã¯Â¸Â No hay cambios para guardar')
      return
    }
    ...
    try {
      ...
      if (cambios.cliente && clienteData.cliente_id) {
        ...
        if (Object.keys(clienteUpdate).length > 0) {
          try {
            ...
            toast.error(`Ã¢ÂÂ Error en cliente: ${errorMsg}`)
            console.error('Error guardando cliente:', err)
          }
        }
      }

      // Solo guardar prÃÂ©stamo si hay cambios
      if (cambios.prestamo && prestamoData.prestamo_id) {
        ...
        if (Object.keys(prestamoUpdate).length > 0) {
          try {
            ...
            errorOccurred = true
            const errorMsg = err?.response?.data?.detail || 'Error al guardar prÃÂ©stamo'
            toast.error(`Ã¢ÂÂ Error en prÃÂ©stamo: ${errorMsg}`)
            console.error('Error guardando prÃÂ©stamo:', err)
          }
        }
      }
          ...
          if (cuota.cuota_id) {
            ...
            if (Object.keys(cuotaUpdate).length > 0) {
              try {
                ...
                toast.error(`Ã¢ÂÂ Error en cuota #${cuota.numero_cuota}: ${errorMsg}`)
      ...
      if (!errorOccurred && savedSomething) {
        toast.success('Ã¢ÂÂ Cambios parciales guardados en BD')
        setCambios({ cliente: false, prestamo: false, cuotas: false })
        // Invalidar todas las vistas que muestran datos de prÃÂ©stamos, clientes y cuotas
        ...
        toast.warning('Ã¢ÂÂ Ã¯Â¸Â Algunos cambios no se guardaron. Revisa los errores arriba')
      }
    } catch (err: any) {
      const errorMsg = err?.response?.data?.detail || 'Error desconocido'
      toast.error(`Ã¢ÂÂ Error general: ${errorMsg}`)
  ...
  const handleGuardarYCerrar = async () => {
    ...
    const confirmar = window.confirm(
      'Ã¢ÂÂ Ã¯Â¸Â CONFIRMAR FINALIZACIÃÂN DE REVISIÃÂN\n\n' +
      'Ã¢ÂÂ Se guardarÃÂ¡n todos los cambios pendientes\n' +
      'Ã¢ÂÂ El prÃÂ©stamo se marcarÃÂ¡ como REVISADO\n' +
      'Ã¢ÂÂ NO PODRÃÂS EDITAR ESTE PRÃÂSTAMO DE NUEVO\n\n' +
      'ÃÂ¿EstÃÂ¡s completamente seguro?'
    )
    if (!confirmar) {
      toast.info('Ã¢ÂÂ¹Ã¯Â¸Â FinalizaciÃÂ³n cancelada')
      return
    }
    ...
    try {
      ...
      if (cambios.prestamo && prestamoData.prestamo_id) {
        ...
        if (Object.keys(prestamoUpdate).length > 0) {
          try {
            ...
            throw new Error(`Error en prÃÂ©stamo: ${err?.response?.data?.detail || 'Error desconocido'}`)
          }
        }
      }
      ...
      try {
        ...
        // PequeÃÂ±o delay antes de navegar para que el usuario vea el mensaje
        setTimeout(() => {
  ...
  const handleCerrar = () => {
    ...
    if (cambios.cliente || cambios.prestamo || cambios.cuotas) {
      const confirmar = window.confirm(
        'Ã¢ÂÂ Ã¯Â¸Â Tienes cambios sin guardar.\n\n' +
        'Si cierras ahora, se perderÃÂ¡n todos los cambios realizados.\n' +
        'ÃÂ¿EstÃÂ¡s seguro de que deseas cerrar sin guardar?'
      )
      if (!confirmar) return
    }
    ...
    return (
      <div className="flex justify-center items-center min-h-screen">
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-4">No se pudieron cargar los datos del prÃÂ©stamo</p>
          <Button onClick={() => navigate('/revision-manual')}>Volver a RevisiÃÂ³n Manual</Button>
        </div>
      </div>
    )
  }
      ...
      <div className="flex items-center justify-between sticky top-0 z-10 bg-white p-4 -mx-6 mb-4 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">
              RevisiÃÂ³n: {clienteData.nombres}
            </h1>
            <p className="text-sm text-gray-600">Edita los detalles del prÃÂ©stamo (cambios parciales permitidos)</p>
          </div>
        </div>
        <div className="flex gap-2">
          <Button
            ...
            className="gap-2"
            title="Guarda los cambios sin finalizar la revisiÃÂ³n"
          >
          ...
          <Button
            ...
            disabled={guardandoParcial || guardandoFinal}
            title="Guarda todos los cambios y finaliza la revisiÃÂ³n"
          >
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="grid sm:grid-cols-2 gap-4">
              ...
              <div>
                <label className="text-sm font-medium">CÃÂ©dula</label>
                <input
                  type="text"
              ...
              <div>
                <label className="text-sm font-medium">TelÃÂ©fono</label>
                <input
                  type="text"
                  ...
                  className="w-full border rounded px-3 py-2 mt-1"
                  placeholder="Ingresa telÃÂ©fono"
                />
              </div>
              <div>
              ...
              <div className="sm:col-span-2">
                <label className="text-sm font-medium">DirecciÃÂ³n</label>
                <textarea
                  ...
                  className="w-full border rounded px-3 py-2 mt-1"
                  placeholder="Ingresa direcciÃÂ³n"
                  rows={2}
                />
              </div>
              <div>
                <label className="text-sm font-medium">OcupaciÃÂ³n</label>
                <input
                  type="text"
                  ...
                  className="w-full border rounded px-3 py-2 mt-1"
                  placeholder="Ingresa ocupaciÃÂ³n"
                />
              </div>
              <div>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="grid sm:grid-cols-2 gap-4">
              ...
              <div>
                <label className="text-sm font-medium">NÃÂºmero de Cuotas</label>
                <input
                  type="number"
              ...
              <div>
                <label className="text-sm font-medium">Tasa de InterÃÂ©s (%)</label>
                <input
                  type="number"
                  step="0.01"
              ...
              <div>
                ...
                >
                  <SelectTrigger className="w-full mt-1">
                    <SelectValue placeholder="Ã¢ÂÂ" />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="-">Ã¢ÂÂ</SelectItem>
              ...
              <div>
                <label className="text-sm font-medium">CÃÂ©dula (prÃÂ©stamo)</label>
                <input
                  type="text"
                  ...
                  className="w-full border rounded px-3 py-2 mt-1"
                  placeholder="CÃÂ©dula"
                />
              </div>
              <div>
                <label className="text-sm font-medium">Nombres (prÃÂ©stamo)</label>
                <input
                  type="text"
              ...
              <div>
                <label className="text-sm font-medium">Cuota PerÃÂ­odo</label>
                <input
                  type="number"
                  step="0.01"
              ...
              <div>
                <label className="text-sm font-medium">Fecha Base CÃÂ¡lculo</label>
                <input
                  type="date"
              ...
              <div>
                <label className="text-sm font-medium">Fecha AprobaciÃÂ³n</label>
                <input
                  type="date"
              ...
              <div>
                <label className="text-sm font-medium">Estado PrÃÂ©stamo</label>
                <input
                  type="text"
              ...
              <div>
                <label className="text-sm font-medium">Modelo VehÃÂ­culo</label>
                <Select
