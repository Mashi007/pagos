...
export function EditarRevisionManual() {
  ...
  const { isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      if (!prestamoId) throw new Error('ID de prÃ©stamo requerido')
  ...
  // Incluir estado actual del cliente si no estÃ¡ en la lista (valor legacy en BD)
  ...
  const handleGuardarParciales = async () => {
    ...
    if (!cambios.cliente && !cambios.prestamo && !cambios.cuotas) {
      toast.info('â¹ï¸ No hay cambios para guardar')
      return
    }
    ...
    try {
      ...
      if (cambios.cliente && clienteData.cliente_id) {
        ...
        if (Object.keys(clienteUpdate).length > 0) {
          try {
            ...
            toast.error(`â Error en cliente: ${errorMsg}`)
            console.error('Error guardando cliente:', err)
          }
        }
      }

      // Solo guardar prÃ©stamo si hay cambios
      if (cambios.prestamo && prestamoData.prestamo_id) {
        ...
        if (Object.keys(prestamoUpdate).length > 0) {
          try {
            ...
            errorOccurred = true
            const errorMsg = err?.response?.data?.detail || 'Error al guardar prÃ©stamo'
            toast.error(`â Error en prÃ©stamo: ${errorMsg}`)
            console.error('Error guardando prÃ©stamo:', err)
          }
        }
      }
          ...
          if (cuota.cuota_id) {
            ...
            if (Object.keys(cuotaUpdate).length > 0) {
              try {
                ...
                toast.error(`â Error en cuota #${cuota.numero_cuota}: ${errorMsg}`)
      ...
      if (!errorOccurred && savedSomething) {
        toast.success('â Cambios parciales guardados en BD')
        setCambios({ cliente: false, prestamo: false, cuotas: false })
        // Invalidar todas las vistas que muestran datos de prÃ©stamos, clientes y cuotas
        ...
        toast.warning('â ï¸ Algunos cambios no se guardaron. Revisa los errores arriba')
      }
    } catch (err: any) {
      const errorMsg = err?.response?.data?.detail || 'Error desconocido'
      toast.error(`â Error general: ${errorMsg}`)
  ...
  const handleGuardarYCerrar = async () => {
    ...
    const confirmar = window.confirm(
      'â ï¸ CONFIRMAR FINALIZACIÃN DE REVISIÃN\n\n' +
      'â Se guardarÃ¡n todos los cambios pendientes\n' +
      'â El prÃ©stamo se marcarÃ¡ como REVISADO\n' +
      'â NO PODRÃS EDITAR ESTE PRÃSTAMO DE NUEVO\n\n' +
      'Â¿EstÃ¡s completamente seguro?'
    )
    if (!confirmar) {
      toast.info('â¹ï¸ FinalizaciÃ³n cancelada')
      return
    }
    ...
    try {
      ...
      if (cambios.prestamo && prestamoData.prestamo_id) {
        ...
        if (Object.keys(prestamoUpdate).length > 0) {
          try {
            ...
            throw new Error(`Error en prÃ©stamo: ${err?.response?.data?.detail || 'Error desconocido'}`)
          }
        }
      }
      ...
      try {
        ...
        // PequeÃ±o delay antes de navegar para que el usuario vea el mensaje
        setTimeout(() => {
  ...
  const handleCerrar = () => {
    ...
    if (cambios.cliente || cambios.prestamo || cambios.cuotas) {
      const confirmar = window.confirm(
        'â ï¸ Tienes cambios sin guardar.\n\n' +
    ...
    return (
      <div className="flex justify-center items-center min-h-screen">
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-4">No se pudieron cargar los datos del prÃ©stamo</p>
          <Button onClick={() => navigate('/revision-manual')}>Volver a RevisiÃ³n Manual</Button>
        </div>
      </div>
    )
  }
      ...
      <div className="flex items-center justify-between sticky top-0 z-10 bg-white p-4 -mx-6 mb-4 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">
              RevisiÃ³n: {clienteData.nombres}
            </h1>
            <p className="text-sm text-gray-600">Edita los detalles del prÃ©stamo (cambios parciales permitidos)</p>
          </div>
        </div>
        <div className="flex gap-2">
          <Button
            ...
            className="gap-2"
            title="Guarda los cambios sin finalizar la revisiÃ³n"
          >
          ...
          <Button
            ...
            disabled={guardandoParcial || guardandoFinal}
            title="Guarda todos los cambios y finaliza la revisiÃ³n"
          >
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full border rounded px-3 py-2 mt-1"
                  placeholder="Ingresa ocupaciÃ³n"
                />
              </div>
              <div>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="grid sm:grid-cols-2 gap-4">
              ...
              <div>
                <label className="text-sm font-medium">Tasa de InterÃ©s (%)</label>
                <input
                  type="number"
                  step="0.01"
              ...
              <div>
                <label className="text-sm font-medium">CÃ©dula (prÃ©stamo)</label>
                <input
                  type="text"
              ...
              <div>
                <label className="text-sm font-medium">Nombres (prÃ©stamo)</label>
                <input
                  type="text"
              ...
              <div>
                <label className="text-sm font-medium">Cuota PerÃ­odo</label>
                <input
                  type="number"
                  step="0.01"
              ...
              <div>
                <label className="text-sm font-medium">Fecha Base CÃ¡lculo</label>
                <input
                  type="date"
              ...
              <div>
                <label className="text-sm font-medium">Fecha AprobaciÃ³n</label>
                <input
                  type="date"
              ...
              <div>
                <label className="text-sm font-medium">Estado PrÃ©stamo</label>
                <input
                  type="text"
              ...
              <div>
                <label className="text-sm font-medium">Modelo VehÃ­culo</label>
                <Select
