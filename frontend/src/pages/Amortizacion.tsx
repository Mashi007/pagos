...
const DEFAULT_MONTO_FINANCIADO_1 = 20000.0

const DEFAULT_MONTO_FINANCIADO_2 = 15000.0

const DEFAULT_MONTO_FINANCIADO_3 = 18000.0
...
export function Amortizacion() {
  ...
  const filteredAmortizaciones = mockAmortizaciones.filter(amortizacion => {
    const matchesSearch =
      amortizacion.cliente.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const totalFinanciado = mockAmortizaciones.reduce(
    (sum, a) => sum + a.monto_financiado,
    0
  )

  const totalSaldoPendiente = mockAmortizaciones.reduce(
    (sum, a) => sum + a.saldo_pendiente,
    0
  )

  const clientesEnMora = mockAmortizaciones.filter(
    a => a.estado === 'en_mora'
  ).length
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-gray-600">
              Monitorea el progreso de los préstamos y su estado de pago
            </p>
          </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-blue-600">
              $
              {totalFinanciado.toLocaleString('es-ES', {
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-orange-600">
              $
              {totalSaldoPendiente.toLocaleString('es-ES', {
                minimumFractionDigits: 2,
              })}
            </div>
            ...
            <div className="space-y-4">
              {filteredAmortizaciones.map(amortizacion => {
                const progreso = calcularProgreso(
                  amortizacion.cuotas_pagadas,
                  amortizacion.cuotas_totales
                )
                  ...
                  >
                      ...
                      <div className="flex-1">
                        ...
                        <div className="grid grid-cols-2 gap-4 text-sm md:grid-cols-4">
                          ...
                          <div>
                            ...
                            <p className="font-medium">
                              $
                              {amortizacion.cuota_mensual.toLocaleString(
                                'es-ES',
                                { minimumFractionDigits: 2 }
                              )}
                            </p>
                          ...
                          <div>
                            ...
                            <p className="font-medium">
                              $
                              {amortizacion.saldo_pendiente.toLocaleString(
                                'es-ES',
                                { minimumFractionDigits: 2 }
                              )}
                            </p>
                    ...
                    <div className="mt-4">
                      <div className="mb-2 flex justify-between text-sm text-gray-600">
                        ...
                        <span>
                          {amortizacion.cuotas_pagadas}/
                          {amortizacion.cuotas_totales} cuotas (
                          {progreso.toFixed(1)}%)
                        </span>
                      </div>
              ...
              {filteredAmortizaciones.length === 0 && (
                <div className="py-8 text-center text-gray-500">
                  ...
                  <p>
                    No se encontraron amortizaciones con los filtros aplicados
                  </p>
                </div>
              )}
            </div>
