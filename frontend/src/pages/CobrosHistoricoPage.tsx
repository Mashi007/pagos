/**
 * HistÃ³rico de pagos reportados por cliente (bÃºsqueda por cÃ©dula).
 */
...
export default function CobrosHistoricoPage() {
  ...
  const handleBuscar = async () => {
    if (!cedula.trim()) {
      toast.error('Ingrese la cÃ©dula.')
      return
    }
    setLoading(true)
    try {
  ...
  return (
    <div className="p-6 space-y-6">
      <h1 className="text-2xl font-semibold">HistÃ³rico por cliente</h1>

      <Card>
        <CardHeader>
          <CardTitle>Buscar por cÃ©dula</CardTitle>
        </CardHeader>
      ...
      {cedulaBusqueda && (
        <Card>
          <CardHeader>
            <CardTitle>Pagos reportados â CÃ©dula {cedulaBusqueda}</CardTitle>
          </CardHeader>
          <CardContent>
            {!items.length ? (
              <p className="text-gray-500">No hay registros para esta cÃ©dula.</p>
            ) : (
                  ...
                  <tbody>
                    {items.map((row) => (
                      <tr key={row.id} className="border-b">
                        ...
                        <td className="py-2">{row.fecha_pago ? 'â'}</td>
                        ...
                        <td className="py-2">{row.tiene_recibo ? 'SÃ­' : 'â'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
