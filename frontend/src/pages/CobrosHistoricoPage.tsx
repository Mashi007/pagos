/**
 * HistÃÂ³rico de pagos reportados por cliente (bÃÂºsqueda por cÃÂ©dula).
 */
...
export default function CobrosHistoricoPage() {
  ...
  const handleBuscar = async () => {
    if (!cedula.trim()) {
      toast.error('Ingrese la cÃÂ©dula.')
      return
    }
    setLoading(true)
    try {
  ...
  return (
    <div className="p-6 space-y-6">
      <h1 className="text-2xl font-semibold">HistÃÂ³rico por cliente</h1>

      <Card>
        <CardHeader>
          <CardTitle>Buscar por cÃÂ©dula</CardTitle>
        </CardHeader>
      ...
      {cedulaBusqueda && (
        <Card>
          <CardHeader>
            <CardTitle>Pagos reportados Ã¢ÂÂ CÃÂ©dula {cedulaBusqueda}</CardTitle>
          </CardHeader>
          <CardContent>
            {!items.length ? (
              <p className="text-gray-500">No hay registros para esta cÃÂ©dula.</p>
            ) : (
                  ...
                  <tbody>
                    {items.map((row) => (
                      <tr key={row.id} className="border-b">
                        ...
                        <td className="py-2">{row.fecha_pago ? 'Ã¢ÂÂ'}</td>
                        ...
                        <td className="py-2">{row.tiene_recibo ? 'SÃÂ­' : 'Ã¢ÂÂ'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
