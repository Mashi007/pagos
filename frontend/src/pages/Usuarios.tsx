ï»¿ï»¿import { useState, useEffect, useCallback } from 'react'
...
export function Usuarios() {
  ...
  const handleEliminar = async (id: number) => {
    try {
      ...
      const confirmar = window.confirm(
        'Ã¢Å¡ Ã¯Â¸Â Â¿EstÃ¡s seguro de que quieres ELIMINAR PERMANENTEMENTE este usuario?\n\n' +
      ...
      toast.success('Ã¢Åâ¦ Usuario eliminado PERMANENTEMENTE de la base de datos')
      cargarUsuarios() // Recargar lista
    } catch (err) {
      logger.apiError(`/api/v1/usuarios/${id}`, err, { action: 'eliminarUsuario', userId: id })
      toast.error('Ã¢ÂÅ Error al eliminar usuario permanentemente')
    }
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingUsuario) {
        ...
        // Ã¢Åâ¦ CRÃTICO: Siempre incluir is_admin explÃ­citamente, incluso si es False
        ...
        // Ã¢Åâ¦ CRÃTICO: Validar que editingUsuario.id existe y es vÃ¡lido
        if (!editingUsuario || !editingUsuario.id) {
          console.error('Ã¢ÂÅ [Usuarios] ERROR: editingUsuario o editingUsuario.id es invÃ¡lido:', editingUsuario)
          toast.error('Error: Usuario a editar no vÃ¡lido')
          return
        }

        // Ã¢Åâ¦ CRÃTICO: Logging detallado del usuario que se estÃ¡ actualizando
        console.log('Ã°Å¸âÂ¤ [Usuarios] Enviando actualizaciÃ³n:', {
        ...
        // Ã¢Åâ¦ Si se cambiÃ³ la contraseÃ±a del usuario actual, forzar logout y redirigir al login
        if (passwordChanged && currentUser && editingUsuario.id === currentUser.id) {
          ...
          // Actualizar lista en local para evitar GET extra que podrÃ­a 401 y redirigir al login
        ...
        // AÃ±adir a la lista en local para evitar GET extra que podrÃ­a 401 y redirigir al login
  ...
  const getRoleBadgeColor = (rol: string) => {  // Cambio clave: rol Ã¢â â is_admin
    return rol === 'administrador' ? 'bg-red-600' : 'bg-blue-600'  // Cambio clave: rol Ã¢â â is_admin
  }
      ...
      <div className="grid gap-4 md:grid-cols-4">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-red-600">
                  {usuarios.filter(u => (u.rol || 'operativo') === 'administrador').length}  {/* Cambio clave: rol Ã¢â â is_admin */}
                </p>
                <p className="text-xs text-gray-400 mt-1">
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-500">Ãltimo Mes</p>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead>Ãltimo Acceso</TableHead>
              ...
              {loading ? (
                ...
                usuariosFiltrados.map((usuario) => (
                  <TableRow key={usuario.id}>
                    ...
                    <TableCell>
                      <Badge className={getRoleBadgeColor(usuario.rol || 'operativo')}>  {/* Cambio clave: rol Ã¢â â is_admin */}
                        {(usuario.rol || 'operativo') === 'administrador' ? 'Administrador' : 'Operativo'}  {/* Cambio clave: rol Ã¢â â is_admin */}
                      </Badge>
                    </TableCell>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col flex-1 min-h-0">
              ...
              <div>
                ...
                <PasswordField
                  ...
                  showGenerateButton={true}  // Ã¢Åâ¦ SIEMPRE mostrar botÃ³n de generar
                  showCopyButton={true}      // Ã¢Åâ¦ SIEMPRE mostrar botÃ³n de copiar
                  className="mt-1"
                />
              </div>
