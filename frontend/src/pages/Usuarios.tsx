...
export function Usuarios() {
  ...
  const cargarUsuarios = async () => {
    try {
      ...
      if (response.items && Array.isArray(response.items)) {
        ...
        logger.debug('Usuarios cargados exitosamente', {
  ...
  const handleEliminar = async (id: number) => {
    try {
      ...
      logger.apiError(`/api/v1/usuarios/${id}`, err, {
  ...
  const handleToggleActivo = async (usuario: User) => {
    try {
      ...
      toast.success(
        `Usuario ${usuario.is_active ? 'desactivado' : 'activado'} exitosamente`
      )
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isFormValid()) {
      toast.error(
        'Por favor completa todos los campos requeridos y asegúrate de que la contraseña cumpla con todos los requisitos'
      )
    ...
    try {
      ...
      if (editingUsuario) {
        ...
        if (!editingUsuario || !editingUsuario.id) {
          console.error(
            'âŒ [Usuarios] ERROR: editingUsuario o editingUsuario.id es inválido:',
            editingUsuario
          )
        ...
        if (
          ...
          toast.success(
            'Contraseña cambiada exitosamente. Debes volver a iniciar sesión.'
          )
          ...
          setUsuarios(prev =>
            prev.map(u => (u.id === editingUsuario.id ? updatedUser : u))
          )
        }
      } else {
        // Crear nuevo usuario
      ...
      logger.userAction(
        editingUsuario ? 'actualizar_usuario' : 'crear_usuario',
        {
          userId: editingUsuario?.id,
        ...
        >
          ...
          >
            ...
            >
              <div className="flex-1 space-y-4 overflow-y-auto p-6">
                ...
                <div>
                  ...
                  <p className="mt-1 text-xs text-gray-500">
                    Dejar vacío para "Sin especificar"
                  </p>
                </div>
                ...
                <div>
                  ...
                  >
                    Contraseña{' '}
                    {editingUsuario && '(dejar vacío para mantener la actual)'}
                  </label>
              ...
              <div className="flex flex-shrink-0 justify-end space-x-2 border-t border-gray-200 bg-gray-50 p-6">
                <Button
                  type="button"
                  variant="outline"
                  onClick={handleCloseForm}
                ...
                <Button
                  ...
                  className="flex items-center gap-2 disabled:cursor-not-allowed disabled:bg-gray-400"
                >
                  {isSubmitting ? (
