...
export function CampanasPage() {
  ...
  const [programarCampana, setProgramarCampana] = useState<CampanaCrm | null>(
    null
  )
  ...
  const load = async () => {
    ...
    try {
      const res = await campanasService.list({
        page,
        per_page: 20,
  ...
  const handleCreateSubmit = async () => {
    ...
    try {
      ...
      const cuerpoHtml = form.modoHtml
        ? (form.cuerpoHtml || form.cuerpoTexto || '').trim() || null
        : null
      ...
      await campanasService.create({
        ...
        destinatarios_cliente_ids: destinatarios_cliente_ids?.length
          ? destinatarios_cliente_ids
          : undefined,
      })
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <span className="ml-2 text-sm text-slate-700">
                Correo del sistema (Configuración &gt; Email)
              </span>
            </div>
            ...
            <div>
              ...
              <input
                ...
                onChange={e =>
                  setForm(f => ({
                    ...f,
                    destinatariosClienteIds: e.target.value,
                  }))
                }
              ...
              <div className="mt-2 rounded-lg border border-slate-200 bg-slate-50/80 p-3">
                ...
                {form.destinatariosClienteIds.trim() === '' ? (
                  <p className="text-sm text-slate-600">
                    Todos los correos de tabla Clientes ({previewTotal}{' '}
                    contactos)
                  </p>
                    ...
                    <ul className="max-h-40 space-y-1 overflow-y-auto text-sm text-slate-700">
                      {seleccionadosPreview.muestra.length === 0 ? (
                        <li className="text-amber-700">
                          Ningún contacto con email válido para los IDs
                          indicados.
                        </li>
                      ) : (
                  ...
                  <p className="text-sm text-slate-500">
                    Cargando contactos seleccionados…
                  </p>
                )}
              </div>
            </div>
            ...
            <div>
              <div className="mb-1 flex items-center gap-2">
                ...
                <label className="flex items-center gap-1 text-sm text-slate-600">
                  <input
                    ...
                    onChange={e =>
                      setForm(f => ({ ...f, modoHtml: e.target.checked }))
                    }
                  />
                  Usar HTML
                </label>
              ...
              <textarea
                ...
                onChange={e =>
                  setForm(f =>
                    f.modoHtml
                      ? { ...f, cuerpoHtml: e.target.value }
                      : { ...f, cuerpoTexto: e.target.value }
                  )
                }
                placeholder={
                  form.modoHtml
                    ? 'Escribe HTML aquí...'
                    : 'Texto del mensaje (o activa HTML para formato)'
                }
                rows={8}
            ...
            <div>
              <label className="mb-1 block text-sm font-medium text-slate-700">
                Adjunto (un solo archivo, opcional)
              </label>
      ...
      <Dialog
        open={!!programarCampana}
        onOpenChange={open => !open && setProgramarCampana(null)}
      >
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <p className="mt-1 text-sm text-slate-600">
              {programarCampana?.nombre}. Indica cada cuántos días y/o horas se
              enviará la campaña.
            </p>
          </DialogHeader>
            ...
            <div>
              ...
              <input
                ...
                onChange={e =>
                  setProgramarForm(f => ({
                    ...f,
                    cada_dias: parseInt(e.target.value, 10) || 0,
                  }))
                }
                className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm"
              />
            ...
            <div>
              ...
              <input
                ...
                onChange={e =>
                  setProgramarForm(f => ({
                    ...f,
                    cada_horas: parseInt(e.target.value, 10) || 0,
                  }))
                }
                className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm"
              />
          ...
          <DialogFooter>
            <Button
              variant="outline"
              onClick={() => setProgramarCampana(null)}
              disabled={programarLoading}
            >
              Cancelar
            </Button>

            <Button
              onClick={handleProgramarSubmit}
              disabled={
                programarLoading ||
                (!programarForm.cada_dias && !programarForm.cada_horas)
              }
            >
                ...
                {list.items.map(c => (
                  <tr key={c.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3">
                      ...
                      {c.estado === 'programada' &&
                        (c.programado_cada_dias || c.programado_cada_horas) && (
                          <div className="mt-0.5 text-xs text-slate-500">
                            {[
                              c.programado_cada_dias &&
                                `cada ${c.programado_cada_dias} día(s)`,
                              c.programado_cada_horas &&
                                `cada ${c.programado_cada_horas} h`,
                            ]
                              .filter(Boolean)
                              .join(' · ')}
                          </div>
                        ...
                        {c.estado === 'borrador' &&
                          c.total_destinatarios > 0 && (
                            <>
                              <Button
                                size="sm"
                                onClick={() => handleIniciarEnvio(c)}
                                title="Enviar ahora (una vez)"
                              >
                              ...
                              <Button
                                ...
                                size="sm"
                                onClick={() => {
                                  setProgramarCampana(c)
                                  setProgramarForm({
                                    cada_dias: 0,
                                    cada_horas: 0,
                                  })
                                }}
                                title="Programar envíos recurrentes"
                              >
