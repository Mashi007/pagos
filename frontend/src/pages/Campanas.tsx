...
export function CampanasPage() {
  ...
  const handleCreateSubmit = async () => {
    ...
    if (!nombre) {
      toast.error('Escribe un nombre para la campaÃ±a.')
      return
    }
    if (!asunto) {
    ...
    try {
      ...
      toast.success('CampaÃ±a creada en borrador.')
      setShowCreate(false)
      load()
    } catch (e: unknown) {
      toast.error((e as Error)?.message || 'Error al crear la campaÃ±a.')
    } finally {
  ...
  const handleEliminar = async (c: CampanaCrm) => {
    ...
    if (!window.confirm(`Â¿Eliminar la campaÃ±a Â«${c.nombre}Â»?`)) return
    try {
  ...
  const handleProgramarSubmit = async () => {
    ...
    if (!dias && !horas) {
      toast.error('Indica cada cuÃ¡ntos dÃ­as o cada cuÃ¡ntas horas.')
      return
    }
    ...
    try {
      ...
      toast.success('CampaÃ±a programada. Se enviarÃ¡ en el intervalo indicado.')
    ...
    <div className="space-y-6">
      ...
      <Dialog open={showCreate} onOpenChange={setShowCreate}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>Nueva campaÃ±a (correo masivo)</DialogTitle>
          ...
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Nombre de la campaÃ±a</label>
              <input
                type="text"
                value={form.nombre}
            ...
            <div>
              ...
              <span className="ml-2 text-slate-700 text-sm">Correo del sistema (ConfiguraciÃ³n &gt; Email)</span>
            </div>
            <div>
              ...
              <input
                ...
                placeholder="VacÃ­o = todos. O escribe IDs de cliente separados por coma (ej. 123 o 123, 456)"
                className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm"
              />
              <div className="mt-2 rounded-lg border border-slate-200 bg-slate-50/80 p-3">
                ...
                {form.destinatariosClienteIds.trim() === '' ? (
                  ...
                  <>
                    ...
                    <ul className="max-h-40 overflow-y-auto space-y-1 text-sm text-slate-700">
                      {seleccionadosPreview.muestra.length === 0 ? (
                        <li className="text-amber-700">NingÃºn contacto con email vÃ¡lido para los IDs indicados.</li>
                      ) : (
                        seleccionadosPreview.muestra.map((d, i) => (
                          <li key={d.cliente_id ? i} className="flex items-center gap-2">
                            ...
                            {d.nombres && <span className="text-slate-500">â {d.nombres}</span>}
                          </li>
                        ))
                      )}
                    </ul>
                  ...
                  <p className="text-sm text-slate-500">Cargando contactos seleccionadosâ¦</p>
                )}
              </div>
            </div>
            <div>
            ...
            <div>
              ...
              <textarea
                ...
                placeholder={form.modoHtml ? 'Escribe HTML aquÃ­...' : 'Texto del mensaje (o activa HTML para formato)'}
                rows={8}
          ...
          <DialogFooter>
            ...
            <Button onClick={handleCreateSubmit} disabled={createLoading}>
              {createLoading ? 'Creandoâ¦' : 'Crear campaÃ±a (borrador)'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!programarCampana} onOpenChange={(open) => !open && setProgramarCampana(null)}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Programar envÃ­os recurrentes</DialogTitle>
            <p className="text-sm text-slate-600 mt-1">
              {programarCampana?.nombre}. Indica cada cuÃ¡ntos dÃ­as y/o horas se enviarÃ¡ la campaÃ±a.
            </p>
          </DialogHeader>
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Cada cuÃ¡ntos dÃ­as (0 = no usar)</label>
              <input
                type="number"
                min={0}
                max={365}
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Cada cuÃ¡ntas horas (0 = no usar)</label>
              <input
                type="number"
                min={0}
                max={8760}
          ...
          <DialogFooter>
            ...
            <Button onClick={handleProgramarSubmit} disabled={programarLoading || (!programarForm.cada_dias && !programarForm.cada_horas)}>
              {programarLoading ? 'Programandoâ¦' : 'Programar'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
                  ...
                  <tr key={c.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3">
                      ...
                      {c.estado === 'programada' && (c.programado_cada_dias || c.programado_cada_horas) && (
                        <div className="text-xs text-slate-500 mt-0.5">
                          {[c.programado_cada_dias && `cada ${c.programado_cada_dias} dÃ­a(s)`, c.programado_cada_horas && `cada ${c.programado_cada_horas} h`].filter(Boolean).join(' Â· ')}
                        </div>
                      )}
                    </td>
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex flex-wrap items-center justify-end gap-1">
                        ...
                        {c.estado === 'enviando' && (
                          <Button variant="outline" size="sm" onClick={() => handleParar(c)} title="Parar envÃ­o">
                            <Pause className="h-4 w-4" /> Parar
                          </Button>
                        )}
                        {(c.estado === 'borrador' || c.estado === 'cancelada') && (
                          <Button variant="outline" size="sm" onClick={() => handleEliminar(c)} title="Eliminar campaÃ±a" className="text-red-600 hover:text-red-700">
                        ...
                        {c.estado === 'borrador' && c.total_destinatarios > 0 && (
                          <>
                            ...
                            <Button variant="outline" size="sm" onClick={() => { setProgramarCampana(c); setProgramarForm({ cada_dias: 0, cada_horas: 0 }) }} title="Programar envÃ­os recurrentes">
