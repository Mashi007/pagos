...
export function Auditoria() {
  ...
  // Cargar auditorÃÂ­a cuando cambian los filtros o la pÃÂ¡gina
  useEffect(() => {
    cargarAuditoria()
    // Actualizar estadÃÂ­sticas cuando se recarga la lista
    cargarEstadisticas()
  ...
  // Cargar estadÃÂ­sticas al montar y luego con polling mÃÂ¡s frecuente
  useEffect(() => {
    cargarEstadisticas()
  ...
  const cargarAuditoria = async () => {
    try {
      ...
      if (response.items && response.items.length > 0) {
        ...
        if (response.total === 0) {
          console.warn('[Auditoria] No hay registros de auditorÃÂ­a en la base de datos')
        }
      }
    } catch (err: any) {
      console.error('[Auditoria] Error API:', err)
      const errorMessage = err?.response?.data?.detail || err?.message || 'Error al cargar auditorÃÂ­a'
      setError(errorMessage)
      toast.error(`Error al cargar auditorÃÂ­a: ${errorMessage}`)
      setAuditorias([])
      setTotal(0)
    } finally {
      setLoading(false)
  ...
  const cargarEstadisticas = async (mostrarToast = false) => {
    try {
      console.log('[Auditoria] Cargando estadÃÂ­sticas de auditorÃÂ­a...')
      const response = await auditoriaService.obtenerEstadisticas()
      console.log('EstadÃÂ­sticas recibidas:', response)
      setStats(response)
      if (mostrarToast) {
        toast.success('KPIs actualizados correctamente')
      }
    } catch (err: any) {
      console.error('[Auditoria] Error cargando estadÃÂ­sticas:', err)
      const errorMessage = err?.response?.data?.detail || err?.message || 'Error al cargar estadÃÂ­sticas'
  ...
  const handleExportarExcel = async () => {
    try {
      ...
      toast.success('AuditorÃÂ­a exportada exitosamente')
    } catch (err) {
      toast.error('Error al exportar auditorÃÂ­a')
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">AuditorÃÂ­a del Sistema</h1>
        ...
        <div className="flex gap-2">
          <Button
            ...
            variant="outline"
            disabled={loading}
            title="Actualizar estadÃÂ­sticas"
          >
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid gap-4 md:grid-cols-3">
            ...
            <div>
              <label className="block text-sm font-medium mb-1">MÃÂ³dulo</label>
              <Select value={filtros.modulo} onValueChange={(value) => setFiltros({ ...filtros, modulo: value })}>
                <SelectTrigger>
                  <SelectValue placeholder="Seleccionar mÃÂ³dulo" />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="ALL">Todos los mÃÂ³dulos</SelectItem>
                  <SelectItem value="USUARIOS">Usuarios</SelectItem>
                  <SelectItem value="CLIENTES">Clientes</SelectItem>
                  <SelectItem value="PRESTAMOS">PrÃÂ©stamos</SelectItem>
                  <SelectItem value="PAGOS">Pagos</SelectItem>
                  <SelectItem value="COBRANZAS">Cobranzas</SelectItem>
                  <SelectItem value="REPORTES">Reportes</SelectItem>
                  <SelectItem value="AUDITORIA">AuditorÃÂ­a</SelectItem>
                  <SelectItem value="CONFIGURACION">ConfiguraciÃÂ³n</SelectItem>
                </SelectContent>
              </Select>
            </div>

            <div>
              <label className="block text-sm font-medium mb-1">AcciÃÂ³n</label>
              <Select value={filtros.accion} onValueChange={(value) => setFiltros({ ...filtros, accion: value })}>
                <SelectTrigger>
                  <SelectValue placeholder="Seleccionar acciÃÂ³n" />
                </SelectTrigger>
                <SelectContent>
          ...
          <Table className="[&_table]:table-fixed">
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="w-[120px]">AcciÃÂ³n</TableHead>
                <TableHead className="w-[120px]">MÃÂ³dulo</TableHead>
                <TableHead className="w-[140px]">Campo</TableHead>
                <TableHead className="min-w-[200px]">DescripciÃÂ³n</TableHead>
              ...
              {loading ? (
                <TableRow>
                  <TableCell colSpan={7} className="text-center py-8">
                    ...
                    <p className="text-gray-500">Cargando auditorÃÂ­a...</p>
                  </TableCell>
                </TableRow>
              ) : error ? (
                <TableRow>
                ...
                <TableRow>
                  <TableCell colSpan={7} className="text-center py-8">
                    <p className="text-gray-500">No se encontraron registros de auditorÃÂ­a</p>
                  </TableCell>
                </TableRow>
              ) : (
