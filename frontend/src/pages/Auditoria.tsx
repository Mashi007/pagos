...
export function Auditoria() {
  ...
  const cargarAuditoria = async () => {
    try {
      ...
      if (response.items && response.items.length > 0) {
        ...
        if (response.total === 0) {
          console.warn('[Auditoria] No hay registros de auditorÃ­a en la base de datos')
        }
      }
    } catch (err: any) {
  ...
  const cargarEstadisticas = async (mostrarToast = false) => {
    try {
      console.log('[Auditoria] Cargando estadÃ­sticas de auditorÃ­a...')
      const response = await auditoriaService.obtenerEstadisticas()
      console.log('EstadÃ­sticas recibidas:', response)
      setStats(response)
      if (mostrarToast) {
        toast.success('KPIs actualizados correctamente')
      }
    } catch (err: any) {
      console.error('[Auditoria] Error cargando estadÃ­sticas:', err)
  ...
  const handleExportarExcel = async () => {
    try {
      ...
      toast.success('AuditorÃ­a exportada exitosamente')
    } catch (err) {
      toast.error('Error al exportar auditorÃ­a')
