...
// SubmenÃÂºs eliminados: financiamiento, cuotas, cobranza, analisis, pagos
...
export function DashboardMenu() {
  ...
  const [periodo, setPeriodo] = useState('ultimos_12_meses') // Por defecto ÃÂºltimos 12 meses para que los grÃÂ¡ficos muestren datos recientes
  /** PerÃÂ­odo por grÃÂ¡fico: cada grÃÂ¡fico puede usar el general o uno propio. Key = id del grÃÂ¡fico, value = dÃÂ­a|semana|mes|aÃÂ±o o '' = usar general */
  ...
  /** PerÃÂ­odo efectivo para un grÃÂ¡fico: el del grÃÂ¡fico si estÃÂ¡ definido, si no el general */
  ...
  // ÃÂÃÂ¢ÃÂÃ¢ÂÂÃ¢ÂÂ¦ OPTIMIZACIÃÂÃ¢ÂÂN PRIORIDAD 1: Carga por batches con priorizaciÃÂ³n
  // Batch 1: CRÃÂTICO - Opciones de filtros y KPIs principales (carga inmediata)
  const { data: opcionesFiltros, isLoading: loadingOpcionesFiltros, isError: errorOpcionesFiltros } = useQuery({
    ...
    refetchOnWindowFocus: false, // No recargar automÃÂ¡ticamente
    // ÃÂÃÂ¢ÃÂÃ¢ÂÂÃ¢ÂÂ¦ Prioridad mÃÂ¡xima - carga inmediatamente
  })

  // Batch 1: CRÃÂTICO - KPIs principales (visible primero para el usuario)
  // Los KPIs siempre reflejan solo el mes actual (ej. febrero) - independiente del perÃÂ­odo de los grÃÂ¡ficos
  ...
  const { data: kpisPrincipales, isLoading: loadingKPIs, isError: errorKPIs, refetch } = useQuery({
    ...
    staleTime: 5 * 60 * 1000, // 5 min para datos mÃÂ¡s frescos
  ...
  // Batch 2: IMPORTANTE - Dashboard admin (grÃÂ¡fico principal). Siempre con perÃÂ­odo que incluya 2025 si hay datos.
  ...
  const { data: datosDashboard, isLoading: loadingDashboard, isError: errorDashboardAdmin } = useQuery({
    ...
    staleTime: 4 * 60 * 60 * 1000, // 4 h: backend actualiza cachÃÂ© a las 6:00, 13:00, 16:00
    retry: 1,
    refetchOnWindowFocus: false, // Evitar refetch al cambiar de pestaÃÂ±a (carga lenta)
    enabled: true,
  })

  // Batch 3: Morosidad por dÃÂ­a (desde tabla cuotas). Respeta rango del perÃÂ­odo (ej. desde 2025).
  ...
  const { data: datosMorosidadPorDia, isLoading: loadingMorosidadPorDia } = useQuery({
    ...
    queryFn: async () => {
      ...
      // No enviar fecha_inicio/fecha_fin: el backend siempre usa hoy y hacia atrÃÂ¡s
  ...
  // Batch 3: GrÃÂ¡ficos secundarios rÃÂ¡pidos. PerÃÂ­odo por grÃÂ¡fico; filtros (fecha_inicio/fecha_fin) se envÃÂ­an siempre.
  // Batch 4: BAJA - GrÃÂ¡ficos menos crÃÂ­ticos. PerÃÂ­odo con fallback para incluir 2025.
  ...
  const { data: datosFinanciamientoRangos, isLoading: loadingFinanciamientoRangos, isError: errorFinanciamientoRangos, error: errorFinanciamientoRangosDetail, refetch: refetchFinanciamientoRangos } = useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        // Si es otro error, retornar respuesta vacÃÂ­a para no romper el dashboard
    ...
    refetchOnWindowFocus: false,
    enabled: true,
    retry: 1, // ÃÂÃÂ¢ÃÂÃ¢ÂÂÃ¢ÂÂ¦ Permitir 1 reintento para errores de red
  ...
  const { data: datosCobranzasSemanales, isLoading: loadingCobranzasSemanales } = useQuery({
    ...
    queryFn: async () => {
      ...
      queryParams.append('semanas', '12') // ÃÂltimas 12 semanas
      const response = await apiClient.get(
  ...
  // Mostrar toast cuando falla la carga del grÃÂ¡fico principal (auditorÃÂ­a: no fallar en silencio)
  useEffect(() => {
    if (errorDashboardAdmin) {
      toast.error('No se pudo cargar el grÃÂ¡fico de evoluciÃÂ³n mensual. Intenta de nuevo o recarga la pÃÂ¡gina.')
    }
  }, [errorDashboardAdmin])

  // NOTA: No necesitamos invalidar queries manualmente aquÃÂ­
  // React Query detecta automÃÂ¡ticamente los cambios en queryKey (que incluye JSON.stringify(filtros))
  // y refetch automÃÂ¡ticamente cuando cambian los filtros o el perÃÂ­odo

  const handleRefresh = async () => {
    setIsRefreshing(true)
    try {
      // Invalidar y refrescar solo las queries usadas por esta pÃÂ¡gina (auditorÃÂ­a: alinear con queryKeys reales)
  ...
  // Etiqueta del perÃÂ­odo activo (general)
  ...
  /** Etiqueta de rango de fechas para un grÃÂ¡fico (usa perÃÂ­odo del grÃÂ¡fico o el general) */
  ...
  /** Selector de perÃÂ­odo por grÃÂ¡fico (dropdown para cada tarjeta) */
  ...
  // Estilos de mayor calidad para todos los grÃÂ¡ficos (tooltips, ejes, grid)
  ...
  // Bandas desde backend: $0-$200 ... $1000-$1200, $1200-$1400, MÃÂ¡s de $1400 (cantidad de prÃÂ©stamos por banda)
  const datosBandas200 = useMemo(() => {
    try {
      if (!datosFinanciamientoRangos?.rangos || datosFinanciamientoRangos.rangos.length === 0) {
        return []
      }
      // Orden descendente (mayor banda arriba): MÃÂ¡s de $1400, $1200-$1400, $1000-$1200, ...
      const orden = ['MÃÂ¡s de $1,400', '$1,200 - $1,400', '$1,000 - $1,200', '$800 - $1,000', '$600 - $800', '$400 - $600', '$200 - $400', '$0 - $200']
  ...
  // Si hay un error crÃÂ­tico en las queries principales, mostrar mensaje pero no bloquear
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
      <div className="container mx-auto px-4 py-8 space-y-8">
        ...
        {/* Mensaje de error si hay problemas crÃÂ­ticos */}
        {hasCriticalError && (
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            <div className="flex items-center gap-2 text-yellow-800">
              ...
              <p className="text-sm font-medium">
                Algunos datos no se pudieron cargar. Por favor, recarga la pÃÂ¡gina o intenta mÃÂ¡s tarde.
              </p>
            </div>
          </motion.div>
        )}

        {/* Barra de filtros: perÃÂ­odo general (cada grÃÂ¡fico puede usar este o uno propio) */}
        <motion.div
            ...
            {loadingKPIs ? (
              ...
              >
                ...
                <KpiCardLarge
                  ...
                  subtitle="Cuotas vencidas sin pagar (solo si ya pasÃÂ³ la fecha de vencimiento)"
        ...
        {loadingDashboard ? (
          ...
          <Card className="border-red-200 bg-red-50">
            <CardContent className="p-6">
              <div className="flex items-center gap-3 text-red-700">
                ...
                <div>
                  <p className="font-medium">Error al cargar el grÃÂ¡fico de evoluciÃÂ³n mensual</p>
                  <p className="text-sm mt-1">Usa el botÃÂ³n ÃÂ«ActualizarÃÂ» en la barra de filtros o recarga la pÃÂ¡gina.</p>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <div className="space-y-6">
            {/* Aviso cuando no hay datos en los grÃÂ¡ficos */}
                ...
                <Card className="border-amber-200 bg-amber-50/80">
                  <CardContent className="p-4 flex items-center gap-3">
                    ...
                    <p className="text-sm text-amber-800">
                      Los grÃÂ¡ficos estÃÂ¡n vacÃÂ­os porque no hay datos en el perÃÂ­odo. Cargue <strong>prÃÂ©stamos</strong> y <strong>cuotas</strong> en el sistema para ver la informaciÃÂ³n. Puede usar la opciÃÂ³n <strong>ÃÂltimos 12 meses</strong> si ya tiene datos de meses anteriores.
                    </p>
                  </CardContent>
                </Card>
              </motion.div>
            ...
            {/* GrÃÂ¡fico de EvoluciÃÂ³n Mensual */}
            <motion.div
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                ...
                <Card className="shadow-lg border border-gray-200/90 rounded-xl overflow-hidden bg-white">
                  <CardHeader className="bg-gradient-to-r from-cyan-50/90 to-blue-50/90 border-b border-gray-200/80 pb-3">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <CardTitle className="flex items-center gap-2 text-lg font-bold text-gray-800">
                        ...
                        <span>EvoluciÃÂ³n Mensual</span>
                      </CardTitle>
                  ...
                  <CardContent className="p-6 pt-4">
                    {evolucionMensual.length > 0 ? (
                      ...
                      <div className="flex items-center justify-center py-16 text-gray-500">No hay datos para el perÃÂ­odo seleccionado</div>
                    )}
                  </CardContent>
                </Card>
              </motion.div>

            {/* Pago vencido por dÃÂ­a - desde tabla cuotas */}
            <motion.div
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                ...
                <Card className="shadow-lg border border-gray-200/90 rounded-xl overflow-hidden bg-white">
                  <CardHeader className="bg-gradient-to-r from-red-50/90 to-orange-50/90 border-b border-gray-200/80 pb-3">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <CardTitle className="flex items-center gap-2 text-lg font-bold text-gray-800">
                        ...
                        <span>Pago vencido por dÃÂ­a</span>
                      </CardTitle>
                      <div className="flex items-center gap-2">
                  ...
                  <CardContent className="p-6 pt-4">
                    {loadingMorosidadPorDia ? (
                      <div className="flex items-center justify-center py-16 text-gray-500">Cargando pago vencido por dÃÂ­a...</div>
                      ...
                      <div className="flex items-center justify-center py-16 text-gray-500">No hay datos de pago vencido por dÃÂ­a para el perÃÂ­odo seleccionado</div>
                    )}
                  </CardContent>
                </Card>
              </motion.div>

            {/* Monto programado por dÃÂ­a: hoy hasta una semana despu\u00E9s */}
            <motion.div
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                ...
                <Card className="shadow-lg border border-gray-200/90 rounded-xl overflow-hidden bg-white">
                  <CardHeader className="bg-gradient-to-r from-emerald-50/90 to-teal-50/90 border-b border-gray-200/80 pb-3">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <CardTitle className="flex items-center gap-2 text-lg font-bold text-gray-800">
                        ...
                        <span>Monto programado por dÃÂ­a</span>
                      </CardTitle>
                      <Badge variant="secondary" className="text-xs font-medium text-gray-600 bg-white/80 border border-gray-200">
                        Hoy hasta 1 semana
                      </Badge>
                  ...
                  <CardContent className="p-6 pt-4">
                    {loadingMontoProgramadoSemana ? (
                      ...
                      <div className="flex items-center justify-center py-16 text-gray-500">No hay datos de monto programado para los prÃÂ³ximos 7 d\u00EDas</div>
                    )}
                  </CardContent>
                </Card>
              </motion.div>
        ...
        {/* GRÃÂFICOS: BANDAS DE $200 USD Y COBRANZA PLANIFICADA VS REAL */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {/* GRÃÂFICO DE BANDAS DE $200 USD */}
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
                ...
                <CardHeader className="bg-gradient-to-r from-indigo-50/90 to-purple-50/90 border-b border-gray-200/80 pb-3">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <CardTitle className="flex items-center gap-2 text-lg font-bold text-gray-800">
                      ...
                      <span>DistribuciÃÂ³n por Bandas de $200 USD</span>
                    </CardTitle>
                    <div className="flex items-center gap-2">
                    ...
                    {(filteredData) => (
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={filteredData} layout="vertical" margin={{ top: 14, right: 24, left: 120, bottom: 24 }}>
                          <CartesianGrid {...chartCartesianGrid} />
                          <XAxis type="number" domain={[600, 2000]} tick={chartAxisTick} tickFormatter={(value) => value.toLocaleString('es-EC')} label={{ value: 'Cantidad de PrÃÂ©stamos', position: 'insideBottom', offset: -10, style: { textAnchor: 'middle', fill: '#374151', fontSize: 13, fontWeight: 600 } }} allowDecimals={false} />
                          ...
                          <Tooltip contentStyle={chartTooltipStyle.contentStyle} labelStyle={chartTooltipStyle.labelStyle} formatter={(value: number) => [`${value.toLocaleString('es-EC')} prÃÂ©stamos`, 'Cantidad']} labelFormatter={(label) => `Banda: ${label}`} cursor={{ fill: 'rgba(99, 102, 241, 0.08)' }} />
                          <Legend {...chartLegendStyle} />
                          <Bar dataKey="cantidad" radius={[0, 6, 6, 0]} name="Cantidad de PrÃÂ©stamos">
          ...
          {/* PrÃÂ©stamos aprobados por modelo de vehÃÂ­culo (barras horizontales) */}
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
              ...
              <CardHeader className="bg-gradient-to-r from-violet-50/90 to-purple-50/90 border-b border-gray-200/80 pb-3">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <CardTitle className="flex items-center gap-2 text-lg font-bold text-gray-800">
                    ...
                    <span>PrÃÂ©stamos aprobados por modelo de vehÃÂ­culo</span>
                  </CardTitle>
                  <div className="flex items-center gap-2">
                    <SelectorPeriodoGrafico chartId="rangos" />
                    <Badge variant="secondary" className="text-xs font-medium text-gray-600 bg-white/80 border border-gray-200">
                ...
                <p className="text-xs text-gray-600 mt-1">
                  DistribuciÃÂ³n en porcentaje (acumulado). Usa el filtro Modelo en la barra superior para filtrar por vehÃÂ­culo.
                </p>
              </CardHeader>
              <CardContent className="p-6 flex-1">
                      ...
                      <ResponsiveContainer width="100%" height={chartHeight}>
                        <BarChart data={dataBarras} layout="vertical" margin={{ top: 12, right: 48, left: 16, bottom: 12 }} barCategoryGap="10%" barGap={4}>
                          ...
                          <YAxis
                            ...
                            tickLine={false}
                            axisLine={{ stroke: '#e5e7eb' }}
                            tickFormatter={(name) => (name && name.length > 22 ? `${name.slice(0, 20)}Ã¢ÂÂ¦` : name)}
                          />
                          <Tooltip
                            contentStyle={chartTooltipStyle.contentStyle}
                            labelStyle={chartTooltipStyle.labelStyle}
                            formatter={(value: number, _name: string, props: { payload?: { cantidad: number; porcentaje: number } }) => [
                              `${props.payload?.cantidad?.toLocaleString('es-EC') ? value} prÃÂ©stamos (${Number(value ? props.payload?.porcentaje ? 0).toFixed(1)}%)`,
                              '% del total',
                            ]}
        ...
        {/* GRÃÂFICOS DE MOROSIDAD */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {/* ComposiciÃÂ³n de Pago vencido (monto en USD) */}
          <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
                ...
                <CardHeader className="bg-gradient-to-r from-red-50/90 to-rose-50/90 border-b border-gray-200/80 pb-3">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <CardTitle className="flex items-center gap-2 text-lg font-bold text-gray-800">
                      ...
                      <span>ComposiciÃÂ³n de Pago vencido (USD)</span>
                    </CardTitle>
                    <div className="flex items-center gap-2">
                  ...
                  <p className="text-xs text-gray-600 mt-1">
                  Cuotas vencidas sin pagar. 1-30, 31-60 y 61-89 d\u00EDas = Vencido; 90+ d\u00EDas = Moroso (snapshot al dÃÂ­a de hoy).
                </p>
                </CardHeader>
                <CardContent className="p-6 flex-1">
          ...
          {/* Cantidad de prÃÂ©stamos en mora por rango de d\u00EDas */}
          <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
                ...
                <CardHeader className="bg-gradient-to-r from-rose-50/90 to-red-50/90 border-b border-gray-200/80 pb-3">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <CardTitle className="flex items-center gap-2 text-lg font-bold text-gray-800">
                      ...
                      <span>Cantidad de prÃÂ©stamos con pago vencido</span>
                    </CardTitle>
                    <div className="flex items-center gap-2">
                      <SelectorPeriodoGrafico chartId="composicion-morosidad" />
                      <Badge variant="secondary" className="text-xs font-medium text-gray-600 bg-white/80 border border-gray-200">
                  ...
                  <p className="text-xs text-gray-600 mt-1">
                    PrÃÂ©stamos con cuotas vencidas sin pagar. 1-30, 31-60 y 61-89 d\u00EDas = Vencido; 90+ d\u00EDas = Moroso (snapshot al dÃÂ­a de hoy).
                  </p>
                </CardHeader>
                <CardContent className="p-6 flex-1">
                  {datosComposicionMorosidad?.puntos?.length ? (
                    ...
                    {(filteredData) => (
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={filteredData} margin={{ top: 12, right: 24, left: 12, bottom: 12 }}>
                          <CartesianGrid {...chartCartesianGrid} />
                          <XAxis dataKey="categoria" tick={chartAxisTick} />
                          <YAxis tick={chartAxisTick} allowDecimals={false} label={{ value: 'Cantidad de prÃÂ©stamos', angle: -90, position: 'insideLeft', style: { fill: '#374151', fontSize: 12 } }} />
                          <Tooltip contentStyle={chartTooltipStyle.contentStyle} labelStyle={chartTooltipStyle.labelStyle} formatter={(value: number) => [typeof value === 'number' ? value.toLocaleString('es-EC') : value, 'PrÃÂ©stamos']} />
                          <Legend {...chartLegendStyle} />
                          <Bar dataKey="cantidad_prestamos" fill="#be123c" name="Cantidad de prÃÂ©stamos" radius={[4, 4, 0, 0]} />
                        </BarChart>
                      </ResponsiveContainer>
                    )}
                  </ChartWithDateRangeSlider>
        ...
        {/* Pago vencido por Analista: 1 grÃÂ¡fico por fila, bloques independientes */}
        <div className="flex flex-col gap-6 mt-6">
                ...
                <CardHeader className="bg-gradient-to-r from-orange-50/90 to-amber-50/90 border-b border-gray-200/80 pb-3">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <CardTitle className="flex items-center gap-2 text-lg font-bold text-gray-800">
                      ...
                      <span>Pago vencido por Analista Ã¢ÂÂ Cuotas vencidas</span>
                    </CardTitle>
                    <div className="flex items-center gap-2">
                      ...
                      <ResponsiveContainer width="100%" height={400}>
                        <RadarChart data={datosMorosidadAnalista} margin={{ top: 24, right: 32, left: 32, bottom: 24 }} cx="50%" cy="50%" outerRadius="75%">
                          ...
                          <PolarAngleAxis
                            dataKey="analista"
                            tick={{ fontSize: 11, fill: '#374151', fontWeight: 500 }}
                            tickFormatter={(name) => (name && name.length > 18 ? `${name.slice(0, 16)}Ã¢ÂÂ¦` : name)}
                          />
          ...
          {/* Fila 2: solo DÃÂ³lares vencidos por analista (barras) */}
          <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              ...
              <Card className="shadow-lg border border-gray-200/90 rounded-xl overflow-hidden bg-white w-full">
                <CardHeader className="bg-gradient-to-r from-amber-50/90 to-orange-50/90 border-b border-gray-200/80 pb-3">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <CardTitle className="flex items-center gap-2 text-lg font-bold text-gray-800">
                      ...
                      <span>Pago vencido por Analista Ã¢ÂÂ DÃÂ³lares vencidos</span>
                    </CardTitle>
                    <div className="flex items-center gap-2">
                      <SelectorPeriodoGrafico chartId="morosidad-analista" />
                      <Badge variant="secondary" className="text-xs font-medium text-gray-600 bg-white/80 border border-gray-200">
                ...
                <CardContent className="p-6">
                  {loadingMorosidadAnalista ? (
                    ...
                    <>
                      <h4 className="text-sm font-semibold text-gray-700 mb-1 text-center">DÃÂ³lares vencidos por analista</h4>
                      <ResponsiveContainer width="100%" height={Math.max(380, Math.min(620, datosMorosidadAnalista.length * 28))}>
                        <BarChart data={datosMorosidadAnalista} layout="vertical" margin={{ top: 12, right: 32, left: 16, bottom: 12 }} barCategoryGap="12%">
                          ...
                          <YAxis
                            ...
                            tickLine={false}
                            axisLine={{ stroke: '#e5e7eb' }}
                            tickFormatter={(name) => (name && name.length > 24 ? `${name.slice(0, 22)}Ã¢ÂÂ¦` : name)}
                          />
                          <Tooltip contentStyle={chartTooltipStyle.contentStyle} labelStyle={chartTooltipStyle.labelStyle} formatter={(value: number) => [formatCurrency(value), 'DÃÂ³lares vencidos']} labelFormatter={(label) => `Analista: ${label}`} cursor={{ fill: 'rgba(234, 88, 12, 0.06)' }} />
                          <Legend {...chartLegendStyle} />
                          <Bar dataKey="monto_vencido" name="DÃÂ³lares vencidos" fill="#ea580c" radius={[0, 4, 4, 0]} maxBarSize={28} />
                        </BarChart>
                      </ResponsiveContainer>
                    </>
                  ) : (
