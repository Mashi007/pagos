...
export function DashboardMenu() {
  ...
  // ÃÂ¢Ãââ¦ OPTIMIZACIÃâN PRIORIDAD 1: Carga por batches con priorizaciÃ³n
  ...
  const { data: opcionesFiltros, isLoading: loadingOpcionesFiltros, isError: errorOpcionesFiltros } = useQuery({
    ...
    // ÃÂ¢Ãââ¦ Prioridad mÃ¡xima - carga inmediatamente
  })
  ...
  const { data: kpisPrincipales, isLoading: loadingKPIs, isError: errorKPIs, refetch } = useQuery({
    ...
    staleTime: 5 * 60 * 1000, // 5 min para datos mÃ¡s frescos
  ...
  const { data: datosFinanciamientoRangos, isLoading: loadingFinanciamientoRangos, isError: errorFinanciamientoRangos, error: errorFinanciamientoRangosDetail, refetch: refetchFinanciamientoRangos } = useQuery({
    ...
    refetchOnWindowFocus: false,
    enabled: true,
    retry: 1, // ÃÂ¢Ãââ¦ Permitir 1 reintento para errores de red
  ...
  const { data: datosCobranzasSemanales, isLoading: loadingCobranzasSemanales } = useQuery({
    ...
    queryFn: async () => {
      ...
      queryParams.append('semanas', '12') // Ãltimas 12 semanas
      const response = await apiClient.get(
                ...
                <Card className="border-amber-200 bg-amber-50/80">
                  <CardContent className="p-4 flex items-center gap-3">
                    ...
                    <p className="text-sm text-amber-800">
                      Los grÃ¡ficos estÃ¡n vacÃ­os porque no hay datos en el perÃ­odo. Cargue <strong>prÃ©stamos</strong> y <strong>cuotas</strong> en el sistema para ver la informaciÃ³n. Puede usar la opciÃ³n <strong>Ãltimos 12 meses</strong> si ya tiene datos de meses anteriores.
                    </p>
                  </CardContent>
                </Card>
              </motion.div>
