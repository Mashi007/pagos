...
export function DashboardAnalisis() {
  ...
  const [isDonutConcesionariosOpen, setIsDonutConcesionariosOpen] =
    useState(false)
  ...
  const { data: cobrosDiariosData, isLoading: loadingCobrosDiarios } = useQuery(
    {
      ...
      queryFn: async () => {
        try {
          ...
          const response = (await apiClient.get(
            `/api/v1/dashboard/cobros-diarios?${queryParams.toString()}`
          )) as {
            datos: CobroDiario[]
          }
  ...
  if (datosEvolucionMensual.length >= 12) {
    ...
    const mesAnoAnterior =
      datosEvolucionMensual[datosEvolucionMensual.length - 13] // Hace 13 meses (incluye mes actual)

    if (mesAnoAnterior && mesActual && mesAnoAnterior.cartera > 0) {
      crecimientoAnual =
        ((mesActual.cartera - mesAnoAnterior.cartera) /
          mesAnoAnterior.cartera) *
        100
    }
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
      <div className="container mx-auto space-y-8 px-4 py-8">
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          ...
          >
            <Card className="border-2 border-gray-200 shadow-lg">
              <CardHeader className="border-b-2 border-amber-200 bg-gradient-to-r from-amber-50 to-orange-50">
          ...
          >
            <Card className="border-2 border-gray-200 shadow-lg">
              <CardHeader className="border-b-2 border-blue-200 bg-gradient-to-r from-blue-50 to-indigo-50">
                <CardTitle className="flex items-center space-x-2 text-xl font-bold text-gray-800">
        ...
        <motion.div
          ...
          className="mt-8"
        >
          <div className="rounded-xl bg-gradient-to-r from-amber-600 to-orange-600 p-6 shadow-xl">
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
              <Button
                variant="secondary"
                className="flex h-auto flex-col items-center space-y-2 border-2 border-transparent bg-white py-4 text-gray-800 hover:border-amber-300 hover:bg-gray-50"
