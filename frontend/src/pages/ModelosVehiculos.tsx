...
import {
  useModelosVehiculos,
  useDeleteModeloVehiculo,
  useUpdateModeloVehiculo,
  useCreateModeloVehiculo,
} from '../hooks/useModelosVehiculos'
...
export function ModelosVehiculos() {
  ...
  const [editingModelo, setEditingModelo] = useState<ModeloVehiculo | null>(
    null
  )

  const [formData, setFormData] = useState<
    ModeloVehiculoCreate & { precioInput?: number | '' }
  >({
    modelo: '',
  ...
  const handleCreateOrUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const precioVal =
        formData.precioInput === '' || formData.precioInput == null
          ? null
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-green-600">
              {
                (modelos || []).filter(m => m.activo === true || m.activo === 1)
                  .length
              }
            </div>
          </CardContent>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-red-600">
              {
                (modelos || []).filter(
                  m => m.activo === false || m.activo === 2 || m.activo === 0
                ).length
              }
            </div>
          </CardContent>
            ...
            <TableBody>
              {paginatedModelos.map(modelo => (
                <TableRow key={modelo.id}>
                  ...
                  <TableCell>
                    {modelo.precio != null
                      ? Number(modelo.precio).toLocaleString('es-VE', {
                  ...
                  <TableCell>
                    <Badge
                      variant={
                        modelo.activo === true || modelo.activo === 1
                          ? 'default'
                          : 'secondary'
                      }
                    >
          ...
          {filteredModelos.length > itemsPerPage && (
            <div className="flex items-center justify-between border-t px-2 py-4">
              <div className="text-sm text-gray-500">
                ...
                {Math.min(endIndex, filteredModelos.length)} de{' '}
                {filteredModelos.length} modelos
              </div>
                ...
                <form onSubmit={handleCreateOrUpdate} className="space-y-4">
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      min={0}
                      value={
                        formData.precioInput === '' ? '' : formData.precioInput
                      }
                      onChange={e => {
                        ...
                        setFormData(prev => ({
                          ...prev,
                          precioInput: num,
                          precio: num === '' ? undefined : (num as number),
                        }))
                      }}
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          <div className="text-sm text-gray-600">
            Columnas requeridas: <b>modelo</b>, <b>precio</b>. Opcional:{' '}
          ...
          <div className="flex items-center gap-2">
            <Button
              ...
              onClick={async () => {
                ...
                try {
                  const res =
                    await modeloVehiculoService.importarDesdeExcel(archivoExcel)

                  toast.success(
                    `Importado: ${res.creados} creados, ${res.actualizados} actualizados`
                  )
                  ...
                  if (ax?.response?.status === 501) {
                    toast.error(
                      'Importación desde Excel no disponible por el momento.'
                    )
            ...
            <div className="text-xs text-gray-500">
              Precios en USD (Valor Activo en préstamos)
            </div>
          </div>
        </CardContent>
      </Card>
