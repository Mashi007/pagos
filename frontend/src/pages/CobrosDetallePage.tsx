 ...
 * Al aprobar se invalidan cuotas y prÃÂ©stamos para que la tabla de amortizaciÃÂ³n se actualice al ir a PrÃÂ©stamos.
 */
...
const ESTADO_BADGE: Record<string, string> = {
  pendiente: 'Pendiente Ã°ÂÂÂ¡',
  en_revision: 'En revisiÃÂ³n Ã°ÂÂÂ ',
  aprobado: 'Aprobado Ã°ÂÂÂ¢',
  rechazado: 'Rechazado Ã°ÂÂÂ´',
}
...
export default function CobrosDetallePage() {
  ...
  const handleAprobar = async () => {
    ...
    try {
      ...
      setAccion('idle')
      load()
      // Actualizar tabla de amortizaciÃÂ³n y listado de prÃÂ©stamos para que reflejen el pago aplicado
    ...
    <div className="p-6 space-y-6 max-w-4xl">
      ...
      <Card>
        ...
        <CardContent className="space-y-2 text-sm">
          ...
          <p><strong>CÃÂ©dula:</strong> {detalle.tipo_cedula}{detalle.numero_cedula}</p>
          <p><strong>Fecha de pago:</strong> {detalle.fecha_pago}</p>
          <p><strong>InstituciÃÂ³n:</strong> {detalle.institucion_financiera}</p>
          <p><strong>NÃÂºmero de operaciÃÂ³n:</strong> {detalle.numero_operacion}</p>
          <p><strong>Monto:</strong> {detalle.monto} {detalle.moneda}</p>
          {detalle.observacion && <p><strong>ObservaciÃÂ³n:</strong> {detalle.observacion}</p>}
          <p><strong>Correo enviado a:</strong> {detalle.correo_enviado_a ? 'Ã¢ÂÂ'}</p>
      ...
      {detalle.estado !== 'aprobado' && detalle.estado !== 'rechazado' && (
        <Card>
          <CardHeader>
            <CardTitle>
              {detalle.estado === 'en_revision' ? 'RevisiÃÂ³n manual' : 'Acciones'}
            </CardTitle>
            {detalle.estado === 'en_revision' && (
              <p className="text-sm text-muted-foreground mt-1">
                No coincidiÃÂ³ 100% con la revisiÃÂ³n automÃÂ¡tica (Gemini). Use los mismos botones: Aprobar (envÃÂ­a recibo) o Rechazar (se notifica al cliente por correo electrÃÂ³nico).
              </p>
            )}
            <p className="text-sm text-muted-foreground">
              Al rechazar se envÃÂ­a un correo al cliente desde <strong>notificaciones@rapicreditca.com</strong> con el motivo de rechazo y el comprobante adjunto (misma pantalla).
            </p>
          </CardHeader>
            ...
            <ul className="space-y-2 text-sm">
              {detalle.historial.map((h, i) => (
                <li key={i} className="border-b pb-2">
                  {h.estado_anterior} Ã¢ÂÂ {h.estado_nuevo}
