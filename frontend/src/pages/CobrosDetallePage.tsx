 ...
 * Al aprobar se invalidan cuotas y prÃ©stamos para que la tabla de amortizaciÃ³n se actualice al ir a PrÃ©stamos.
 */
...
const ESTADO_BADGE: Record<string, string> = {
  pendiente: 'Pendiente ð¡',
  en_revision: 'En revisiÃ³n ð ',
  aprobado: 'Aprobado ð¢',
  rechazado: 'Rechazado ð´',
}
...
export default function CobrosDetallePage() {
  ...
  const handleAprobar = async () => {
    ...
    try {
      ...
      setAccion('idle')
      load()
      // Actualizar tabla de amortizaciÃ³n y listado de prÃ©stamos para que reflejen el pago aplicado
    ...
    <div className="p-6 space-y-6 max-w-4xl">
      ...
      <Card>
        ...
        <CardContent className="space-y-2 text-sm">
          ...
          <p><strong>CÃ©dula:</strong> {detalle.tipo_cedula}{detalle.numero_cedula}</p>
          <p><strong>Fecha de pago:</strong> {detalle.fecha_pago}</p>
          <p><strong>InstituciÃ³n:</strong> {detalle.institucion_financiera}</p>
          <p><strong>NÃºmero de operaciÃ³n:</strong> {detalle.numero_operacion}</p>
          <p><strong>Monto:</strong> {detalle.monto} {detalle.moneda}</p>
          {detalle.observacion && <p><strong>ObservaciÃ³n:</strong> {detalle.observacion}</p>}
          <p><strong>Correo enviado a:</strong> {detalle.correo_enviado_a ? 'â'}</p>
      ...
      {detalle.estado !== 'aprobado' && detalle.estado !== 'rechazado' && (
        <Card>
          <CardHeader>
            <CardTitle>
              {detalle.estado === 'en_revision' ? 'RevisiÃ³n manual' : 'Acciones'}
            </CardTitle>
            {detalle.estado === 'en_revision' && (
              <p className="text-sm text-muted-foreground mt-1">
                No coincidiÃ³ 100% con la revisiÃ³n automÃ¡tica (Gemini). Use los mismos botones: Aprobar (envÃ­a recibo) o Rechazar (se notifica al cliente por correo electrÃ³nico).
              </p>
            )}
            <p className="text-sm text-muted-foreground">
              Al rechazar se envÃ­a un correo al cliente desde <strong>notificaciones@rapicreditca.com</strong> con el motivo de rechazo y el comprobante adjunto (misma pantalla).
            </p>
          </CardHeader>
