/**
 * Formulario PÃBLICO de reporte de pago (sin login).
 * Validadores alineados con el backend: cÃ©dula (V/E/J/Z + 6-11 dÃ­gitos), monto (>0, mÃ¡x 999.999.999,99),
 * fecha (obligatoria, no futura, desde calendario), instituciÃ³n y nÂº documento (longitud), archivo (JPG/PNG/PDF, mÃ¡x 5 MB).
 * Notificaciones claras por cada error para guiar al cliente.
 * Marca flujo pÃºblico para que, si intentan ir a login, vean "Acceso prohibido" y puedan volver.
 */
...
// LÃ­mites iguales al backend (cobros_publico)
...
// CÃ©dula: solo letra (V|E|G|J) + 6-11 dÃ­gitos; no puntos ni signos intermedios. Si solo dÃ­gitos, al procesar se antepone V.
const CEDULA_REGEX = /^[VEGJ]\d{6,11}$/i

/** Normaliza para validar: quita espacios, guiones y puntos. Rechaza si queda otro signo o punto. Si solo 6-11 dÃ­gitos, devuelve con V antepuesto. */
...
function validarMonto(val: string): { valido: boolean; valor?: number; error?: string } {
  ...
  if (Number.isNaN(num)) return { valido: false, error: 'Monto no vÃ¡lido. Ingrese un nÃºmero (ej: 150.50).' }
...
function validarFechaPago(fecha: string): { valido: boolean; error?: string } {
  ...
  if (Number.isNaN(d.getTime())) return { valido: false, error: 'Fecha no vÃ¡lida. Use el calendario para elegir la fecha.' }
  d.setHours(0, 0, 0, 0)
...
function validarArchivo(file: File | null): { valido: boolean; error?: string } {
  ...
  if (file.size > MAX_FILE_SIZE) {
    return { valido: false, error: 'El comprobante no puede superar 5 MB. Reduzca el tamaÃ±o del archivo.' }
  }
  if (file.size < 4) return { valido: false, error: 'El archivo estÃ¡ vacÃ­o o no es vÃ¡lido.' }
  return { valido: true }
}
...
export default function ReportePagoPage({ variant = 'cobros' }: { variant?: ReportePagoVariant }) {
  ...
  // Marcar flujo pÃºblico para que, si intentan ir a login, vean "Acceso prohibido" y puedan volver
  useEffect(() => {
  ...
  const stepAnnouncements: Record<number, string> = {
    ...
    1: 'Paso 1: Ingrese su nÃºmero de cÃ©dula',
    2: 'Paso 2: Datos del pago',
    3: 'Paso 3: InstituciÃ³n financiera',
    4: 'Paso 4: Fecha y monto del pago',
    5: 'Paso 5: NÃºmero de documento u operaciÃ³n',
  ...
  const handleValidarCedula = async () => {
    ...
    try {
      ...
      showNotification('error', e?.message || 'Error al validar cÃ©dula.')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleEnviar = async () => {
    ...
    if (!vCedula.valido) {
      showNotification('error', vCedula.error ? 'CÃ©dula invÃ¡lida.')
      return
    }
    const cedulaEnviar = vCedula.valorParaEnviar!
    if (!institucionFinal.trim()) {
      showNotification('error', 'Seleccione la instituciÃ³n financiera.')
      return
    }
    if (institucionFinal.length > MAX_LENGTH_INSTITUCION) {
      showNotification('error', 'Nombre de instituciÃ³n demasiado largo. RedÃºzcalo.')
      return
    }
    const vFecha = validarFechaPago(fechaPago)
    if (!vFecha.valido) {
      showNotification('error', vFecha.error ? 'Fecha invÃ¡lida.')
      return
    }
    const vMonto = validarMonto(monto)
    if (!vMonto.valido) {
      showNotification('error', vMonto.error ? 'Monto invÃ¡lido.')
      return
    }
    if (!numeroDocumento.trim()) {
      showNotification('error', 'Ingrese el nÃºmero de documento u operaciÃ³n.')
      return
    }
    if (numeroDocumento.length > MAX_LENGTH_NUMERO_OPERACION) {
      showNotification('error', 'NÃºmero de documento u operaciÃ³n demasiado largo.')
      return
    }
    const vArchivo = validarArchivo(archivo)
    if (!vArchivo.valido) {
      showNotification('error', vArchivo.error ? 'Archivo invÃ¡lido.')
      return
    }
    // Honeypot: si un bot rellenÃ³ el campo oculto, no enviar
    const honeypotValue = honeypotRef.current?.value?.trim() ? ''
    if (honeypotValue) {
      showNotification('error', 'No se pudo procesar el envÃ­o. Intente de nuevo.')
      return
    }
    ...
    form.append('contact_website', '') // honeypot: siempre vacÃ­o para usuarios reales
  ...
  // Pantalla de bienvenida con instrucciones generales (logo y colores RapiCredit: azul oscuro, naranja/marrÃ³n)
  const LOGO_PUBLIC_SRC = `${(import.meta.env.BASE_URL || '/').replace(/\/?$/, '')}/logos/rapicredit-public.png`
  if (step === 0) {
    const steps = isInfopagos
      ? [
          { icon: 'id' as const, text: 'Ingrese la cÃ©dula del deudor (V, E, G o J + dÃ­gitos).' },
          { icon: 'bank' as const, text: 'Indique instituciÃ³n financiera, fecha, monto y nÃºmero de operaciÃ³n.' },
          { icon: 'file' as const, text: 'Adjunte el comprobante de pago (JPG, PNG o PDF, mÃ¡x. 5 MB).' },
          { icon: 'check' as const, text: 'Al enviar, el recibo se enviarÃ¡ al correo del deudor y podrÃ¡ descargarlo aquÃ­.' },
        ]
      : [
          { icon: 'id' as const, text: 'Ingrese su nÃºmero de cÃ©dula (V, E, G o J + dÃ­gitos).' },
          { icon: 'bank' as const, text: 'Indique instituciÃ³n financiera, fecha, monto y nÃºmero de operaciÃ³n.' },
          { icon: 'file' as const, text: 'Adjunte el comprobante de pago (JPG, PNG o PDF, mÃ¡x. 5 MB).' },
          { icon: 'check' as const, text: 'Revise los datos y envÃ­e. RecibirÃ¡ confirmaciÃ³n al correo registrado.' },
        ]
    return (
          ...
          <CardContent className="p-4 sm:p-6 md:p-8 space-y-5 sm:space-y-6">
            <div className="text-center">
              ...
              <p className="text-slate-600 mt-2 text-sm leading-relaxed">
                {isInfopagos
                  ? 'Registre el pago del deudor. El recibo se enviarÃ¡ a su correo y podrÃ¡ descargarlo aquÃ­ para compartirlo.'
            ...
            <p className="text-xs text-slate-500 text-center bg-slate-50 rounded-lg px-3 py-2 border border-slate-100">
              Los datos se comprobarÃ¡n y almacenarÃ¡n Ãºnicamente para validaciÃ³n del pago.
            </p>
            <p className="text-xs text-slate-500 text-center">
              Si desea reportar mÃ¡s de un pago, al finalizar cada envÃ­o use el botÃ³n Â«Ingresar otro pagoÂ» o reinicie el proceso.
            </p>
            <Button
  ...
  if (step === 1) {
    return (
      <div className="min-h-screen min-h-[100dvh] bg-slate-50 flex flex-col items-center justify-center p-3 sm:p-4 overflow-x-hidden">
        ...
        {/* Honeypot: campo oculto para usuarios, visible para bots. No usar en navegaciÃ³n ni leer en pantalla. */}
        <input
          ref={honeypotRef}
          type="text"
        ...
        <div className="w-full max-w-md min-w-0 flex flex-col items-center gap-3 px-1 sm:px-0">
          ...
          <CardHeader className="px-4 sm:px-6 pb-2">
            <CardTitle className="text-lg sm:text-xl">{isInfopagos ? 'CÃ©dula del deudor' : 'Reporte de pago'}</CardTitle>
            <p className="text-sm text-gray-600 mt-1">Solo letra (V, E, G o J) y 6 a 11 dÃ­gitos. No use puntos ni signos. Si solo ingresa nÃºmeros se procesarÃ¡ con V.</p>
          </CardHeader>
      ...
      <div className="min-h-screen min-h-[100dvh] bg-slate-50 flex flex-col items-center justify-center p-3 sm:p-4 overflow-x-hidden">
        ...
        <div className="w-full max-w-md min-w-0 flex flex-col items-center gap-3 px-1 sm:px-0">
          ...
          <CardHeader className="px-4 sm:px-6">
            <CardTitle className="text-lg sm:text-xl">{isInfopagos ? 'Deudor: ' : 'Hola, '}{nombre || (isInfopagos ? 'â' : 'Cliente')}</CardTitle>
            <p className="text-sm text-gray-600 mt-2">
              {isInfopagos
                ? 'Ingrese los datos del pago del deudor. El recibo se enviarÃ¡ a su correo registrado.'
                : 'Recuerda ingresar Ãºnicamente datos de tu pago que se comprobarÃ¡n y almacenarÃ¡n para fines de validaciÃ³n de pago.'}
            </p>
          </CardHeader>
    ...
    return (
      <div className="min-h-screen min-h-[100dvh] bg-slate-50 flex flex-col items-center justify-center p-3 sm:p-4 overflow-x-hidden">
        ...
        <div className="w-full max-w-md min-w-0 flex flex-col items-center gap-3 px-1 sm:px-0">
          ...
          <Card className="w-full max-w-md min-w-0">
          <CardHeader className="px-4 sm:px-6">
            <CardTitle className="text-lg sm:text-xl">InstituciÃ³n financiera</CardTitle>
          </CardHeader>
          <CardContent className="px-4 sm:px-6 space-y-4">
            <select
              ...
              aria-label="Seleccione la instituciÃ³n financiera"
            >
            ...
            <div className="flex gap-2 flex-wrap sm:flex-nowrap">
              <Button variant="outline" className="min-h-[48px] flex-1 min-w-[100px] touch-manipulation" onClick={() => setStep(2)}>AtrÃ¡s</Button>
              <Button
        ...
        <div className="w-full max-w-md min-w-0 flex flex-col items-center gap-3 px-1 sm:px-0">
          ...
          <CardContent className="px-4 sm:px-6 space-y-4">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1">Monto mayor a 0. MÃ¡ximo permitido: 999.999.999,99</p>
            </div>
            <div className="flex gap-2 flex-wrap sm:flex-nowrap">
              <Button variant="outline" className="min-h-[48px] flex-1 min-w-[100px] touch-manipulation" onClick={() => setStep(3)}>AtrÃ¡s</Button>
              <Button
                className="flex-1 min-h-[48px] min-w-0 touch-manipulation"
                onClick={() => {
                  const vF = validarFechaPago(fechaPago)
                  if (!vF.valido) {
                    showNotification('error', vF.error ? 'Fecha invÃ¡lida.')
                    return
                  }
                  const vM = validarMonto(monto)
                  if (!vM.valido) {
                    showNotification('error', vM.error ? 'Monto invÃ¡lido.')
                    return
                  }
                  setStep(5)
                }}
    ...
    return (
      <div className="min-h-screen min-h-[100dvh] bg-slate-50 flex flex-col items-center justify-center p-3 sm:p-4 overflow-x-hidden">
        ...
        <div className="w-full max-w-md min-w-0 flex flex-col items-center gap-3 px-1 sm:px-0">
          ...
          <Card className="w-full max-w-md min-w-0">
          <CardHeader className="px-4 sm:px-6">
            <CardTitle className="text-lg sm:text-xl">NÃºmero de documento / operaciÃ³n</CardTitle>
          </CardHeader>
          <CardContent className="px-4 sm:px-6 space-y-4">
            <Input
              className="min-h-[44px] touch-manipulation"
              placeholder="NÃºmero de serie, operaciÃ³n o referencia"
              value={numeroDocumento}
              onChange={(e) => setNumeroDocumento(e.target.value)}
              maxLength={MAX_LENGTH_NUMERO_OPERACION}
            />
            <p className="text-xs text-gray-500">MÃ¡ximo {MAX_LENGTH_NUMERO_OPERACION} caracteres.</p>
            <div className="flex gap-2 flex-wrap sm:flex-nowrap">
              <Button variant="outline" className="min-h-[48px] flex-1 min-w-[100px] touch-manipulation" onClick={() => setStep(4)}>AtrÃ¡s</Button>
              <Button
                className="flex-1 min-h-[48px] min-w-0 touch-manipulation"
                onClick={() => {
                  if (!numeroDocumento.trim()) {
                  ...
                  if (numeroDocumento.length > MAX_LENGTH_NUMERO_OPERACION) {
                    showNotification('error', 'NÃºmero de documento demasiado largo.')
                    return
                  }
                  setStep(6)
                }}
      ...
      <div className="min-h-screen min-h-[100dvh] bg-slate-50 flex flex-col items-center justify-center p-3 sm:p-4 overflow-x-hidden">
        ...
        <div className="w-full max-w-md min-w-0 flex flex-col items-center gap-3 px-1 sm:px-0">
          ...
          <CardHeader className="px-4 sm:px-6">
            ...
            <p className="text-sm text-gray-600">Un solo archivo por envÃ­o. JPG, PNG o PDF. MÃ¡ximo 5 MB.</p>
            <p className="text-xs text-amber-700 mt-1">Si desea reportar otro pago, al finalizar use Â«Ingresar otro pagoÂ» o reinicie el proceso.</p>
          </CardHeader>
          <CardContent className="px-4 sm:px-6 space-y-4">
            {archivo ? (
              <>
                <div className="rounded-lg border border-gray-200 bg-gray-50 px-3 py-3 text-sm text-gray-700 break-words">
                  ...
                  <p className="text-xs text-amber-700 mt-2">No se puede agregar otra imagen en este envÃ­o. Para otro pago, finalice y use Â«Ingresar otro pagoÂ».</p>
                </div>
              </>
            ) : (
              <Input
            ...
            <div className="flex gap-2 flex-wrap sm:flex-nowrap">
              <Button variant="outline" className="min-h-[48px] flex-1 min-w-[100px] touch-manipulation" onClick={() => { setArchivo(null); setStep(5); }}>AtrÃ¡s</Button>
              <Button
                className="flex-1 min-h-[48px] min-w-0 touch-manipulation"
                onClick={() => {
                  const v = validarArchivo(archivo)
                  if (!v.valido) {
                    showNotification('error', v.error ? 'Archivo invÃ¡lido.')
                    return
                  }
                  setStep(7)
                }}
      ...
      <div className="min-h-screen min-h-[100dvh] bg-slate-50 flex flex-col items-center justify-center p-3 sm:p-4 overflow-x-hidden">
        ...
        <div className="w-full max-w-md min-w-0 flex flex-col items-center gap-3 px-1 sm:px-0">
          ...
          <CardContent className="px-4 sm:px-6 space-y-2 text-sm break-words">
            ...
            <p><strong>InstituciÃ³n:</strong> {institucionFinal}</p>
            <p><strong>Fecha de pago:</strong> {fechaPago}</p>
            <p><strong>Monto:</strong> {monto} {moneda}</p>
            <p><strong>NÃºmero de operaciÃ³n:</strong> {numeroDocumento}</p>
            <p><strong>Comprobante:</strong> {archivo?.name}</p>
          </CardContent>
          <CardContent className="px-4 sm:px-6 pt-0 space-y-3">
            <p className="text-sm text-gray-600 break-words">
              {isInfopagos
                ? <>El recibo se enviarÃ¡ al correo del deudor (<span className="font-semibold text-[#1e3a5f] bg-blue-50 px-1.5 py-0.5 rounded break-all">{emailParaVerificacion || 'correo registrado'}</span>) y podrÃ¡ descargarlo aquÃ­ al finalizar.</>
                : <>Tu pago se procesarÃ¡ y se enviarÃ¡ al correo registrado en tu contrato de financiamiento (
                  ...
                  ). Si tienes algÃºn problema con el correo, contacta a{' '}
                  ...
                  o a tu asesor para actualizaciÃ³n.</>}
            </p>
            <div className="flex gap-2 flex-wrap sm:flex-nowrap">
      ...
      <Card className="w-full max-w-md min-w-0 text-center mx-1 sm:mx-0">
        ...
        <CardContent className="px-4 sm:px-6 space-y-4">
          <div className="text-base sm:text-lg break-words">
            <p className="font-semibold">NÃºmero de referencia:</p>
          ...
          {isInfopagos ? (
            <>
              <p className="text-sm text-gray-600">
                Se enviÃ³ el recibo al correo registrado del deudor (segÃºn cÃ©dula). Puede descargar el recibo aquÃ­ para compartirlo.
              </p>
            ...
            <p className="text-sm text-gray-600">
              El recibo (PDF) se enviarÃ¡ a tu correo registrado en un plazo de hasta 24 horas.
            </p>
          )}
          <p className="text-sm break-words">
            Si necesitas informaciÃ³n adicional, comunÃ­cate con nosotros por WhatsApp:{' '}
