  ...
  const handleEliminar = async (id: number) => {
    try {
      ...
      const confirmar = window.confirm(
        'Ã¢Å¡ Ã¯Â¸Â Â¿EstÃ¡s seguro de que quieres ELIMINAR PERMANENTEMENTE este concesionario?\n\n' +
  ...
  const handleCreateOrUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingConcesionario) {
        ...
        toast.success('Ã¢Åâ¦ Concesionario actualizado exitosamente')
      } else {
        // Al crear, ya tiene activo: true por defecto
        await createConcesionarioMutation.mutateAsync({ ...formData, nombre: nombreFormateado })
        toast.success('Ã¢Åâ¦ Concesionario creado exitosamente')
      }
      resetForm()
      refetch()
    } catch (err) {
      console.error('Error:', err)
      toast.error('Ã¢ÂÅ Error al guardar concesionario')
    }
  }
