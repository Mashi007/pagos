...
export function Concesionarios() {
  ...
  const handleEliminar = async (id: number) => {
    try {
      ...
      const confirmar = window.confirm(
        'ÃÂ¢ÃÂ¡ ÃÂ¯ÃÂ¸ÃÂ ÃÂ¿EstÃÂ¡s seguro de que quieres ELIMINAR PERMANENTEMENTE este concesionario?\n\n' +
        'Esta acciÃÂ³n NO se puede deshacer y el concesionario serÃÂ¡ borrado completamente de la base de datos.'
      )
  ...
  const validateNombre = (nombre: string): string => {
    ...
    // Verificar cantidad de palabras (mÃÂ­nimo 2, mÃÂ¡ximo 4)
    ...
    if (palabras.length > 4) {
      return 'Debe ingresar mÃÂ¡ximo 4 palabras'
    }
  ...
  const handleCreateOrUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingConcesionario) {
        ...
        toast.success('ÃÂ¢ÃÂÃ¢ÂÂ¦ Concesionario actualizado exitosamente')
      } else {
        // Al crear, ya tiene activo: true por defecto
        await createConcesionarioMutation.mutateAsync({ ...formData, nombre: nombreFormateado })
        toast.success('ÃÂ¢ÃÂÃ¢ÂÂ¦ Concesionario creado exitosamente')
      }
      resetForm()
      refetch()
    } catch (err) {
      console.error('Error:', err)
      toast.error('ÃÂ¢ÃÂÃÂ Error al guardar concesionario')
    }
  }
  ...
  // Filtrar concesionarios por tÃÂ©rmino de bÃÂºsqueda
        ...
        <CardContent className="space-y-3">
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              className="hidden"
              id="excel-file-concesionarios"
            />
            <label
              htmlFor="excel-file-concesionarios"
            ...
            <Button
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  const fileInput = document.getElementById('excel-file-concesionarios') as HTMLInputElement
                  ...
                  <div>
                    ...
                    {!editingConcesionario && !validationError && (
                      <p className="text-xs text-gray-500 mt-1">
                        Ejemplo: Juan PÃÂ©rez (mÃÂ­nimo 2, mÃÂ¡ximo 4 palabras)
                      </p>
                    )}
                  </div>
