/**
 * Editar pago reportado: modificar valores para que cumplan con los validadores (cÃÂ©dula, fecha, monto, etc.).
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (montoNum !== undefined && (isNaN(montoNum) || montoNum < 0)) {
        toast.error('Monto debe ser un nÃÂºmero mayor o igual a 0.')
        setSaving(false)
        return
      }
      ...
      <Card>
        <CardHeader>
          ...
          <p className="text-sm text-muted-foreground">
            Modifica los valores para que cumplan con los validadores (cÃÂ©dula, fecha, monto, etc.).
          </p>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium mb-1">Tipo cÃÂ©dula</label>
                <select
              ...
              <div>
                <label className="block text-sm font-medium mb-1">NÃÂºmero cÃÂ©dula</label>
                <Input
                  value={form.numero_cedula}
                  onChange={(e) => setForm((f) => ({ ...f, numero_cedula: e.target.value.replace(/\D/g, '').slice(0, 11) }))}
                  placeholder="Solo dÃÂ­gitos"
                />
              </div>
            </div>
            <div>
            ...
            <div>
              <label className="block text-sm font-medium mb-1">InstituciÃÂ³n financiera</label>
              <select
            ...
            <div>
              <label className="block text-sm font-medium mb-1">NÃÂºmero de operaciÃÂ³n</label>
              <Input
            ...
            <div>
              <label className="block text-sm font-medium mb-1">Correo (para envÃÂ­o de recibo)</label>
              <Input
                type="email"
            ...
            <div>
              <label className="block text-sm font-medium mb-1">ObservaciÃÂ³n (opcional)</label>
              <Input
