...
export function TicketsAtencion() {
  ...
  const { data: clientesBuscados = [], isLoading: isLoadingSearch } =
    useSearchClientes(searchCliente, true)
  ...
  const {
    ...
    queryFn: async () => {
      ...
      return (
        response ?? {
          tickets: [],
          paginacion: { page: 1, per_page: 20, total: 0, pages: 0 },
        }
      )
    },
  ...
  const ticketsFiltrados = tickets.filter(ticket => {
    ...
    const descripcion =
      typeof ticket.descripcion === 'string' ? ticket.descripcion : ''
    ...
    const matchSearch =
      ...
      (clienteStr &&
        clienteStr.toLowerCase().includes(searchTerm.toLowerCase())) ||
      (ticket.clienteData &&
        (String(ticket.clienteData.cedula ?? '')
          .toLowerCase()
          .includes(searchTerm.toLowerCase()) ||
          (ticket.clienteData.telefono &&
            String(ticket.clienteData.telefono).includes(searchTerm)))) ||
  ...
  const handleCrearTicket = () => {
    ...
    const ticketData: TicketCreate = {
      ...
      asignado_a:
        nuevoTicket.asignado_a ||
        (user ? `${user.nombre} ${user.apellido}` : undefined),
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    <h3 className="font-semibold text-red-900">
                      {ticketsVencidos.length} Ticket
                      {ticketsVencidos.length > 1 ? 's' : ''} Vencido
                      {ticketsVencidos.length > 1 ? 's' : ''}
                    </h3>
                    ...
                    <p className="text-sm text-red-700">
                      {ticketsVencidos
                        .map(
                          t =>
                            `#${t.id}: ${typeof t.titulo === 'string' ? t.titulo : ''}`
                        )
                        .join(', ')}
                    </p>
                  </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    <h3 className="font-semibold text-yellow-900">
                      {ticketsProximosAVencer.length} Ticket
                      {ticketsProximosAVencer.length > 1 ? 's' : ''} Próximo
                      {ticketsProximosAVencer.length > 1 ? 's' : ''} a Vencer
                    </h3>

                    <p className="text-sm text-yellow-700">
                      {ticketsProximosAVencer
                        .map(t => {
                          ...
                          const titulo =
                            typeof t.titulo === 'string' ? t.titulo : ''
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-5">
        <Card>
          <CardContent className="pt-6">
            ...
            <p className="mt-1 text-xs text-gray-600">Total Tickets</p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="pt-6">
            ...
            <p className="mt-1 text-xs text-gray-600">Abiertos</p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="pt-6">
            ...
            <p className="mt-1 text-xs text-gray-600">En Proceso</p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="pt-6">
            ...
            <p className="mt-1 text-xs text-gray-600">Resueltos</p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="pt-6">
            ...
            <p className="mt-1 text-xs text-gray-600">Cerrados</p>
          </CardContent>
        </Card>
      </div>
        ...
        <CardContent>
          ...
          {ticketsFiltrados.length === 0 ? (
              ...
              {tickets.length === 0 ? (
                <>
                  ...
                  <p className="mb-4 text-sm text-gray-400">
                    Crea tu primer ticket usando el botón "Nuevo Ticket"
                  </p>
                </>
              ) : (
            ...
            <div className="relative">
              ...
              <div className="absolute bottom-0 left-8 top-0 w-0.5 bg-gradient-to-b from-blue-200 via-blue-300 to-blue-200" />
              ...
              <div className="space-y-6">
                {ticketsFiltrados
                  ...
                  .map((ticket, index) => {
                    const estadoStr =
                      typeof ticket.estado === 'string'
                        ? ticket.estado
                        : 'abierto'

                    const prioridadStr =
                      typeof ticket.prioridad === 'string'
                        ? ticket.prioridad
                        : 'media'
                    ...
                    if (
                      ticket.fecha_limite &&
                      (estadoStr === 'abierto' || estadoStr === 'en_proceso')
                    ) {
                      const fechaLimite = new Date(ticket.fecha_limite)
                      ...
                      if (fechaLimite <= ahora) {
                        ...
                        minutosRestantes = Math.round(
                          (fechaLimite.getTime() - ahora.getTime()) /
                            (60 * 1000)
                        )
                      }
                    }
                    ...
                    const ticketTitulo =
                      typeof ticket.titulo === 'string' ? ticket.titulo : ''
                    ...
                    const ticketCliente =
                      typeof ticket.cliente === 'string'
                        ? ticket.cliente
                        : 'N/A'

                    const ticketAsignadoA =
                      typeof ticket.asignadoA === 'string'
                        ? ticket.asignadoA
                        : 'Sin asignar'
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="relative z-10 flex-shrink-0">
                          <div
                            className={`flex h-16 w-16 items-center justify-center rounded-full border-4 border-white shadow-lg ${
                              estadoFecha === 'vencido'
                          ...
                          {!isLast && (
                            <div className="absolute left-1/2 top-16 h-6 w-0.5 -translate-x-1/2 transform bg-gradient-to-b from-blue-300 to-blue-200" />
                          )}
                        </div>
                        ...
                        <div className="min-w-0 flex-1">
                          <motion.div
                            ...
                            className={`rounded-lg border-2 bg-white p-6 shadow-md transition-all duration-200 hover:shadow-lg ${
                              estadoFecha === 'vencido'
                          ...
                          >
                                    ...
                                    <div>
                                      ...
                                      <div className="mt-1 space-y-0.5">
                                        ...
                                        {ticket.clienteData.email && (
                                          <p className="text-xs text-gray-600">
                                            Email:{' '}
                                            <span className="font-medium">
                                              {String(ticket.clienteData.email)}
                                            </span>
                                          </p>
                                        )}
                                      </div>
                                      ...
                                      {ticket.clienteId != null && (
                                        <Button
                                          ...
                                          onClick={() =>
                                            window.open(
                                              `${BASE_PATH || ''}/clientes/${ticket.clienteId}`,
                                              '_blank'
                                            )
                                          }
                                        >
                            ...
                            <div className="flex flex-wrap items-center gap-3 border-t border-gray-200 pt-4">
                              <Badge
                                className={`${estadoInfo.color} flex items-center gap-1`}
                              >
          ...
          {ticketsData?.paginacion && ticketsData.paginacion.pages > 1 && (
            <div className="mt-6 flex items-center justify-between border-t pt-4">
              <div className="text-sm text-gray-600">
                Página {ticketsData.paginacion.page} de{' '}
                {ticketsData.paginacion.pages} ({ticketsData.paginacion.total}{' '}
                total)
              </div>
              ...
              <div className="flex gap-2">
                ...
                <Button
                  ...
                  size="sm"
                  onClick={() =>
                    setPage(p => Math.min(ticketsData.paginacion.pages, p + 1))
                  }
                  disabled={
                    page === ticketsData.paginacion.pages || isLoadingTickets
                  }
                >
                  Siguiente
                </Button>
      ...
      <Dialog open={showEditDialog} onOpenChange={setShowEditDialog}>
        <DialogContent className="max-h-[90vh] max-w-4xl overflow-y-auto">
          ...
          <div className="mt-4 space-y-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="descripcion-edit">
