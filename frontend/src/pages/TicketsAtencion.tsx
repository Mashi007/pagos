...
export function TicketsAtencion() {
  ...
  // BÃÂºsqueda de clientes para agregar al ticket (incluir todos los estados)
  const { data: clientesBuscados = [], isLoading: isLoadingSearch } = useSearchClientes(searchCliente, true)

  // Query: placeholderData para que la pÃÂ¡gina se vea desde el inicio (no pantalla en blanco)
  const {
    data: ticketsData,
  ...
  // KPIs desde la API (estadÃÂ­sticas globales en BD). Si el backend no envÃÂ­a estadisticas, fallback a contar solo la pÃÂ¡gina actual.
  ...
  // ÃÂ¢ÃÂÃ¢ÂÂ¦ Calcular tickets vencidos y prÃÂ³ximos a vencer
  ...
  const handleSeleccionarCliente = (cliente: Cliente) => {
    // Capturar rÃÂ¡pidamente todos los datos del cliente
    ...
    setNuevoTicket(prev => ({
      ...
      clienteData: cliente,
      // Pre-llenar descripciÃÂ³n con datos del cliente si estÃÂ¡ vacÃÂ­a para agilizar
      descripcion: prev.descripcion || `Cliente: ${nombreCompleto}\nCÃÂ©dula: ${cliente.cedula}${cliente.telefono ? `\nTelÃÂ©fono: ${cliente.telefono}` : ''}${cliente.email ? `\nEmail: ${cliente.email}` : ''}${cliente.direccion ? `\nDirecciÃÂ³n: ${cliente.direccion}` : ''}${cliente.ocupacion ? `\nOcupaciÃÂ³n: ${cliente.ocupacion}` : ''}\n\n`,
    }))
    setSearchCliente('')
  }
  ...
  return (
    <div className="space-y-6">
        ...
        <Dialog open={showAddDialog} onOpenChange={setShowAddDialog}>
          ...
          <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
                ...
                {clienteSeleccionado ? (
                      ...
                      <div className="flex-1">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm ml-7">
                          <div className="flex items-center gap-2">
                            <span className="text-gray-600 font-medium">CÃÂ©dula:</span>
                            <span className="text-gray-900 font-semibold">{String(clienteSeleccionado.cedula ? '')}</span>
                          </div>
                          {clienteSeleccionado.telefono && (
                            <div className="flex items-center gap-2">
                              <span className="text-gray-600 font-medium">TelÃÂ©fono:</span>
                          ...
                          {clienteSeleccionado.direccion && (
                            <div className="flex items-center gap-2 sm:col-span-2">
                              <span className="text-gray-600 font-medium">DirecciÃÂ³n:</span>
                              <span className="text-gray-900">{String(clienteSeleccionado.direccion)}</span>
                            </div>
                          )}
                          {clienteSeleccionado.ocupacion && (
                            <div className="flex items-center gap-2">
                              <span className="text-gray-600 font-medium">OcupaciÃÂ³n:</span>
                  ...
                  <div className="space-y-2">
                    <div className="relative">
                      ...
                      <Input
                        placeholder="Buscar cliente por nombre, cÃÂ©dula o telÃÂ©fono (mÃÂ­n. 2 caracteres)..."
                        value={searchCliente}
                                ...
                                <div className="flex-1 min-w-0">
                                  ...
                                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5 ml-6 text-xs">
                                    <div className="flex items-center gap-1.5">
                                      <span className="text-gray-500">CÃÂ©dula:</span>
                                    ...
                                    {cliente.direccion && (
                                      <div className="flex items-center gap-1.5 sm:col-span-2">
                                        <span className="text-gray-500">DirecciÃÂ³n:</span>
                                        <span className="font-medium text-gray-700 truncate">{String(cliente.direccion)}</span>
                                      </div>
                                    )}
                                    {cliente.ocupacion && (
                                      <div className="flex items-center gap-1.5">
                                        <span className="text-gray-500">OcupaciÃÂ³n:</span>
              ...
              {/* TÃÂ­tulo */}
              <div className="space-y-2">
                <Label htmlFor="titulo">TÃÂ­tulo *</Label>
                <Input
                  id="titulo"
                  placeholder="Ej: Consulta sobre estado de prÃÂ©stamo"
              ...
              {/* DescripciÃÂ³n */}
              <div className="space-y-2">
                <Label htmlFor="descripcion">DescripciÃÂ³n *</Label>
                <Textarea
                  id="descripcion"
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  placeholder="Selecciona fecha y hora lÃÂ­mite"
                />
                <p className="text-xs text-gray-500">
                  El sistema alertarÃÂ¡ cuando se alcance esta fecha y hora
                </p>
              </div>
            </div>
            <DialogFooter>
      ...
      {/* ÃÂ¢ÃÂÃ¢ÂÂ¦ Alertas de Tickets Vencidos */}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    <h3 className="font-semibold text-yellow-900">
                      {ticketsProximosAVencer.length} Ticket{ticketsProximosAVencer.length > 1 ? 's' : ''} PrÃÂ³ximo{ticketsProximosAVencer.length > 1 ? 's' : ''} a Vencer
                    </h3>
          ...
          <div className="flex flex-col md:flex-row gap-4">
            <div className="flex-1">
              <div className="relative">
                <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 h-4 w-4" />
                <Input
                  placeholder="Buscar por tÃÂ­tulo, cliente o descripciÃÂ³n..."
                  value={searchTerm}
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              <CardTitle>Tickets de AtenciÃÂ³n</CardTitle>
              <CardDescription>
                GestiÃÂ³n de incidencias y actividades que generan seguimiento
              </CardDescription>
            </div>
            {isLoadingTickets && (
        ...
        <CardContent>
          ...
          {ticketsFiltrados.length === 0 ? (
            <div className="text-center py-12 text-gray-500">
              ...
              {tickets.length === 0 ? (
                <>
                  ...
                  <p className="text-sm text-gray-400 mb-4">Crea tu primer ticket usando el botÃÂ³n "Nuevo Ticket"</p>
                </>
              ) : (
            ...
            <div className="relative">
              {/* LÃÂ­nea vertical de la timeline */}
              ...
              <div className="space-y-6">
                {ticketsFiltrados
                  .sort((a, b) => {
                    // Ordenar por fecha mÃÂ¡s reciente primero
                  ...
                  .map((ticket, index) => {
                    ...
                    // Verificar si el ticket estÃÂ¡ vencido o prÃÂ³ximo a vencer
                    const ahora = new Date()
                    const enUnaHora = new Date(ahora.getTime() + 60 * 60 * 1000)
                    ...
                    return (
                      ...
                      >
                        {/* Punto en la lÃÂ­nea de tiempo */}
                          ...
                          >
                              ...
                              <div className="flex gap-2 ml-4">
                                {ticket.conversacion_whatsapp_id && (
                                  <Button
                                    ...
                                    title="Ver conversaciÃÂ³n de WhatsApp"
                                  >
                                  ...
                                  {ticket.clienteData ? (
                                    <div>
                                      ...
                                      <div className="mt-1 space-y-0.5">
                                        <p className="text-xs text-gray-600">CÃÂ©dula: <span className="font-medium">{String(ticket.clienteData.cedula ? '')}</span></p>
                                      ...
                                      {ticket.clienteId != null && (
                                        ...
                                        >
                                          Ver cliente completo Ã¢ÂÂ ÃÂ¢Ã¢ÂÂ Ã¢ÂÂ
                                        </Button>
                                      )}
                                    </div>
                                  ) : (
                            ...
                            <div className="flex flex-wrap items-center gap-3 pt-4 border-t border-gray-200">
                              ...
                              {ticket.fecha_limite && (
                                ...
                                >
                                  <Clock className="h-3 w-3 mr-1" />
                                  LÃÂ­mite: {formatDate(ticket.fecha_limite)}
                                </Badge>
                              )}
          ...
          {ticketsData?.paginacion && ticketsData.paginacion.pages > 1 && (
            <div className="flex items-center justify-between pt-4 border-t mt-6">
              <div className="text-sm text-gray-600">
                PÃÂ¡gina {ticketsData.paginacion.page} de {ticketsData.paginacion.pages} ({ticketsData.paginacion.total} total)
              </div>
      ...
      <Dialog open={showEditDialog} onOpenChange={setShowEditDialog}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4 mt-4">
            {/* TÃÂ­tulo y descripciÃÂ³n no se pueden editar despuÃÂ©s de guardar (solo estado, prioridad, fecha lÃÂ­mite) */}
            <div className="space-y-2">
              <Label htmlFor="titulo-edit">TÃÂ­tulo (no editable)</Label>
              <Input
                id="titulo-edit"
                value={nuevoTicket.titulo || ''}
                disabled
            ...
            <div className="space-y-2">
              <Label htmlFor="descripcion-edit">DescripciÃÂ³n (no editable)</Label>
              <Textarea
                id="descripcion-edit"
                value={nuevoTicket.descripcion || ''}
                disabled
