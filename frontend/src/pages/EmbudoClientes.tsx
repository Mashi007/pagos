...
const mapearEstadoEmbudo = (
  ...
  // 1. RECHAZADO: Solo si en mÃÂ³dulo prÃÂ©stamos estÃÂ¡ con ese estado
  ...
  // 2. APROBADO: Todos los que tienen estado aprobado en prÃÂ©stamos
  ...
  // 3. EN EVALUACIÃÂN: Solo si estÃÂ¡n en el mÃÂ³dulo prÃÂ©stamos en evaluaciÃÂ³n (EN_REVISION o DRAFT)
  ...
  // Cualquier cliente de la base de datos puede ser prospecto (no requiere tener prÃÂ©stamos)
  // Si no tiene prÃÂ©stamos o no estÃÂ¡ en ningÃÂºn estado especÃÂ­fico, es prospecto
  return 'prospecto'
}
...
export function EmbudoClientes() {
  ...
  // Obtener prÃÂ©stamos para vincular concesionarios
  const { data: prestamosData } = usePrestamos(undefined, 1, 1000)

  // Obtener estadÃÂ­sticas del embudo desde la API
  ...
  // BÃÂºsqueda de clientes para agregar (incluir todos los estados para el embudo)
  ...
  const clientesEmbudo = useMemo(() => {
    ...
    // Si un cliente estÃÂ¡ en clientesEnEmbudo, usar esa versiÃÂ³n (tiene estado actualizado)
    ...
    // Agregar clientes que no estÃÂ¡n en la API pero sÃÂ­ en clientesEnEmbudo
    ...
    return todosClientes.map(cliente => {
      // Buscar prÃÂ©stamos del cliente
      const prestamosCliente = prestamos.filter(p => p.cliente_id === cliente.id)
      const tienePrestamos = prestamosCliente.length > 0

      // Verificar si los prÃÂ©stamos estÃÂ¡n al dÃÂ­a:
      // - El cliente debe estar ACTIVO
      // - No debe tener dÃÂ­as de mora (dias_mora === 0)
      // - Los prÃÂ©stamos deben estar APROBADOS (no en revisiÃÂ³n o rechazados)
  ...
  // EstadÃÂ­sticas basadas en los embudos (deben coincidir exactamente)
  ...
  const handleAgregarCliente = (cliente: Cliente) => {
    ...
    // Asignar automÃÂ¡ticamente al embudo "agregar_otro"
  ...
  // FunciÃÂ³n auxiliar para mapear estado del embudo al estado del cliente en BD
  ...
  const mapearEstadoEmbudoACliente = (
    ...
    switch (estadoEmbudo) {
      case 'prospecto':
        return 'ACTIVO'
      case 'evaluacion':
        // Si el cliente tiene prÃÂ©stamos en aprobaciÃÂ³n de riesgo, mantener ACTIVO
        // Solo cambiar a MORA si realmente estÃÂ¡ en mora
        const tienePrestamosEnRiesgo = prestamosCliente.some(p =>
          p.estado === 'EN_REVISION' || p.estado === 'DRAFT'
        )
        if (tienePrestamosEnRiesgo) {
          // Mantener ACTIVO si estÃÂ¡ en aprobaciÃÂ³n de riesgo
          return cliente.estado === 'ACTIVO' ? null : 'ACTIVO'
        }
        // Si no estÃÂ¡ en aprobaciÃÂ³n de riesgo, puede ser MORA
        return 'MORA'
      case 'aprobado':
        return 'FINALIZADO'
      case 'rechazado':
  ...
  const handleDrop = async (e: React.DragEvent, estadoDestino: string) => {
    ...
    if (clienteArrastrando && estadoDestino) {
      ...
      if (cliente && cliente.cliente) {
        // Obtener prÃÂ©stamos del cliente para la lÃÂ³gica de mapeo
        const prestamos = prestamosData?.data || []
        const prestamosCliente = prestamos.filter(p => p.cliente_id === cliente.id)

        // Guardar el estado manual del embudo (esto tiene prioridad sobre el cÃÂ¡lculo automÃÂ¡tico)
        setEstadosManuales(prev => {
          const nuevo = new Map(prev)
        ...
        // Si hay un nuevo estado vÃÂ¡lido y es diferente al actual, actualizar en BD
        if (nuevoEstadoCliente && nuevoEstadoCliente !== cliente.cliente.estado) {
          ...
          // Actualizar tambiÃÂ©n en clientesEnEmbudo para mantener consistencia
          setClientesEnEmbudo(prev => {
            const nuevo = new Map(prev)
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <div>
          ...
          <p className="text-gray-600 mt-1 text-sm">
            GestiÃÂ³n de ventas y servicios
          </p>
        </div>
          ...
          <DialogContent className="max-w-2xl">
            ...
            <div className="space-y-4">
              <div className="relative">
                ...
                <Input
                  placeholder="Buscar por nombre, cÃÂ©dula o telÃÂ©fono..."
                  value={searchCliente}
                      ...
                      <CardContent className="p-4">
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            <p className="text-sm text-gray-500">CÃÂ©dula: {cliente.cedula}</p>
                            {cliente.telefono && (
      ...
      {/* EstadÃÂ­sticas - Deben coincidir exactamente con los embudos */}
          ...
          <div className="flex flex-col md:flex-row gap-4">
            <div className="flex-1">
              <div className="relative">
                <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 h-4 w-4" />
                <Input
                  placeholder="Buscar por nombre, cÃÂ©dula, telÃÂ©fono o concesionario..."
                  value={searchTerm}
                    ...
                    <CardContent className="p-3 space-y-3 max-h-[calc(100vh-400px)] overflow-y-auto">
                      {columna.id === 'agregar_otro' ? (
                                    ...
                                    <div className="flex items-start justify-between">
                                      <div className="flex items-start gap-2 flex-1">
                                        ...
                                        <div className="flex-1 min-w-0">
                                          ...
                                          <p className="text-xs text-gray-500 mt-1">CÃÂ©dula: {cliente.cedula}</p>
                                        </div>
                                      </div>
                        ...
                        <div className={`text-center py-8 text-gray-400 border-2 border-dashed rounded-lg ${
                          ...
                          <p className="text-xs">Suelta aquÃÂ­ para mover</p>
                        </div>
                      ) : (
                        columna.clientes.map((cliente) => (
                          <motion.div
