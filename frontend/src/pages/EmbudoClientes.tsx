...
const mapearEstadoEmbudo = (
  ...
  const prestamosRechazados = prestamosCliente.filter(
  ...
  const prestamosAprobados = prestamosCliente.filter(
    p => p.estado === 'APROBADO'
  )
  ...
  const prestamosEnEvaluacion = prestamosCliente.filter(
    p => p.estado === 'EN_REVISION' || p.estado === 'DRAFT'
  )
...
export function EmbudoClientes() {
  ...
  const [clientesEnEmbudo, setClientesEnEmbudo] = useState<
    Map<number, Cliente>
  >(new Map())

  const [estadosManuales, setEstadosManuales] = useState<Map<number, string>>(
    new Map()
  ) // Guardar estados manuales del embudo

  const [clienteArrastrando, setClienteArrastrando] = useState<number | null>(
    null
  )
  ...
  const { data: estadisticasEmbudo, isLoading: isLoadingEstadisticas } =
    useQuery({
  ...
  const clientesEmbudo = useMemo(() => {
    ...
    return todosClientes.map(cliente => {
      ...
      const estadoCalculado = mapearEstadoEmbudo(
        cliente,
        tienePrestamos,
        prestamosAlDia,
        prestamosCliente
      ...
      return {
        ...
        montoSolicitado:
          prestamoCliente?.total_financiamiento ||
          cliente.total_financiamiento ||
          cliente.monto_financiado ||
          0,
  ...
  const estadisticas = useMemo(() => {
    ...
    const prospectos =
      clientesPorEstado.find(e => e.id === 'prospecto')?.count || 0

    const evaluacion =
      clientesPorEstado.find(e => e.id === 'evaluacion')?.count || 0

    const aprobados =
      clientesPorEstado.find(e => e.id === 'aprobado')?.count || 0

    const rechazados =
      clientesPorEstado.find(e => e.id === 'rechazado')?.count || 0

    const agregarOtro =
      clientesPorEstado.find(e => e.id === 'agregar_otro')?.count || 0
  ...
  const mapearEstadoEmbudoACliente = (
    ...
    switch (estadoEmbudo) {
      ...
      case 'evaluacion':
        ...
        const tienePrestamosEnRiesgo = prestamosCliente.some(
          p => p.estado === 'EN_REVISION' || p.estado === 'DRAFT'
        )
  ...
  const handleDrop = async (e: React.DragEvent, estadoDestino: string) => {
    ...
    if (clienteArrastrando && estadoDestino) {
      ...
      if (cliente && cliente.cliente) {
        ...
        if (
          nuevoEstadoCliente &&
          nuevoEstadoCliente !== cliente.cliente.estado
        ) {
          try {
          ...
          <div className="flex min-w-max gap-4">
            {clientesPorEstado.map(columna => {
              ...
              return (
                ...
                >
                  <Card
                    className={`h-full ${columna.color} border-2 ${
                      columnaDestino === columna.id
                  ...
                  >
                    ...
                    <CardContent className="max-h-[calc(100vh-400px)] space-y-3 overflow-y-auto p-3">
                      {columna.id === 'agregar_otro' ? (
                            ...
                            <div className="mt-3 space-y-3">
                              {columna.clientes.map(cliente => (
                                <motion.div
                                  ...
                                  className={`cursor-move rounded-lg border-2 border-gray-200 bg-white p-4 shadow-sm transition-all hover:border-purple-300 hover:shadow-md ${
                                    clienteArrastrando === cliente.id
                                  ...
                                  <div className="space-y-3">
                                    <div className="flex items-start justify-between">
                                      <div className="flex flex-1 items-start gap-2">
                                        <Menu className="mt-0.5 h-4 w-4 flex-shrink-0 text-gray-400" />
                                    ...
                                    <div className="space-y-2 border-t border-gray-100 pt-2">
                                      ...
                                      <div className="pt-1 text-xs text-gray-500">
                                        Ingreso:{' '}
                                        {formatDate(cliente.fechaIngreso)}
                                      </div>
                                    </div>
                                  </div>
                                </motion.div>
                        ...
                        <div
                          className={`rounded-lg border-2 border-dashed py-8 text-center text-gray-400 ${
                            columnaDestino === columna.id
                        ...
                        columna.clientes.map(cliente => (
                          <motion.div
                            ...
                            animate={{
                              opacity:
                                clienteArrastrando === cliente.id ? 0.5 : 1,

                              scale:
                                clienteArrastrando === cliente.id ? 0.95 : 1,
                            }}
                            whileHover={{
                              scale:
                                clienteArrastrando === cliente.id ? 0.95 : 1.02,
                            }}
                            draggable
                            onDragStart={() => handleDragStart(cliente.id)}
                            onDragEnd={handleDragEnd}
                            onClick={() => navigate(`/clientes/${cliente.id}`)}
                            className={`cursor-move rounded-lg border-2 border-gray-200 bg-white p-4 shadow-sm transition-all hover:border-blue-300 hover:shadow-md ${
                              clienteArrastrando === cliente.id
                                ? 'scale-95 cursor-grabbing opacity-50'
                                : 'cursor-grab'
                            }`}
                              ...
                              <div className="space-y-2 border-t border-gray-100 pt-2">
                                ...
                                {cliente.montoSolicitado > 0 && (
                                  <div className="flex items-center gap-2 text-xs text-gray-600">
                                    ...
                                    <span className="font-medium">
                                      {formatCurrency(cliente.montoSolicitado)}
                                    </span>
                                  </div>
                                )}
