...
import {
  useConcesionarios,
  useCreateConcesionario,
} from '../hooks/useConcesionarios'
...
const mapearEstadoConcesionario = (
  ...
  const prestamosAprobados = prestamos.filter(
    p => p.concesionario === concesionario.nombre && p.estado === 'APROBADO'
  )
...
export function EmbudoConcesionarios() {
  ...
  const [concesionarioSeleccionadoId, setConcesionarioSeleccionadoId] =
    useState<string>('')
  ...
  const [concesionariosEnEmbudo, setConcesionariosEnEmbudo] = useState<
    Map<number, Concesionario>
  >(new Map())

  const [estadosManuales, setEstadosManuales] = useState<Map<number, string>>(
    new Map()
  )

  const [nuevoConcesionario, setNuevoConcesionario] =
    useState<ConcesionarioCreate>({
      nombre: '',
  ...
  const todosConcesionarios = useMemo(() => {
    const concesionariosAPI = concesionarios.map(concesionario => {
      const concesionarioActualizado = concesionariosEnEmbudo.get(
        concesionario.id
      )
  ...
  const concesionariosConEstadisticas = useMemo(() => {
    return todosConcesionarios
      .map(concesionario => {
        const prestamosConcesionario = prestamos.filter(
          p => p.concesionario === concesionario.nombre
        )

        const prestamosAprobados = prestamosConcesionario.filter(
          p => p.estado === 'APROBADO'
        )
        ...
        const clientesIds = new Set(
          prestamosConcesionario.map(p => p.cliente_id)
        )
        ...
        const montoTotal = prestamosAprobados.reduce(
          (sum, p) => sum + Number(p.total_financiamiento || 0),
          0
        )
  ...
  const concesionarioDetalle = concesionarioSeleccionado
    ? concesionariosConEstadisticas.find(
        c => c.id === concesionarioSeleccionado
      )
    : null
  ...
  const concesionariosParaMostrar = concesionarioSeleccionado
    ? concesionariosConEstadisticas.filter(
        c => c.id === concesionarioSeleccionado
      )
  ...
  const concesionariosFiltrados = concesionariosParaMostrar.filter(
    concesionario => {
  ...
  const handleAgregarConcesionario = (concesionario: Concesionario) => {
    setConcesionariosEnEmbudo(prev =>
      new Map(prev).set(concesionario.id, concesionario)
    )
  ...
  const handleCrearConcesionario = async () => {
    ...
    try {
      const creado =
        await createConcesionarioMutation.mutateAsync(nuevoConcesionario)
  ...
  const concesionariosPorEstado = useMemo(() => {
    return ESTADOS_EMBUDO.map(estado => ({
      ...
      concesionarios: concesionariosFiltrados.filter(
        c => c.estado === estado.id
      ),
  ...
  const estadisticas = useMemo(() => {
    ...
    const activos =
      concesionariosPorEstado.find(e => e.id === 'activo')?.count || 0

    const pendientes =
      concesionariosPorEstado.find(e => e.id === 'pendiente')?.count || 0

    const inactivos =
      concesionariosPorEstado.find(e => e.id === 'inactivo')?.count || 0
    ...
    return {
      ...
      totalClientes: new Set(
        concesionariosParaEstadisticas.flatMap(c =>
          c.prestamos.map(p => p.cliente_id)
        )
      ).size,

      totalPrestamos: concesionariosParaEstadisticas.reduce(
        (sum, c) => sum + c.prestamosActivos,
        0
      ),

      montoTotal: concesionariosParaEstadisticas.reduce(
      ...
      <div className="flex min-h-[400px] flex-col items-center justify-center space-y-4">
        ...
        <div className="text-center">
          <h3 className="mb-2 text-lg font-semibold text-gray-900">
            Error al cargar concesionarios
          </h3>

          <p className="mb-4 text-sm text-gray-600">
            {errorConcesionarios instanceof Error
              ? errorConcesionarios.message
              : 'Error desconocido'}
          </p>

          <Button onClick={() => refetchConcesionarios()}>Reintentar</Button>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-4 lg:grid-cols-7">
        <Card>
          <CardContent className="pt-6">
            ...
            <p className="mt-1 text-xs text-gray-600">Total Concesionarios</p>
          </CardContent>
        </Card>
          ...
          return (
            <Card key={estado.id}>
              <CardContent className="pt-6">
                ...
                <p className="mt-1 text-xs text-gray-600">{estado.label}</p>
              </CardContent>
            </Card>
          )
        })}
        ...
        <Card>
          <CardContent className="pt-6">
            ...
            <p className="mt-1 text-xs text-gray-600">Total Clientes</p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="pt-6">
            ...
            <p className="mt-1 text-xs text-gray-600">Préstamos Activos</p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="pt-6">
            ...
            <p className="mt-1 text-xs text-gray-600">Monto Total</p>
          </CardContent>
        </Card>
      </div>
      ...
      {concesionarios.length === 0 ? (
        <Card>
          <CardContent className="py-12">
            <div className="text-center">
              ...
              <p className="mb-4 text-sm text-gray-600">
                No se encontraron concesionarios en el sistema. Puedes crear uno
                nuevo desde la configuración.
              </p>
        ...
        <div
          className={`grid grid-cols-1 gap-6 ${concesionarioDetalle ? 'lg:grid-cols-3' : 'lg:grid-cols-1'}`}
        >
          {/* Kanban Board - Embudo Visual */}

          <div
            className={`overflow-x-auto pb-4 ${concesionarioDetalle ? 'lg:col-span-2' : 'lg:col-span-1'}`}
          >
                ...
                return (
                  ...
                  >
                    <Card className={`h-full ${columna.color} border-2`}>
                      ...
                      <CardContent className="max-h-[calc(100vh-400px)] space-y-3 overflow-y-auto p-3">
                        {columna.id === 'inactivo' ? (
                                  ...
                                  <motion.div
                                    ...
                                    onClick={() =>
                                      setConcesionarioSeleccionadoId(
                                        concesionarioSeleccionadoId ===
                                          String(concesionario.id)
                                          ? ''
                                          : String(concesionario.id)
                                      )
                                    }
                                    className={`cursor-pointer rounded-lg border-2 border-gray-200 bg-white p-4 shadow-sm transition-all hover:border-gray-300 hover:shadow-md`}
                                  >
                                    <div className="space-y-3">
                                      ...
                                      <div className="space-y-2 border-t border-gray-100 pt-2">
                                        <div className="grid grid-cols-2 gap-2 pt-2">
                                          <div className="rounded bg-blue-50 p-2">
                                            ...
                                            <div className="text-sm font-bold text-blue-700">
                                              {concesionario.clientesAsignados}
                                            </div>
                                          </div>
                                          ...
                                          <div className="rounded bg-purple-50 p-2">
                                            ...
                                            <div className="text-sm font-bold text-purple-700">
                                              {concesionario.prestamosActivos}
                                            </div>
                                          </div>
                                        </div>
                                        ...
                                        <div className="pt-1 text-xs text-gray-500">
                                          Registro:{' '}
                                          {formatDate(
                                            new Date(concesionario.created_at)
                                          )}
                                        </div>
                                      </div>
                                    </div>
                            ...
                            >
                              <div className="space-y-3">
                                ...
                                {(camposTarjeta.mostrarClientes ||
                                  ...
                                  camposTarjeta.mostrarMontoTotal ||
            ...
            <div className="lg:col-span-1">
              <Card className="h-full">
                <CardHeader>
                  ...
                  <CardDescription>
                    {concesionarioDetalle.clientesAsignados} clientes,{' '}
                    {concesionarioDetalle.prestamosActivos} préstamos activos
                  </CardDescription>
                </CardHeader>

                <CardContent className="max-h-[calc(100vh-400px)] space-y-3 overflow-y-auto">
                  {clientesYprestamosDetalle.length === 0 ? (
                    <div className="py-12 text-center text-gray-500">
                      <Users className="mx-auto mb-4 h-12 w-12 text-gray-400" />

                      <p className="text-sm">
                        Este concesionario no tiene clientes asignados
                      </p>
                    </div>
                  ) : (
                    clientesYprestamosDetalle.map(
                      ({
                        cliente,
                        ...
                        <motion.div
                          ...
                          animate={{ opacity: 1, y: 0 }}
                          className="rounded-lg border border-gray-200 bg-gray-50 p-3 transition-colors hover:border-blue-300"
                        >
