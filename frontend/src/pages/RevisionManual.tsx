...
export function RevisionManual() {
  ...
  // Al volver tras finalizar (Guardar y Cerrar), mostrar "Todos" para que el prÃÂ©stamo finalizado sea visible
  useEffect(() => {
  ...
  // Persistir estado para mantener posiciÃÂ³n al volver de editar (Guardar y cerrar)
  useEffect(() => {
  ...
  const handleConfirmarSi = async (prestamoId: number, nombres: string) => {
    const confirmar = window.confirm(
      `Ã¢ÂÂ Ã¯Â¸Â CONFIRMAR REVISIÃÂN - ${nombres}\n\n` +
      'Ã¢ÂÂ Se marcarÃÂ¡n TODOS los datos como correctos:\n' +
      '  - Datos del cliente\n' +
      '  - Datos del prÃÂ©stamo\n' +
      '  - Cuotas y pagos\n\n' +
      'Ã¢ÂÂ El prÃÂ©stamo desaparecerÃÂ¡ de esta lista\n' +
      'Ã¢ÂÂ NO PODRÃÂS EDITAR ESTE PRÃÂSTAMO DE NUEVO\n\n' +
      'ÃÂ¿Confirmas que todo estÃÂ¡ correcto?'
    )
    if (!confirmar) {
      toast.info('Ã¢ÂÂ¹Ã¯Â¸Â ConfirmaciÃÂ³n cancelada')
      return
    }

    try {
      const res = await revisionManualService.confirmarPrestamoRevisado(prestamoId)
      toast.success(`Ã¢ÂÂ ${res.mensaje}`)
  ...
  const handleEditarNo = (prestamoId: number) => {
    ...
    const confirmar = window.confirm(
      'Ã¢ÂÂ Ã¯Â¸Â INICIAR EDICIÃÂN\n\n' +
      'Al presionar "No", accederÃÂ¡s a la interfaz de ediciÃÂ³n donde podrÃÂ¡s:\n' +
      'Ã¢ÂÂ Editar datos del cliente\n' +
      'Ã¢ÂÂ Editar datos del prÃÂ©stamo\n' +
      'Ã¢ÂÂ Editar cuotas y pagos\n\n' +
      'Ã¢ÂÂ Puedes guardar cambios parciales (Guardar Parciales)\n' +
      'Ã¢ÂÂ O finalizar la revisiÃÂ³n (Guardar y Cerrar)\n\n' +
      'ÃÂ¿Deseas continuar?'
    )
    if (!confirmar) {
      toast.info('Ã¢ÂÂ¹Ã¯Â¸Â EdiciÃÂ³n cancelada')
      return
    }

    // Inicia revisiÃÂ³n (cambia estado a 'revisando')
    revisionManualService.iniciarRevision(prestamoId).then(() => {
      toast.info('Ã¢ÂÂ¹Ã¯Â¸Â EdiciÃÂ³n iniciada. Abriendo editor...')
      queryClient.invalidateQueries({ queryKey: ['revision-manual-prestamos'] })
      // Navega a pÃÂ¡gina de ediciÃÂ³n
      navigate(`/revision-manual/editar/${prestamoId}`)
    }).catch((err: any) => {
      const errorMsg = err?.response?.data?.detail || 'Error al iniciar revisiÃÂ³n'
      toast.error(`Ã¢ÂÂ ${errorMsg}`)
      console.error('Error iniciando revisiÃÂ³n:', err)
    })
  }

  const handleEliminar = async (prestamoId: number, nombres: string) => {
    const confirmar = window.confirm(
      `Ã¢ÂÂ Ã¯Â¸Â ELIMINAR PRÃÂSTAMO - ${nombres}\n\n` +
      'Esta acciÃÂ³n eliminarÃÂ¡ permanentemente:\n' +
      '  - El prÃÂ©stamo\n' +
      '  - Todas las cuotas asociadas\n' +
      '  - El registro de revisiÃÂ³n manual\n\n' +
      'ÃÂ¿EstÃÂ¡s seguro de que deseas eliminar?'
    )
    if (!confirmar) {
      toast.info('Ã¢ÂÂ¹Ã¯Â¸Â EliminaciÃÂ³n cancelada')
      return
    }

    try {
      await revisionManualService.eliminarPrestamo(prestamoId)
      toast.success('Ã¢ÂÂ PrÃÂ©stamo eliminado')
      queryClient.invalidateQueries({ queryKey: ['revision-manual-prestamos'] })
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="space-y-1">
          <h1 className="text-3xl font-bold text-gray-900">RevisiÃÂ³n Manual de PrÃÂ©stamos</h1>
          <p className="text-sm text-gray-600">
            Verifica y confirma los detalles de cada prÃÂ©stamo post-migraciÃÂ³n
          </p>
        </div>
        <Button
          variant="outline"
      ...
      {data && (
        <Card className="bg-gradient-to-r from-blue-50 to-indigo-50 border-blue-200">
          <CardHeader className="pb-3">
            <CardTitle className="text-lg">Progreso de RevisiÃÂ³n</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-700">
                  {data.prestamos_revisados} de {data.total_prestamos} prÃÂ©stamos revisados
                </p>
                <p className="text-xs text-gray-500 mt-1">
                  Faltan: {data.prestamos_pendientes} prÃÂ©stamos por revisar
                </p>
              </div>
      ...
      <div className="flex gap-2 flex-wrap">
        ...
        >
          Ã¢ÂÂ Revisados ({data?.prestamos_revisados || 0})
        </Button>
      </div>

      {/* BÃÂºsqueda por cÃÂ©dula */}
        ...
        <CardContent className="pt-4">
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="flex-1 flex gap-2">
              ...
              <Input
                placeholder="Buscar por cÃÂ©dula para acceder a un caso especÃÂ­fico..."
                value={cedulaInput}
            ...
            {cedulaBuscar && (
              <span className="text-sm text-gray-600 self-center">
                Mostrando resultados para cÃÂ©dula: <strong>{cedulaBuscar}</strong>
              </span>
            )}
          </div>
        </CardContent>
        ...
        <CardContent>
          {isLoading ? (
            ...
            <div className="text-center py-8 text-red-600">
              <p>Error al cargar prÃÂ©stamos</p>
            </div>
          ) : datosVisibles.length === 0 ? (
            <div className="text-center py-8 text-gray-500">
              <p>No hay prÃÂ©stamos para mostrar</p>
            </div>
          ) : (
              ...
              <table className="w-full text-sm">
                <thead className="bg-gray-50 border-b">
                  <tr>
                    ...
                    <th className="px-4 py-2 text-left font-semibold">CÃÂ©dula</th>
                    <th className="px-4 py-2 text-right font-semibold">Total PrÃÂ©stamo</th>
                    ...
                    <th className="px-4 py-2 text-center font-semibold">DecisiÃÂ³n</th>
                  </tr>
                </thead>
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-center">
                        {prestamo.estado_revision === 'revisado' && (
                          <span className="px-2 py-1 rounded text-xs font-semibold bg-green-100 text-green-800">Ã¢ÂÂ Revisado</span>
                        )}
                        {prestamo.estado_revision === 'revisando' && (
                          <span className="px-2 py-1 rounded text-xs font-semibold bg-yellow-100 text-yellow-800">Ã°ÂÂÂ Revisando</span>
                        )}
          ...
          {!isLoading && !error && totalPrestamos > 0 && (
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mt-4 pt-4 border-t">
              <p className="text-sm text-gray-600">
                ...
                <strong>{totalPrestamos}</strong> prÃÂ©stamos
              </p>
