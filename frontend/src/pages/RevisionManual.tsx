...
export function RevisionManual() {
  ...
  // Al volver tras finalizar (Guardar y Cerrar), mostrar "Todos" para que el prÃ©stamo finalizado sea visible
  useEffect(() => {
  ...
  // Persistir estado para mantener posiciÃ³n al volver de editar (Guardar y cerrar)
  useEffect(() => {
  ...
  const handleConfirmarSi = async (prestamoId: number, nombres: string) => {
    const confirmar = window.confirm(
      `â ï¸ CONFIRMAR REVISIÃN - ${nombres}\n\n` +
      'â Se marcarÃ¡n TODOS los datos como correctos:\n' +
      '  - Datos del cliente\n' +
      '  - Datos del prÃ©stamo\n' +
      '  - Cuotas y pagos\n\n' +
      'â El prÃ©stamo desaparecerÃ¡ de esta lista\n' +
      'â NO PODRÃS EDITAR ESTE PRÃSTAMO DE NUEVO\n\n' +
      'Â¿Confirmas que todo estÃ¡ correcto?'
    )
    if (!confirmar) {
      toast.info('â¹ï¸ ConfirmaciÃ³n cancelada')
      return
    }

    try {
      const res = await revisionManualService.confirmarPrestamoRevisado(prestamoId)
      toast.success(`â ${res.mensaje}`)
  ...
  const handleEditarNo = (prestamoId: number) => {
    ...
    const confirmar = window.confirm(
      'â ï¸ INICIAR EDICIÃN\n\n' +
      'Al presionar "No", accederÃ¡s a la interfaz de ediciÃ³n donde podrÃ¡s:\n' +
      'â Editar datos del cliente\n' +
      'â Editar datos del prÃ©stamo\n' +
      'â Editar cuotas y pagos\n\n' +
      'â Puedes guardar cambios parciales (Guardar Parciales)\n' +
      'â O finalizar la revisiÃ³n (Guardar y Cerrar)\n\n' +
      'Â¿Deseas continuar?'
    )
    if (!confirmar) {
      toast.info('â¹ï¸ EdiciÃ³n cancelada')
      return
    }

    // Inicia revisiÃ³n (cambia estado a 'revisando')
    revisionManualService.iniciarRevision(prestamoId).then(() => {
      toast.info('â¹ï¸ EdiciÃ³n iniciada. Abriendo editor...')
      queryClient.invalidateQueries({ queryKey: ['revision-manual-prestamos'] })
      // Navega a pÃ¡gina de ediciÃ³n
      navigate(`/revision-manual/editar/${prestamoId}`)
    }).catch((err: any) => {
      const errorMsg = err?.response?.data?.detail || 'Error al iniciar revisiÃ³n'
      toast.error(`â ${errorMsg}`)
      console.error('Error iniciando revisiÃ³n:', err)
    })
  }

  const handleEliminar = async (prestamoId: number, nombres: string) => {
    const confirmar = window.confirm(
      `â ï¸ ELIMINAR PRÃSTAMO - ${nombres}\n\n` +
      'Esta acciÃ³n eliminarÃ¡ permanentemente:\n' +
      '  - El prÃ©stamo\n' +
      '  - Todas las cuotas asociadas\n' +
      '  - El registro de revisiÃ³n manual\n\n' +
      'Â¿EstÃ¡s seguro de que deseas eliminar?'
    )
    if (!confirmar) {
      toast.info('â¹ï¸ EliminaciÃ³n cancelada')
      return
    }

    try {
      await revisionManualService.eliminarPrestamo(prestamoId)
      toast.success('â PrÃ©stamo eliminado')
      queryClient.invalidateQueries({ queryKey: ['revision-manual-prestamos'] })
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="space-y-1">
          <h1 className="text-3xl font-bold text-gray-900">RevisiÃ³n Manual de PrÃ©stamos</h1>
          <p className="text-sm text-gray-600">
            Verifica y confirma los detalles de cada prÃ©stamo post-migraciÃ³n
          </p>
        </div>
        <Button
          variant="outline"
      ...
      {data && (
        <Card className="bg-gradient-to-r from-blue-50 to-indigo-50 border-blue-200">
          <CardHeader className="pb-3">
            <CardTitle className="text-lg">Progreso de RevisiÃ³n</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-700">
                  {data.prestamos_revisados} de {data.total_prestamos} prÃ©stamos revisados
                </p>
                <p className="text-xs text-gray-500 mt-1">
                  Faltan: {data.prestamos_pendientes} prÃ©stamos por revisar
                </p>
              </div>
      ...
      <div className="flex gap-2 flex-wrap">
        ...
        >
          â Revisados ({data?.prestamos_revisados || 0})
        </Button>
      </div>
        ...
        <CardContent className="pt-4">
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="flex-1 flex gap-2">
              ...
              <Input
                placeholder="Buscar por cÃ©dula para acceder a un caso especÃ­fico..."
                value={cedulaInput}
            ...
            {cedulaBuscar && (
              <span className="text-sm text-gray-600 self-center">
                Mostrando resultados para cÃ©dula: <strong>{cedulaBuscar}</strong>
              </span>
            )}
          </div>
        </CardContent>
        ...
        <CardContent>
          {isLoading ? (
            ...
            <div className="text-center py-8 text-red-600">
              <p>Error al cargar prÃ©stamos</p>
            </div>
          ) : datosVisibles.length === 0 ? (
            <div className="text-center py-8 text-gray-500">
              <p>No hay prÃ©stamos para mostrar</p>
            </div>
          ) : (
              ...
              <table className="w-full text-sm">
                <thead className="bg-gray-50 border-b">
                  <tr>
                    ...
                    <th className="px-4 py-2 text-left font-semibold">CÃ©dula</th>
                    <th className="px-4 py-2 text-right font-semibold">Total PrÃ©stamo</th>
                    ...
                    <th className="px-4 py-2 text-center font-semibold">DecisiÃ³n</th>
                  </tr>
                </thead>
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-center">
                        {prestamo.estado_revision === 'revisado' && (
                          <span className="px-2 py-1 rounded text-xs font-semibold bg-green-100 text-green-800">â Revisado</span>
                        )}
                        {prestamo.estado_revision === 'revisando' && (
                          <span className="px-2 py-1 rounded text-xs font-semibold bg-yellow-100 text-yellow-800">ð Revisando</span>
                        )}
          ...
          {!isLoading && !error && totalPrestamos > 0 && (
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mt-4 pt-4 border-t">
              <p className="text-sm text-gray-600">
                ...
                <strong>{totalPrestamos}</strong> prÃ©stamos
              </p>
